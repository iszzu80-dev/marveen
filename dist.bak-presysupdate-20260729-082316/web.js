...
export function startWebServer(port = 3420) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        const isPublicApi = (path === '/api/auth/status' && method === 'GET') ||
            (path === '/api/settings' && method === 'GET') ||
            (method === 'GET' && (path === '/api/marveen/avatar' ||
        ...
        if ((path.startsWith('/api/') && !isPublicApi) || isFleetManifest) {
            ...
            const isFedPath = (path === '/api/federation/manifest' && method === 'GET') ||
        ...
        try {
            const routeCtx = { req, res, path, method, url, fedPeer: fedPeerForCtx };
