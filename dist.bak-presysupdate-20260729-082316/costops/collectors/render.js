...
export function loadRenderPricing() {
    const empty = { version: 1, currency: 'HUF', fx_usd_huf: 0, fx_eur_huf: 0, plans: {} };
    ...
    try {
        const raw = JSON.parse(readFileSync(RENDER_PRICING_PATH, 'utf-8'));
        return {
            pricing: {
 ...
 */
export function mapRenderPlanCost(raw, opts) {
    ...
    const addUnit = (type, plan, suspended) => {
        count++;
        if (suspended) {
            suspended_count++;
            return;
        } // suspended -> not billed
        if (type === 'static_site') {
            staticCount++;
            return;
        } // free flat; bandwidth uncounted
        ...
        b.count++;
        b.usd = Math.round((b.usd + price) * 100) / 100;
        by_type_plan[key] = b;
    };
    for (const it of serviceItems) {
        const svc = (it && typeof it === 'object' && 'service' in it)
            ? it.service
            : it;
        ...
        if (typeof instances !== 'number' || instances < 1) {
            instances = 1;
            if (type !== 'static_site' && !suspended)
                missingInstances++;
        }
        for (let i = 0; i < instances; i++)
            addUnit(type, plan, suspended);
    }
    for (const it of pgItems) {
        const pg = (it && typeof it === 'object' && 'postgres' in it)
            ? it.postgres
            : it;
    ...
    if (staticCount > 0)
        flags.push(`${staticCount} static_site: bandwidth overage not counted (0 Ft)`);
    if (missingInstances > 0)
        flags.push(`${missingInstances} service(s): instance count missing -> counted as 1`);
    if (unpriced.length > 0)
        flags.push(`${unpriced.reduce((s, u) => s + u.count, 0)} service(s): unknown/unpriced plan -> excluded from total`);
    if (pgItems.length === 0)
        flags.push('no postgres resources seen (if you have DBs, add /v1/postgres pricing)');
    if (fx <= 0)
        flags.push('fx_usd_huf missing/zero -> HUF total is 0; set it in the pricing config');
...
export function getRenderApiKey() {
    ...
    if (fromEnv && fromEnv.trim())
        return fromEnv.trim();
    return (readKeyFromEnvFile(join(PROJECT_ROOT, '.env')) ??
 ...
 */
export async function syncRenderCollector(db, now, deps = {}) {
    ...
    if (!apiKey)
        return { ok: false, provider: 'render', status: 'error', imported_count: 0, error: 'no RENDER_API_KEY configured' };
    const httpGetJson = deps.httpGetJson || (async (url, headers) => {
        const r = await fetch(url, { method: 'GET', headers });
        if (!r.ok)
            throw new Error(`render api ${r.status}`);
        return r.json();
    });
...
export function makeRenderCollector(pricing) {
    return {
        provider: 'render',
        ...
        async collectRaw(opts) {
            ...
            let postgres = [];
            try {
                postgres = await opts.httpGetJson(RENDER_POSTGRES_URL, headers);
            }
            catch {
                postgres = [];
            }
