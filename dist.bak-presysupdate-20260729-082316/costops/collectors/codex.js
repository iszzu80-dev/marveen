 ...
 */
export function parseCodexRateLimit(raw) {
 ...
 */
export const readCodexRateLimitLive = async () => {
    ...
    return await new Promise((resolve, reject) => {
        ...
        p.on('error', (e) => { clearTimeout(timer); done(e); });
        p.stdout.on('data', (d) => {
            buf += d.toString();
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (o && o.id === 2 && o.result !== undefined) {
                        clearTimeout(timer);
                        done(null, o.result);
                    }
                }
 ...
 */
export function forecastCodexLimitExhaustion(snapsAsc, now) {
    if (snapsAsc.length < 2)
        return null;
    const spanSec = snapsAsc[snapsAsc.length - 1].captured_at - snapsAsc[0].captured_at;
    if (spanSec < 3600)
        return null; // < 1h of history is too noisy
    let rise = 0;
    ...
    if (rise <= 0)
        return null; // flat or only resets -> no exhaustion to project
    const perSec = rise / spanSec;
    if (!isFinite(perSec) || perSec <= 0)
        return null;
    const remaining = 100 - snapsAsc[snapsAsc.length - 1].used_percent;
    if (remaining <= 0)
        return now;
    const secsToFull = remaining / perSec;
    if (!isFinite(secsToFull) || secsToFull < 0)
        return null;
    return Math.floor(now + secsToFull);
}
function recordRun(db, status, count, now, errMsg) {
 ...
 */
export async function syncCodexRateLimit(db, now, deps = {}) {
    ...
    VALUES (?,?,?,?,?,?,?)`).run(CODEX_PROVIDER, snap.limitId, snap.usedPercent, snap.windowDurationMins, snap.resetsAt, snap.planType, now);
