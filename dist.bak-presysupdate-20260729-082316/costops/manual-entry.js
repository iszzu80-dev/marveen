...
function manualCostDedupKey(sourceId, month) {
    return `manual|${sourceId}|${month}`;
}
export function createManualCost(db, input, opts) {
    if (!input || typeof input.source_id !== 'string' || !input.source_id)
        return { ok: false, error: 'missing source_id', status: 400 };
    if (!input.name)
        return { ok: false, error: 'missing name', status: 400 };
    if (!input.provider)
        return { ok: false, error: 'missing provider', status: 400 };
    const win = monthWindow(input.month);
    if (!win)
        return { ok: false, error: `bad month '${input.month}'`, status: 400 };
    ...
    if (!writable.writable)
        return { ok: false, error: writable.reason, status: 409 };
    const cur = (input.currency || 'HUF').toUpperCase();
    const amountHuf = toHuf(Number(input.amount), cur, opts.fxUsdHuf, opts.fxEurHuf ?? 0);
    if (amountHuf === null || !isFinite(amountHuf))
        return { ok: false, error: `unconvertible currency '${input.currency}'`, status: 400 };
    ...
    if (existing)
        return { ok: false, error: `a manual entry for '${input.source_id}' / '${input.month}' already exists -- use PATCH to update it`, status: 409 };
...
export function updateManualCost(db, patch, opts) {
    if (!patch || typeof patch.source_id !== 'string' || !patch.source_id)
        return { ok: false, error: 'missing source_id', status: 400 };
    const win = monthWindow(patch.month);
    if (!win)
        return { ok: false, error: `bad month '${patch.month}'`, status: 400 };
    const dedup = manualCostDedupKey(patch.source_id, patch.month);
    const existing = db.prepare(`SELECT confidence FROM cost_line_items WHERE dedup_key = ?`).get(dedup);
    if (!existing)
        return { ok: false, error: `no manual entry for '${patch.source_id}' / '${patch.month}' -- use POST to create one`, status: 404 };
    if (existing.confidence !== 'manual')
        return { ok: false, error: `'${patch.source_id}' / '${patch.month}' is not a manual entry (confidence='${existing.confidence}')`, status: 409 };
    // Phase 2 (GAP-13): a closed month accepts no direct write -- use a correction instead.
    ...
    if (amountHuf === null || !isFinite(amountHuf))
        return { ok: false, error: `unconvertible currency '${patch.currency}'`, status: 400 };
    const wasConverted = cur !== 'HUF';
    const appliedFxRate = fxRateFor(cur, opts.fxUsdHuf, opts.fxEurHuf ?? 0);
    db.prepare(`
...
export function deleteManualCost(db, input, opts) {
    if (!input || typeof input.source_id !== 'string' || !input.source_id)
        return { ok: false, error: 'missing source_id', status: 400 };
    const win = monthWindow(input.month);
    if (!win)
        return { ok: false, error: `bad month '${input.month}'`, status: 400 };
    const dedup = manualCostDedupKey(input.source_id, input.month);
    const existing = db.prepare(`SELECT confidence, voided_at FROM cost_line_items WHERE dedup_key = ?`).get(dedup);
    if (!existing)
        return { ok: false, error: `no manual entry for '${input.source_id}' / '${input.month}'`, status: 404 };
    if (existing.confidence !== 'manual')
        return { ok: false, error: `'${input.source_id}' / '${input.month}' is not a manual entry (confidence='${existing.confidence}')`, status: 409 };
    if (existing.voided_at != null)
        return { ok: false, error: `'${input.source_id}' / '${input.month}' is already voided`, status: 409 };
...
function entitlementDedupKey(e) {
    return `manual|${e.provider}|${e.product}|${e.entitlement_type}|${e.billing_period}`;
}
export function createManualEntitlement(db, input, now) {
    if (!input?.provider)
        return { ok: false, error: 'missing provider', status: 400 };
    if (!input?.product)
        return { ok: false, error: 'missing product', status: 400 };
    if (!input?.entitlement_type)
        return { ok: false, error: 'missing entitlement_type', status: 400 };
    if (!input?.billing_period)
        return { ok: false, error: 'missing billing_period', status: 400 };
    if (!input?.status)
        return { ok: false, error: 'missing status', status: 400 };
    const dedup = entitlementDedupKey(input);
    const existing = db.prepare(`SELECT 1 FROM entitlements WHERE dedup_key = ?`).get(dedup);
    if (existing)
        return { ok: false, error: `a manual entitlement for '${dedup}' already exists -- use PATCH to update it`, status: 409 };
    db.prepare(`
...
export function updateManualEntitlement(db, patch, now) {
    ...
    const existing = db.prepare(`SELECT usage_source FROM entitlements WHERE dedup_key = ?`).get(dedup);
    if (!existing)
        return { ok: false, error: `no manual entitlement for '${dedup}' -- use POST to create one`, status: 404 };
    if (existing.usage_source !== 'manual')
        return { ok: false, error: `'${dedup}' is not a manual entitlement (usage_source='${existing.usage_source}')`, status: 409 };
    db.prepare(`
...
export function deleteManualEntitlement(db, input) {
    ...
    const existing = db.prepare(`SELECT usage_source FROM entitlements WHERE dedup_key = ?`).get(dedup);
    if (!existing)
        return { ok: false, error: `no manual entitlement for '${dedup}'`, status: 404 };
    if (existing.usage_source !== 'manual')
        return { ok: false, error: `'${dedup}' is not a manual entitlement (usage_source='${existing.usage_source}')`, status: 409 };
