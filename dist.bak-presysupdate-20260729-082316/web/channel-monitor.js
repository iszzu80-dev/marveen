...
import { agentHasChannel, agentSessionName, capturePane, captureParkedInputView, clearInputBuffer, dismissResumeSummaryModalIfPresent, isAgentRunning, sendPromptToSession, startAgentProcess, stopAgentProcess, scheduleIdentitySetup, ensureMainAgentIsolatedConfigDir, ensureSharedClaudeOnboarded, hasFleetOauthToken, FLEET_OAUTH_TOKEN_PATH, } from './agent-process.js';
...
import { detectPaneState, decidePaneErrorAlert, detectsBlockingMenu, stuckInputSignature, decideStuckInputRecovery, parkedChannelInput, parkedInputText, shouldClearTruncatedPreamble, parkedInputRowCount, submitLanded, decideStuckInputAction, } from '../pane-state.js';
...
export function applyStuckRestartBusyGuard(paneState, decision) {
...
function maybeRestartWedgedMainChannel(state) {
    ...
    const action = applyStuckRestartBusyGuard(paneState, decideStuckInputRestart(parked, state.attempts, MAIN_STUCK_THRESHOLDS.maxAttempts, Date.now(), lastStuckRestartAt, stuckRestartCount, STUCK_RESTART_MIN_INTERVAL_MS, STUCK_RESTART_MAX_CONSECUTIVE));
