...
import { evaluateDispatch, parseTrustedProviders, normalizeConfig, } from '../data-sensitivity-gate.js';
...
let cachedTrustedPrefixes = null;
export function readTrustedProviders() {
    if (cachedTrustedPrefixes)
        return cachedTrustedPrefixes;
    cachedTrustedPrefixes = parseTrustedProviders(process.env['TRUSTED_PROVIDERS']);
    logger.info({ trusted: [...cachedTrustedPrefixes] }, 'data-sensitivity-gate: trusted providers resolved');
    return cachedTrustedPrefixes;
}
// For testing: invalidate caches.
export function invalidateGateConfigCache() {
    cachedConfig = null;
    cachedConfigMtime = 0;
...
export function checkDispatchGate(input) {
...
export function checkGateLiveness() {
    try {
        const db = getDb();
        const row = db
        ...
        if (lastTs === null) {
            logger.warn('data-sensitivity-gate: LIVENESS CHECK FAILED — audit log is EMPTY. ' +
        ...
        if (ageMs > GATE_SILENCE_THRESHOLD_MS) {
            ...
            logger.warn({ lastEntry: new Date(lastTs * 1000).toISOString(), ageHours }, `data-sensitivity-gate: LIVENESS CHECK FAILED — last audit entry was ${ageHours}h ago. ` +
