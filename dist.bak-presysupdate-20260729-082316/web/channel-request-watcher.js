...
function scanAuditLog(agent, auditPath) {
...
async function lookupChannelName(agent, channelId) {
    ...
    try {
        ...
        if (data.ok && data.channel?.name) {
            ...
            if (match)
                updateChannelRequestName(match.id, data.channel.name);
        }
    }
    catch (err) {
...
function runScanTick() {
    ...
    for (const name of listAgentNames()) {
        ...
        for (const req of listPendingChannelRequests(name)) {
            if (!req.channel_name)
                lookupChannelName(name, req.channel_id).catch(() => { });
        }
    }
...
let intervalId = null;
export function startChannelRequestWatcher(intervalMs = 10_000) {
    if (intervalId)
        return;
    runScanTick();
...
export function stopChannelRequestWatcher() {
    if (intervalId) {
