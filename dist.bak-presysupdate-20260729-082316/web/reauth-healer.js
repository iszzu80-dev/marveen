...
export const NO_REAUTH_STATE = { consecutiveDead: 0, lastActionAtMs: null };
/**
 ...
 */
export function decideReauthAction(input, t) {
...
function hostCanInteractiveLogin() {
...
async function sendBestEffortLogin(session) {
...
export function isQuietHour(hourLocal) {
...
export function localHour(nowMs) {
    return parseInt(new Intl.DateTimeFormat('en-GB', { timeZone: APP_TZ, hour: '2-digit', hour12: false }).format(new Date(nowMs)), 10);
}
const quietSuppressed = new Map();
export function buildEscalationMessage(label, reason, consecutiveDead) {
...
export function buildQuietSummaryMessage(entries) {
 ...
 */
export function routeEscalation(entry, quiet, notify, suppressed = quietSuppressed) {
    if (quiet) {
 ...
 */
export function flushQuietSummary(quiet, stillDeadCount, notify, stampAlert, suppressed = quietSuppressed) {
    ...
    for (const e of stillDead)
        stampAlert(e.session);
}
function sendNotify(msg) {
    execFile('/bin/bash', [NOTIFY_SCRIPT, msg], { timeout: 10_000 }, (err) => {
        if (err)
            logger.warn({ err }, 'reauth-healer: notify.sh escalation failed');
    });
}
function checkSession(label, session, isMain, quiet) {
    ...
    const decision = decideReauthAction({ isDeadToken: reauth.needsReauth, sessionAlive, isMain, canInteractiveLogin: hostCanInteractiveLogin(), isFirstRunGate, prev, nowMs: Date.now() }, { threshold: DEAD_PROBE_THRESHOLD, cooldownMs: ESCALATION_COOLDOWN_MS });
    ...
    if (decision.escalate) {
        ...
        routeEscalation({ session, label, reason: reauth.reason ?? 'auth failure', consecutiveDead: decision.next.consecutiveDead }, quiet, sendNotify);
        ...
        if (!isFirstRunGate) {
            quarantineFleetTokenIfDead()
                .then((r) => { if (r !== 'no-token')
                logger.warn({ label, result: r }, 'reauth-healer: fleet-token liveness check'); })
...
async function restartFirstRunGatedAgent(name, session) {
    await new Promise((resolve) => {
    ...
    try {
        ...
        if (!r.ok)
            logger.warn({ name, error: r.error }, 'reauth-healer: first-run-gate relaunch failed');
    }
    catch (err) {
        logger.warn({ err, name }, 'reauth-healer: first-run-gate relaunch threw');
    }
}
export function startReauthHealer() {
    ...
    function sweep() {
        ...
        flushQuietSummary(quiet, (session) => watchState.get(session)?.consecutiveDead ?? 0, sendNotify, (session) => {
