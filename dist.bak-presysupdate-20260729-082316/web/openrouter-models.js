...
export function loadOpenRouterCatalog() {
    try {
        if (existsSync(OPENROUTER_MODELS_FILE)) {
            const parsed = JSON.parse(readFileSync(OPENROUTER_MODELS_FILE, 'utf-8'));
...
export async function fetchAllOpenRouterModels(nowMs) {
    if (allModelsCache && nowMs - allModelsCache.at < ALL_MODELS_TTL_MS)
        return allModelsCache.models;
    const resp = await fetch('https://openrouter.ai/api/v1/models');
    if (!resp.ok)
        throw new Error(`openrouter models fetch: HTTP ${resp.status}`);
...
export function loadCuratedManual() {
    try {
        if (existsSync(OPENROUTER_MANUAL_FILE)) {
            const parsed = JSON.parse(readFileSync(OPENROUTER_MANUAL_FILE, 'utf-8'));
...
function saveCuratedManual(models) {
...
export function addCuratedManual(id, name) {
...
export function removeCuratedManual(id) {
    const list = loadCuratedManual();
    const next = list.filter(m => m.id !== id);
    if (next.length !== list.length)
        saveCuratedManual(next);
    return next;
}
...
export function resolveOpenRouterModel(model) {
    if (!model.startsWith(AUTO_PREFIX))
        return model;
