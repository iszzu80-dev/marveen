...
import { createBackgroundTaskAtomic, finishBackgroundTask, getBackgroundTasks, getBackgroundTask, getRunningBackgroundTasks, } from '../../db.js';
...
function bgSessionName(id) {
    return `bg-${id}`;
}
function isBgSessionAlive(session) {
    try {
...
export function spawnBackgroundTask(agentId, prompt) {
...
export function sweepOrphanedBackgroundTasks() {
    ...
    if (orphaned)
        logger.info({ orphaned }, 'Swept orphaned background tasks on startup');
}
const TASK_ID_RE = /^\/api\/background-tasks\/([A-F0-9]{8})$/;
export async function tryHandleBackgroundTasks(ctx) {
    ...
    if (taskMatch && method === 'GET') {
        ...
        if (!task) {
            json(res, { error: 'Háttérfeladat nem található' }, 404);
            return true;
        }
        let liveOutput = null;
