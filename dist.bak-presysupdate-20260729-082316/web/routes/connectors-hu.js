...
async function isInstalled() {
    ...
    if (existsSync(localPath))
        return { installed: true, path: localPath };
...
export async function tryHandleConnectorsHu(ctx) {
    ...
    if (path === '/api/connectors-hu/install' && method === 'POST') {
        try {
            ...
            if (result.ok)
                logger.info('connectors CLI installed');
            else
                logger.warn({ output: result.output }, 'connectors CLI install failed');
        }
        catch (err) {
    ...
    if (path === '/api/connectors-hu/configure' && method === 'POST') {
        try {
            ...
            if (result.ok)
                logger.info('connectors-hu configured and synced');
            else
                logger.warn({ output: result.output }, 'connectors sync returned non-zero');
        }
        catch (err) {
