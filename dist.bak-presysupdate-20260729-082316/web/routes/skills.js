...
function getSkillAgents(skillDirName) {
    const agents = [];
...
export async function tryHandleSkills(ctx) {
    ...
    if (path === '/api/skills' && method === 'GET') {
        ...
        if (existsSync(USER_SKILLS_DIR)) {
            ...
            const dirs = readdirSync(USER_SKILLS_DIR).filter(f => {
                ...
                if (f.startsWith('.'))
                    return false;
                try {
                    return statSync(join(USER_SKILLS_DIR, f)).isDirectory();
                }
                catch {
                    return false;
                }
            ...
            for (const dir of dirs) {
                ...
                let mtime = 0;
                try {
                    mtime = statSync(skillMdPath).mtimeMs;
                }
                catch { /* no-op */ }
                skills.push({
                    name: dir,
        ...
        if (existsSync(PLUGINS_CACHE_DIR)) {
            const walkForSkills = (dir, depth, packagePath) => {
                if (depth > 4)
                    return;
                let entries = [];
                try {
                ...
                if (entries.includes('skills')) {
                    ...
                    for (const sd of skillDirs) {
                        ...
                        let pluginMtime = 0;
                        try {
                            pluginMtime = statSync(skillMdPath).mtimeMs;
                        }
                        catch { /* no-op */ }
                        skills.push({
                ...
                for (const entry of entries) {
                    if (entry.startsWith('.') || entry === 'skills')
                        continue;
    ...
    if (path === '/api/skills/export' && method === 'GET') {
        ...
        try {
            execSync(`cd ${shellEscape(USER_SKILLS_DIR)} && zip -r ${shellEscape(tmpZip)} . --include "*/SKILL.md" --include "*/references/*"`, { timeout: 15000 });
    ...
    if (globalSkillDetailMatch && method === 'GET') {
        ...
        if (agentParam) {
            ...
            try {
                agentDetailMtime = statSync(skillMdPath).mtimeMs;
            }
            catch { /* no-op */ }
            json(res, {
                name: skillName,
        ...
        if (skillName.includes(':')) {
            ...
            try {
                pluginDetailMtime = statSync(skillMdPath).mtimeMs;
            }
            catch { /* no-op */ }
            json(res, {
                name: skillName,
        ...
        try {
            userDetailMtime = statSync(skillMdPath).mtimeMs;
        }
        catch { /* no-op */ }
        const files = [];
        try {
    ...
    if (path === '/api/skills' && method === 'POST') {
        ...
        const { name: rawSkillName, description } = JSON.parse(body.toString());
        ...
        if (!description) {
            json(res, { error: 'Skill description is required' }, 400);
            return true;
        }
        const GLOBAL_SKILLS_DIR = join(homedir(), '.claude', 'skills');
        const skillDir = join(GLOBAL_SKILLS_DIR, skillName);
        ...
        if (existsSync(skillDir)) {
            json(res, { error: 'Skill already exists' }, 409);
            return true;
        }
    ...
    if (path === '/api/skills/import' && method === 'POST') {
        ...
        try {
            ...
            const rejectSymlinks = (dir) => {
                for (const entry of readdirSync(dir)) {
                    const p = join(dir, entry);
                    const st = lstatSync(p);
                    if (st.isSymbolicLink())
                        return true;
                    if (st.isDirectory() && rejectSymlinks(p))
                        return true;
                }
                return false;
            };
            ...
            if (tainted.length > 0) {
                for (const f of after) {
                    try {
                        rmSync(join(skillsDir, f), { recursive: true, force: true });
                    }
                    catch { /* best effort */ }
                }
    ...
    if (globalSkillAssignMatch && method === 'POST') {
        ...
        const body = await readBody(req);
        const { agents: targetAgents } = JSON.parse(body.toString());
