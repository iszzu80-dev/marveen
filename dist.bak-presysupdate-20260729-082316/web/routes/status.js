...
export async function tryHandleStatus(ctx) {
    ...
    if (path === '/api/status' && method === 'GET') {
        try {
            ...
            while ((match = itemRegex.exec(rssText)) !== null) {
                ...
                if (cleanDesc.toLowerCase().includes('resolved'))
                    status = 'resolved';
                else if (cleanDesc.toLowerCase().includes('monitoring'))
                    status = 'monitoring';
                else if (cleanDesc.toLowerCase().includes('identified'))
                    status = 'identified';
