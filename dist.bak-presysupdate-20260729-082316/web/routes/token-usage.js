import { collectTokenUsage, getTokenSummary, getTokenTimeline, getTokenDetails, getModelDistribution, getToolStats, correlateWithKanban, } from '../token-usage.js';
import { json } from '../http-helpers.js';
import { logger } from '../../logger.js';
export async function tryHandleTokenUsage(ctx) {
    ...
    if (path === '/api/token-usage/summary' && method === 'GET') {
        ...
        const summary = getTokenSummary(from ? parseInt(from) : undefined, to ? parseInt(to) : undefined);
        json(res, summary);
        return true;
    }
    if (path === '/api/token-usage/timeline' && method === 'GET') {
        ...
        const timeline = getTokenTimeline(bucketMinutes, from ? parseInt(from) : undefined, to ? parseInt(to) : undefined, agent);
        json(res, timeline);
        return true;
    }
    if (path === '/api/token-usage/model-dist' && method === 'GET') {
        ...
        const to = url.searchParams.get('to');
        const agent = url.searchParams.get('agent') || undefined;
        json(res, getModelDistribution(from ? parseInt(from) : undefined, to ? parseInt(to) : undefined, agent));
        return true;
    }
    if (path === '/api/token-usage/tool-stats' && method === 'GET') {
        ...
        const to = url.searchParams.get('to');
        const agent = url.searchParams.get('agent') || undefined;
        json(res, getToolStats(from ? parseInt(from) : undefined, to ? parseInt(to) : undefined, agent));
        return true;
    }
