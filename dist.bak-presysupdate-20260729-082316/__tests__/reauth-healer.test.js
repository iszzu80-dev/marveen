...
import { decideReauthAction, NO_REAUTH_STATE } from '../web/reauth-healer.js';
...
describe('decideReauthAction', () => {
    ...
    it('a heal between dead spells lets the next spell alert immediately', () => {
        ...
        let s = b.next;
        let last = { escalate: false };
        for (let i = 0; i < 3; i++) {
            const r = decideReauthAction(base({ prev: s, nowMs: 9_000_000 }), T);
            s = r.next;
            last = r;
        }
...
describe('decideReauthAction: first-run gate', () => {
    const gated = { consecutiveDead: 2, lastActionAtMs: null };
