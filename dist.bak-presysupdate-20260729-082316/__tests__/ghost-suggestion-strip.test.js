...
import { stripGhostSuggestion, stuckInputSignature, detectPaneState, parkedInputText, paneLooksIdle, paneShowsContextSaturation, } from '../pane-state.js';
const ESC = '\x1b';
...
describe('isSessionReadyForPrompt wiring (dim-ghost tolerant idle)', () => {
    it('resolves idle through idleOrGhost/captureParkedInputView and saturation on the plain capture', () => {
        const src = readFileSync(join(dirname(fileURLToPath(import.meta.url)), '../web/agent-process.ts'), 'utf-8');
