...
describe('describeShape (types only, no values)', () => {
    it('returns structure with no scalar values', () => {
        ...
        expect(shape.keys.data).toMatchObject({ type: 'array', length: 1 });
        expect(shape.keys.data.of.keys.results).toMatchObject({ type: 'array', length: 2 });
        expect(shape.keys.has_more).toBe('boolean');
    });
});
describe('dryRunCollector (offline, persists NOTHING to cost_line_items)', () => {
    ...
    it('normalizes and shows planned lines + dedup_keys + response shape', async () => {
        ...
        expect(rep.responseShape.keys.has_more).toBe('boolean');
    });
    it('does NOT write any provider_api cost_line_item', async () => {
        ...
        const n = db.prepare("SELECT COUNT(*) n FROM cost_line_items WHERE confidence='provider_api'").get();
        expect(n.n).toBe(0);
        const anyLine = db.prepare('SELECT COUNT(*) n FROM cost_line_items').get();
        expect(anyLine.n).toBe(0);
    });
    it('does NOT write a final import; only a dry_run audit row with count 0', async () => {
        const db = getDb();
        await dryRunCollector({ db, collector: anthropicCollector, opts: opts(async () => FIXTURE), now: NOW });
        const rows = db.prepare('SELECT status, imported_count FROM import_runs').all();
        expect(rows).toHaveLength(1);
        expect(rows[0].status).toBe('dry_run');
        expect(rows[0].imported_count).toBe(0);
        // NO 'ok' import ever recorded by a dry-run
        const okRuns = db.prepare("SELECT COUNT(*) n FROM import_runs WHERE status='ok'").get();
    ...
    it('recordRun:false persists absolutely nothing', async () => {
        ...
        const runs = db.prepare('SELECT COUNT(*) n FROM import_runs').get();
        const lines = db.prepare('SELECT COUNT(*) n FROM cost_line_items').get();
        expect(runs.n).toBe(0);
        expect(lines.n).toBe(0);
    });
    it('on error: status=error, sanitized, writes NO cost line, secret never leaks', async () => {
        const db = getDb();
        const throwing = async () => { throw new Error(`rate limited, key ${SECRET} and token abcdef1234567890abcdef1234567890`); };
