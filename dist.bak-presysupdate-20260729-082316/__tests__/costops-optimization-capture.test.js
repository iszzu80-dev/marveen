...
import { gatherRecommendationCandidates, captureRecommendations, listRecommendations, } from '../costops/optimization-capture.js';
const NOW = Math.floor(Date.UTC(2026, 6, 15, 12, 0, 0) / 1000); // 2026-07-15
function insertSource(db, id, provider, source_type) {
...
function writeSubscriptionsFixture(content) {
...
describe('gatherRecommendationCandidates -- subscription-based (real file fixture)', () => {
    beforeEach(() => { initDatabase(':memory:'); });
    afterEach(() => { if (existsSync(SUBSCRIPTIONS_PATH))
...
describe('captureRecommendations persistence round-trip', () => {
    ...
    it('dedups on a second capture -- no duplicate row, only a touch', () => {
        ...
        const rows = db.prepare(`SELECT COUNT(*) as n FROM costops_recommendations WHERE type = 'duplicate_hosting_saas'`).get();
    ...
    it('freezes an accepted recommendation -- captureRecommendations never reverts it back to open', () => {
        ...
        const row = db.prepare(`SELECT status FROM costops_recommendations WHERE type = 'duplicate_hosting_saas'`).get();
