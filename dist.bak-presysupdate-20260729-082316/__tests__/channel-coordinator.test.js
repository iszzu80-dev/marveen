...
import { initIngestDb, insertIncomingEvent, createHandoffMessage, markEventDelivered, getEventsNeedingHandoff, getOffset, setOffset, closeIngestDb, COORDINATOR_AGENT_ID, } from '../channel-coordinator/ingest.js';
import { neutralizeChannelTags, buildHandoffContent, transientBackoffMs, inNative409Cooldown, } from '../channel-coordinator.js';
...
describe('ingest', () => {
    ...
    it('getUpdates_to_agent_message_flow', () => {
        ...
        const am = db.prepare('SELECT * FROM agent_messages WHERE id = ?').get(amId);
        ...
        const ev = db.prepare('SELECT * FROM incoming_events WHERE id = ?').get(ins.eventId);
    ...
    it('reconcile re-handoff is idempotent against the source event (dedup still holds)', () => {
        ...
        const count = db.prepare("SELECT COUNT(*) c FROM incoming_events WHERE update_id = 304").get();
        expect(count.c).toBe(1);
        // And the event now points at the fresh (pending) handoff.
        const ev = db.prepare('SELECT agent_message_id FROM incoming_events WHERE id = ?').get(ins.eventId);
