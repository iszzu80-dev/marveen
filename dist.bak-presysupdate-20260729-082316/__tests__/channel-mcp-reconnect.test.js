...
vi.mock('../platform.js', () => ({
    resolveFromPath: (name) => `/usr/local/bin/${name}`,
}));
vi.mock('../logger.js', () => ({
    logger: {
        info: vi.fn(),
...
vi.mock('../web/agent-config.js', () => ({
    readAgentChannelProvider: (name) => name === 'slacker' ? 'slack' : '',
    AGENTS_BASE_DIR: '/tmp/test-claudeclaw/agents',
}));
const mockCapturePane = vi.fn();
vi.mock('../web/agent-process.js', () => ({
    agentSessionName: (name) => `agent-${name}`,
    capturePane: (session) => mockCapturePane(session),
}));
...
import { attemptChannelMcpReconnect, resolveAgentSession, resolveAgentProviderType, selectedSubmenuLine, chooseSubmenuTarget, } from '../web/channel-mcp-reconnect.js';
...
describe('attemptChannelMcpReconnect', () => {
    ...
    it('defers (sends NO keys) when the pane is actively generating', async () => {
        ...
        const sendKeyCalls = mockExecFileSync.mock.calls.filter((c) => Array.isArray(c[1]) && c[1].includes('send-keys'));
    ...
    it('connected state: steps Down onto Reconnect, then activates it', async () => {
        ...
        const submenuEnters = mockExecFileSync.mock.calls.filter((c) => Array.isArray(c[1]) && c[1].includes('Enter') && !c[1].includes('/mcp'));
    ...
    it('failed state: Reconnect is already selected, activates WITHOUT pressing Down', async () => {
        ...
        const downCalls = mockExecFileSync.mock.calls.filter((c) => Array.isArray(c[1]) && c[1].includes('Down'));
    ...
    it('uses correct session for sub-agents', async () => {
        ...
        expect(mockExecFileSync).toHaveBeenCalledWith('/usr/local/bin/tmux', ['send-keys', '-t', 'agent-slacker', 'Escape'], expect.any(Object));
    });
    it('sends Escape on error to clean up menu state', async () => {
        ...
        const result = await attemptChannelMcpReconnect('marveen');
        expect(result.ok).toBe(false);
        const escapeCalls = mockExecFileSync.mock.calls.filter((c) => Array.isArray(c[1]) && c[1].includes('Escape'));
