...
import { channelsSessionName, channelsLaunchdLabel, channelsPlistPath, } from '../web/main-agent.js';
...
function assertNoMarveen(value, label) {
...
describe('identity payload core resolves from config, not the literal', () => {
    it('maps display name / brand / canonical id for a non-marveen identity', () => {
        ...
        for (const [k, v] of Object.entries(core))
            assertNoMarveen(String(v), `identity.${k}`);
    });
...
describe('brand-population fallback contract (initSidebarBrand)', () => {
    ...
    const pickBrand = (m) => m.brandName || m.name;
