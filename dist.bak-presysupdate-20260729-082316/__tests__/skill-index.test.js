...
function makeSkillMd(name, description) {
...
describe('skill-index.sh -- no-arg mode (backward compat)', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(tmpHome, '.claude', 'skills', 'skill-alpha', 'SKILL.md'), makeSkillMd('skill-alpha', 'Global skill alpha description'));
    });
    afterEach(() => {
    ...
    it('does NOT create an index in any other directory', () => {
        ...
        writeFileSync(join(agentDir, '.claude', 'skills', 'skill-beta', 'SKILL.md'), makeSkillMd('skill-beta', 'Agent-specific skill beta'));
        runScript([], { HOME: tmpHome });
...
describe('skill-index.sh -- AGENT_DIR mode (merged index)', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(tmpHome, '.claude', 'skills', 'skill-global', 'SKILL.md'), makeSkillMd('skill-global', 'A global skill visible to all agents'));
        // Agent-specific skill
        agentDir = join(tmpHome, 'agents', 'agent-a');
        mkdirSync(join(agentDir, '.claude', 'skills', 'skill-local'), { recursive: true });
        writeFileSync(join(agentDir, '.claude', 'skills', 'skill-local', 'SKILL.md'), makeSkillMd('skill-local', 'An agent-local skill for agent-a only'));
    });
    afterEach(() => {
        rmSync(tmpHome, { recursive: true, force: true });
    });
    ...
    it('two different agents get independent indexes with their own agent-local skills', () => {
        ...
        writeFileSync(join(agentBDir, '.claude', 'skills', 'skill-b-only', 'SKILL.md'), makeSkillMd('skill-b-only', 'Only for agent-b'));
        runScript([agentDir], { HOME: tmpHome });
