...
export function getAgentMemories(agentId, limit = 20) {
    const key = `${agentId}:${limit}`;
    const cached = memoryCacheGet(key);
    if (cached)
        return cached;
    const result = db.prepare("SELECT * FROM memories WHERE (agent_id = ? OR category = 'shared') ORDER BY accessed_at DESC LIMIT ?").all(agentId, limit);
...
export function createAgentMessage(from, to, content, originNote) {
    const now = Math.floor(Date.now() / 1000);
    const info = db.prepare('INSERT INTO agent_messages (from_agent, to_agent, content, status, created_at, origin_note) VALUES (?, ?, ?, ?, ?, ?)').run(from, to, content, 'pending', now, originNote ?? null);
    return {
...
// --- Tool Call Log ---
export function logToolCall(sessionId, toolName, inputSummary, success = true) {
    const now = Math.floor(Date.now() / 1000);
