...
describe('COS unified document store', () => {
  ...
  it('stores bytes content-addressed, defaults to UNKNOWN/not-shareable, links to case', () => {
    ...
    const rel = (db.prepare(`SELECT related_document_ids FROM zst_cases WHERE case_id='zst-c1'`).get() as any).related_document_ids
  ...
  it('dedups identical bytes for the same case (one file, one row)', () => {
    ...
    expect((db.prepare(`SELECT COUNT(*) n FROM cos_documents`).get() as any).n).toBe(1)
  })
  ...
  it('keeps personal and zst namespaces separate (same bytes, two docs)', () => {
    ...
    expect((db.prepare(`SELECT COUNT(*) n FROM cos_documents`).get() as any).n).toBe(2)
  ...
  it('a document can be stored before a case exists (nullable case_id)', () => {
    ...
    expect(r.duplicate).toBe(false)
    const row = db.prepare(`SELECT case_id FROM cos_documents WHERE document_id=?`).get(r.documentId) as any
