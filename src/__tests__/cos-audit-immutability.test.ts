...
describe('audit log immutability (personal_case_events)', () => {
  ...
  it('events accumulate on writes', () => {
    const n = (getDb().prepare(`SELECT COUNT(*) c FROM personal_case_events WHERE case_id='c1'`).get() as any).c
  ...
  it('the event history survives a tamper attempt intact', () => {
    const db = getDb()
    const before = (db.prepare(`SELECT COUNT(*) c FROM personal_case_events`).get() as any).c
    try { db.prepare(`DELETE FROM personal_case_events`).run() } catch { /* blocked */ }
    const after = (db.prepare(`SELECT COUNT(*) c FROM personal_case_events`).get() as any).c
