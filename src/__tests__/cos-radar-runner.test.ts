...
function offer(car: string, category: string, full: number, supplier = 'Centauro'): RentalOffer {
  return {
...
class MockRental implements RentalAdapter {
  ...
  constructor(private readonly offers: RentalOffer[]) {}
  async search(_p: RentalSearchParams): Promise<RentalOffer[]> { return this.offers }
}
...
describe('COS radar runner (rental)', () => {
  ...
  it('searches, filters by category, records the cheapest as the observation', async () => {
    ...
    const obs = db.prepare(`SELECT best_price, offer_count, offer_ref FROM radar_observations WHERE radar_id='r1'`).get() as any
  ...
  it('records a null best price when no offers match', async () => {
    ...
    const obs = db.prepare(`SELECT best_price, offer_count FROM radar_observations WHERE radar_id='r1'`).get() as any
