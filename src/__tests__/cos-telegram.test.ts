...
import { CosSendRefused } from '../cos/cos-telegram.js'
...
// W10: the send is brokered, so these transport tests must now say WHO is
// sending. Passing an identity here is not weakening them -- the refusal path
// gets its own test below, which is the assertion that did not exist before.
const SENDER: ExecutionIdentity = {
  actorId: 'schedule:cos-channel-send',
  actorType: 'SYSTEM_AUTOMATION',
  onBehalfOf: 'istvan',
  runId: 'test-run',
  capabilityScope: ['READ', 'WRITE_LOCAL', 'EXTERNAL_EFFECT'],
}
const AS = { identity: SENDER }
...
describe('the token never travels in an error', () => {
  ...
  it('an HTTP failure surfaces WITHOUT the token', async () => {
    ...
    await expect(sendCosMessage(CFG, 'szia', { ...AS, fetchImpl: impl })).rejects.toThrow(/telegram sendMessage: 401/)
    await sendCosMessage(CFG, 'szia', { ...AS, fetchImpl: impl }).catch((e: Error) => {
  ...
  it('a network throw is redacted too', async () => {
    ...
    await sendCosMessage(CFG, 'x', { ...AS, fetchImpl: impl }).catch((e: Error) => {
...
describe('sending', () => {
  it('posts to the configured chat and returns the message id', async () => {
    ...
    const r = await sendCosMessage(CFG, 'kérdés', { ...AS, fetchImpl: impl })
    expect(r).toEqual({ messageId: 42, chatId: '999' })
    expect(seen[0].body.chat_id).toBe('999')
    expect(seen[0].body.text).toBe('kérdés')
  })

  it('REFUSES a send with no identity, and never reaches the wire', async () => {
    // The point of the broker: a caller that cannot say who it is does not get
    // a degraded send, it gets no send. `identity: null` is the explicit form of
    // "nobody" -- there is no value of the field that skips the gate.
    const { impl, seen } = fake(true, { message_id: 1, chat: { id: 999 } })
    await expect(sendCosMessage(CFG, 'kérdés', { identity: null, fetchImpl: impl }))
      .rejects.toThrow(CosSendRefused)
    expect(seen).toHaveLength(0)
  })

  it('REFUSES a sender that holds no EXTERNAL_EFFECT capability', async () => {
    const { impl, seen } = fake(true, { message_id: 1, chat: { id: 999 } })
    await expect(sendCosMessage(CFG, 'kérdés', {
      identity: { ...SENDER, capabilityScope: ['READ', 'WRITE_LOCAL'] },
      fetchImpl: impl,
    })).rejects.toThrow(CosSendRefused)
    expect(seen).toHaveLength(0)
  })

  it('REFUSES to carry a credential out, even from a fully capable sender', async () => {
    const { impl, seen } = fake(true, { message_id: 1, chat: { id: 999 } })
    await expect(sendCosMessage(CFG, 'a token: abc', {
      identity: { ...SENDER, capabilityScope: ['READ', 'WRITE_LOCAL', 'EXTERNAL_EFFECT', 'ADMIN', 'SECRET_READ', 'MODEL_EGRESS'] },
      classification: { level: 'RESTRICTED', tags: ['AUTH_TOKEN'], basis: 'test' },
      fetchImpl: impl,
    })).rejects.toThrow(CosSendRefused)
    expect(seen).toHaveLength(0)
  })
  ...
  it('says NOT CONFIGURED when the chat is unknown, rather than failing at the wire', async () => {
    ...
    await expect(sendCosMessage({ token: TOKEN }, 'x', { ...AS, fetchImpl: impl }))
