...
import {
  ...
  resolveSessionIdForCwd,
  discoverAgentSources,
  encodeProjectPath,
} from '../web/transcript-sources.js'
 ...
 */
function writeTranscript(dir: string, fileBase: string, opts: { sessionId?: string | null; mtimeSec: number }) {
...
const AGENT_DIR = (agent: string) => join(projectsDir, `${encodeProjectPath(FIXTURE_PROJECT_ROOT)}-agents-${agent}`)
...
afterEach(() => {
  try { chmodSync(projectsDir, 0o755) } catch { /* may not exist */ }
...
describe('one copy of the transcript->agent rule', () => {
  it('discoverAgentSources and resolveCurrentSessionId agree on the mapping', () => {
    ...
    for (const a of agents) expect(resolveCurrentSessionId(a, roots())).not.toBeNull()
  })
...
describe('END-TO-END: resolved session_id makes token rows attributable', () => {
  ...
  function insertTokenUsage(agent: string, sessionId: string, ts: number) {
    getDb().prepare(`
  ...
  it('origin-resolved session_id -> correlateTokenUsageToDispatches attributes the rows', () => {
    ...
    const dispatchId = createDispatch(
      db,
      { source: 'kanban', agent: 'codeworker', cardId: 'card-1', sessionId: resolved },
      T0 * 1000,
    )
    expect((db.prepare('SELECT session_id FROM dispatches WHERE dispatch_id = ?').get(dispatchId) as any).session_id)
    ...
    const attributed = db.prepare(
      'SELECT agent, session_id, timestamp FROM token_usage WHERE dispatch_id = ? ORDER BY timestamp',
    ).all(dispatchId) as { agent: string; session_id: string; timestamp: number }[]
    ...
    const unattributed = db.prepare('SELECT COUNT(*) AS n FROM token_usage WHERE dispatch_id IS NULL').get() as { n: number }
