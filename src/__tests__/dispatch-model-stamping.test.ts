...
import { deriveProvider, type PricingConfig } from '../costops/pricing.js'
import {
  resolveDispatchIdentity,
  resolveDispatchIdentitySafe,
  resolveAuthProfile,
  configDirIdentifier,
  UNRESOLVED_DISPATCH_IDENTITY,
  HOST_DEFAULT_AUTH_PROFILE,
  defaultDispatchIdentityDeps,
  type DispatchIdentityDeps,
} from '../costops/dispatch-identity.js'
...
function deps(over: Partial<DispatchIdentityDeps> = {}): DispatchIdentityDeps {
  return {
...
describe('P2-C: every dispatch origin stamps the identity columns', () => {
  ...
  it('the worker stamps its OWN launched model + config dir, not the main agent id alone', () => {
    ...
    expect(WORKER).toMatch(
      /\.\.\.resolveDispatchIdentitySafe\(MAIN_AGENT_ID, \{[\s\S]{0,200}?configuredModel: WORKER_MODEL,[\s\S]{0,200}?configDir: ctx\.configDir,/,
    )
...
describe('P2-C: billing_mode is resolved from the billing map only', () => {
  ...
  it('the committed example documents the auth_profile key space P2-C really produces', () => {
    ...
    const example = JSON.parse(
      readFileSync(join(__dirname, '..', '..', 'config-examples', 'billing-map.example.json'), 'utf-8'),
    ) as BillingMap
...
describe('P2-C: a resolver fault stamps un-attributed instead of blocking the dispatch', () => {
  ...
  it('the dispatch row is STILL created, with the four descriptive columns NULL', () => {
    ...
    const row = getDb().prepare('SELECT * FROM dispatches WHERE dispatch_id = ?').get(id) as Record<string, unknown>
...
describe('P2-C runtime: a real row through the REAL resolver has the columns populated', () => {
  ...
  it('stamps non-NULL configured/runtime/provider/auth_profile/billing_mode', () => {
    ...
    const row = getDb().prepare(
      'SELECT configured_model, runtime_model, provider, auth_profile, billing_mode, model_profile FROM dispatches WHERE dispatch_id = ?',
    ...
    expect(row.billing_mode).toBe(
      resolveBillingMode(loadBillingMap(), row.provider, row.auth_profile),
    )
