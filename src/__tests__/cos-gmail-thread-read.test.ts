...
const reader = (messages: ThreadMessage[]) => ({ fetchThread: async () => messages })
const failing = (msg: string) => ({ fetchThread: async () => { throw new Error(msg) } })
...
describe('full thread read', () => {
  ...
  describe('storing it on the case', () => {
    ...
    it('stores the thread and reports how many messages it held', async () => {
      ...
      expect((docs[0] as { doc_kind: string }).doc_kind).toBe('email_thread')
    })
