...
import { resolveRuntimeRouting, capacityKeyId, type FallbackCandidate } from '../capacity-routing.js'

const NOW_SEC = Math.floor(Date.UTC(2026, 6, 30, 12, 0, 0) / 1000)
const NOW_MS = NOW_SEC * 1000

function insertBalanceSnapshot(balance: number, capturedAtSec: number, provider = 'deepseek'): void {
  const db = getDb()
  db.prepare(
    `INSERT INTO provider_balance_snapshots (provider, currency, balance, captured_at) VALUES (?, 'USD', ?, ?)`,
  ).run(provider, balance, capturedAtSec)
}

const DEEPSEEK_CANDIDATE: FallbackCandidate = {
  provider: 'deepseek',
...
describe('card 6976aaa2: DeepSeek balance wired into capacity-routing availability', () => {
  ...
  describe('end-to-end: resolveRuntimeRouting actually fires the armed fallback', () => {
    it('GUARD TEST 1 (full path): a constrained Max primary + available DeepSeek -> fallback decision names deepseek-v4-pro', () => {
      ...
      if (decision.action === 'fallback') expect(decision.to.model).toBe('deepseek-v4-pro')
    })
