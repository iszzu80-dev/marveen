...
describe('COS Slice 0 schema', () => {
  ...
  function insertCase(db: any, over: Partial<Record<string, unknown>> = {}): string {
    const caseId = (over.case_id as string) ?? 'case-1'
    db.prepare(
      `INSERT INTO personal_cases (case_id, title, case_type, status, sensitivity, created_at, updated_at)
       VALUES (@case_id, @title, @case_type, @status, @sensitivity, @created_at, @updated_at)`
    ).run({
      case_id: caseId,
  ...
  describe('personal_cases', () => {
    it('defaults version to 1 and status to a valid state', () => {
      const db = getDb()
      insertCase(db)
      const row: any = db.prepare(`SELECT version, status, scope, priority, owner FROM personal_cases WHERE case_id='case-1'`).get()
    ...
    it('optimistic-concurrency UPDATE bumps version only when the seen version matches (P0.5)', () => {
      ...
      const bump = db.prepare(
        `UPDATE personal_cases SET status=@status, version=version+1, updated_at=@t
      ...
      const row: any = db.prepare(`SELECT version, status FROM personal_cases WHERE case_id='case-1'`).get()
  ...
  describe('personal_case_events (append-only)', () => {
    function addEvent(db: any) {
      insertCase(db)
      db.prepare(
        `INSERT INTO personal_case_events (case_id, case_version, actor, event_type, new_status, created_at)
         VALUES ('case-1', 1, 'marveen', 'CREATED', 'NEW', @t)`
      ).run({ t: nowSec() })
      return db.prepare(`SELECT event_id FROM personal_case_events WHERE case_id='case-1'`).get() as any
    }
    ...
    it('blocks UPDATE via trigger', () => {
      ...
      const ev = addEvent(db)
      expect(() =>
        db.prepare(`UPDATE personal_case_events SET reason='tampered' WHERE event_id=?`).run(ev.event_id)
    ...
    it('blocks DELETE via trigger', () => {
      ...
      const ev = addEvent(db)
      expect(() =>
        db.prepare(`DELETE FROM personal_case_events WHERE event_id=?`).run(ev.event_id)
      ).toThrow(/append-only/i)
    })
  })
  ...
  describe('case_claims (P0.2 fencing / P0.3 UNIQUE / atomic upsert)', () => {
    ...
    function claim(db: any, owner: string, now: number, ttl = 100) {
      db.prepare(CLAIM_UPSERT).run({ key: 'case-1', owner, now, expires: now + ttl })
      return db.prepare(`SELECT owner_run_id, claim_fence FROM case_claims WHERE claim_key='case-1'`).get() as any
    }
