...
describe('accordion guard (card 9193eedd follow-up)', () => {
  it('clicks inside .cos-owner-ctrl do not toggle the tile', () => {
    ...
    container.addEventListener('click', ((e: MouseEvent) => {
      ...
      if ((e.target as HTMLElement).closest?.('.cos-owner-ctrl')) return
      const t = (e.target as HTMLElement).closest?.('.cos-case-tile') as HTMLElement | null
      if (!t) return
      const d = t.querySelector('.cos-case-detail') as HTMLElement | null
      if (!d) return
      toggled = true
      d.hidden = !d.hidden
    }) as EventListener)
