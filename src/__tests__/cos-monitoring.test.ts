...
describe('COS monitoring view (listMonitoring)', () => {
  ...
  it('rolls up connector health, outbound statuses, needs-attention rows, and quotas', () => {
    ...
    expect((mon.connectors as any[]).find((c) => c.connector_id === 'rental').status).toBe('DOWN')
    // outbound roll-up
    ...
    expect((mon.outboundHealth.needsAttention[0] as any).status).toBe('RECOVERY_REQUIRED')
    expect((mon.outboundHealth.needsAttention[0] as any).last_error).toBe('marker absent')
    // quotas
    expect((mon.quotas[0] as any)).toMatchObject({ quota_key: 'EMAIL_SEND:daily', used_count: 4, max_count: 10 })
  })
