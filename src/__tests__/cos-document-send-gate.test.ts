...
import { GmailSendAdapter, type MailTransport, type OutboundEmail } from '../cos/adapters/gmail-send.js'
...
function storeDoc(shareable: boolean) {
  const db = getDb()
  ...
  if (shareable) setDocumentShareable(db, r.documentId, true, 'ZST_INTERNAL', T0)
  return r.documentId
}

// A capturing transport so the test sees exactly what would be sent.
function captureTransport(): { t: MailTransport; sent: OutboundEmail[] } {
...
function action(payload: unknown) {
  return { ledgerId: 'L1', externalIdempotencyMarker: 'MARK-1', payload } as never
}
...
describe('P4 document send gate', () => {
  ...
  it('setDocumentShareable is an explicit, auditable clearance (default stays blocked)', () => {
    const db = getDb()
    const id = storeDoc(false)
    const row0 = db.prepare(`SELECT external_share_allowed FROM cos_documents WHERE document_id=?`).get(id) as any
    expect(row0.external_share_allowed).toBe(0)
    setDocumentShareable(db, id, true, 'ZST_INTERNAL', T0)
    const row1 = db.prepare(`SELECT external_share_allowed, sensitivity FROM cos_documents WHERE document_id=?`).get(id) as any
