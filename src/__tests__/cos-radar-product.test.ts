...
import { runProductRadarCheck, minorToMajor, type ProductRadarQuery } from '../cos/radar-runner.js'
import type { ShoppingAdapter, ProductSearchResult } from '../cos/shopping-adapter.js'
...
const p = (id: string, name: string, priceMinor: number | null, currency = 'HUF'): ProductSearchResult =>
  ({ ref: { adapterId: 'mock', productId: id }, name, priceMinor, currency, available: true })
...
describe('PRODUCT radar price-watch engine', () => {
  ...
  it('picks the cheapest matching offer and records an observation', async () => {
    ...
    const res = await runProductRadarCheck(getDb(), getRadarItem(getDb(), 'BUY-TEST')!,
      mockAdapter([p('a', 'On Cloud 6 white', 52000), p('b', 'On Cloud 6 midnight', 47000)]), T0 + 1)
    expect(res.bestPrice).toBe(47000)
    expect(res.hit).toBe(false) // 47000 > 45000 target
    const obs = getDb().prepare(`SELECT COUNT(*) n FROM radar_observations WHERE radar_id='BUY-TEST'`).get() as any
    expect(obs.n).toBe(1)
  })
  ...
  it('flips to HIT and notifies when the target is met', async () => {
    makeItem()
    const res = await runProductRadarCheck(getDb(), getRadarItem(getDb(), 'BUY-TEST')!,
      mockAdapter([p('b', 'On Cloud 6 midnight', 44000)]), T0 + 1)
  ...
  it('skips offers in a different currency (no silent FX)', async () => {
    makeItem()
    const res = await runProductRadarCheck(getDb(), getRadarItem(getDb(), 'BUY-TEST')!,
      mockAdapter([p('e', 'On Cloud 6', 120, 'EUR'), p('h', 'On Cloud 6', 44000, 'HUF')]), T0 + 1)
  ...
  it('records an honest 0-offer observation (no fake price) when nothing matches', async () => {
    ...
    const obs = getDb().prepare(`SELECT best_price, offer_count FROM radar_observations WHERE radar_id='BUY-TEST'`).get() as any
