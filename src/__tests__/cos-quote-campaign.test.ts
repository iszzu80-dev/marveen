...
import {
  ensureQuoteSchema, createQuoteCampaign, approveShortlist, transitionQuote,
  recordQuote, compareQuotes, chooseProvider, getQuoteCampaign, QUOTE_TRANSITIONS,
} from '../cos/quote-campaign.js'
...
function seed(state?: string) {
  ...
  if (state) {
    ...
    if (state !== 'SHORTLIST_READY') {
      ...
      if (state !== 'REQUESTS_SENDING') transitionQuote(db, 'q1', 'WAITING_EXTERNAL', NOW - 70)
    }
  }
  return db
}

const quote = (i: number, amount: number) => ({
...
describe('quote-request campaign (§13.1)', () => {
  ...
  it('every state can be cancelled except the terminal ones', () => {
    for (const [from, to] of Object.entries(QUOTE_TRANSITIONS)) {
      if (from === 'COMPLETED' || from === 'CANCELLED') expect(to).toEqual([])
      else expect(to, from).toContain('CANCELLED')
    }
  })
