...
function seed(rung: 'PREPARE' | 'EXECUTE_WITH_APPROVAL' = 'EXECUTE_WITH_APPROVAL') {
  initDatabase(':memory:')
  const db = getDb()
...
describe('outbound approval door', () => {
  ...
  it('approving records an approval bound to the campaign version', () => {
    ...
    const a = db.prepare(
      `SELECT campaign_id, campaign_version, status, allowed_recipients FROM campaign_approvals`
    ).get() as { campaign_id: string; campaign_version: number; status: string; allowed_recipients: string }
