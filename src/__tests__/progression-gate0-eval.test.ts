...
const ALL_CORPUS: CorpusCase[] = [...PRI_CORPUS, ...ZST_CORPUS]
...
describe('Gate 0 — Progression foundation schema + eval harness (card 89c5e317)', () => {
  ...
  it('creates case_progression_state table with all required columns', () => {
    ...
    const pcCols = db.prepare('PRAGMA table_info(personal_cases)').all() as Array<{ name: string }>
  ...
  it('creates required indexes on progression tables', () => {
    const indexes = db.prepare(
      "SELECT name FROM sqlite_master WHERE type='index' AND (name LIKE '%cps%' OR name LIKE '%cpruns%')",
  ...
  it('enforces progression_mode CHECK constraint', () => {
    expect(() =>
      db.prepare(
        `INSERT INTO case_progression_state (domain, case_id, progression_mode, created_at, updated_at)
         VALUES ('personal', 'test-2', 'invalid_mode', 1, 1)`,
      ).run(),
    ).toThrow()
  })
  ...
  it('progresses all 50 real corpus cases through stub harness with zero safety violations', () => {
    ...
    const stateCount = db.prepare('SELECT count(*) as c FROM case_progression_state').get() as { c: number }
    expect(stateCount.c).toBe(50)

    // Verify run rows created (one per case)
    const runCount = db.prepare('SELECT count(*) as c FROM case_progression_runs').get() as { c: number }
    ...
    const runsWithAssertions = db.prepare(
      'SELECT count(*) as c FROM case_progression_runs WHERE safety_assertions_json IS NOT NULL',
    ).get() as { c: number }
  ...
  it('existing personal_cases and zst_cases tables are untouched (Option B invariant)', () => {
    ...
    const zcCols = db.prepare('PRAGMA table_info(zst_cases)').all() as Array<{ name: string }>
  ...
  it('RED-PROOF: premature_completion assertion catches deliberately-broken completion', () => {
    ...
    const redRun = db.prepare(
      "SELECT * FROM case_progression_runs WHERE case_id = 'RED-PROOF-FAKE-001'",
    ).get() as { status: string; error_code: string | null; safety_assertions_json: string } | undefined
    ...
    const prem = parsed.find((a: { assertion: string; passed: boolean }) => a.assertion === 'premature_completion')
  ...
  it('progression_enabled defaults to 0 (OFF) for newly created state rows', () => {
    ...
    const col = (db.prepare('PRAGMA table_info(case_progression_state)').all() as Array<{ name: string; dflt_value: string | null }>)
