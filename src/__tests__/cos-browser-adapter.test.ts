...
import { assessBrowserAdapter, REQUIRED_BROWSER_METHODS, type BrowserExecuteAdapter } from '../cos/browser-adapter.js'
...
function fullAdapter(readback: BrowserExecuteAdapter['readback']): BrowserExecuteAdapter {
  return {
...
describe('browser adapter capability (P1.4)', () => {
  it('requires all six methods', () => {
    expect([...REQUIRED_BROWSER_METHODS].sort()).toEqual(
      ['compensateOrCancel', 'dedupeKey', 'execute', 'prepare', 'readback', 'verify'].sort())
  })
