...
import {
  createCase, getCase, transitionCase, appendCaseEvent,
  acquireClaim, releaseClaim, CaseConcurrencyError,
...
describe('COS case-store commands', () => {
  ...
  function events(caseId: string) {
    return getDb().prepare(
      `SELECT event_type, previous_status, new_status, case_version FROM personal_case_events
  ...
  describe('transitionCase (optimistic concurrency P0.5)', () => {
    ...
    it('a stale writer throws CaseConcurrencyError and does NOT clobber (no phantom event)', () => {
      ...
      expect(() =>
        transitionCase(db, { caseId: 'c1', seenVersion: 1, newStatus: 'BLOCKED', actor: 'B' }, T0 + 2)
    ...
    it('throws a plain Error for a nonexistent case', () => {
      const db = getDb()
      expect(() =>
        transitionCase(db, { caseId: 'ghost', seenVersion: 1, newStatus: 'TRIAGE', actor: 'x' }, T0)
    ...
    it('rejects an out-of-set status at the DB CHECK even through the command', () => {
      ...
      createCase(db, { caseId: 'c1', title: 't', caseType: 'X' }, T0)
      expect(() =>
        transitionCase(db, { caseId: 'c1', seenVersion: 1, newStatus: 'DONE' as any, actor: 'x' }, T0)
  ...
  describe('appendCaseEvent', () => {
    it('JSON-encodes the payload and is append-only (trigger blocks tampering)', () => {
      ...
      const row: any = db.prepare(`SELECT payload FROM personal_case_events WHERE event_id = ?`).get(id)
      expect(JSON.parse(row.payload)).toEqual({ k: 'v', n: 3 })
      expect(() =>
        db.prepare(`UPDATE personal_case_events SET reason='x' WHERE event_id=?`).run(id)
