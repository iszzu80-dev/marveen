...
import {
  interpretGoal,
  interpretGoalDomainScoped,
  AnthropicLlmClient,
  type LlmClient,
  type GoalInterpretation,
} from '../cos/progression-interpreter.js'
...
class MockLlmClient implements LlmClient {
  ...
  constructor(response: string | GoalInterpretation) {
...
function mockOk(title = 'Test Case Title', summary = 'Test summary.', goal = 'Resolve the issue.'): MockLlmClient {
  return new MockLlmClient({ title, summary, goal })
}

/** Create a mock that returns raw text (simulating garbled LLM output). */
function mockRaw(text: string): MockLlmClient {
...
function seedPersonalCase(
  ...
  db.prepare(
    `INSERT INTO personal_cases (case_id, title, case_type, description, status, sensitivity, created_at, updated_at)
     VALUES (?, ?, ?, ?, 'NEW', 'PERSONAL', ?, ?)`,
  ).run(caseId, title, caseType, description, now, now)
}
...
function seedZstCase(
  ...
  const now = Math.floor(Date.now() / 1000)
  db.prepare(
    `INSERT INTO zst_cases (case_id, title, case_type, description, status, sensitivity, created_at, updated_at)
...
`
      ...
      await expect(
        interpretGoal(client, 'Invoice', 'INVOICE_INCOMING', null, injectedEmailContent),
      ).rejects.toThrow('injection marker')
    })
...
Due date: 2026-08-15.
  ...
  describe('interpretGoalDomainScoped()', () => {
    ...
    it('succeeds when case is in the claimed domain (personal)', async () => {
      ...
      const result = await interpretGoalDomainScoped(
        client, db, 'personal', 'pri-001',
        'Test case', 'ADMIN', null, '',
      )
    ...
    it('RED-PROOF: throws CrossDomainReadError when personal resolver reads ZST case', async () => {
      ...
      await expect(
        interpretGoalDomainScoped(
          client, db, 'personal', 'zst-cross-001',
          'Test', 'ADMIN', null, '',
        ),
    ...
    it('returns successfully when case exists in neither domain (domainGuard only blocks cross-domain reads)', async () => {
      ...
      const result = await interpretGoalDomainScoped(
        client, db, 'personal', 'nonexistent-case',
        'Test', 'ADMIN', null, '',
      )
  ...
  describe('enrichCaseGoal()', () => {
    ...
    it('interprets and writes goal + summary to case_progression_state on first run', async () => {
      ...
      const client = mockOk(
        ...
        'An external invoice needs to be paid for project services.',
      ...
      const row = db.prepare(
        'SELECT goal, summary, goal_version FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'pri-001') as { goal: string; summary: string; goal_version: number }
    ...
    it('is idempotent — second run is a no-op (lazy enrichment)', async () => {
      ...
      const row = db.prepare(
        'SELECT goal, summary, goal_version FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'pri-002') as { goal: string; summary: string; goal_version: number }
    ...
    it('writes to ZST domain correctly', async () => {
      ...
      const client = mockOk(
        ...
        'A partner is inquiring about potential collaboration.',
      ...
      ).get('zst', 'zst-001') as { goal: string; summary: string }
    ...
    it('passes email thread content to interpretGoal', async () => {
      ...
      const captureClient: LlmClient = {
        async complete(_system: string, userMessage: string): Promise<string> {
  ...
  describe('runProgressionCycle() with enriched goal', () => {
    ...
    it('uses enriched goal from DB instead of heuristic when available', () => {
      ...
      db.prepare(
        `INSERT INTO case_progression_state
         ...
         VALUES (?, ?, ?, ?, 0, 'shadow', 1, 1, ?, ?)`,
      ).run('personal', 'pri-010', 'Pay the Q3 supplier invoice by August 15', 'Supplier invoice for Q3 services needs payment.', now, now)
      ...
      const row = db.prepare(
        'SELECT goal FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'pri-010') as { goal: string }
    ...
    it('falls back to heuristic when no enriched goal exists', () => {
      ...
      const row = db.prepare(
        'SELECT goal FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'pri-011') as { goal: string }
    ...
    it('preserves summary on UPDATE when already set', () => {
      ...
      db.prepare(
        `INSERT INTO case_progression_state
         (domain, case_id, goal, summary, progression_enabled, progression_mode,
          case_version, goal_version, created_at, updated_at)
         VALUES (?, ?, ?, ?, 0, 'shadow', 1, 1, ?, ?)`,
      ).run('personal', 'pri-012', 'Original goal', 'Original summary.', now, now)
      ...
      const row = db.prepare(
        'SELECT goal, summary FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'pri-012') as { goal: string; summary: string | null }
    ...
    it('sets summary to NULL on INSERT when no enrichment exists', () => {
      ...
      const row = db.prepare(
        'SELECT summary FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'pri-013') as { summary: string | null }
    ...
    it('enriched goal survives multiple progression runs', () => {
      ...
      db.prepare(
        `INSERT INTO case_progression_state
         (domain, case_id, goal, summary, progression_enabled, progression_mode,
          case_version, goal_version, created_at, updated_at)
         VALUES (?, ?, ?, ?, 0, 'shadow', 1, 1, ?, ?)`,
      ).run('personal', 'pri-014', enrichedGoal, 'Monthly electricity bill payment.', now, now)
      ...
      const row = db.prepare(
        'SELECT goal, case_version FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'pri-014') as { goal: string; case_version: number }
  ...
  describe('Schema — summary column', () => {
    ...
    it('ensureColumns adds summary to existing table without the column', () => {
      ...
      const now = Math.floor(Date.now() / 1000)
      db.prepare(
        `INSERT INTO case_progression_state (domain, case_id, summary, created_at, updated_at)
         VALUES ('personal', 'test-1', 'A test summary.', ?, ?)`,
      ).run(now, now)

      const row = db.prepare(
        'SELECT summary FROM case_progression_state WHERE domain = ? AND case_id = ?',
      ).get('personal', 'test-1') as { summary: string }
  ...
  describe('Live acceptance — real cryptic-subject cases (one PRI, one ZST)', () => {
    it('produces meaningfully better title for real PRI case with cryptic subject', async (ctx) => {
      ...
      const crypticCase = liveDb.prepare(
        `SELECT case_id, title, case_type, description
         FROM personal_cases
      ...
      const docs = liveDb.prepare(
        `SELECT extracted_text FROM cos_documents
         WHERE namespace = 'personal' AND case_id = ? AND extracted_text IS NOT NULL
         LIMIT 5`,
      ).all(crypticCase.case_id) as Array<{ extracted_text: string }>
