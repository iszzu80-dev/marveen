...
describe('rejected-candidate regressions', () => {
  ...
  it('59 post-lease setup failure records LAUNCH_ERROR, releases, and remains retryable', async () => { const s = new ReviewJobStore(root); const failed = await runReview({ request: request(), repoRoot: root, store: s, verifyCheckout: noCheckoutVerification, setupAttempt: () => { throw new Error('disk setup exploded') }, launcher: writes(result()) }); expect(failed.metadata.status).toBe('LAUNCH_ERROR'); expect(failed.metadata.failure).toContain('attempt setup failed'); expect(existsSync(s.paths('job-1').lease)).toBe(false); const retry = await runReview({ request: request(), repoRoot: root, store: s, verifyCheckout: noCheckoutVerification, launcher: writes(result()) }); expect(retry.metadata.attempt).toBe(2); expect(retry.result?.verdict).toBe('PASS') })
})
