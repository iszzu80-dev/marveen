...
import {
  ensureLadderSchema, getLadder, setLadder, permits, pauseAll,
  promotionEligibility, recordCampaignOutcome, fleetLevelFor,
  PROMOTION_THRESHOLD, RUNGS,
} from '../cos/autonomy-ladder.js'
...
describe('COS autonomy ladder', () => {
  ...
  describe('the two rungs the fleet 1-3 switch cannot express', () => {
    ...
    it('both map onto fleet level 2 and 3 — one truth, one mirror', () => {
      ...
      for (const r of RUNGS) expect([1, 2, 3]).toContain(fleetLevelFor(r))
    })
  })
  ...
  describe('promotion is earned and capped', () => {
    it('needs the threshold before it is even eligible', () => {
      ...
      for (let i = 0; i < PROMOTION_THRESHOLD; i++) recordCampaignOutcome(d, 'QUOTE', true, NOW + i)
    ...
    it('eligibility does NOT raise the rung — the ladder never climbs itself', () => {
      setLadder(d, 'QUOTE', { rung: 'PREPARE', ceiling: 'LIMITED_AUTONOMOUS' }, NOW)
      for (let i = 0; i < PROMOTION_THRESHOLD + 5; i++) recordCampaignOutcome(d, 'QUOTE', true, NOW + i)
    ...
    it('the owner ceiling stops promotion however good the record is', () => {
      ...
      for (let i = 0; i < 50; i++) recordCampaignOutcome(d, 'FINANCE', true, NOW + i)
  ...
  it('every refusal carries a code and a reason', () => {
    ...
    for (const a of ['OBSERVE', 'DRAFT', 'SEND', 'PAYMENT'] as const) {
