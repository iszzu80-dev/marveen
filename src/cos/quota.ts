...
interface Row { window_start: number; window_sec: number; max_count: number; used_count: number }
...
export function reserveQuota(
  db: Database.Database, quotaKey: string, maxCount: number, windowSec: number, now: number,
): ReserveResult {
  const tx = db.transaction((): ReserveResult => {
    const row = db.prepare(`SELECT window_start, window_sec, max_count, used_count FROM send_quotas WHERE quota_key = ?`).get(quotaKey) as Row | undefined

    // No row, or the window has rolled over → start a fresh window at count 1.
    if (!row || now >= row.window_start + row.window_sec) {
      if (maxCount < 1) return { reserved: false, remaining: 0, windowStart: now }
      db.prepare(
        `INSERT INTO send_quotas (quota_key, window_start, window_sec, max_count, used_count, updated_at)
         VALUES (@key, @now, @windowSec, @max, 1, @now)
         ON CONFLICT(quota_key) DO UPDATE SET window_start=@now, window_sec=@windowSec, max_count=@max, used_count=1, updated_at=@now`
      ).run({ key: quotaKey, now, windowSec, max: maxCount })
...
export function releaseQuota(db: Database.Database, quotaKey: string, now: number): void {
  const row = db.prepare(`SELECT window_start, window_sec, used_count FROM send_quotas WHERE quota_key = ?`).get(quotaKey) as
    | { window_start: number; window_sec: number; used_count: number } | undefined
  if (!row || now >= row.window_start + row.window_sec || row.used_count <= 0) return
  db.prepare(`UPDATE send_quotas SET used_count = used_count - 1, updated_at = @now WHERE quota_key = @key`).run({ key: quotaKey, now })
}

export function quotaUsage(db: Database.Database, quotaKey: string): { used: number; max: number } | undefined {
  const row = db.prepare(`SELECT used_count, max_count FROM send_quotas WHERE quota_key = ?`).get(quotaKey) as { used_count: number; max_count: number } | undefined
