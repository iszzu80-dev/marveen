...
export interface CampaignRow {
  ...
  autonomous_spend_limit: number
}

export function createCampaign(db: Database.Database, c: NewCampaign, now: number): CampaignRow {
  db.prepare(
    `INSERT INTO campaigns (campaign_id, case_id, campaign_type, template_id, template_hash,
        status, version, allows_free_text, autonomous_spend_limit, created_at, updated_at)
     VALUES (@campaignId, @caseId, @campaignType, @templateId, @templateHash, 'DRAFT', 1, @free, 0, @now, @now)`
  ).run({
...
export function getCampaign(db: Database.Database, campaignId: string): CampaignRow | undefined {
  return db.prepare(`SELECT * FROM campaigns WHERE campaign_id = ?`).get(campaignId) as CampaignRow | undefined
}

/** Move DRAFT → APPROVED (the campaign itself is greenlit; individual sends
 *  still need a matching approval row via authorizeSend). */
export function approveCampaign(db: Database.Database, campaignId: string, now: number): void {
  ...
  if (info.changes === 0) throw new Error(`campaign not DRAFT (or missing): ${campaignId}`)
}
...
export function revokeCampaign(db: Database.Database, campaignId: string, seenVersion: number, now: number): number {
  return setCampaignStatusVersioned(db, campaignId, 'REVOKED', seenVersion, now)
}
export function pauseCampaign(db: Database.Database, campaignId: string, seenVersion: number, now: number): number {
  return setCampaignStatusVersioned(db, campaignId, 'PAUSED', seenVersion, now)
}
function setCampaignStatusVersioned(db: Database.Database, campaignId: string, status: CampaignStatus, seenVersion: number, now: number): number {
  const info = db.prepare(
    `UPDATE campaigns SET status=@status, version=version+1, updated_at=@now WHERE campaign_id=@id AND version=@seen`
  ).run({ status, now, id: campaignId, seen: seenVersion })
  if (info.changes === 0) throw new Error(`campaign ${campaignId}: version ${seenVersion} is stale or campaign missing`)
...
export function resumeCampaign(db: Database.Database, campaignId: string, now: number): void {
