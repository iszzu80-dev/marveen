...
export interface ExtractedContract extends ZstContractInput {
  confidence: 'HIGH' | 'PARTIAL' | 'LOW'
  extracted: string[]  // which fields were parsed
}

export interface ContractEmailSource {
  caseId?: string
  from: string
  subject: string
  body: string
...
function parseExpiryNear(text: string): string | null {
...
export function contractDuplicateHash(i: ZstContractInput): string {
  const key = [i.counterpartyId ?? '', i.title, i.expiryDate ?? ''].join('|')
  return createHash('sha256').update(key).digest('hex').slice(0, 32)
}

export interface UpsertContractResult { contractId: string; duplicate: boolean }
...
export function upsertZstContract(db: Database.Database, input: ZstContractInput, now: number): UpsertContractResult {
  const dupHash = contractDuplicateHash(input)
  const existing = db.prepare(
    `SELECT contract_id FROM zst_contracts WHERE counterparty_id IS ? AND title = ? AND expiry_date IS ?`
  ).get(input.counterpartyId ?? null, input.title, input.expiryDate ?? null) as { contract_id: string } | undefined
  if (existing) return { contractId: existing.contract_id, duplicate: true }

  const contractId = input.contractId ?? `zst-ctr-${dupHash.slice(0, 12)}`
  db.prepare(
    `INSERT INTO zst_contracts
...
export function ingestZstContractEmail(
  db: Database.Database, src: ContractEmailSource, now: number,
): { contractId: string; duplicate: boolean; confidence: string; extracted: string[] } | null {
