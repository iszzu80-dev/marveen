...
const SENSITIVITY_RANK: Record<CaseSensitivity, number> = {
  PUBLIC: 0,
  PERSONAL: 1,
...
export const MOST_SENSITIVE: CaseSensitivity = 'HIGHLY_SENSITIVE'

// Coerce any value into a known tier, FAIL-CLOSED to HIGHLY_SENSITIVE.
export function coerceSensitivity(value: unknown): CaseSensitivity {
  return typeof value === 'string' && (CASE_SENSITIVITIES as readonly string[]).includes(value)
    ? (value as CaseSensitivity)
    : MOST_SENSITIVE
}

// Return the MORE sensitive of two tiers (the escalation rule).
export function escalateSensitivity(a: CaseSensitivity, b: CaseSensitivity): CaseSensitivity {
...
const SENSITIVE_PERSONAL_PATTERNS: SensitivityPattern[] = [
...
const PERSONAL_PATTERNS: SensitivityPattern[] = [
  { name: 'email_address', pattern: '[\\w.+-]+@[\\w.-]+\\.[\\w]{2,}', description: 'Email address (personal contact)' },
]

export interface PersonalClassification {
  tier: CaseSensitivity
  matched: string[]
}
...
export function classifyPersonalSensitivity(content: string): PersonalClassification {
  const highly = matchSensitivityPatterns(content, HIGHLY_SENSITIVE_PATTERNS)
  if (highly.length > 0) return { tier: 'HIGHLY_SENSITIVE', matched: highly }

  const sensitive = matchSensitivityPatterns(content, SENSITIVE_PERSONAL_PATTERNS)
  if (sensitive.length > 0) return { tier: 'SENSITIVE_PERSONAL', matched: sensitive }

  const personal = matchSensitivityPatterns(content, PERSONAL_PATTERNS)
  if (personal.length > 0) return { tier: 'PERSONAL', matched: personal }
...
export function effectiveSensitivity(declared: unknown, content: string): CaseSensitivity {
...
const PROFILE_ALLOWLIST: Record<CaseSensitivity, ReadonlySet<ModelProfileId>> = {
  ...
  SENSITIVE_PERSONAL: new Set<ModelProfileId>(['premium_reasoning', 'build_strong']),
  HIGHLY_SENSITIVE: new Set<ModelProfileId>(['premium_reasoning']),
}
...
export function isProfileAllowedForSensitivity(profile: string, tier: unknown): boolean {
  const t = coerceSensitivity(tier)
  const allowed = PROFILE_ALLOWLIST[t]
  return allowed.has(profile as ModelProfileId)
}

// The set of profiles allowed for a tier (fail-closed to the HIGHLY_SENSITIVE set).
export function allowedProfilesFor(tier: unknown): ReadonlySet<ModelProfileId> {
  return PROFILE_ALLOWLIST[coerceSensitivity(tier)]
}

export type PersonalGateVerdict = 'allow' | 'block'

export interface PersonalGateResult {
  verdict: PersonalGateVerdict
  tier: CaseSensitivity
  matched: string[]
  reason: string
}
...
// throughout.
export function evaluatePersonalDispatch(
  content: string,
