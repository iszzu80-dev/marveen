...
export function buildCaseContext(
  ...
  // 1b. THE DOSSIER ITSELF: what this case is made of, and WHY each piece is
  //    part of it.
  //
  ...
  // PLACED BEFORE the events and the documents, deliberately. The packet has a
  // 40-item bound, and when the pool case's documents arrived through the graph
  // the relation summary -- the cheapest, most structural thing in the packet,
  // the part that says this case owns TWO threads and on what evidence -- was
  // the half that fell off the end. It was named in `excluded`, so nothing lied,
  // but a map that gets dropped whenever the territory grows is a map nobody
  // reads. Twenty engine events do not outrank knowing what the case is made of.
