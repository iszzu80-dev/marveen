...
import { loadBillingMap, resolveBillingMode, type BillingMap, type BillingMode } from './dispatch.js'
...
import type { ModelResolution } from '../model-profiles.js'

/** The six identity columns of a `dispatches` row, as a stampable bag. */
 ...
 */
export const UNRESOLVED_DISPATCH_IDENTITY: DispatchIdentity = {
  modelProfile: null,
...
/**
 * Origin-supplied values that override what the per-AGENT resolvers would say.
 * Exists for exactly one real case: the agent-worker runs as a SEPARATE Claude
 * Code session (its own isolated CLAUDE_CONFIG_DIR, its own --model) while its
 * dispatch row is stamped with the MAIN agent id. Resolving its identity from
 * that agent id would attribute the main pane's model/login to worker requests
 * -- the same mis-attribution the session_id resolver already avoids.
 */
export interface DispatchIdentityOverrides {
  /** The exact model id the origin launched with (worker: WORKER_MODEL). */
  configuredModel?: string | null
  /** The exact CLAUDE_CONFIG_DIR the origin launched with (worker: ctx.configDir). */
  configDir?: string | null
}

/**
 * Injected seams, so the resolver is testable without the real fs and so there
 * is exactly ONE place that names each upstream resolver. Mirrors the
 * aliasResolver injection in model-profiles.ts and the roots injection in
 * transcript-sources.ts.
 */
export interface DispatchIdentityDeps {
  resolveModel: (agent: string) => ModelResolution
  readModelProfile: (agent: string) => string | null
  resolveConfigDir: (agent: string) => {
    configDir: string | null
    planId: string | null
    planUnresolved: boolean
  }
  deriveProvider: (model: string | null | undefined) => string
  loadBillingMap: () => BillingMap | null
  homeDir: string
}
 ...
 */
export function defaultDispatchIdentityDeps(): DispatchIdentityDeps {
  return {
 ...
 */
export function configDirIdentifier(configDir: string, homeDir: string): string {
  if (configDir === homeDir) return 'home'
  if (homeDir && configDir.startsWith(homeDir + sep)) return configDir.slice(homeDir.length + 1)
...
export function resolveAuthProfile(
  agent: string,
  deps: Pick<DispatchIdentityDeps, 'resolveConfigDir' | 'homeDir'>,
): string {
  ...
  if (!configDir) return HOST_DEFAULT_AUTH_PROFILE
 ...
 */
export function resolveDispatchIdentity(
  agent: string,
  overrides: DispatchIdentityOverrides = {},
  deps: DispatchIdentityDeps = defaultDispatchIdentityDeps(),
): DispatchIdentity {
  ...
  if (overrides.configuredModel) {
    ...
    if (resolution.source === 'model_profile') {
      ...
      logger.warn(
        { agent, modelProfileError: resolution.error, model: resolution.model },
        'dispatch identity: agent names a model profile that could not be honoured; model_profile stays NULL',
      )
    }
  }
 ...
 */
export function resolveDispatchIdentitySafe(
  agent: string,
  overrides: DispatchIdentityOverrides = {},
  resolve: (
    agent: string,
    overrides: DispatchIdentityOverrides,
    deps?: DispatchIdentityDeps,
  ) => DispatchIdentity = resolveDispatchIdentity,
): DispatchIdentity {
  try {
    ...
    logger.warn(
      { err, agent },
      'resolveDispatchIdentity failed; dispatch stamped un-attributed (send unaffected)',
    )
