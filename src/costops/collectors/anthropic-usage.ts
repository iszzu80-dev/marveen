...
import { loadSubscriptionsConfig, type SubscriptionsConfig } from '../subscriptions.js'
import { writeRateLimitSnapshot } from '../capacity-snapshots.js'

export const ANTHROPIC_USAGE_COLLECTOR = 'anthropic-usage-snapshot'

export interface AnthropicUsageSyncResult {
  ok: boolean
  provider: 'anthropic'
  status: 'ok' | 'skipped'
  imported_count: number
  /** Precise, non-secret reason when nothing was imported. */
  blocker: string | null
  /** Which subscription ids contributed a snapshot. */
 ...
 */
export function syncAnthropicUsageSnapshot(
  db: Database.Database,
  now: number,
  deps: { config?: SubscriptionsConfig } = {},
): AnthropicUsageSyncResult {
