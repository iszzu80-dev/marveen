...
import type { ApgMode, ApgUiSummary } from './ui-types.js'
import {
  buildApgUiSummary,
  resolveApgKernelDbPath,
...
export function getCachedApgSummary(mode: ApgMode): ApgUiSummary {
  ...
  if (
    ...
    if (summaryCache.dbMtimeMs === null) return summaryCache.value
    const currentMtimeMs = readDbMtimeMs()
    if (currentMtimeMs === summaryCache.dbMtimeMs) return summaryCache.value
  }
...
export function invalidateApgCache(): void {
  summaryCache = null
}
