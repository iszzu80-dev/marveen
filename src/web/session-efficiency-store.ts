...
export interface SessionEfficiencyConfig {
  saturation: SaturationThresholds
  taskSizePolicy: TaskSizePolicy
  /** Per-agent overrides. An absent agent simply uses the fleet values. */
  agents: Record<string, { saturation: SaturationThresholds; agentDefaultTaskSize: TaskSize }>
}
...
export function normalizeSessionEfficiencyConfig(raw: unknown): SessionEfficiencyConfig {
  ...
  const agents: SessionEfficiencyConfig['agents'] = {}
  if (o.agents && typeof o.agents === 'object') {
    for (const [name, entry] of Object.entries(o.agents as Record<string, unknown>)) {
      ...
      const saturation = 'saturation' in e
        ? normalizeSaturationThresholds({ ...fleetSaturation, ...(e.saturation as object ?? {}) })
        : fleetSaturation
      const declared = e.agentDefaultTaskSize
      const agentDefaultTaskSize: TaskSize =
        (declared === 'small' || declared === 'normal' || declared === 'large')
          ? declared
...
export function readSessionEfficiencyConfig(path: string = SESSION_EFFICIENCY_PATH): SessionEfficiencyConfig {
...
export function readSaturationThresholdsFor(agent: string, path: string = SESSION_EFFICIENCY_PATH): SaturationThresholds {
...
export function readAgentDefaultTaskSize(agent: string, path: string = SESSION_EFFICIENCY_PATH): TaskSize {
  const cfg = readSessionEfficiencyConfig(path)
