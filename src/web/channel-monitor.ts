...
// Lazy, not module-level consts. `resolveFromPath(...)` at import time is the
// anti-pattern platform.ts names by hand three lines above makeLazyBinResolver:
// it throws during IMPORT, so a transient PATH gap takes the whole dashboard --
// and the scheduler living inside it -- down at boot.
//
// It also made this module, and the ~20 files that import it transitively,
// unimportable on any machine without the claude CLI installed. The first CI
// run outside the author's laptop (2026-08-14) failed exactly there:
//
//     Error: Required binary not found on PATH: claude
//       at Module.resolveFromPath src/platform.ts:58
//       at src/web/channel-monitor.ts:57
//
// The resolver this replaces it with was already written, already memoised and
// already re-validates a stale cache. It just was not used here.
...
export function startChannelPluginMonitor(): NodeJS.Timeout | null {
  ...
  async function check() {
    ...
    if (shouldRunPeriodicReap(lastDetachedReapAt, Date.now(), DETACHED_REAP_INTERVAL_MS)) {
      ...
      try {
        const reaped = await reapDetachedChannelClaudes({ tmuxPath: tmuxBin() })
        if (reaped.length > 0) {
