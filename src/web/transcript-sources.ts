...
/**
 * Injectable roots so both consumers -- and their tests -- can point the same
 * rule at a fixture tree instead of the caller's real homedir (the module-level
 * PROJECTS_DIR const is what made the old mapping effectively untestable).
 */
...
export function discoverAgentSources(roots: TranscriptRoots = {}): AgentTranscriptSource[] {
 ...
 */
function readSessionIdFromHead(filePath: string): string | null {
  ...
  try {
    ...
    if (nl < 0 && n === HEAD_BYTES) return null // first line longer than the peek
    const line = (nl >= 0 ? text.slice(0, nl) : text).trim()
    if (!line) return null
    const sid = (JSON.parse(line) as { sessionId?: unknown }).sessionId
 ...
 * local dir.
 */
export function resolveCurrentSessionId(agent: string, roots: TranscriptRoots = {}): string | null {
  try {
 ...
 */
export function resolveSessionIdForCwd(cwd: string, roots: TranscriptRoots = {}): string | null {
  try {
    if (!cwd) return null
