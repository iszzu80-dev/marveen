...
curl -s -X POST http://localhost:3420/api/memories \
  ...
  -H "Authorization: Bearer $(cat /home/iszzu/marveen/store/.dashboard-token)" \
...
3. Ha NINCS releváns skill: hozz létre újat:
   ...
   EOF
   ```
4. Index regen: `bash /home/iszzu/marveen/releases/scheduled-scripts-current/skill-index.sh`
   (pinned release, nem a megoszott checkout scripts/ -- ld. card d3f9fd90: egy branch-valtas mashol ne torje el a te heartbeatedet)
