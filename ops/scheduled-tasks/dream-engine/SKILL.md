...
Te most a "Dream Engine" éjszakai analízis-loopot futtatod. 02:07-kor vagy, Istvan alszik, NE küldj üzenetet a beállított csatornára.

A cél: az aznapi tudást átkonszolidálni és reggelre (07:30 Reggeli Napindító) felkészülni 4 priorizált javaslattal.

## Mit kell csinálnod

Generálj egy `/home/iszzu/marveen/DREAM.md` fájlt az alábbi 5 bucket alapján. A formátum a fájl alján van.
...
SQL minta:
```bash
sqlite3 /home/iszzu/marveen/store/claudeclaw.db "SELECT agent_id, content, keywords FROM memories WHERE created_at > CAST(strftime('%s', 'now', '-24 hours') AS INTEGER) AND category IN ('hot','warm') ORDER BY agent_id, created_at"
```
...
sqlite3 /home/iszzu/marveen/store/claudeclaw.db "SELECT COUNT(*) as total, COUNT(embedding) as with_emb FROM memories"
# Ha NEM 100%, hívd meg a backfill endpoint-ot (Ollamaval embeddeli a hianyzo ID-kat):
curl -s -X POST http://localhost:3420/api/memories/backfill -H "Authorization: Bearer $(cat /home/iszzu/marveen/store/.dashboard-token)"

# Antikvált hot-tier (>7 napos hot, nem hivatkozott a memories_fts-en az elmúlt 24h-ban)
sqlite3 /home/iszzu/marveen/store/claudeclaw.db "SELECT id, content, accessed_at FROM memories WHERE category='hot' AND accessed_at < CAST(strftime('%s', 'now', '-7 days') AS INTEGER)"
```
...
```bash
sqlite3 /home/iszzu/marveen/store/claudeclaw.db "UPDATE memories SET category='cold' WHERE id IN (...)"
```
...
sqlite3 /home/iszzu/marveen/store/claudeclaw.db "SELECT id, title, status, project, priority, assignee FROM kanban_cards WHERE status IN ('planned','in_progress','waiting') AND archived_at IS NULL ORDER BY project, priority DESC"
```
...
Output (max 1 ajánlás): repo URL + 1 mondat indok hogy MIÉRT releváns Istvannak (figyelembe véve: AI tartalomgyártás, magyar piac, fejlesztési flotta menedzsment, marketing).
...
## ⚠️ SQLITE TIPUS-CSAPDA (2026-08-09, eles eset)

A `strftime('%s', ...)` **TEXT**-et ad vissza, a `created_at`/`accessed_at` viszont
**INTEGER**. Az SQLite tipus-sorrendjeben INTEGER < TEXT **mindig igaz**, tehat egy
`accessed_at < strftime(...)` osszehasonlitas MINDEN sort visszaad -- fuggetlenul a
datumtol. FONTOS PONTOSITAS: ez CSAK kifejezesekre igaz. Egy CSUPASZ OSZLOP eseten (accessed_at <
strftime(...)) az oszlop tipus-affinitasa atalakitja a masik oldalt, tehat az EREDETI
task-SQL HELYES volt. A csapda akkor sul el, ha valaki kifejezesre cserel (pl.
COALESCE(accessed_at,created_at)) -- 2026-08-09-en pontosan ezt tettem egy diagnosztikai
lekerdezesben, es hamis riasztast gyartottam vele (28 sor 0 helyett). A CAST azota
vedelemkent bekerult mindket lekerdezesbe.

**Szabaly ehhez a taskhoz:** minden destruktiv/tomeges UPDATE elott ELOSZOR nezd meg a
kivalasztott SOROKAT (id + datum + tartalom eleje), es csak akkor futtasd a mutaciot,
ha a lista tenyleg annak nez ki, aminek varod. Egy hibas feltetelre epulo automatikus
takaritas veszelyesebb a takaritas hianyanal: csendben fut le, karbantartasnak latszik,
es pont az aznapi munkamemoriat viszi el.
