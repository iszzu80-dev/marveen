...
import { detectUnderusedSubscriptionRecommendation, detectDuplicateSubscriptionRecommendations, detectDuplicateHostingSaasRecommendations, detectAutomateLongManualSourceRecommendation, reconcileRecommendations, serializeEvidence, deserializeEvidence, } from './optimization.js';
...
// ---- signal gathering ------------------------------------------------------------------
function gatherUnderusedSubscriptionCandidates(now) {
    ...
    for (const ls of limitStatuses) {
        ...
        if (!sub || sub.amount == null || sub.status !== 'active')
            continue; // no known real cost -- never fabricated
...
function gatherAutomateLongManualSourceCandidates(db) {
    const out = [];
    const sources = db.prepare(`SELECT id, provider FROM cost_sources WHERE active = 1 AND lifecycle_state != 'decommissioned'`).all();
    ...
    `).all(s.id, AUTOMATION_LOOKBACK_PERIODS);
        if (periodRows.length < AUTOMATION_LOOKBACK_PERIODS)
            continue;
        let allManual = true;
        let latestCost = 0;
        periodRows.forEach((p, i) => {
            const lines = db.prepare(`SELECT billed_cost, confidence FROM cost_line_items WHERE source_id = ? AND charge_period_start = ? AND voided_at IS NULL`).all(s.id, p.charge_period_start);
            const resolved = lines.reduce((best, l) => (CONF_PRIORITY[l.confidence] || 0) > (CONF_PRIORITY[best.confidence] || 0) ? l : best, lines[0]);
            if (!resolved || !AUTOMATION_CONF.has(resolved.confidence))
                allManual = false;
            if (i === 0 && resolved)
                latestCost = resolved.billed_cost;
        });
        if (allManual) {
...
function gatherAnnualBillingCandidates() { return []; }
function gatherUnusedDomainOrStorageCandidates() { return []; }
function gatherForgottenServiceCandidates() { return []; }
function gatherOversizedPackageCandidates() { return []; }
function gatherProviderCreditCandidates() { return []; }
...
export function gatherRecommendationCandidates(db, now) {
    return [
...
function rowToRecord(r) {
    return {
        type: r.type, dedup_key: r.dedup_key, evidence: deserializeEvidence(r.evidence_json),
...
function loadExistingRecommendations(db) {
    const rows = db.prepare(`SELECT * FROM costops_recommendations`).all();
 ...
 */
export function captureRecommendations(db, now, opts = {}) {
