...
function outcome(provider, collector, status, imported, blocker) {
 ...
 */
export function buildCollectorPlan(deps = {}) {
    const plan = [
        {
...
export function lastCollectorRunAt(db, collectorName) {
    const row = db.prepare('SELECT MAX(started_at) AS last FROM import_runs WHERE collector_name = ?').get(collectorName);
 ...
 */
export function isCollectorDue(db, collectorName, cadenceSeconds, now) {
...
function recordScheduledRun(db, entry, res, now) {
    db.prepare(`
 ...
 */
export async function runScheduledCollectorSync(db, now, deps = {}) {
    ...
    for (const entry of plan) {
        ...
        try {
            if (lastCollectorRunAt(db, entry.collectorName) === before)
                recordScheduledRun(db, entry, res, now);
        }
        catch (err) {
 ...
 */
export async function runScheduledCollectorSyncSafe(db, now) {
    try {
        const report = await runScheduledCollectorSync(db, now);
        const landed = report.outcomes.filter(o => o.status === 'ok').length;
        if (report.outcomes.length > 0) {
            logger.info({ ran: report.outcomes.length, ok: landed, blocked: report.outcomes.filter(o => o.status !== 'ok').map(o => o.collector) }, 'CostOps scheduled collector sync completed');
        }
        return report;
    }
    catch (err) {
