...
function isSsePaneStream(path, method) {
    return method === 'GET' && /^\/api\/agents\/[^/]+\/pane\/stream$/.test(path);
}
export function isFederationWireEndpoint(path, method) {
    return ((path === '/api/federation/manifest' && method === 'GET') ||
...
export function requiresAuth(path, method) {
    ...
    if (path === '/api/settings' && method === 'GET')
        return false;
    if (method === 'GET' && (path === '/api/marveen/avatar' || /^\/api\/agents\/[^/]+\/avatar$/.test(path)))
        return false;
    if (path === '/.well-known/fleetq' && method === 'GET')
        return true;
    return path.startsWith('/api/');
}
export function resolveAuth(req, url, path, method, dashboardToken) {
    ...
    if (bearerMatch) {
        ...
        if (dk)
            return { kind: 'device', device: dk.name, deviceId: dk.id };
    }
    ...
    if (isSsePaneStream(path, method)) {
        ...
        if (checkBearerToken(`Bearer ${qtoken}`, dashboardToken))
            return { kind: 'token' };
