...
import { slugify as slugifyMcp, catalogMatchesConfigured, } from '../../mcp-list-parser.js';
...
import { getExternalProjectPaths, addExternalProjectPath, removeExternalProjectPath, getGitHubRepos, installGitHubRepo, removeGitHubRepo, updateGitHubRepo } from '../dashboard-settings.js';
import { listSecrets, setSecret, getSecret, deleteSecret } from '../vault.js';
import { getBindings, addBinding, removeBinding, removeBindingsForSecret, syncSecret, syncAllBindings, scanMcpConfigs, unsyncBinding, } from '../vault-bindings.js';
...
function loadMcpCatalog() {
    const central = JSON.parse(readFileSync(join(PROJECT_ROOT, 'mcp-catalog.json'), 'utf-8'));
    const byId = new Map();
...
function collectConfiguredServerSlugs() {
    const slugs = new Set();
    const files = [
...
function upsertLocalCatalogEntry(entry) {
...
export function vaultAndBindEnvSecrets(serverName, mcpFilePath, envSecrets) {
    for (const [key, value] of Object.entries(envSecrets)) {
        ...
        if (result.errors.length) {
            throw new Error(`A(z) "${key}" titkot nem sikerult a configba kotni: ${result.errors.join('; ')}. ` +
...
export async function tryHandleConnectors(ctx) {
    ...
    if (path === '/api/connectors/external-paths' && method === 'POST') {
        ...
        const { path: p } = JSON.parse(body.toString());
    ...
    if (path === '/api/connectors/github-repos' && method === 'POST') {
        const body = await readBody(req);
        const { url, env } = JSON.parse(body.toString());
        if (!url?.trim()) {
    ...
    if (connectorDetailMatch && method === 'GET' && !path.includes('/assign')) {
        ...
        if (name.startsWith('plugin:')) {
            try {
                ...
                const match = Object.keys(enabled).find(k => enabled[k] && k.split('@')[0].toLowerCase() === plain);
                if (!match) {
    ...
    if (path === '/api/connectors' && method === 'POST') {
        ...
        try {
            ...
            if ((data.type === 'http' || data.type === 'sse') && data.url) {
                ...
                const userSecrets = Object.fromEntries(Object.entries(data.env || {}).filter(([, v]) => v !== ''));
    ...
    if (connectorAssignMatch && method === 'POST') {
        ...
        const body = await readBody(req);
        const { agents: rawTargetAgents, allAgents: rawVisibleAgents } = JSON.parse(body.toString());
        ...
        for (const agentName of targetAgents) {
            ...
            let mcpConfig = {};
            try {
                mcpConfig = JSON.parse(readFileOr(mcpPath, '{}'));
            }
            catch { }
    ...
    if (catalogInstallMatch && method === 'POST') {
        ...
        try {
            ...
            if (!item) {
                json(res, { error: 'Item not found in catalog' }, 404);
                return true;
            }
            const body = await readBody(req);
            let envData = {};
            ...
            if (item.type === 'local') {
                ...
                // stay as -e flags.
                const userSecrets = Object.fromEntries(Object.entries(envData).filter(([, v]) => v !== ''));
                const defaultEnv = Object.fromEntries(Object.entries(item.env || {}).filter(([k, v]) => v !== '' && !(k in userSecrets)));
                const envFlags = Object.entries(defaultEnv)
                    .map(([k, v]) => `-e ${shellEscape(k)}=${shellEscape(v)}`)
                    .join(' ');
                const argsStr = (item.args || []).map((a) => shellEscape(a)).join(' ');
                ...
                const url = item.url;
                if (!url) {
                    json(res, { error: 'Remote item has no URL' }, 400);
                    return true;
                }
    ...
    if (path === '/api/vault' && method === 'POST') {
        const body = await readBody(req);
        const { id, label, value } = JSON.parse(body.toString());
        if (!id?.trim() || !value) {
            json(res, { error: 'id and value required' }, 400);
            return true;
        }
    ...
    if (path === '/api/vault/bindings' && method === 'POST') {
        ...
        const binding = { vaultSecretId: data.vaultSecretId, envVar: bindingKey, targets };
    ...
    if (bindingDeleteMatch && method === 'DELETE') {
        ...
        if (!removeBinding(secretId, envVar)) {
            json(res, { error: 'Binding not found' }, 404);
            return true;
        }
        json(res, { ok: true });
        return true;
    ...
    if (path === '/api/vault/import' && method === 'POST') {
        const body = await readBody(req);
        const { imports: importRequests } = JSON.parse(body.toString());
        let imported = 0;
        let bound = 0;
        const errors = [];
