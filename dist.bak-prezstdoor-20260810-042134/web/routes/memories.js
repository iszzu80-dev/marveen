import { saveAgentMemory, getAgentMemories, searchAgentMemories, getMemoryStats, updateMemory, hybridSearch, backfillEmbeddings, clearMemoryCache, searchMemories, getMemoriesForChat, getDb, touchMemoriesAccessed, } from '../../db.js';
...
export async function tryHandleMemories(ctx) {
    ...
    if (path === '/api/memories' && method === 'POST') {
        ...
        const result = saveAgentMemory(data.agent_id || MAIN_AGENT_ID, data.content.trim(), category, data.keywords || undefined, true);
    ...
    if (path === '/api/memories' && method === 'GET') {
        ...
        if (q && mode === 'hybrid') {
            ...
            if (results.length === 0) {
                const db2 = getDb();
                results = db2.prepare('SELECT * FROM memories WHERE content LIKE ? ORDER BY accessed_at DESC LIMIT ?').all(`%${q}%`, limit);
            }
        }
        else if (agentId) {
        ...
        if (tier)
            results = results.filter(m => m.category === tier);
        ...
        if (q && results.length)
            touchMemoriesAccessed(results.map(m => m.id));
    ...
    if (path === '/api/memories/import' && method === 'POST') {
        const body = await readBody(req);
        const { agent_id, chunks } = JSON.parse(body.toString());
        ...
        try {
            const ollamaModels = await fetch(`${OLLAMA_URL}/api/tags`, { signal: AbortSignal.timeout(3000) })
                .then(r => r.json())
                .then((d) => (d.models || []).filter((m) => !m.name.includes('embed')).map((m) => m.name))
                .catch(() => []);
            categorizeModel = ollamaModels.find((m) => m.includes('gemma4')) || ollamaModels[0] || null;
        }
        catch {
...
{"tier": "warm", "keywords": "keyword1, keyword2, keyword3"}`,
    ...
    if (memUpdateMatch && method === 'PUT') {
        ...
        const body = await readBody(req);
        const { content, category, tier, agent_id, keywords } = JSON.parse(body.toString());
