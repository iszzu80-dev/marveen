...
export async function tryHandleStatic(ctx, webDir) {
    ...
    if (path === '/style.css') {
        serveFile(req, res, join(webDir, 'style.css'));
        return true;
    }
    if (path === '/app.js') {
        serveFile(req, res, join(webDir, 'app.js'));
        return true;
    }
    ...
    if (path === '/coscontrol.js') {
        serveFile(req, res, join(webDir, 'coscontrol.js'));
        return true;
    } // LOCAL-FORK: cos seam (keep on rebase)
    if (path === '/apg.js') {
        serveFile(req, res, join(webDir, 'apg.js'));
        return true;
    }
    if (path === '/apg.css') {
        serveFile(req, res, join(webDir, 'apg.css'));
        return true;
    }
