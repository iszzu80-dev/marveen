...
import { hashPassword, verifyPassword, assertPasswordPolicy, PasswordPolicyError, } from '../password-hash.js';
import { createSession, revokeSession, revokeAllForUser, listUserSessions, } from '../auth-sessions.js';
import { checkThrottle, recordFailure, recordSuccess, runDummyVerify, } from '../login-throttle.js';
import { createDeviceKey, listDeviceKeys, revokeDeviceKey, getDeviceKey, } from '../auth-device-keys.js';
import { removeBridgeSshAccess, sshDirOverride } from '../bridge-enroll.js';
import { createDashboardUser, getDashboardUser, listDashboardUsers, countDashboardUsers, updateDashboardUserPassword, deleteDashboardUser, logConfigChange, } from '../../db.js';
...
function kindAllowed(auth, kinds) {
...
const USER_ADMIN_KINDS = ['token', 'session'];
...
const DEVICE_KEY_ADMIN_KINDS = ['token', 'session'];
...
export async function tryHandleAuth(ctx) {
    ...
    if (path === '/api/auth/login' && method === 'POST') {
        ...
        if (throttle.locked) {
            if (throttle.global)
                logger.warn('login: global failure cap reached -- all logins throttled');
        ...
        const token = createSession({ userId: user.id, username: user.username }, { userAgent: str(req.headers['user-agent']) || null, remoteNote: isHttps(req) ? 'https' : 'loopback' });
