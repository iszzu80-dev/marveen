...
import { deployProgressionSchema, seedProgressionState, runInitialProgressionForAll, deployAndSeedProgression, } from '../cos/progression-migrate.js';
...
describe('deployProgressionSchema', () => {
    it('creates tables when they do not exist', () => {
        ...
        const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('case_progression_state','case_progression_runs') ORDER BY name").all();
    ...
    it('is safe to call multiple times', () => {
        ...
        deployProgressionSchema(db);
        // Tables still exist
        const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('case_progression_state','case_progression_runs')").all();
...
describe('seedProgressionState', () => {
    it('seeds progression state for existing cases without one', () => {
        ...
        const rows = db.prepare('SELECT domain, case_id, progression_enabled, progression_mode FROM case_progression_state ORDER BY domain, case_id').all();
    ...
    it('skips cases that already have progression state (idempotent)', () => {
        ...
        // Only one row
        const count = db.prepare('SELECT count(*) as c FROM case_progression_state').get().c;
        expect(count).toBe(1);
    });
    it('skips COMPLETED, CANCELLED, and ARCHIVED cases', () => {
        ...
        const rows = db.prepare('SELECT case_id FROM case_progression_state').all();
...
describe('runInitialProgressionForAll', () => {
    it('runs progression cycle for every newly-seeded case', () => {
        ...
        const states = db.prepare('SELECT case_id, last_progressed_at FROM case_progression_state ORDER BY case_id').all();
        for (const s of states) {
            expect(s.last_progressed_at).toBeGreaterThan(0);
        }
        // Each case should have at least one progression run
        const runCount = db.prepare('SELECT count(*) as c FROM case_progression_runs').get().c;
    ...
    it('produces a decision and reason for each case', () => {
        ...
        const runs = db.prepare('SELECT decision, reason, domain FROM case_progression_runs ORDER BY started_at').all();
    ...
    it('is deterministic: same input → same decision', () => {
        ...
        const r1 = db1.prepare('SELECT decision, reason FROM case_progression_runs WHERE case_id = ? AND domain = ?').get('c1', 'personal');
        const r2 = db2.prepare('SELECT decision, reason FROM case_progression_runs WHERE case_id = ? AND domain = ?').get('c1', 'personal');
...
describe('runProgressionHeartbeat', () => {
    ...
    it('skips cases already claimed by another runner', () => {
        ...
        db.prepare('UPDATE case_progression_state SET progression_claimed_by = ?, progression_claim_expires_at = ? WHERE case_id = ? AND domain = ?').run('other-runner', t + 300, 'c1', 'personal');
        const result = runProgressionHeartbeat(db, t, 50);
    ...
    it('releases claims after processing (no leaked claims)', () => {
        ...
        const claimed = db.prepare("SELECT case_id FROM case_progression_state WHERE progression_claimed_by IS NOT NULL").all();
...
describe('intake seeds progression state when tables exist', () => {
    ...
    function seedBatch(messageId, threadId) {
        openBatch(db, {
            batchId: `b-${messageId}`,
            accountId: 'iszzu80',
            cursorBefore: '1',
    ...
    it('personal intake creates case_progression_state row', () => {
        ...
        const state = db.prepare("SELECT * FROM case_progression_state WHERE domain = 'personal' AND case_id = ?").get(result.caseId);
    ...
    it('ZST intake creates case_progression_state row', () => {
        ...
        expect(result.outcome).toBe('CASE_CREATED');
        const state = db.prepare("SELECT * FROM case_progression_state WHERE domain = 'zst' AND case_id = ?").get(result.caseId);
        expect(state).not.toBeUndefined();
        expect(state.progression_enabled).toBe(1);
        expect(state.progression_mode).toBe('internal');
    });
    it('intake return values unchanged (outcome, caseId, case shape)', () => {
        ...
        const c = db.prepare('SELECT * FROM personal_cases WHERE case_id = ?').get(result.caseId);
