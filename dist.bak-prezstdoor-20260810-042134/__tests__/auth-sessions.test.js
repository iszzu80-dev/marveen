...
import { createSession, resolveSession, revokeSession, revokeAllForUser, listUserSessions, sweepExpiredSessions, _clearSessionCacheForTest, } from '../web/auth-sessions.js';
...
describe('createSession / resolveSession', () => {
    ...
    it('stores only sha256(token) at rest, never the raw token', () => {
        const u = makeUser('alice');
        const token = createSession({ userId: u.id, username: u.username });
        const rows = getDb().prepare('SELECT id_hash FROM auth_sessions').all();
...
describe('TTLs', () => {
    ...
    it('debounces last_seen writes to >= 60s apart', () => {
        ...
        const token = createSession({ userId: u.id, username: u.username });
        const initial = getDb().prepare('SELECT last_seen_at c FROM auth_sessions').get().c;
        // 30s later: within debounce window -> no DB write
        vi.setSystemTime(new Date('2026-01-01T00:00:30Z'));
        resolveSession(token);
        expect(getDb().prepare('SELECT last_seen_at c FROM auth_sessions').get().c).toBe(initial);
        // 61s later: past debounce -> DB write
        vi.setSystemTime(new Date('2026-01-01T00:01:01Z'));
        resolveSession(token);
        expect(getDb().prepare('SELECT last_seen_at c FROM auth_sessions').get().c).toBeGreaterThan(initial);
    });
});
describe('revocation', () => {
