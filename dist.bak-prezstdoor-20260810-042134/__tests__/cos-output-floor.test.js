...
import { evaluateOutputFloors, breachedFloors, OUTPUT_FLOORS, } from '../cos/output-floor.js';
...
describe('COS output floors', () => {
    ...
    it('a missing table reports 0 rather than throwing — an undeployed pipeline is producing nothing', () => {
        ...
        const specs = [{
                ...
                measure: (d, since) => {
                    try {
                        return d.prepare('SELECT COUNT(*) AS n FROM table_that_does_not_exist WHERE t >= ?').get(since).n;
                    }
                    catch {
                        return 0;
                    }
