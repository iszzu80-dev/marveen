...
const MANUAL_OPTS = { triggerType: 'MANUAL', triggerReference: 'owner-answer-test' };
 ...
 *  the owner answers. */
function seedAwaitingSelectionCase(db, caseId, t) {
    createCase(db, {
    ...
    db.prepare(`INSERT INTO case_progression_state
     ...
     VALUES ('personal', ?, 1, 'internal', ?, ?, ?, ?, ?, ?)`).run(caseId, 'Test goal', 'Test summary', t, JSON.stringify(seedDod), t, t);
}
...
function advanceToRequestDecision(db, caseId, t) {
...
function getLastRun(db, caseId) {
    return db.prepare(`SELECT decision, reason, nbaStep
     FROM (
...
function getCaseStatus(db, caseId) {
    return db.prepare('SELECT status FROM personal_cases WHERE case_id = ?').get(caseId).status;
}
function getState(db, caseId) {
    return db.prepare(`SELECT completed_plan_step, plan_version, no_progress_run_count
     FROM case_progression_state WHERE domain = ? AND case_id = ?`).get('personal', caseId);
}
...
describe('stale answer (question changed)', () => {
    it('does NOT consume an answer when the question has genuinely changed (different decision + nbaStep)', () => {
        ...
        db.prepare(`UPDATE personal_cases SET status = 'READY', version = version + 1 WHERE case_id = ?`).run('c-stale');
...
describe('REQUEST_APPROVAL via AWAITING_APPROVAL', () => {
    ...
    function seedApprovalCase(db, caseId, t) {
        createCase(db, {
       ...
       (domain, case_id, progression_enabled, progression_mode,
        goal, summary, next_progression_at, dod_verification_json, created_at, updated_at)
       VALUES ('personal', ?, 1, 'internal', ?, ?, ?, ?, ?, ?)`).run(caseId, 'Approval goal', 'Approval summary', t, JSON.stringify(seedDod), t, t);
    }
