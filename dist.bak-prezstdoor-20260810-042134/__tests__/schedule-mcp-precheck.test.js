...
import { deriveProcessPattern, collectSubtreeCmdlines, decideMcpPrecheck, } from '../web/schedule-mcp-precheck.js';
...
describe('parseRequires (task-config requires.mcp_servers)', () => {
    ...
    it('hibás alakot csendben eldob (nem tömbszerű, üres, nem-string elemek)', () => {
        ...
        expect(parseRequires({ mcp_servers: 'gmail' })).toBeUndefined();
        expect(parseRequires({ mcp_servers: [] })).toBeUndefined();
        expect(parseRequires({ mcp_servers: [42, '', '  '] })).toBeUndefined();
        expect(parseRequires({ mcp_servers: [42, 'gmail-readonly'] }))
