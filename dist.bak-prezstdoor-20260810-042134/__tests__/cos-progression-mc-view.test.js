...
function freshDom() {
    ...
    const dom = new JSDOM('<!DOCTYPE html><html><body><div id="cosBody"></div></body></html>', { url: 'http://localhost:3420', runScripts: 'dangerously' });
    return dom;
}
function mockAllFetch(dom, responses) {
    const fn = vi.fn(async (url) => {
        const s = String(url);
        if (responses[s] !== undefined)
            return { ok: true, json: async () => responses[s] };
...
function loadAndMount(dom) {
    ...
    const cc = dom.window.CosControl;
    if (!cc)
        throw new Error('CosControl not found on window after eval');
    cc.mount();
    return cc;
}
...
function BASE_RESPONSES(overrides = {}) {
    return {
        '/api/cos/today': { cases: [] },
        '/api/cos/cases': { cases: [] },
        '/api/cos/outbound': {},
...
describe('Mission Control progression view (card 969e5c3b)', () => {
    ...
    describe('closed tile', () => {
        ...
        it('survives progression fetch failure without crashing', async () => {
            ...
            const fn = vi.fn(async (url) => {
                const s = String(url);
                if (s.includes('/api/cos/progression'))
    ...
    describe('expanded detail panel', () => {
        it('shows goal, status, last decision on expand', async () => {
            ...
            loadAndMount(dom);
            await tick(100);
            const tile = dom.window.document.querySelector('.cos-case-tile');
            tile?.click();
            await tick(100);
    ...
    describe('ZST namespace', () => {
        it('applies ZST progression data when ZST tab is active', async () => {
            ...
            await tick(100);
            // Click ZST tab
            const zstTab = dom.window.document.querySelector('[data-tab="zst"]');
            zstTab?.click();
            await tick(50);
