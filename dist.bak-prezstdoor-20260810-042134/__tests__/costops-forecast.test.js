...
import { forecastFixedSubscription, forecastTimeProportionalUsage, forecastBalanceDelta, forecastInvoiceCadence, forecastOneTime, forecastManual, resolveSourceForecast, computeForecastError, summarizeForecastAccuracy, forecastSnapshotDedupKey, initForecastSchema, } from '../costops/forecast.js';
// Hand-built MonthWindow fixtures -- these functions are pure over the
// interface shape, not tied to monthWindow()'s calendar math.
function win(fractionElapsed) {
...
describe('forecastBalanceDelta', () => {
    ...
    it('falls back to MTD-only, low confidence, with fewer than 2 snapshots', () => {
        const snaps = [{ balance: 100, captured_at: 1 * 86400 }];
    ...
    it('extrapolates month-end from a daily burn rate over a longer span', () => {
        ...
        const snaps = [];
        for (let d = 0; d <= 10; d++)
            snaps.push({ balance: 200 - d * 10, captured_at: d * 86400 });
...
describe('resolveSourceForecast dispatcher precedence', () => {
    const base = { mtd_amount: 1000, win: win(0.5), now: 15 * 86400 };
    it('manual_override wins over everything else', () => {
        const ctx = { ...base, manual_override: 999, charge_category: 'usage', balance_snapshots: [{ balance: 1, captured_at: 0 }, { balance: 0, captured_at: 86400 * 5 }] };
        expect(resolveSourceForecast(ctx).method).toBe('manual_forecast');
    });
    it('balance_snapshots beats cadence and charge_category', () => {
        const ctx = { ...base, balance_snapshots: [{ balance: 1, captured_at: 0 }], cadence: 'annual', last_invoice_amount: 1000, charge_category: 'usage' };
    ...
    it('non-monthly cadence beats charge_category when a last invoice amount is known', () => {
        const ctx = { ...base, cadence: 'annual', last_invoice_amount: 12000, charge_category: 'purchase' };
    ...
    it('monthly cadence does NOT trigger invoice_cadence (falls through)', () => {
        const ctx = { ...base, cadence: 'monthly', last_invoice_amount: 12000, charge_category: 'subscription' };
        expect(resolveSourceForecast(ctx).method).toBe('fixed_subscription');
    });
    it('purchase charge_category resolves to one_time', () => {
        const ctx = { ...base, charge_category: 'purchase', occurred_amount: 500 };
    ...
    it('usage charge_category resolves to time_proportional_usage', () => {
        const ctx = { ...base, charge_category: 'usage' };
    ...
    it('defaults to fixed_subscription for subscription/no category', () => {
        const ctx = { ...base, charge_category: 'subscription' };
        expect(resolveSourceForecast(ctx).method).toBe('fixed_subscription');
...
describe('initForecastSchema', () => {
    ...
    `).run({ now, dk: forecastSnapshotDedupKey('render-hosting', '2026-07', now) });
        const row = db.prepare(`SELECT * FROM forecast_snapshots WHERE source_id = 'render-hosting'`).get();
