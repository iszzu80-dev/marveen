...
import { renderFederationBlock, renderRoutingDirective, renderSubAgentFederationBlock, renderPolicySeed, applyFederationBlock, FEDERATION_BLOCK_BEGIN, FEDERATION_BLOCK_END, FEDERATION_POLICY_ANCHOR, } from '../web/federation/onboarding.js';
const ID = { botName: 'Arthur', mainAgentId: 'arthur', webPort: 3420, lang: 'hu' };
function cfg(peers = [{ id: 'teodor' }]) {
    return {
        enabled: true,
        systemId: 'arthur',
        peers: peers.map((p) => ({
...
describe('renderFederationBlock', () => {
    ...
    it('stays within the byte budget in BOTH languages AND EVERY routing mode at the 5-system scale (L5 raised 2048->3072)', () => {
        ...
        for (const lang of ['hu', 'en']) {
            for (const routingMode of ['strong', 'catalog-first', 'advisory']) {
...
describe('routing modes (configurable delegation eagerness)', () => {
    it('renders a DISTINCT directive per mode in BOTH languages, each still pointing at the catalog', () => {
        for (const lang of ['hu', 'en']) {
            ...
            for (const d of [strong, catalog, advisory])
                expect(d).toContain('/api/federation/directory');
