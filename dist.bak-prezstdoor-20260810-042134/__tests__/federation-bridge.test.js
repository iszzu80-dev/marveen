...
import { sendFederatedMessage, isPeerInBackoff, _resetBackoffForTest, FEDERATION_REQUEST_TIMEOUT_MS, FEDERATION_MAX_CONTENT_BYTES, } from '../web/federation/bridge.js';
...
function enabledConfig(peerOverrides = {}) {
    writeConfigFile({
        enabled: true,
        systemId: 'teodor',
...
describe('sendFederatedMessage -- terminal failures (no network)', () => {
    ...
    it('fails when the target system is the own system', async () => {
        ...
        expect(r).toMatchObject({ kind: 'failed' });
        if (r.kind === 'failed')
            expect(r.error).toContain('this system');
    });
    ...
    it('fails terminally (no network attempt) while pairing is incomplete (empty outboundToken)', async () => {
        ...
        let calls = 0;
        const counting = (async () => { calls++; return new Response('{}', { status: 202 }); });
        const r = await sendFederatedMessage(MSG, NOW, counting);
        expect(r).toMatchObject({ kind: 'failed' });
        if (r.kind === 'failed')
            expect(r.error).toContain('Pairing incomplete');
        expect(calls).toBe(0);
    });
