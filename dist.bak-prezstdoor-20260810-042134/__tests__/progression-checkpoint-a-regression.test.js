...
import { createZstCase, transitionZstCase, listActiveZstCases } from '../cos/zst-case-store.js';
...
describe('Checkpoint A — Brownfield integrity (card 31188085)', () => {
    ...
    it('personal intake produces identical output WITH and WITHOUT progression tables', () => {
        ...
        function seedIntake(db, messageId, threadId) {
            openBatch(db, {
                batchId: `b-${messageId}`,
                accountId: 'iszzu80',
                cursorBefore: '1',
    ...
    it('personal case create + transition produce identical shapes', () => {
        ...
        expect(shapeKey(caseWith)).toBe(shapeKey(caseWithout));
        // Event count and shape identical
        const evWith = dbWithProg.prepare('SELECT * FROM personal_case_events WHERE case_id = ?').all(caseWith.case_id);
        const evWithout = dbWithoutProg.prepare('SELECT * FROM personal_case_events WHERE case_id = ?').all(caseWithout.case_id);
        expect(evWith.length).toBe(evWithout.length);
    ...
    it('event history triggers are intact (append-only, FK enforcement)', () => {
        for (const db of [dbWithProg, dbWithoutProg]) {
            ...
            expect(() => db.prepare("INSERT INTO personal_case_events (case_id, case_version, actor, event_type, reason, created_at) VALUES ('nonexistent', 1, 'test', 'CREATED', 'bad', 1)").run()).toThrow();
        }
    });
    // ─── 5. Mission Control view unchanged ───────────────────────────────
    it('listActiveCases and listTodayCases return identical shapes', () => {
        ...
        const activeWith = listActiveCases(dbWithProg).filter(c => c.title.startsWith('MC View Test'));
        const activeWithout = listActiveCases(dbWithoutProg).filter(c => c.title.startsWith('MC View Test'));
        expect(activeWith.length).toBe(activeWithout.length);
        expect(shapeKey(activeWith)).toBe(shapeKey(activeWithout));
        // Personal today
        const todayWith = listTodayCases(dbWithProg, horizon).filter(c => c.title.startsWith('MC View Test'));
        const todayWithout = listTodayCases(dbWithoutProg, horizon).filter(c => c.title.startsWith('MC View Test'));
    ...
    it('progression tables are populated after intake (thin slice active)', () => {
        ...
        const stateCount = dbWithProg.prepare('SELECT count(*) as c FROM case_progression_state').get().c;
        const runCount = dbWithProg.prepare('SELECT count(*) as c FROM case_progression_runs').get().c;
    ...
    it('intake seeds progression_enabled=1, progression_mode=internal', () => {
        ...
        const rows = dbWithProg.prepare('SELECT progression_enabled, progression_mode FROM case_progression_state').all();
    ...
    it('schema defaults enforce progression_enabled=0, progression_mode=off', () => {
        ...
        const cols = dbWithProg.prepare('PRAGMA table_info(case_progression_state)').all();
    ...
    it('schema snapshot: progression tables are the ONLY difference', () => {
        ...
        const tablesWith = dbWithProg.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all().map(r => r.name);
        const tablesWithout = dbWithoutProg.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all().map(r => r.name);
    ...
    it('schema snapshot: no progression columns leaked into existing tables', () => {
        for (const table of ['personal_cases', 'zst_cases', 'personal_case_events', 'zst_case_events']) {
            const cols = dbWithProg.prepare(`PRAGMA table_info(${table})`).all().map(c => c.name);
    ...
    it('integrity: after ALL operations, progression state exists (thin slice active)', () => {
        ...
        const withoutCount = dbWithoutProg.prepare("SELECT count(*) as c FROM sqlite_master WHERE type='table' AND name IN ('case_progression_state','case_progression_runs')").get().c;
