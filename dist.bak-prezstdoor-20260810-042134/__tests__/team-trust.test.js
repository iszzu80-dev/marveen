...
function makeCtx(teams, mainAgentId = 'main') {
    const known = new Set([mainAgentId, ...Object.keys(teams)]);
    return {
        mainAgentId,
        isKnownAgent: (name) => !!name && known.has(name),
        readAgentTeam: (name) => teams[name] ?? { reportsTo: null, delegatesTo: [], trustFrom: [] },
    };
}
