...
import { personalApprovals, zstApprovals, makeApprovalEngine } from '../cos/approval-core.js';
...
describe('COS approval envelope (§3.2)', () => {
    ...
    describe('quotas (v4.2.1 A.4)', () => {
        let seq = 0;
        function ledgerRow(id, kind, status = 'VERIFIED') {
            seq += 1;
         ...
         VALUES (?, 'c1', 'EMAIL_SEND', ?, ?, ?, ?, ?, ?, ?)`).run(id, seq, `k-${id}`, status, CAMP, kind, NOW, NOW);
        }
        beforeEach(() => { seq = 0; });
        it('refuses once the per-kind quota is used up', () => {
            ...
            ledgerRow('l1', 'FOLLOW_UP');
            ledgerRow('l2', 'FOLLOW_UP');
    ...
    it('refuses to record an approval that would allow autonomous spend', () => {
        expect(() => personalApprovals.recordApproval(getDb(), { ...baseApproval(), autonomousSpendLimit: 500 }, NOW)).toThrow(/never spends/);
    });
