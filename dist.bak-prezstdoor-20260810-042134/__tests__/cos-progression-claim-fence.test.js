...
const claims = (db) => db.prepare(`SELECT claim_key, owner_run_id, claim_fence FROM case_claims`).all();
describe('progression claims carry a fence', () => {
    ...
    it('a heartbeat run takes a fenced claim — the fence advances on takeover', () => {
        ...
        db.prepare(`INSERT INTO case_claims (claim_key, owner_run_id, claim_fence, claimed_at, claim_expires_at)
       VALUES (?, 'halott-runner', 7, ?, ?)`).run(key, NOW - 5000, NOW - 4000); // long expired
    ...
    it('the fence is monotonic across re-claims — a stale run cannot match it later', () => {
        ...
        const second = db.prepare(`SELECT claim_fence FROM case_claims WHERE claim_key LIKE 'progression:%'`)
            .get();
        if (first && second)
            expect(second.claim_fence).toBeGreaterThanOrEqual(first.claim_fence);
    });
    it('the run does not leave stale claims behind — they would read as findings', () => {
        const db = seed(2);
        runProgressionHeartbeat(db, NOW);
        const held = db.prepare(`SELECT COUNT(*) AS n FROM case_claims WHERE claim_key LIKE 'progression:%' AND claim_expires_at > ?`).get(NOW);
