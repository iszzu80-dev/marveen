...
const cfg = { version: 1, currency: 'HUF', fixed_costs: [], budgets: [] };
describe('costops manual cost entry (card a1552362, item 3)', () => {
    ...
    it('Phase 1 (GAP-09): a converted manual entry also carries fx_source/conversion_method', () => {
        ...
        const row = db.prepare(`SELECT fx_source, conversion_method FROM cost_line_items WHERE dedup_key = 'manual|figma|2026-07'`).get();
    ...
    it('an already-HUF manual entry has null fx_source/conversion_method (nothing converted)', () => {
        ...
        const row = db.prepare(`SELECT fx_source, conversion_method FROM cost_line_items WHERE dedup_key = 'manual|notion|2026-07'`).get();
    ...
    it('DELETE voids the manual cost_line_items row instead of hard-deleting it (card 73e8914a, Phase 0)', () => {
        ...
        const voided = db.prepare(`SELECT voided_at, void_reason, billed_cost FROM cost_line_items WHERE dedup_key = 'manual|notion|2026-07|voided|${NOW + 5}'`).get();
...
describe('costops manual entitlement entry (card a1552362, item 3)', () => {
    ...
    it('creates a manual entitlement row', () => {
        ...
        expect(r.ok).toBe(true);
        const row = db.prepare(`SELECT * FROM entitlements WHERE dedup_key = 'manual|notion|notion-plus|seats|monthly'`).get();
    ...
    it('rejects a duplicate create (409)', () => {
        const db = getDb();
        const input = { provider: 'notion', product: 'notion-plus', billing_period: 'monthly', entitlement_type: 'seats', status: 'ok' };
