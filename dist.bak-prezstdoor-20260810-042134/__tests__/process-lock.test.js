...
import { findOwnNodeHolders, findOwnBinaryMatches, terminateProcesses, acquirePortLock, acquirePidfileLock, writeBufferFully, DeferToPeerError, } from '../process-lock.js';
...
function makeCtx(options) {
    ...
    const ctx = {
        ...
        listPortHolders: (port) => options.portHolders?.[port] ?? [],
        ...
        getProcessCommand: (pid) => table.get(pid)?.cmd ?? null,
        getProcessUid: (pid) => {
            const p = table.get(pid);
            return p ? p.uid : null;
        },
        signal: (pid, sig) => {
            ...
            if (options.signalOverride)
                return options.signalOverride(pid, sig);
...
describe('findOwnNodeHolders', () => {
    ...
    it('skips non-positive or non-finite PIDs defensively', () => {
        const { ctx } = makeCtx({ portHolders: { 3420: [0, -1, NaN] } });
        expect(findOwnNodeHolders(3420, ctx)).toEqual([]);
    });
...
describe('terminateProcesses', () => {
    ...
    it('escalates to SIGKILL if SIGTERM did not kill the process', async () => {
        ...
        const signalOverride = (pid, sig) => {
            const p = procs.find(x => x.pid === pid);
            if (sig === 'SIGTERM')
                return 'sent'; // no-op (hung process)
            if (sig === 0)
                return p.alive ? 'sent' : 'gone';
            if (sig === 'SIGKILL') {
                p.alive = false;
                return 'sent';
            }
...
function makePidfileCtx(state) {
    ...
    return {
        ...
        probeAlive(pid) {
            if (state.probeAliveOverride)
                return state.probeAliveOverride(pid);
...
describe('acquirePidfileLock', () => {
    ...
    it('gives up after maxAttempts if every attempt keeps seeing EEXIST', async () => {
        ...
        await expect(acquirePidfileLock(state.path, 100, ctx, { graceMs: 5, maxAttempts: 3 })).rejects.toThrow(/Failed to acquire pidfile lock/);
    });
    ...
    it('throws DeferToPeerError when onLiveLegitimate=defer and peer is alive-legit', async () => {
        ...
        state.legitimatePids.add(999);
        const ctx = makePidfileCtx(state);
        await expect(acquirePidfileLock(state.path, 100, ctx, { graceMs: 10, onLiveLegitimate: 'defer' })).rejects.toBeInstanceOf(DeferToPeerError);
    ...
    it('does NOT unlink a third peer that took the slot during our SIGTERM-wait', async () => {
        ...
        await expect(acquirePidfileLock(state.path, 100, ctx, { graceMs: 5, maxAttempts: 1 })).rejects.toThrow(/Failed to acquire pidfile lock/);
...
describe('writeBufferFully', () => {
    ...
    it('throws if the writer returns 0', () => {
        expect(() => writeBufferFully(() => 0, Buffer.from('x'))).toThrow(/returned 0/);
    });
    it('throws if the writer returns a negative number', () => {
        expect(() => writeBufferFully(() => -1, Buffer.from('x'))).toThrow(/returned -1/);
    });
    it('throws if the writer returns NaN', () => {
        expect(() => writeBufferFully(() => NaN, Buffer.from('x'))).toThrow(/returned NaN/);
    });
