...
describe('COS schema completion', () => {
    ...
    describe('A.3 — thread_id in the unique key', () => {
        ...
        it('the same message cannot enter twice — not even once without a thread', () => {
            ...
            expect(() => db.prepare(`INSERT INTO email_processing (gmail_account_id, message_id, thread_id, batch_id, status, created_at, updated_at)
         VALUES (?, 'm1', NULL, 'b1', 'DISCOVERED', ?, ?)`).run(ACC, NOW, NOW)).toThrow(/UNIQUE/i);
        });
        it('two different messages on the SAME thread both fit', () => {
            ...
            const n = db.prepare(`SELECT COUNT(*) AS n FROM email_processing WHERE thread_id='t1'`).get();
            expect(n.n).toBe(2);
        });
        it('re-discovery of the same message is still a no-op, not a duplicate', () => {
            ...
            expect(rows[0].status).toBe('LOCAL_APPLIED'); // not reset by re-discovery
        });
        it('the table indexes survive the rebuild', () => {
            ...
            // passes just as well.
            const idx = (n) => getDb().prepare(`SELECT 1 FROM sqlite_master WHERE type='index' AND name=?`).get(n);
        ...
        it('the pre-migration table is gone, not left behind as a shadow copy', () => {
            const t = getDb().prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='email_processing_pre_a3'`).get();
    ...
    it('§17 — a migrated case is marked MIGRATED_UNVERIFIED, a native one is not', () => {
        ...
        const scope = (id) => db2.prepare(`SELECT scope FROM personal_cases WHERE case_id=?`)
            .get(id).scope;
