...
import { buildCollectorPlan, isCollectorDue, lastCollectorRunAt, runScheduledCollectorSync, HOURLY, DAILY, COLLECTOR_TICK_MS, } from '../costops/collectors/scheduled-sync.js';
const NOW = Math.floor(Date.UTC(2026, 6, 30, 12, 0, 0) / 1000);
/** A runner that records what it was asked and reports a clean import. */
function okRunner(provider, collector, calls) {
    return async () => {
        calls.push(collector);
        return { provider, collector, status: 'ok', imported_count: 1, blocker: null };
    };
}
function stubAll(calls) {
    const out = {};
    for (const e of buildCollectorPlan())
        out[e.collectorName] = okRunner(e.provider, e.collectorName, calls);
    return out;
}
describe('P2-C collector plan', () => {
    it('1. covers every required Phase 2 signal, with capacity on a faster cadence than cost', () => {
        ...
        for (const e of plan)
            expect(['capacity', 'cost']).toContain(e.kind);
...
describe('P2-C due-check (restart-safe, derived from import_runs)', () => {
    ...
    function recordRun(collector, at, status = 'ok') {
        getDb().prepare(`
...
describe('P2-C sweep', () => {
    ...
    it('5b. a thrown secret-shaped string is sanitized before it reaches the report or the DB', async () => {
        ...
        const row = getDb().prepare("SELECT error_message_sanitized m FROM import_runs WHERE collector_name='openai-costs' ORDER BY id DESC LIMIT 1").get();
        if (row?.m)
            expect(row.m).not.toContain('sk-abcdefghijklmnopqrstuvwx');
    });
    it('6. a collector that bails without recording gets a run row anyway, so it stays paced', async () => {
        ...
        runners['github-billing-usage'] = async () => ({
            provider: 'github', collector: 'github-billing-usage', status: 'skipped',
    ...
    it('6b. a collector that records its OWN run row is not double-recorded', async () => {
      ...
      `).run(now, now);
            return { provider: 'deepseek', collector: 'deepseek-balance', status: 'ok', imported_count: 1, blocker: null };
        };
        await runScheduledCollectorSync(getDb(), NOW, { runners });
        const n = getDb().prepare("SELECT COUNT(*) c FROM import_runs WHERE collector_name='deepseek-balance'").get();
        expect(n.c).toBe(1);
    });
    it('skips a collector that is not due, and names it', async () => {
        ...
        expect(third.outcomes.map(o => o.collector).sort()).toEqual(['anthropic-usage-snapshot', 'codex-ratelimit', 'deepseek-balance']);
    });
