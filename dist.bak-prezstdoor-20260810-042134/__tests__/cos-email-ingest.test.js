...
import { openBatch, isBatchTerminal, tryAdvanceCheckpoint, getCheckpoint, sourceCommit, excludeMessage, markDuplicate, markRecoveryRequired, quarantineMessage, localApply, } from '../cos/email-ingest.js';
...
describe('COS email ingestion', () => {
    ...
    it('openBatch records DISCOVERED messages; re-discovery across batches is a no-op', () => {
        const db = getDb();
        batch3();
        expect(db.prepare(`SELECT COUNT(*) n FROM email_processing WHERE batch_id='b1'`).get().n).toBe(3);
        ...
        expect(db.prepare(`SELECT COUNT(*) n FROM email_processing WHERE gmail_account_id=? AND message_id='m1'`).get(ACC).n).toBe(1);
    });
    it('P0.2: the cursor does NOT advance while any message is non-terminal', () => {
        const db = getDb();
        batch3();
        sourceCommit(db, ACC, 'm1', 1010);
        localApply(db, ACC, 'm2', 'c1', 1011);
        sourceCommit(db, ACC, 'm2', 1012);
        ...
        expect(db.prepare(`SELECT status FROM email_processing_batches WHERE batch_id='b1'`).get().status).toBe('PROCESSING');
    });
    it('advances only once EVERY message is terminal (mixed terminal states)', () => {
        ...
        expect(db.prepare(`SELECT status FROM email_processing_batches WHERE batch_id='b1'`).get().status).toBe('TERMINAL');
    });
