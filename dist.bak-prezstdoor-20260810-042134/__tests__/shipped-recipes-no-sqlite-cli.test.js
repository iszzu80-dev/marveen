        ...
        for (const file of SHIPPED) {
            for (const block of shellBlocks(readFileSync(file, 'utf-8'))) {
                for (const line of block.split('\n')) {
                    ...
                    if (/(^|[|;&(]\s*|\$\()\s*sqlite3\s/.test(code))
                        offenders.push(`${file}: ${line.trim().slice(0, 90)}`);
                    if (/(^|[|;&(]\s*|\$\()\s*jq\s/.test(code))
                        offenders.push(`${file}: ${line.trim().slice(0, 90)}`);
                }
            }
        }
...
describe('both seeding paths substitute the same placeholders', () => {
    ...
    function placeholdersAfter(src, anchor) {
        const start = src.indexOf(anchor);
        if (start < 0)
            throw new Error(`anchor not found: ${anchor}`);
        const sed = src.indexOf('sed -e', start);
        if (sed < 0)
            throw new Error(`no sed block after ${anchor}`);
        const end = src.indexOf('> "$target/$(basename "$f")"', sed);
        if (end < 0)
            throw new Error(`unterminated sed block after ${anchor}`);
