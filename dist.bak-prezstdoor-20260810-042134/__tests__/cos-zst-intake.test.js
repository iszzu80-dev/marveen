...
describe('ZST email intake (Slice 1, read-only)', () => {
    ...
    const led = (mid) => getDb().prepare(`SELECT status, case_id FROM zst_email_processing WHERE gmail_account_id='zst' AND message_id=?`).get(mid);
    it('creates a zst_case from an actionable ZST email', () => {
        ...
        const c = getZstCase(getDb(), r.caseId);
