...
import { createDeviceKey, resolveDeviceKey, listDeviceKeys, revokeDeviceKey, revokeAllDeviceKeys, sweepExpiredDeviceKeys, _clearDeviceKeyCacheForTest, } from '../web/auth-device-keys.js';
...
describe('mint + storage discipline', () => {
    it('mints a prefixed key and stores ONLY its sha256', () => {
        ...
        const rows = getDb().prepare('SELECT key_hash, name FROM device_keys').all();
