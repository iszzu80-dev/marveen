...
import { runProgressionCycle, getMissionControlProgressionView, VALID_DECISIONS, deriveOutcomeContract, resolveContext, buildRollingPlan, determineNextBestAction, decide, } from '../cos/progression-pipeline.js';
// ── Helper: snapshot case row before/after progression ──────────────────
function snapshotCase(table, caseId, db) {
    const row = db.prepare(`SELECT * FROM ${table} WHERE case_id = ?`).get(caseId);
    if (!row)
        throw new Error(`Case not found: ${table}/${caseId}`);
    return { ...row };
}
...
function snapshotProgressionRuns(domain, caseId, db) {
    return db.prepare('SELECT * FROM case_progression_runs WHERE domain = ? AND case_id = ? ORDER BY started_at').all(domain, caseId);
}
// ── Tests ───────────────────────────────────────────────────────────────
describe('Checkpoint B — Thin shadow vertical slice (card 4a809934)', () => {
    ...
    describe('Stage 1: Pipeline stages in isolation', () => {
        it('deriveOutcomeContract returns valid contract for a HOME_REPAIR case', () => {
            const contract = deriveOutcomeContract('Medence WPC-alkatresz ajanlatkeres', 'HOME_REPAIR', 'WAITING_EXTERNAL', 'PERSONAL');
        ...
        it('deriveOutcomeContract returns valid contract for an INVOICE_INCOMING case', () => {
            const contract = deriveOutcomeContract('ZST: Bejovo szamla - Biztositas', 'INVOICE_INCOMING', 'NEW', 'ZST_INTERNAL');
            expect(contract.goal).toBeTruthy();
        ...
        it('determineNextBestAction picks first non-external step', () => {
            const plan = [
                { step: 1, label: 'Verify', kind: 'VERIFY', needsExternal: false },
                { step: 2, label: 'Wait', kind: 'AWAIT_EXTERNAL', needsExternal: true },
                { step: 3, label: 'Execute', kind: 'EXECUTE', needsExternal: false },
            ];
        ...
        it('decide returns WAIT_EXTERNAL for a waiting case with external dependency', () => {
            const nba = { planStep: 2, description: 'Check for external response', kind: 'AWAIT_EXTERNAL', canProceedAutonomously: false, estimatedEffortMinutes: 5 };
            const ctx = { eventCount: 2, lastEventType: null, lastEventReason: null, hasParent: false, hasChildren: false, ageDays: 3, sensitivity: 'PERSONAL' };
        ...
        it('decide returns RECOVERY_REQUIRED for overdue waiting (>7 days)', () => {
            const nba = { planStep: 2, description: 'Check', kind: 'AWAIT_EXTERNAL', canProceedAutonomously: false, estimatedEffortMinutes: 5 };
        ...
        it('decide returns RECOVERY_REQUIRED for BLOCKED status', () => {
            const nba = { planStep: 1, description: 'Identify', kind: 'GATHER_INFO', canProceedAutonomously: true, estimatedEffortMinutes: 5 };
        ...
        it('decide returns COMPLETE for COMPLETED status', () => {
            const nba = { planStep: 1, description: 'Verify', kind: 'VERIFY', canProceedAutonomously: true, estimatedEffortMinutes: 5 };
            const ctx = { eventCount: 2, lastEventType: null, lastEventReason: null, hasParent: false, hasChildren: false, ageDays: 1, sensitivity: 'PERSONAL' };
    ...
    describe('Stage 2: End-to-end pipeline — real cases', () => {
        ...
        it('PRI: decision is a valid progression decision type', () => {
            expect(VALID_DECISIONS).toContain(priResult.decision);
        });
        ...
        it('PRI: safety assertions JSON contains all 7 assertions all passed', () => {
            const runs = snapshotProgressionRuns('personal', PRI_CASE_ID, db);
            const parsed = JSON.parse(runs[0].safety_assertions_json);
        ...
        it('PRI: no new columns appeared in personal_cases', () => {
            const cols = db.prepare('PRAGMA table_info(personal_cases)').all();
        ...
        it('ZST: decision is a valid progression decision type', () => {
            expect(VALID_DECISIONS).toContain(zstResult.decision);
        });
        ...
        it('ZST: no progression columns leaked into zst_cases', () => {
            const cols = db.prepare('PRAGMA table_info(zst_cases)').all();
            const names = cols.map(c => c.name);
            const forbidden = ['goal', 'rolling_plan', 'progression_enabled', 'semantic_completion'];
            for (const f of forbidden) {
                const found = names.some(n => n.toLowerCase().includes(f));
        ...
        it('domain isolation: personal progression state does NOT appear in zst query', () => {
            const personalStates = db.prepare("SELECT count(*) as c FROM case_progression_state WHERE domain = 'personal'").get();
            const zstStates = db.prepare("SELECT count(*) as c FROM case_progression_state WHERE domain = 'zst'").get();
    ...
    describe('Stage 3: Mission Control read view (read-only projection)', () => {
        ...
        it('getMissionControlProgressionView is read-only — zero writes to case tables', () => {
            ...
            const beforeCount = db.prepare("SELECT count(*) as c FROM case_progression_runs").get().c;
            getMissionControlProgressionView(db, 'personal');
            getMissionControlProgressionView(db, 'zst');
            const afterCount = db.prepare("SELECT count(*) as c FROM case_progression_runs").get().c;
    ...
    describe('Stage 4: Idempotency — second progression run', () => {
        it('PRI: second progression cycle keeps plan_version stable (plan unchanged) and adds a second run row', () => {
            ...
            expect(stateAfter.plan_version).toBe(stateBefore.plan_version);
            // A second run row is created
            expect(runsAfter.length).toBe(runsBefore.length + 1);
            // Still only one state row (upsert)
            const stateCount = db.prepare("SELECT count(*) as c FROM case_progression_state WHERE domain='personal' AND case_id=?").get(PRI_CASE_ID).c;
    ...
    describe('Stage 5: Decision type coverage — at least 3 distinct decisions across tests', () => {
        ...
        it('All decisions used are in the 10 valid decision types', () => {
            const allRuns = db.prepare('SELECT decision FROM case_progression_runs').all();
            for (const run of allRuns) {
                expect(VALID_DECISIONS).toContain(run.decision);
            }
        });
    });
});
