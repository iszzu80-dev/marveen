...
// exactly that).
const mockUpdatePendingRetry = vi.fn((taskName, _agent, _now, reason) => {
    for (const row of mockListPendingRetries()) {
        if (row.task_name === taskName)
            row.last_reason = reason;
    }
    return true;
});
const mockListPendingRetries = vi.fn(() => []);
...
vi.mock('../db.js', () => ({
    appendTaskRun: (...a) => mockAppendTaskRun(...a),
    listPendingTaskRetries: () => mockListPendingRetries(),
    deletePendingTaskRetry: (...a) => mockDeletePendingRetry(...a),
...
vi.mock('../web/agent-process.js', () => ({
    ...
    startAgentProcess: (...a) => mockStartAgent(...a),
