...
export function makeApprovalEngine(T) {
    function getCampaign(db, campaignId) {
        return db.prepare(`SELECT campaign_id, status, version, allows_free_text FROM ${T.campaigns} WHERE campaign_id = ?`).get(campaignId);
    }
    ...
    function recordApproval(db, a, now) {
        ...
        db.prepare(`INSERT INTO ${T.approvals} (approval_id, campaign_id, campaign_version, approved_by,
    ...
    function tripStopCondition(db, approvalId, reason, now) {
        db.prepare(`UPDATE ${T.approvals} SET stopped_reason=@reason, updated_at=@now WHERE approval_id=@id`).run({ reason, now, id: approvalId });
    }
    /** Count what this campaign has already sent, for the quota checks. */
    function outboundCount(db, campaignId, kind) {
        try {
            const sql = kind
            ...
            const row = kind
                ? db.prepare(sql).get(campaignId, kind)
     ...
     */
    function authorizeSend(db, q, now) {
        const refuse = (code, reason) => ({ authorized: false, code, reason });
        const c = getCampaign(db, q.campaignId);
        if (!c)
            return refuse('campaign_missing', `campaign not found: ${q.campaignId}`);
        if (c.status !== 'APPROVED')
            return refuse('campaign_not_approved', `campaign status ${c.status} (not APPROVED)`);
        if (c.allows_free_text) {
            return refuse('free_text_campaign', 'free-text campaign → PREPARE only, never autonomous send');
        }
        const appr = db.prepare(`SELECT * FROM ${T.approvals}
       WHERE campaign_id=@id AND status='APPROVED' AND campaign_version=@version
         AND template_hash=@th AND rendered_payload_hash=@rh
       LIMIT 1`).get({ id: q.campaignId, version: c.version, th: q.templateHash, rh: q.renderedPayloadHash });
        if (!appr) {
            return refuse('no_matching_approval', 'no APPROVED approval for this template + rendered payload at the campaign current version');
        }
        ...
        if (!recipients.ok)
            return refuse('envelope_unreadable', 'allowed_recipients is not a readable list');
        ...
        if (q.channel) {
            ...
            if (!channels.ok)
                return refuse('envelope_unreadable', 'allowed_channels is not a readable list');
        ...
        if (q.usedVariables?.length) {
            ...
            if (!forbidden.ok)
                return refuse('envelope_unreadable', 'forbidden_variables is not a readable list');
            const hit = forbidden.value?.find((f) => q.usedVariables.includes(f));
            if (hit)
                return refuse('forbidden_variable', `rendered payload uses forbidden variable: ${hit}`);
            const schema = parseList(appr.allowed_variable_schema);
            if (!schema.ok)
                return refuse('envelope_unreadable', 'allowed_variable_schema is not a readable list');
            if (schema.value) {
                const extra = q.usedVariables.find((v) => !schema.value.includes(v));
                if (extra)
                    return refuse('variable_not_in_schema', `variable ${extra} is outside the approved schema`);
            }
        }
        const perKind = {
        ...
        if (kindCap !== null && kindCap !== undefined && q.outboundKind) {
            ...
            if (used < 0)
                return refuse('envelope_unreadable', 'ledger cannot be read for the per-kind quota');
            if (used >= kindCap) {
        ...
        if (appr.max_total_outbound !== null) {
            ...
            if (used < 0)
                return refuse('envelope_unreadable', 'ledger cannot be read for the total quota');
