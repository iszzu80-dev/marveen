...
export function buildRadarHitAlert(db, radarId) {
    const item = db.prepare(`SELECT label, kind, target_price, currency, best_seen_price, notification_reason FROM radar_items WHERE radar_id = ?`).get(radarId);
    if (!item)
        return null;
    const obs = db.prepare(`SELECT best_price, offer_ref, original_currency, original_final_price,
            comparison_currency, fx_rate, fx_rate_source
     FROM radar_observations WHERE radar_id = ? ORDER BY observed_at DESC LIMIT 1`).get(radarId);
...
export function alertRadarHit(db, radarId) {
