...
function recordLedger(db, input, status, caseId, now) {
    db.prepare(`INSERT INTO zst_email_processing (gmail_account_id, message_id, thread_id, case_id, status, created_at)
     VALUES (@acc, @mid, @tid, @cid, @status, @now)`).run({ acc: input.accountId, mid: input.messageId, tid: input.threadId ?? null, cid: caseId, status, now });
}
function findActiveZstCaseByThread(db, threadId) {
    return db.prepare(`SELECT case_id FROM zst_cases
     WHERE gmail_thread_ids LIKE ? AND archived_at IS NULL
       AND status NOT IN ('COMPLETED','CANCELLED','ARCHIVED','FAILED_TERMINAL') LIMIT 1`).get(`%"${threadId}"%`);
}
...
export function ingestTriagedZstEmail(db, input, now) {
    const existing = db.prepare(`SELECT status FROM zst_email_processing WHERE gmail_account_id = ? AND message_id = ?`).get(input.accountId, input.messageId);
    if (existing)
    ...
    const tx = db.transaction(() => {
        ...
        const tier = effectiveZstSensitivity(coerceZstSensitivity(input.declaredSensitivity ?? 'ZST_INTERNAL'), `${input.subject}\n${input.snippet}`);
        createZstCase(db, {
            caseId,
        ...
        if (progTableExists) {
         ...
         VALUES ('zst', ?, 1, 'internal', ?, ?, ?)`).run(caseId, now, now, now);
        }
