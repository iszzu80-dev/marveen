...
export const MOST_RESTRICTED = 'ZST_HIGHLY_SENSITIVE';
...
export function coerceZstSensitivity(value) {
    if (typeof value === 'string' && ZST_SENSITIVITIES.includes(value)) {
        return value === 'UNKNOWN' ? MOST_RESTRICTED : value;
    }
...
export function escalateZstSensitivity(a, b) {
...
export function effectiveZstSensitivity(declared, content) {
...
const PROFILE_ALLOWLIST = {
    ...
    ZST_INTERNAL: new Set(['premium_reasoning', 'build_strong', 'analysis_efficient']),
    ZST_CONFIDENTIAL: new Set(['premium_reasoning', 'build_strong']),
    ZST_FINANCIAL: new Set(['premium_reasoning']),
    ZST_LEGAL: new Set(['premium_reasoning']),
    ZST_PERSONAL_DATA: new Set(['premium_reasoning']),
    ZST_HIGHLY_SENSITIVE: new Set(['premium_reasoning']),
    UNKNOWN: new Set(['premium_reasoning']),
};
// Is `profile` allowed to process content of `tier`? FAIL-CLOSED: unknown tier →
// most restricted; unknown profile → never allowed.
export function isProfileAllowedForZstSensitivity(profile, tier) {
    return PROFILE_ALLOWLIST[coerceZstSensitivity(tier)].has(profile);
}
export function allowedProfilesForZst(tier) {
...
// throughout.
export function evaluateZstDispatch(content, declaredSensitivity, targetProfile) {
