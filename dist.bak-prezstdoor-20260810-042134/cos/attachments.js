...
export function addAttachment(db, a, now) {
    const checksum = sha256Hex(a.content);
    db.prepare(`INSERT INTO case_attachments
       (attachment_id, case_id, message_id, filename, mime_type, byte_size, checksum, sensitivity, content, created_at, updated_at)
     VALUES (@id, @caseId, @messageId, @filename, @mimeType, @size, @checksum, @sensitivity, @content, @now, @now)`).run({
...
export function verifyAttachment(db, attachmentId) {
    const row = db.prepare(`SELECT checksum, content, content_purged_at FROM case_attachments WHERE attachment_id = ?`).get(attachmentId);
    if (!row)
        return { integrity: 'NOT_FOUND', storedChecksum: null, computedChecksum: null };
    if (row.content == null) {
