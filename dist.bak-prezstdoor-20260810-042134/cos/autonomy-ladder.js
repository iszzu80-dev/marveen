...
export const RUNGS = ['OFF', 'OBSERVE', 'PREPARE', 'EXECUTE_WITH_APPROVAL', 'LIMITED_AUTONOMOUS'];
export const rungIndex = (r) => RUNGS.indexOf(r);
/** Faultless campaigns required before the next rung becomes available. */
export const PROMOTION_THRESHOLD = 3;
export function ensureLadderSchema(db) {
    db.exec(`
...
export function getLadder(db, caseType) {
    const row = db.prepare(`SELECT case_type, rung, ceiling, faultless_campaigns, paused FROM cos_autonomy_ladder WHERE case_type = ?`).get(caseType);
    const g = db.prepare(`SELECT paused FROM cos_autonomy_global WHERE id = 1`).get();
...
export function setLadder(db, caseType, patch, now) {
    ...
    db.prepare(`INSERT INTO cos_autonomy_ladder (case_type, rung, ceiling, faultless_campaigns, paused, updated_at)
     VALUES (@t, @r, @c, @f, @p, @now)
     ON CONFLICT(case_type) DO UPDATE SET rung=@r, ceiling=@c, faultless_campaigns=@f, paused=@p, updated_at=@now`).run({ t: caseType, r: next.rung, c: next.ceiling, f: next.faultlessCampaigns, p: next.paused ? 1 : 0, now });
}
/** §22 master switch. */
export function pauseAll(db, paused, reason, now) {
    db.prepare(`INSERT INTO cos_autonomy_global (id, paused, reason, updated_at) VALUES (1, @p, @r, @now)
     ON CONFLICT(id) DO UPDATE SET paused=@p, reason=@r, updated_at=@now`).run({ p: paused ? 1 : 0, r: reason, now });
}
/**
 ...
 */
export function permits(db, caseType, action) {
    if (action === 'PAYMENT') {
...
export function promotionEligibility(db, caseType) {
    const s = getLadder(db, caseType);
...
export function recordCampaignOutcome(db, caseType, faultless, now) {
    const s = getLadder(db, caseType);
...
export function fleetLevelFor(rung) {
    switch (rung) {
        case 'OFF':
        case 'OBSERVE': return 1;
        case 'PREPARE':
        case 'EXECUTE_WITH_APPROVAL': return 2;
