...
function loadTokens() {
    let currentMtime = 0;
    try {
        currentMtime = statSync(TOKENS_PATH).mtimeMs;
    }
    catch { /* file missing -- fall through to readFileSync error */ }
...
function saveTokens(tokens) {
    ...
    let mtimeMs = 0;
    try {
        mtimeMs = statSync(TOKENS_PATH).mtimeMs;
    }
    catch { /* unlikely right after writeFileSync */ }
...
function loadClientCredentials() {
    ...
    let currentMtime = 0;
    try {
        currentMtime = statSync(CLIENT_CREDS_PATH).mtimeMs;
    }
    catch { /* missing -- fall through to readFileSync error */ }
...
function httpsRequest(url, options, body, timeoutMs = TOOL_TIMEOUTS['google-calendar']) {
...
async function refreshAccessToken() {
    ...
    const { status, data } = await httpsRequest('https://oauth2.googleapis.com/token', {
        method: 'POST',
...
export async function getCalendarEvents(calendarId, timeMin, timeMax) {
    ...
    if (status === 401) {
        ...
        const parsed = JSON.parse(retry.data);
        return parsed.items ?? [];
    }
    if (status !== 200) {
