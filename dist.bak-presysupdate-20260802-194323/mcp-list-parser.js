...
export function catalogMatchesConfigured(idSlug, nameSlug, configuredSlugs) {
    for (const s of configuredSlugs) {
        if (idSlug && (s === idSlug || s.startsWith(idSlug + '-')))
            return true;
        if (nameSlug && (s === nameSlug || s.startsWith(nameSlug + '-')))
            return true;
    }
    return false;
}
...
export function parseMcpListLine(line) {
...
export function scrubPaths(msg, homeDir) {
    if (!msg)
        return msg;
    ...
    let out = msg.replace(PATH_SCRUB_RX, (_match, prefix, _rootName, trail) => {
