...
const EMPTY_PRICING = { version: 1, currency: 'HUF', models: {} };
...
export function deriveProvider(model) {
    ...
    if (/(claude|anthropic|opus|sonnet|haiku|fable)/.test(m))
        return 'anthropic';
    if (/(deepseek)/.test(m))
        return 'deepseek';
    if (/(gpt|openai|o1|o3|o4|davinci|codex)/.test(m))
        return 'openai';
    if (/(gemini|google|palm|bard)/.test(m))
        return 'google';
    if (/(grok|xai)/.test(m))
        return 'xai';
    if (/(mistral|mixtral)/.test(m))
        return 'mistral';
    if (/(llama|meta)/.test(m))
        return 'meta';
    return 'unknown';
}
...
export function ensurePricingExample() {
    try {
...
export function validatePricing(raw) {
    ...
    const models = {};
    const rawModels = (obj.models && typeof obj.models === 'object') ? obj.models : {};
    for (const [id, v] of Object.entries(rawModels)) {
        ...
        if (inp === null || out === null) {
            errors.push(`models['${id}']: input_per_mtok and output_per_mtok must be non-negative numbers`);
            continue;
        }
        models[id] = {
 ...
 */
export function estimateModelCost(pricing, model, tokens) {
    if (!model)
        return null;
    const rate = pricing.models[model];
    if (!rate)
        return null;
    return round2(((tokens.input ?? 0) / 1e6) * rate.input_per_mtok +
 ...
 */
export function getTokenCostEstimate(db, pricing, pricingExists, start, end) {
  ...
  `).all({ start, end });
    ...
    for (const r of rows) {
        ...
        if (!r.model) {
            unknownTokens += tokenSum;
            unpricedCalls += r.calls;
            anyUnpriced = true;
            continue;
        }
        if (!rate) {
            noRateTokens += tokenSum;
            unpricedCalls += r.calls;
            anyUnpriced = true;
            continue;
        }
    ...
    const pricing_profile_status = !pricingExists || Object.keys(pricing.models).length === 0 ? 'no_pricing_config'
 ...
 */
export function getTokenCostByAgent(db, pricing, start, end, fractionElapsed) {
    const rows = db.prepare(`
    SELECT agent, model,
      COALESCE(SUM(input_tokens),0) as input_tokens,
      COALESCE(SUM(output_tokens),0) as output_tokens,
  ...
  `).all({ start, end });
    ...
    for (const r of rows) {
        const rate = r.model ? pricing.models[r.model] : undefined;
        if (!r.model || !rate)
            continue; // unpriced (unknown model / no rate) -- volume only, not returned here
        const est = estimateModelCost(pricing, r.model, {
            input: r.input_tokens, output: r.output_tokens,
            cache_read: r.cache_read_tokens, cache_creation: r.cache_creation_tokens,
        });
