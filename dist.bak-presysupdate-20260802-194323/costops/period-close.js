...
import { getCostSummary, monthWindow } from './ledger.js';
...
export function initPeriodCloseSchema(db) {
    db.exec(`
...
export function getPeriodStatus(db, month) {
    const row = db.prepare(`SELECT status FROM period_status WHERE month = ?`).get(month);
    return row?.status ?? 'open';
}
/** True only for 'closed' -- 'reopened' and 'provisional' both accept normal writes, matching 'open' rules. */
export function isPeriodClosed(db, month) {
 ...
 */
export function checkPeriodWritable(db, month) {
 ...
 */
export function checkCloseReadiness(db, config, now, month) {
    ...
    const alertRows = db.prepare(`SELECT severity FROM costops_alerts WHERE resolved_at IS NULL`).all();
 ...
 * snapshot).
 */
export function closePeriod(db, config, month, actor, reason, now, opts = {}) {
    if (!actor || !actor.trim())
        return { ok: false, error: 'actor is required for a close event', status: 400 };
    const current = getPeriodStatus(db, month);
    if (current === 'closed')
        return { ok: false, error: `month ${month} is already closed -- reopen it first`, status: 409 };
    ...
    const snapshot = { summary, budgets };
...
export function reopenPeriod(db, month, actor, reason, now) {
    if (!actor || !actor.trim())
        return { ok: false, error: 'actor is required for a reopen event', status: 400 };
    if (!reason || !reason.trim())
        return { ok: false, error: 'reason is required to reopen a closed period', status: 400 };
    const current = getPeriodStatus(db, month);
    if (current !== 'closed')
        return { ok: false, error: `month ${month} is not closed (status='${current}')`, status: 409 };
    const tx = db.transaction(() => {
        db.prepare(`
      INSERT INTO period_close_events (month, event_type, actor, reason, snapshot_json, created_at)
...
export function getPeriodCloseHistory(db, month) {
    return db.prepare(`
...
export function getCloseSnapshot(db, month) {
