 ...
 */
export function ingestWorkspaceAlerts(db, entries, now, idSalt = 'costops-workspace-alert') {
  ...
  `);
    const out = { ingested: 0, errors: [] };
    const tx = db.transaction((list) => {
        for (const e of list) {
            if (!e || typeof e.account !== 'string' || !e.account) {
                out.errors.push({ account: String(e?.account), reason: 'missing account' });
                continue;
            }
            if (!VALID_ISSUE_TYPES.has(e.issue_type)) {
                out.errors.push({ account: e.account, reason: `invalid issue_type '${e.issue_type}'` });
                continue;
            }
            if (typeof e.detected_at !== 'number' || !isFinite(e.detected_at)) {
                out.errors.push({ account: e.account, reason: 'missing/invalid detected_at' });
                continue;
            }
            if (e.suspension_date !== undefined && (typeof e.suspension_date !== 'number' || !isFinite(e.suspension_date))) {
                out.errors.push({ account: e.account, reason: 'invalid suspension_date' });
                continue;
            }
...
export function getActiveWorkspaceAlerts(db, now) {
    return db.prepare(`
  ...
  `).all({ cutoff: now - ACTIVE_WINDOW_SECS });
}
const ISSUE_MESSAGE = {
...
export function buildWorkspaceAlertWarnings(db, now) {
    ...
    for (const r of rows) {
        ...
        if (seen.has(key))
            continue; // rows are DESC by detected_at -- first hit is the most recent
        seen.add(key);
