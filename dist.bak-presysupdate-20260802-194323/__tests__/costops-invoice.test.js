...
import { computeNetAmount, invoiceDedupKey, inferExpectedInvoiceBy, recordInvoice, applyInvoiceAdjustment, voidInvoice, listInvoices, buildInvoiceReconciliation, initInvoiceSchema, } from '../costops/invoice.js';
const NOW = Math.floor(Date.UTC(2026, 6, 15, 12, 0, 0) / 1000); // 2026-07-15
const SALT = 'test-salt';
function amounts(over = {}) {
    return { gross_amount: 100, tax_amount: 0, discount_amount: 0, credit_amount: 0, refund_amount: 0, late_charge_amount: 0, ...over };
}
function insertSource(db, id, provider) {
...
describe('recordInvoice -- first-time recording (no existing ledger line)', () => {
    ...
    it('inserts a new cost_line_items row as actual_invoice/email_invoice', () => {
        ...
        const line = db.prepare(`SELECT billed_cost, confidence, actual_source FROM cost_line_items WHERE id = ?`).get(r.ledgerLineId);
    ...
    it('rejects a duplicate invoice (same provider+ref+period)', () => {
        ...
        const count = db.prepare(`SELECT COUNT(*) as n FROM costops_invoices`).get();
...
describe('recordInvoice -- an active ledger line already exists', () => {
    ...
    `).run({ start: win.start, end: win.end, now: NOW });
        const originalLineId = estInfo.lastInsertRowid;
        const r = recordInvoice(db, {
        ...
        const original = db.prepare(`SELECT billed_cost, voided_at FROM cost_line_items WHERE id = ?`).get(originalLineId);
        expect(original.billed_cost).toBe(10000); // NEVER rewritten
        expect(original.voided_at).not.toBeNull(); // voided, not deleted
        const corrected = db.prepare(`SELECT billed_cost, corrects_line_id FROM cost_line_items WHERE id = ?`).get(r.ledgerLineId);
...
describe('applyInvoiceAdjustment', () => {
    ...
    it('applies a credit and cascades the reduced net through a correction, keeping the original charge intact', () => {
        ...
        const invoiceRow = db.prepare(`SELECT net_amount, credit_amount, ledger_line_id FROM costops_invoices WHERE id = ?`).get(rec.invoiceId);
        expect(invoiceRow.net_amount).toBe(10000);
        expect(invoiceRow.credit_amount).toBe(2000);
        const correctedLine = db.prepare(`SELECT billed_cost FROM cost_line_items WHERE id = ?`).get(invoiceRow.ledger_line_id);
    ...
    it('accumulates multiple partial adjustments rather than overwriting them', () => {
        ...
        const invoiceRow = db.prepare(`SELECT net_amount, refund_amount FROM costops_invoices WHERE id = ?`).get(rec.invoiceId);
