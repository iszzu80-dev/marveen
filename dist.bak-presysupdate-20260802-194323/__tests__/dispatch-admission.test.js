...
function writeCfg(cfg) { writeFileSync(cfgPath, JSON.stringify(cfg)); }
...
describe('P2-B admission gate: fault isolation', () => {
    it('fails OPEN when the signal source throws -- the dispatch proceeds', () => {
        ...
        const out = evaluateDispatchAdmissionSafe({ agent: 'dev1', labels: ['size:large'] }, { get signals() { return boom(); }, configPath: cfgPath });
        expect(out.admit).toBe(true);
    ...
    it('the safe wrapper never throws for any shape of garbage input', () => {
        ...
        for (const req of [
            { agent: '' },
            { agent: 'dev1', labels: null },
            { agent: 'dev1', explicitTaskSize: 'gigantic' },
            { agent: 'dev1', labels: [undefined, 'size:large'] },
        ]) {
