...
import { isQuietHour, localHour, routeEscalation, flushQuietSummary, buildQuietSummaryMessage, buildEscalationMessage, } from '../web/reauth-healer.js';
...
const entry = (session, label = session, consecutiveDead = 10) => ({
    session,
    label,
...
describe('isQuietHour / localHour', () => {
    ...
    it('localHour a host TZ-től függetlenül az install-zóna (APP_TZ) óráját adja', () => {
        ...
        const hourIn = (ms) => parseInt(new Intl.DateTimeFormat('en-GB', { timeZone: APP_TZ, hour: '2-digit', hour12: false }).format(new Date(ms)), 10);
...
describe('flushQuietSummary', () => {
    ...
    it('06:00 után EGY összegző megy ki a még mindig halott agensekről, cooldown-stampeléssel', () => {
        ...
        const stillDead = (s) => (s === 'agent-spock' ? 120 : s === 'agent-scotty' ? 110 : 0);
