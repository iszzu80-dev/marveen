...
import { mapRenderPlanCost, makeRenderCollector } from '../costops/collectors/render.js';
...
describe('render collector via runner (offline)', () => {
    ...
    it('dry-run does NOT persist a render-plan line', async () => {
        const stub = async (url) => url.includes('postgres') ? RAW.postgres : RAW.services;
        ...
        const n = getDb().prepare("SELECT COUNT(*) n FROM cost_line_items").get();
        expect(n.n).toBe(0);
    });
    it('import writes a provider_plan_estimate line; idempotent', async () => {
        ...
        const rows = getDb().prepare("SELECT confidence, billed_cost, data_freshness FROM cost_line_items WHERE source_id='render-plan'").all();
