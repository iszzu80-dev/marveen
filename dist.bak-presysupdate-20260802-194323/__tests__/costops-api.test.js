...
function fakeCtx(path, method = 'GET') {
    const out = { status: 0, body: null, raw: undefined };
    const res = {
...
describe('costops API (route smoke)', () => {
    ...
    it('GET /api/costs/source-inventory returns the full inventory, additive to /summary', async () => {
        ...
        const domain = out.body.sources.find((s) => s.source_id === 'domain');
    ...
    it('GET /api/costs/reconciliation returns a per-source reconciliation view', async () => {
        ...
        const domain = out.body.reconciliation.find((r) => r.source_id === 'domain');
    ...
    `).run(win.start, win.end);
        const originalId = info.lastInsertRowid;
    ...
    describe('budget CRUD + history routes (GAP-11)', () => {
        ...
        it('POST /api/costs/budgets creates a budget, GET reflects its live status', async () => {
            ...
            const found = getOut.body.find(b => b.id === BID);
        ...
        it('PATCH updates an existing budget in place (no duplicate entry)', async () => {
            ...
            expect(getOut.body.filter(b => b.id === BID)).toHaveLength(1);
    ...
    describe('invoice routes (GAP-14)', () => {
        const win = monthWindow(Math.floor(Date.now() / 1000));
        function insertSource(id, provider) {
        ...
        it('GET /api/costs/invoices/reconciliation decorates the base reconciliation with invoice breakdown', async () => {
            ...
            expect(await tryHandleCostOps(ctx)).toBe(true);
            const row = out.body.reconciliation.find(r => r.source_id === 'render-hosting');
