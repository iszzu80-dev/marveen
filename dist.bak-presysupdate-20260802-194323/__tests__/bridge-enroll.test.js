...
import { bridgeEnroll, removeBridgeSshAccess, RemoteEnrollError } from '../web/bridge-enroll.js';
...
function makeKeyLine(installId = randomUUID()) {
...
describe('bridgeEnroll', () => {
    it('writes the restricted line, mints a paired device key, and embeds it in the bundle', async () => {
        ...
        const r = resolveAuth({ headers: { authorization: `Bearer ${bundle.dashboardToken}` } }, new URL('http://127.0.0.1:3420/api/agents'), '/api/agents', 'GET', TOKEN);
    ...
    it('re-pairing the same installId replaces BOTH the ssh line and the device key', async () => {
        ...
        const gate = (k) => resolveAuth({ headers: { authorization: `Bearer ${k}` } }, new URL('http://127.0.0.1:3420/api/agents'), '/api/agents', 'GET', TOKEN);
    ...
    it('fails hard without a host key and leaves NO side effects', async () => {
        ...
        await expect(bridgeEnroll({ keyLine: line, name: 'x' }, testDeps({ keyscan: async () => null }))).rejects.toThrow(RemoteEnrollError);
    ...
    it('rejects a garbage key line before touching anything', async () => {
        await expect(bridgeEnroll({ keyLine: 'ssh-rsa AAAA nope', name: 'x' }, testDeps())).rejects.toThrow(RemoteEnrollError);
        expect(listDeviceKeys()).toHaveLength(0);
    });
...
function mkRes() {
    return {
        ...
        headers: {},
        body: '',
        writeHead(status, headers) { this.statusCode = status; if (headers)
            Object.assign(this.headers, headers); return this; },
...
describe('POST /api/security/bridge-enroll (HTTP)', () => {
    ...
    it('enrolls end-to-end over the route (MARVEEN_SSH_DIR seam) and audits', async () => {
        ...
        const audit = getDb().prepare("SELECT new_value FROM config_change_log WHERE key='security.bridge_enroll'").all();
...
describe('DELETE /api/auth/device-keys/:id for a paired key', () => {
    it('reports ssh_removed:false when the authorized_keys side fails (partial revoke)', async () => {
        ...
        const audit = getDb().prepare("SELECT new_value FROM config_change_log WHERE key='security.bridge_revoke'").all();
