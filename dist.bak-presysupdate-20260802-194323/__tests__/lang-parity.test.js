...
beforeAll(async () => {
    ;
    globalThis.window ||= {};
    await import(/* @vite-ignore */ '../../web/lang/hu.js');
    await import(/* @vite-ignore */ '../../web/lang/en.js');
    const i18n = globalThis.window._i18n;
    hu = i18n.hu;
    en = i18n.en;
});
describe('lang parity (hu.js vs en.js)', () => {
    ...
    it('matching keys carry the same {placeholder} token sets', () => {
        ...
        for (const key of Object.keys(hu)) {
            if (!(key in en))
                continue;
            if (tokens(hu[key]) !== tokens(en[key]))
                mismatches.push(`${key}: hu[${tokens(hu[key])}] vs en[${tokens(en[key])}]`);
        }
