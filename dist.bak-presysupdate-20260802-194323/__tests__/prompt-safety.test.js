...
import { wrapUntrusted, wrapTrustedPeer, wrapScheduledTask, UNTRUSTED_PREAMBLE, TRUSTED_PEER_PREAMBLE, SCHEDULED_TASK_PREAMBLE, sanitizeAgentIdent, sanitizeAgentSource, sanitizeCapabilityTag, } from '../prompt-safety.js';
describe('wrapUntrusted', () => {
    ...
    it('coerces non-string content to string', () => {
        expect(wrapUntrusted('src', 42)).toContain('42');
    });
...
describe('sanitizeAgentIdent', () => {
    ...
    it('returns empty string for null/undefined', () => {
        expect(sanitizeAgentIdent(null)).toBe('');
        expect(sanitizeAgentIdent(undefined)).toBe('');
    });
});
describe('sanitizeAgentSource', () => {
    ...
    it('returns "unknown" for empty input so we never emit source=""', () => {
        ...
        expect(sanitizeAgentSource(null)).toBe('unknown');
        expect(sanitizeAgentSource('!!!')).toBe('unknown');
    });
});
...
describe('sanitizeCapabilityTag', () => {
    ...
    it('returns null for null/undefined input', () => {
        expect(sanitizeCapabilityTag(null)).toBeNull();
        expect(sanitizeCapabilityTag(undefined)).toBeNull();
    });
});
