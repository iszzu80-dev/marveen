...
import { classifySaturation, normalizeSaturationThresholds, resolveTaskSize, normalizeTaskSizePolicy, decideDispatchAdmission, atOrAbove, saturationSeverity, DEFAULT_SATURATION_THRESHOLDS, DEFAULT_TASK_SIZE_POLICY, LARGE_TASK_REFUSAL_FLOOR, SATURATION_ORDER, } from '../session-saturation.js';
...
function state(pct, paneSaturated = false, th = T) {
...
describe('P2-B saturation: all four required states exist and are reachable', () => {
    ...
    it('every one of the four new states is produced by SOME input (no dead state)', () => {
        ...
        for (let pct = 0; pct <= 1.2; pct += 0.005)
            produced.add(state(Number(pct.toFixed(3))));
        for (const s of ['warning', 'no_new_large_task', 'checkpoint_required', 'hard_stop']) {
...
describe('P2-B admission: THE load-bearing rule', () => {
    const sat = (pct, paneSaturated = false) => classifySaturation({ pct, paneSaturated });
...
describe('P2-B: no model invocation on the packet / taskSize decision paths', () => {
    ...
    it('the whole src/ tree has no module named like a packet-size or task-size estimator model call', () => {
        ...
        for (const n of names)
            expect(n).not.toMatch(/llm.*(packet|task-?size)|(packet|task-?size).*llm/i);
    });
});
