...
import { getOptimizationDecisionEvents, initOptimizationDecisionsSchema, listOptimizationDecisions, OPTIMIZATION_DECISIONS_FORBIDDEN_CALLS, setDecisionStatus, upsertDecisionsFromRecommendations, } from '../optimization/optimization-decisions.js';
...
describe('optimization decision schema and audit trail', () => {
    ...
    it('reopens an executed package when its verdict changes and records why', () => {
        ...
        upsertDecisionsFromRecommendations(getDb(), [recommendation({ verdict: 'DOWNGRADE' })], NOW + 20);
        const [record] = listOptimizationDecisions(getDb());
    ...
    it('leaves a deferred human review untouched even when computed verdict data changes', () => {
        ...
        upsertDecisionsFromRecommendations(getDb(), [recommendation({ verdict: 'CANCEL' })], NOW + 20);
        const [record] = listOptimizationDecisions(getDb());
    ...
    it('returns package_not_found without throwing for an unknown package', () => {
        expect(setDecisionStatus(getDb(), 'missing-package', 'viewed', 'owner', NOW)).toEqual({ ok: false, error: 'package_not_found', record: null });
    });
    ...
    it('contains none of the forbidden network or process call substrings', () => {
        const source = readFileSync(join(__dirname, '../optimization/optimization-decisions.ts'), 'utf-8');
