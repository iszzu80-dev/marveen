...
function makeDbFns(db) {
    function stampMessageTrace(id, traceId, spanId, parentSpanId) {
        return db.prepare(`
    ...
    `).run(s.trace_id, s.span_id, s.parent_span_id ?? null, s.agent_id, s.operation, s.start_ms, s.end_ms ?? null, s.status ?? 'running', s.attributes ?? null);
    }
    function closeOtelSpan(traceId, spanId, endMs, status) {
        return db.prepare(`
      UPDATE otel_spans SET end_ms = ?, status = ? WHERE trace_id = ? AND span_id = ?
    `).run(endMs, status, traceId, spanId).changes > 0;
    }
    function getOtelTrace(traceId) {
        return db.prepare('SELECT * FROM otel_spans WHERE trace_id = ? ORDER BY start_ms ASC').all(traceId);
    }
    function insertMsg(from, to) {
        const info = db.prepare('INSERT INTO agent_messages (from_agent, to_agent, content, status, created_at) VALUES (?, ?, ?, ?, ?)').run(from, to, 'test', 'pending', Math.floor(Date.now() / 1000));
