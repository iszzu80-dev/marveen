...
export function setSecret(id, label, value) {
    const store = readVault();
    const now = new Date().toISOString();
    const idx = store.entries.findIndex(e => e.id === id);
    const entry = { id, label, encrypted: encrypt(value), createdAt: now, updatedAt: now };
    if (idx >= 0) {
...
export function getSecretsForEnv(envMap) {
    const result = {};
