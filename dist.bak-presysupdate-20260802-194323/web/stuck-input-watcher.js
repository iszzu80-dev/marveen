...
import { stuckInputSignature, parkedPasteSignature, decideStuckInputRecovery, } from '../pane-state.js';
...
function recoverParkedPaste(label, session, host, thresholds) {
    ...
    if (recover) {
        logger.info({ label, session, attempt: next.attempts }, 'stuck-input-watcher: parked paste placeholder persisted past confirm window, sending recovery Enter');
        sendEnterToSession(session, host);
    }
    else if (next.attempts >= thresholds.maxAttempts && prev.attempts < thresholds.maxAttempts) {
        logger.warn({ label, session }, 'stuck-input-watcher: paste placeholder still parked after max recovery Enters, giving up for this spell');
    }
    return true;
}
...
function bareEnterRecovery(label, session, host) {
    ...
    if (recover) {
        logger.info({ label, session, attempt: next.attempts }, 'stuck-input-watcher: parked input persisted past confirm window, sending recovery Enter');
        sendEnterToSession(session, host);
    }
...
async function checkLocalSession(label, session, alertOnGiveUp, allowPlainReinject) {
    const prev = watchState.get(session) ?? NO_STATE;
...
export function startStuckInputWatcher() {
