...
export function controlDir() {
    ...
    if (xdg && xdg.trim())
        return join(xdg.trim(), 'marveen-ssh');
...
export const CONTROL_PATH = join(controlDir(), 'cm-%r@%h:%p');
 ...
 */
export function buildTmuxInvocation(host, localTmuxBin, tmuxArgs, remoteTmuxBin = 'tmux') {
    if (host == null)
        return { file: localTmuxBin, args: tmuxArgs };
 ...
 */
export function buildSshExec(host, remoteCmd) {
    return { file: 'ssh', args: [...SSH_OPTS, host, remoteCmd] };
}
/** True when `session` appears as an exact line in `tmux list-sessions` output. */
export function sessionInList(listOutput, session) {
 ...
 */
export function classifyRunState(listOutput, session, isRemote) {
    if (listOutput == null)
        return isRemote ? 'unreachable' : 'stopped';
 ...
 */
export function classifyRunStateFromExit(exitStatus, isRemote) {
    if (!isRemote)
        return 'stopped';
    if (typeof exitStatus === 'number' && exitStatus !== 255)
        return 'stopped';
    return 'unreachable';
}
/**
 ...
 */
export function buildContinueProbeCommand(absWorkdir) {
 ...
 */
export function cleanStaleSshSockets(host) {
    try {
