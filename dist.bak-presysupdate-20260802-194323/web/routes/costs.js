...
import { exportCostRows, rowsToCsv, exportSourceInventory, sourceInventoryToCsv, exportProviderSummary, providerSummaryToCsv, exportCategorySummary, categorySummaryToCsv, exportBudgetVariance, exportReconciliationReport, reconciliationReportToCsv, exportForecastHistory, exportDataQualityReport, exportMonthlySnapshot, exportAlerts, alertsToCsv, } from '../../costops/export.js';
...
import { listRecommendations, acceptRecommendationByKey, dismissRecommendationByKey } from '../../costops/recommendations-store.js';
import { recordInvoice, applyInvoiceAdjustment, voidInvoice, listInvoices, buildInvoiceReconciliation } from '../../costops/invoice.js';
export async function tryHandleCostOps(ctx) {
    ...
    if (path === '/api/costs/sync' && method === 'POST') {
        try {
            ...
            if (provider === 'render') {
                ...
                result = { ok: cost.ok || usage.ok, provider: 'anthropic', cost, usage };
            }
                ...
                result = { ok: report.outcomes.some(o => o.status === 'ok'), ...report };
            }
            else {
                json(res, { error: `unsupported provider '${provider}' (supported: render, openai, github, deepseek, codex, anthropic, all)` }, 400);
                return true;
            }
    ...
    if (path === '/api/costs/email-ingest' && method === 'POST') {
        try {
            ...
            const result = ingestEmailCosts(getDb(), entries, { fxUsdHuf, fxEurHuf, now });
    ...
    if (path === '/api/costs/period-close' && method === 'GET') {
        try {
            ...
            if (!month) {
                json(res, { error: 'missing ?month=YYYY-MM' }, 400);
                return true;
            }
            const { config } = loadCostopsConfig();
            const now = Math.floor(Date.now() / 1000);
    ...
    if (path === '/api/costs/corrections' && method === 'GET') {
        try {
            ...
            if (!Number.isFinite(lineId) || lineId <= 0) {
                json(res, { error: 'missing or invalid ?chain=<lineId>' }, 400);
                return true;
            }
    ...
    if (path === '/api/costs/workspace-alerts' && method === 'POST') {
        try {
            ...
            const entries = Array.isArray(body?.entries) ? body.entries : [];
            const now = Math.floor(Date.now() / 1000);
            const result = ingestWorkspaceAlerts(getDb(), entries, now);
            json(res, { ok: result.errors.length === 0, ...result });
        }
        catch (err) {
    ...
    if (path.startsWith('/api/costs/export/') && method === 'GET') {
        ...
        const month = url.searchParams.get('month') || undefined;
        const now = Math.floor(Date.now() / 1000);
        const sendCsvOrJson = (csv, jsonBody) => {
            if (wantCsv && csv != null) {
                res.writeHead(200, { 'Content-Type': 'text/csv; charset=utf-8', 'Cache-Control': 'private, no-store' });
                res.end(csv);
            }
