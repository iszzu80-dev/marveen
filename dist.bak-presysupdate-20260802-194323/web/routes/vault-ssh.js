...
import { listVaultSshServers, getVaultSshServer, createVaultSshServer, updateVaultSshServer, deleteVaultSshServer, computeSshKeyStatus, getVaultSshKey, } from '../../db.js';
import { generateSshKeyPair } from './vault-ssh-keys.js';
import { setSecret } from '../vault.js';
function toApiShape(server, key) {
    return {
        id: server.id,
        name: server.name,
        host: server.host,
...
function buildKeyMap(servers) {
    const keyIds = [...new Set(servers.map(s => s.ssh_key_id).filter(Boolean))];
    const map = new Map();
...
export async function tryHandleVaultSsh(ctx) {
    const { req, res, path, method } = ctx;
    if (!path.startsWith('/api/vault/ssh-servers'))
        return false;
    ...
    if (singleMatch && method === 'PUT') {
        ...
        try {
            ...
            if (typeof data.host === 'string')
                patch.host = data.host.trim();
            if (typeof data.user === 'string')
                patch.username = data.user.trim();
            if (Number.isInteger(data.port) && data.port > 0 && data.port <= 65535)
                patch.port = data.port;
            if (data.desc !== undefined)
                patch.description = typeof data.desc === 'string' ? (data.desc.trim() || null) : null;
    ...
    if (pubKeyMatch && method === 'GET') {
        ...
        if (!server.ssh_key_id) {
            json(res, { error: 'No key assigned to this server' }, 404);
            return true;
        }
        const key = getVaultSshKey(server.ssh_key_id);
        if (!key) {
            json(res, { error: 'Assigned key not found' }, 404);
            return true;
        }
