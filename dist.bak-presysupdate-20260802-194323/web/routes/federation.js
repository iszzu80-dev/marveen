...
import { getFederationConfig, federationFileHealth, validateFederationConfig, writeFederationConfig, setFederationEnabledPreservingFile, setFederationRoutingModePreservingFile, removeFederationStore, generatePeerInboundToken, isAcceptablePeerBaseUrl, FEDERATION_MIN_TOKEN_LENGTH, MIN_ABANDON_WINDOW_MINUTES, MAX_ABANDON_WINDOW_MINUTES, FEDERATION_ROUTING_MODES, DEFAULT_ROUTING_MODE, } from '../federation/config.js';
...
const seenRefs = new Map(); // "<callerPeer>:<ref>" -> local message id
function rememberRef(key, localId) {
    if (seenRefs.size >= DEDUP_CAP) {
        const oldest = seenRefs.keys().next().value;
        if (oldest !== undefined)
            seenRefs.delete(oldest);
    }
...
export function purgeInboxDedup(peerId) {
    if (peerId === undefined) {
        seenRefs.clear();
        return;
    }
...
/** Test seam. */
export function _resetInboxDedupForTest() {
    seenRefs.clear();
}
...
export function validateInboxPayload(payload, cfg, deps, callerPeerId) {
...
function safeOutboundText(text) {
    if (!text)
        return undefined;
    return containsPrivateData(text) ? undefined : text;
}
function buildManifest(cfg, callerPeerId) {
...
async function readJsonBody(ctx) {
    ...
    try {
        ...
        json(ctx.res, { error: 'Invalid JSON' }, 400);
...
function refuseIfConfigUnhealthy(res) {
...
// ---- route handler ------------------------------------------------------------
export async function tryHandleFederation(ctx) {
    const { req, res, path, method } = ctx;
    if (path === '/api/federation/manifest' && method === 'GET') {
        const cfg = getFederationConfig();
        if (!cfg.enabled) {
            json(res, { error: 'Federation disabled' }, 403);
            return true;
        }
    ...
    if (path === '/api/federation/inbox' && method === 'POST') {
        ...
        if (callerPeerId !== null && verdict.ref !== null)
            rememberRef(`${callerPeerId}:${verdict.ref}`, msg.id);
    ...
    if (path === '/api/federation/directory' && method === 'GET') {
        ...
        const lang = resolveLang();
        const capSkills = (skills) => skills.slice(0, DIRECTORY_MAX_SKILLS_PER_AGENT).map((s) => ({
            name: s.name,
    ...
    if (path === '/api/federation/peers' && method === 'PUT') {
        ...
        if (typeof validated === 'string') {
            ...
            // loss).
            const wantsDisable = payload !== null && typeof payload === 'object' && payload.enabled !== true;
            if (wantsDisable) {
        ...
        if (!validated.enabled) {
            ...
            const failed = failPendingFederatedMessages(undefined, 'Federation disabled while pending');
            if (failed.length)
                logger.warn({ fed: true, failedPending: failed.length }, 'federation: pending outbound messages failed on disable');
        }
        ensureFederationClaudeMdSection();
    ...
    if (path === '/api/federation/enabled' && method === 'POST') {
        ...
        if (!flipped) {
            json(res, { error: 'federation.json failed validation -- federation stays disabled; fix or remove the file' }, 409);
            return true;
        }
        if (!enabled) {
            resetPeerBackoff();
    ...
    if (path === '/api/federation/routing-mode' && method === 'POST') {
        ...
        if (isErr(payload))
            return true;
        const mode = payload !== null && typeof payload === 'object' ? payload.mode : undefined;
        if (typeof mode !== 'string' || !FEDERATION_ROUTING_MODES.includes(mode)) {
            json(res, { error: `invalid mode (${FEDERATION_ROUTING_MODES.join('|')})` }, 400);
            return true;
        }
        if (!setFederationRoutingModePreservingFile(mode)) {
            json(res, { error: 'federation.json unreadable -- routing mode not persisted' }, 409);
            return true;
        }
        ensureFederationClaudeMdSection();
    ...
    if (path === '/api/federation/peers' && method === 'POST') {
        ...
        if (refuseIfConfigUnhealthy(res))
            return true;
        const p = payload;
        // --- synchronous read->mutate->write section (no await below) ---
        const cfg = getFederationConfig();
        if (!isValidIdSegment(p.id)) {
            json(res, { error: 'invalid peer id' }, 400);
            return true;
        }
        // Case-insensitive ids, stored lowercase (the operator keeps typing the
        // display name 'Teodor' for the slug 'teodor' -- see L1).
        const id = p.id.toLowerCase();
        if (id === (cfg.systemId || MAIN_AGENT_ID).toLowerCase()) {
            json(res, { error: 'peer id equals own systemId' }, 400);
            return true;
        }
        if (cfg.peers.some((peer) => peer.id === id)) {
            json(res, { error: `peer '${id}' already exists` }, 409);
            return true;
        }
        if (!isAcceptablePeerBaseUrl(p.baseUrl)) {
            json(res, { error: 'invalid baseUrl (https required; http only on loopback)' }, 400);
            return true;
        }
        let addWindow;
        if (p.abandonWindowMinutes !== undefined && p.abandonWindowMinutes !== null) {
            if (typeof p.abandonWindowMinutes !== 'number' || !Number.isInteger(p.abandonWindowMinutes)
                ...
                json(res, { error: `invalid abandonWindowMinutes (${MIN_ABANDON_WINDOW_MINUTES}..${MAX_ABANDON_WINDOW_MINUTES})` }, 400);
                return true;
            }
            addWindow = p.abandonWindowMinutes;
        }
        if (p.shareCapabilitySummaries !== undefined && typeof p.shareCapabilitySummaries !== 'boolean') {
            json(res, { error: 'invalid shareCapabilitySummaries (boolean)' }, 400);
            return true;
        }
        ...
        const next = { enabled: cfg.enabled, systemId: cfg.systemId || MAIN_AGENT_ID, routingMode: cfg.routingMode, peers: [...cfg.peers, peer] };
        const validated = validateFederationConfig(next);
        if (typeof validated === 'string') {
            json(res, { error: `Invalid peer: ${validated}` }, 400);
            return true;
        }
        writeFederationConfig(validated);
        ensureFederationClaudeMdSection();
    ...
    if (peerIdMatch && (method === 'PATCH' || method === 'DELETE')) {
        ...
        if (method === 'PATCH') {
            ...
            if (!existing) {
                json(res, { error: 'Unknown peer' }, 404);
                return true;
            }
            const updated = { ...existing };
            if (p.baseUrl !== undefined) {
                if (!isAcceptablePeerBaseUrl(p.baseUrl)) {
                    json(res, { error: 'invalid baseUrl' }, 400);
                    return true;
                }
                updated.baseUrl = p.baseUrl.replace(/\/+$/, '');
            }
            if (p.outboundToken !== undefined) {
                if (p.outboundToken === '' || p.outboundToken === null)
                    updated.outboundToken = '';
                else if (typeof p.outboundToken === 'string' && p.outboundToken.trim().length >= FEDERATION_MIN_TOKEN_LENGTH)
                    updated.outboundToken = p.outboundToken.trim();
                else {
                    json(res, { error: `invalid outboundToken (min ${FEDERATION_MIN_TOKEN_LENGTH} chars, or empty)` }, 400);
                    return true;
                }
            }
            if (p.abandonWindowMinutes !== undefined) {
                if (p.abandonWindowMinutes === null)
                    delete updated.abandonWindowMinutes;
            ...
            if (p.shareCapabilitySummaries !== undefined) {
                if (typeof p.shareCapabilitySummaries !== 'boolean') {
                    json(res, { error: 'invalid shareCapabilitySummaries (boolean)' }, 400);
                    return true;
                }
                if (p.shareCapabilitySummaries)
                    updated.shareCapabilitySummaries = true;
                else
                    delete updated.shareCapabilitySummaries;
            }
            const next = { ...cfg, peers: cfg.peers.map((peer) => (peer.id === id ? updated : peer)) };
            const validated = validateFederationConfig(next);
            if (typeof validated === 'string') {
                json(res, { error: `Invalid peer update: ${validated}` }, 400);
                return true;
            }
            writeFederationConfig(validated);
        ...
        const next = { ...cfg, peers: cfg.peers.filter((peer) => peer.id !== id) };
        const validated = validateFederationConfig(next);
        if (typeof validated === 'string') {
            json(res, { error: `Invalid config after removal: ${validated}` }, 400);
            return true;
        }
        writeFederationConfig(validated);
    ...
    if (rotateMatch && method === 'POST') {
        ...
        const next = { ...cfg, peers: cfg.peers.map((peer) => (peer.id === id ? { ...peer, inboundToken: fresh } : peer)) };
        const validated = validateFederationConfig(next);
        if (typeof validated === 'string') {
            json(res, { error: `Invalid config after rotation: ${validated}` }, 400);
            return true;
        }
        writeFederationConfig(validated);
