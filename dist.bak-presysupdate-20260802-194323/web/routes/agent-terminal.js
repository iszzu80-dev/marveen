...
import { literalKeyArgs, specialKeyArgs, loginSequence } from '../tmux-keys.js';
...
function isTmuxSessionAlive(session) {
    try {
...
// the text is safe.
export function sanitizeLiteralKeys(keys) {
    if (keys.length <= 1)
        return keys; // single keystroke -- literal passthrough
    const noPasteMarkers = keys.replace(/\x1b\[20[01]~/g, '');
    const singleLine = noPasteMarkers.replace(/[\r\n]+/g, '');
    return singleLine.replace(/^\s+|\s+$/g, '');
}
async function runLoginSteps(session, steps) {
...
export async function tryHandleAgentTerminal(ctx) {
    ...
    if (path === '/api/terminal-input') {
        ...
        if (method === 'POST') {
            ...
            let enabled;
            try {
                enabled = JSON.parse(body.toString()).enabled;
            }
            catch {
    ...
    if (streamMatch && method === 'GET') {
        ...
        const tick = () => {
            if (closed || inFlight)
                return; // skip if a slow capture is still running
            inFlight = true;
        ...
        const stop = () => { closed = true; clearInterval(interval); };
    ...
    if (loginMatch && method === 'POST') {
        ...
        let phase;
        try {
            phase = JSON.parse(body.toString()).phase;
        }
        catch { /* default below */ }
