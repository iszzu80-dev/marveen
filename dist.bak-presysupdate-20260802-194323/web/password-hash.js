...
function parseScryptParams(segment) {
    const out = {};
    ...
    if (out.ln === undefined || out.r === undefined || out.p === undefined)
        return null;
    // Bound ln so a hostile stored hash can't request an absurd N and OOM/hang.
    if (out.ln < 1 || out.ln > 20)
        return null;
    return out;
}
async function verifyScrypt(pw, phc) {
...
export async function verifyPassword(pw, phc) {
    if (typeof pw !== 'string' || typeof phc !== 'string' || phc.length === 0)
        return false;
    if (phc.startsWith('$scrypt$'))
        return verifyScrypt(pw, phc);
