...
export function isProviderTrusted(modelName, trustedPrefixes) {
...
export function parseTrustedProviders(raw) {
    if (!raw || !raw.trim())
        return new Set(['claude']); // safe default
    return new Set(raw
        .split(',')
...
function contextNearMatch(content, matchIndex, contextRe) {
...
export function matchSensitivityPatterns(content, patterns) {
...
export function classifyContent(content, config) {
...
// ---- dispatch evaluation ------------------------------------------------------
const ALLOWED_CATEGORIES_FOR_UNTRUSTED = new Set([
    'public',
]);
export function evaluateDispatch(content, targetModel, config, trustedPrefixes) {
    ...
    const effectiveCategory = category === 'public' || category === 'internal' || category === 'restricted'
        ? category
        : 'restricted';
...
function normalizePatterns(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw
        .filter((p) => p && typeof p === 'object' && typeof p.name === 'string' && typeof p.pattern === 'string')
        .map((p) => ({
