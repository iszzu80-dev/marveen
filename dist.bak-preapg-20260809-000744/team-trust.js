 ...
 *   6. Otherwise → false.
 */
export function isTrustedPeer(from, to, ctx) {
    if (!from || !to)
        return false;
    if (from === to)
        return false;
    ...
    if (!ctx.isKnownAgent(from) || !ctx.isKnownAgent(to))
        return false;
    if (from === ctx.mainAgentId || to === ctx.mainAgentId)
        return true;
    const fromTeam = ctx.readAgentTeam(from);
    const toTeam = ctx.readAgentTeam(to);
    if (fromTeam.reportsTo === to)
        return true;
    if (toTeam.reportsTo === from)
        return true;
    if (fromTeam.delegatesTo.includes(to))
        return true;
    if (toTeam.delegatesTo.includes(from))
        return true;
    if ((fromTeam.trustFrom ?? []).includes(to))
        return true;
    if ((toTeam.trustFrom ?? []).includes(from))
        return true;
    return false;
}
