...
describe('taskInjectionRank: forceSend outranks tasks outranks heartbeats', () => {
    ...
    it('ranks plain tasks before heartbeats', () => {
        expect(taskInjectionRank({ forceSend: false, type: 'task' })).toBeLessThan(taskInjectionRank({ forceSend: false, type: 'heartbeat' }));
        expect(taskInjectionRank({ type: 'command' })).toBeLessThan(taskInjectionRank({ type: 'heartbeat' }));
    });
