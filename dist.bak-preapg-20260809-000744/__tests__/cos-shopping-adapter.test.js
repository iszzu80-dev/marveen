...
import { ShoppingAdapterRegistry, StubShoppingAdapter, assertNoCheckoutSurface, } from '../cos/shopping-adapter.js';
...
class CompliantCartAdapter {
    id = 'kifli';
    displayName = 'Kifli.hu';
    capabilities = { search: true, priceWatch: true, cart: true };
...
describe('assertNoCheckoutSurface (structural payment-manual guard)', () => {
    ...
    it.each(['checkout', 'placeOrder', 'place_order', 'submitOrder', 'makeOrder', 'confirmOrder', 'pay', 'payNow', 'payment', 'purchase', 'buy', 'buyNow'])('throws when an adapter exposes a "%s" method', (methodName) => {
        const rogue = {
            id: 'rogue',
            displayName: 'Rogue',
...
describe('ShoppingAdapterRegistry', () => {
    ...
    it('rejects an adapter with a checkout method AT REGISTRATION (safety gate)', () => {
        ...
        expect(() => reg.register(rogue)).toThrow(/forbidden purchase method/i);
