...
import { parseApiOffer, parseApiOffers, isDebitFriendly, filterByCategory, underFullPrice, cheapestByFullPrice, rentalDayCount, assertRentalAdapterSafe, } from '../cos/rental-adapter.js';
...
// ---- mocked-fetch search() ----------------------------------------------------
function sqFor(pickup, dropoff) {
    return Buffer.from(JSON.stringify({ PickupDateTime: pickup, DropOffDateTime: dropoff })).toString('base64');
}
function jsonResp(body) { return { json: async () => body }; }
const VLC = { countryId: 26, cityId: 462, placeId: 462, label: 'Valencia (all locations)' };
const AGP = { countryId: 26, cityId: 455, placeId: 1848, label: 'Malaga Airport (AGP)' };
...
describe('DiscoverCarsAdapter.search (mocked fetch)', () => {
    ...
    it('resolveLocation maps an autocomplete result to ids', async () => {
        const fetchImpl = (async () => jsonResp({ success: true, result: [{ place: 'Malaga Airport (AGP)', countryID: 26, cityID: 455, placeID: 1848 }] }));
...
describe('no-booking safety guard', () => {
    it('rejects book/reserve/checkout/pay method names', () => {
        for (const n of ['bookRental', 'book', 'reserve', 'checkout', 'payNow'])
            expect(isForbiddenMethodName(n)).toBe(true);
    });
    it('allows search/read names', () => {
        for (const n of ['search', 'resolveLocation', 'parseApiOffer', 'bookmark'])
