...
const DEVICE_AUTH = { kind: 'device' };
const FEDERATION_AUTH = { kind: 'federation', peer: 'test-peer' };
const GOOD_PW = 'super-secret-pw';
async function seedUser(username, password = GOOD_PW) {
...
describe('users CRUD', () => {
    ...
    it('still allows a SESSION principal to manage users (existing behavior)', async () => {
        await seedUser('alice');
        const SESSION_AUTH = { kind: 'session', user: 'alice' };
