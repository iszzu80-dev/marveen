...
describe('syncDeepSeekBalance (offline stub)', () => {
    ...
    it('records a snapshot + baseline line on first sync, then MTD spend on a later drop', async () => {
        ...
        const bal = (v) => async () => ({ is_available: true, balance_infos: [{ currency: 'USD', total_balance: v }] });
        ...
        const line0 = db.prepare("SELECT billed_cost, confidence FROM cost_line_items WHERE source_id='deepseek-api'").get();
        ...
        const line1 = db.prepare("SELECT billed_cost FROM cost_line_items WHERE source_id='deepseek-api'").get();
        expect(line1.billed_cost).toBeCloseTo(1.8 * 360, 2); // idempotent upsert, single line
        expect(db.prepare("SELECT COUNT(*) c FROM cost_line_items WHERE source_id='deepseek-api'").get().c).toBe(1);
    ...
    it('errors (no line) when the vault key is missing', async () => {
        ...
        expect(db.prepare("SELECT COUNT(*) c FROM cost_line_items WHERE source_id='deepseek-api'").get().c).toBe(0);
    });
    ...
    it('upserts a single entitlements row per sync, remaining tracks the live balance, status derives from absolute thresholds', async () => {
        ...
        let row = db.prepare("SELECT * FROM entitlements WHERE dedup_key='deepseek|prepaid_balance'").get();
        ...
        row = db.prepare("SELECT * FROM entitlements WHERE dedup_key='deepseek|prepaid_balance'").get();
        expect(row.remaining).toBe(2.5);
        expect(row.status).toBe('warning');
        expect(db.prepare("SELECT COUNT(*) c FROM entitlements WHERE provider='deepseek'").get().c).toBe(1);
        // critical band
    ...
    it('does not touch entitlements when the sync itself fails (no key)', async () => {
        ...
        expect(db.prepare("SELECT COUNT(*) c FROM entitlements WHERE provider='deepseek'").get().c).toBe(0);
    });
    it('populates forecast_exhaustion_at once there is enough history, null on the first (baseline-only) sync', async () => {
        ...
        let row = db.prepare("SELECT forecast_exhaustion_at FROM entitlements WHERE dedup_key='deepseek|prepaid_balance'").get();
        ...
        row = db.prepare("SELECT forecast_exhaustion_at FROM entitlements WHERE dedup_key='deepseek|prepaid_balance'").get();
    ...
    it('sets actual_source=provider_api on the cost_line_items row (card 7d086cd3, F1)', async () => {
        ...
        const bal = (v) => async () => ({ is_available: true, balance_infos: [{ currency: 'USD', total_balance: v }] });
        await syncDeepSeekBalance(db, t0, { apiKey: 'k', fxUsdHuf: 360, httpGetJson: bal('5.00') });
        const line = db.prepare("SELECT actual_source, confidence FROM cost_line_items WHERE source_id='deepseek-api'").get();
    ...
    it('retains original USD amount + fx_rate on the cost_line_items row once a drop is observed (card a1552362, item 2)', async () => {
        ...
        let line = db.prepare("SELECT original_amount, original_currency, fx_rate FROM cost_line_items WHERE source_id='deepseek-api'").get();
        ...
        line = db.prepare("SELECT original_amount, original_currency, fx_rate FROM cost_line_items WHERE source_id='deepseek-api'").get();
    ...
    it('does not fabricate an original currency when no fx rate is configured (fxUsdHuf 0)', async () => {
        ...
        const line = db.prepare("SELECT original_currency, fx_rate FROM cost_line_items WHERE source_id='deepseek-api'").get();
