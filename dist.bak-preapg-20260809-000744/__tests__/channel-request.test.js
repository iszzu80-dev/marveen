...
import { initDatabase, getDb, upsertChannelRequest, listPendingChannelRequests, updateChannelRequestStatus, updateChannelRequestName, } from '../db.js';
beforeAll(() => {
...
describe('updateChannelRequestStatus', () => {
    it('sets resolved_at when approving', () => {
        ...
        const rows = listPendingChannelRequests('test-agent');
        updateChannelRequestStatus(rows[0].id, 'approved');
        const row = getDb().prepare('SELECT resolved_at, status FROM pending_channel_requests WHERE id = ?').get(rows[0].id);
