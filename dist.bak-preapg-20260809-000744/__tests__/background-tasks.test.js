...
describe('background_tasks schema and CRUD', () => {
    ...
    it('inserts a running task', () => {
        ...
        const row = db.prepare('SELECT * FROM background_tasks WHERE id = ?').get('ABCD1234');
    ...
    it('finishes a task with done status', () => {
        ...
        const row = db.prepare('SELECT * FROM background_tasks WHERE id = ?').get('AAAA1111');
    ...
    it('counts running tasks per agent', () => {
        ...
        const count = db.prepare("SELECT COUNT(*) as c FROM background_tasks WHERE agent_id = ? AND status = 'running'").get('marveen').c;
        expect(count).toBe(2);
        const samuCount = db.prepare("SELECT COUNT(*) as c FROM background_tasks WHERE agent_id = ? AND status = 'running'").get('samu').c;
    ...
    it('supports timeout status', () => {
        ...
        const row = db.prepare('SELECT * FROM background_tasks WHERE id = ?').get('T1000001');
        expect(row.status).toBe('timeout');
    });
    it('atomic create respects concurrency limit', () => {
        const now = Math.floor(Date.now() / 1000);
        const maxConcurrent = 3;
        const atomicCreate = db.transaction((id, agentId, prompt, session) => {
    ...
    it('marks orphaned tasks as failed', () => {
        ...
        const tasks = db.prepare('SELECT * FROM background_tasks ORDER BY id').all();
        expect(tasks.find((t) => t.id === 'OR000001').status).toBe('failed');
        expect(tasks.find((t) => t.id === 'OR000002').status).toBe('failed');
        expect(tasks.find((t) => t.id === 'OR000003').status).toBe('done');
    });
    it('DELETE captures output before kill (order test)', () => {
        const now = Math.floor(Date.now() / 1000);
        db.prepare('INSERT INTO background_tasks (id, agent_id, prompt, status, tmux_session, started_at) VALUES (?, ?, ?, ?, ?, ?)')
            .run('DE000001', 'marveen', 'cancel me', 'running', 'bg-DE000001', now);
        const task = db.prepare('SELECT * FROM background_tasks WHERE id = ?').get('DE000001');
        expect(task.status).toBe('running');
        expect(task.tmux_session).toBe('bg-DE000001');
        db.prepare('UPDATE background_tasks SET status = ?, finished_at = ?, output = ? WHERE id = ?')
            .run('failed', now + 1, 'captured output before kill', 'DE000001');
        const cancelled = db.prepare('SELECT * FROM background_tasks WHERE id = ?').get('DE000001');
