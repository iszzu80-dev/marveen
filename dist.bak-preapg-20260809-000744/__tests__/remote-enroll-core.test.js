...
import { validatePublicKeyLine, buildRestrictedLine, mergeAuthorizedKeys, parseHostKeyPub, buildBundle, encodeBundle, decodeBundle, RemoteEnrollError, RESTRICT_OPTIONS, restrictOptions, parseKeyscanEd25519, resolveHostKey, HOST_KEY_PUB_CANDIDATES, dashboardTokenDecision, } from '../remote-enroll-core.js';
...
function makeEd25519Base64(keyByte = 0x42) {
...
describe('buildRestrictedLine', () => {
    it('produces the verbatim restricted entry', () => {
        ...
        expect(line).toBe(`restrict,port-forwarding,permitopen="127.0.0.1:3420",command="/bin/false" ssh-ed25519 ${B64} marveen-remote:${UUID}`);
    ...
    it('threads a NON-DEFAULT dashboard port into the permitopen, keeping it narrow', () => {
        ...
        expect(line).toBe(`restrict,port-forwarding,permitopen="127.0.0.1:3421",command="/bin/false" ssh-ed25519 ${B64} marveen-remote:${UUID}`);
    ...
    it('restrictOptions defaults to REMOTE_PORT and narrows to the given port', () => {
        ...
        expect(restrictOptions(3421)).toBe('restrict,port-forwarding,permitopen="127.0.0.1:3421",command="/bin/false"');
    });
});
...
describe('bundle', () => {
    const base = validatePublicKeyLine(VALID_LINE);
...
describe('resolveHostKey', () => {
    ...
    it('returns null when every source fails', () => {
        expect(resolveHostKey({ readFile: () => null, keyscan: () => null }, ['/a'])).toBeNull();
        expect(resolveHostKey({ readFile: () => null, keyscan: () => '# nothing\n' }, ['/a'])).toBeNull();
    });
