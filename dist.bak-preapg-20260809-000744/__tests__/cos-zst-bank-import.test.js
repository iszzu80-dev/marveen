...
describe('ZST bank statement importer (v1 CSV)', () => {
    ...
    it('imports rows UNMATCHED and is idempotent on re-import', () => {
        ...
        expect(db.prepare(`SELECT COUNT(*) n FROM zst_bank_transactions`).get().n).toBe(3);
        const st = db.prepare(`SELECT reconciliation_status FROM zst_bank_transactions LIMIT 1`).get();
