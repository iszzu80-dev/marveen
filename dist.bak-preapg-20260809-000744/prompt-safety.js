...
const SECURITY_TAG_NAMES = ['untrusted', 'trusted-peer', 'scheduled-task'];
// The \s* after '<' tolerates "< untrusted>" variants that some LLMs still
// parse as a tag even though real HTML parsers reject them.
const SECURITY_TAG_RX = new RegExp(`<\\s*\\/?\\s*(${SECURITY_TAG_NAMES.join('|')})\\b[^>]*>`, 'gi');
...
export function sanitizeCapabilityTag(raw) {
...
export function sanitizeOriginNote(raw) {
...
export function sanitizeAgentSource(raw) {
...
export function generateFetchNonce() {
...
export function wrapUntrustedFetch(url, content, nonce) {
    if (content == null)
        return '';
    const text = String(content);
    if (text.length === 0)
...
export function wrapTrustedPeer(source, content) {
    if (content == null)
        return '';
    const text = String(content);
    if (text.length === 0)
...
export function wrapScheduledTask(source, content) {
    if (content == null)
        return '';
    const text = String(content);
    if (text.length === 0)
...
export function wrapChannelInbound(content) {
    if (content == null)
        return '';
    const text = String(content);
    if (text.length === 0)
