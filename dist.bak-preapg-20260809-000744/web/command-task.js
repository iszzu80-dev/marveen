...
function load() {
    if (healthMap)
        return healthMap;
    try {
        healthMap = JSON.parse(readFileSync(HEALTH_PATH, "utf-8"));
    }
    catch {
        healthMap = {};
    }
...
function persist() {
    try {
        atomicWriteFileSync(HEALTH_PATH, JSON.stringify(healthMap ?? {}, null, 2));
    }
    catch (err) {
        logger.warn({ err }, "command-task: failed to persist health map");
    }
}
...
export function evaluateCommandResult(prev, success, failThreshold, now) {
...
export function runCommandTask(task, now) {
    if (!task.command) {
    ...
    persist();
    try {
        appendTaskRun(task.name, task.agent || "system");
    }
    catch { /* non-fatal */ }
