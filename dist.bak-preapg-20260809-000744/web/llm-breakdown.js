...
function buildUserPrompt(title, description, agents) {
    const parts = [
        `<card_title>${title}</card_title>`,
    ];
    if (description)
        parts.push(`<card_description>${description}</card_description>`);
    parts.push(`Available team members: ${agents.join(', ')}`);
    return parts.join('\n');
}
function getValidAssignees() {
...
async function callBreakdownAgent(userPrompt) {
...
export function validateSubtasks(raw, validAssignees) {
    if (!Array.isArray(raw))
        throw new Error('LLM response is not an array');
    const maxSubtasks = getMaxSubtasks();
    if (raw.length < 1 || raw.length > maxSubtasks * 2)
        throw new Error(`Expected 1-${maxSubtasks * 2} subtasks, got ${raw.length}`);
    const validPriorities = new Set(['low', 'normal', 'high', 'urgent']);
    const allowed = validAssignees ?? getValidAssignees();
    return raw.map((item, i) => {
        if (!item.title || typeof item.title !== 'string')
            throw new Error(`Subtask ${i}: missing title`);
        if (!item.description || typeof item.description !== 'string')
            throw new Error(`Subtask ${i}: missing description`);
...
export async function generateBreakdown(title, description) {
