...
export function buildManifest(raw, brandName) {
    return raw
        .replace(/^(\s*"name"\s*:\s*)"[^"]*"/m, (_m, p) => `${p}${JSON.stringify(`${brandName} Dashboard`)}`)
        .replace(/^(\s*"short_name"\s*:\s*)"[^"]*"/m, (_m, p) => `${p}${JSON.stringify(brandName)}`);
}
...
function assetVersion(webDir, fileName) {
    try {
...
function serveIndexHtml(ctx, webDir) {
    const { req, res } = ctx;
    try {
        ...
        const html = readFileSync(filePath, 'utf-8')
            .replace(/(<script\s+src=")\/app\.js(")/, `$1/app.js?v=${assetVersion(webDir, 'app.js')}$2`)
            // style.css gets the same ?v= cache-busting as app.js so both can be
            // served with a long max-age (see tryHandleStatic below).
            .replace(/(<link\s+rel="stylesheet"\s+href=")\/style\.css(")/, `$1/style.css?v=${assetVersion(webDir, 'style.css')}$2`)
            ...
            .replace(/(<meta name="apple-mobile-web-app-title" content=")[^"]*(">)/, `$1${escapeAttr(BRAND_NAME)}$2`);
        res.writeHead(200, {
...
function detectAvatarType() {
    for (const ext of ['.png', '.jpg', '.jpeg', '.webp']) {
        if (existsSync(join(PROJECT_ROOT, 'store', `marveen-avatar${ext}`)))
            return MIME[ext];
    }
    return null;
}
...
export async function tryHandleStatic(ctx, webDir) {
    const { req, res, path } = ctx;
    if (path === '/' || path === '/index.html') {
        serveIndexHtml(ctx, webDir);
        return true;
    }
    ...
    if (path === '/style.css') {
        serveFile(req, res, join(webDir, 'style.css'), { cacheSeconds: 86400 });
        return true;
    }
    if (path === '/app.js') {
        serveFile(req, res, join(webDir, 'app.js'), { cacheSeconds: 86400 });
        return true;
    }
    ...
    // "Betoltes..." forever.
    if (path === '/coscontrol.js') {
        serveFile(req, res, join(webDir, 'coscontrol.js'), { cacheSeconds: 86400 });
        return true;
    } // LOCAL-FORK: cos seam (keep on rebase)
    if (path === '/apg.js') {
        serveFile(req, res, join(webDir, 'apg.js'), { cacheSeconds: 86400 });
        return true;
    }
    if (path === '/apg.css') {
        serveFile(req, res, join(webDir, 'apg.css'), { cacheSeconds: 86400 });
        return true;
    }
    ...
    if (path.startsWith('/icons/')) {
        ...
        if (existsSync(iconPath)) {
            serveFile(req, res, iconPath, { cacheSeconds: 3600 });
            return true;
        }
        res.writeHead(404);
        res.end();
