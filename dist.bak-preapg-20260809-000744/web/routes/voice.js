...
function isSafeStateDir(dir) {
    const resolved = dir.replace(/\/$/, '');
    if (resolved.includes('..'))
        return false;
    if (!existsSync(join(resolved, '.env')))
        return false;
    if (resolved.startsWith(CHANNELS_BASE + '/') || resolved === CHANNELS_BASE)
        return true;
...
function voiceOnnxPath(model) {
    if (!KNOWN_VOICE_MODELS.has(model))
        return null;
    const p = join(VOICE_DIR, 'voices', `${model}.onnx`);
    return existsSync(p) ? p : null;
}
function isVoiceInstalled() {
...
function runProc(cmd, args, opts = {}) {
    return new Promise((resolve) => {
        ...
        if (opts.stdinData != null) {
            proc.stdin.write(opts.stdinData, 'utf-8');
            proc.stdin.end();
        }
        proc.on('close', (code) => {
            if (timer)
...
export async function transcribeVoiceFile(fileId, stateDir) {
    if (!isVoiceInstalled())
        return null;
    if (!SAFE_FILE_ID_RE.test(fileId))
        return null;
    if (!isSafeStateDir(stateDir))
        return null;
...
export async function tryHandleVoice(ctx) {
    ...
    if (path === '/api/voice/directive' && method === 'GET') {
        ...
        if (!agentId || !/^[a-zA-Z0-9_-]+$/.test(agentId)) {
            json(res, { error: 'Invalid agent' }, 400);
            return true;
        }
        if (!chatId || !/^\d+$/.test(chatId)) {
            json(res, { error: 'Invalid chat_id' }, 400);
            return true;
        }
    ...
    if (path === '/api/voice/modality' && method === 'GET') {
        ...
        const chatId = ctx.url.searchParams.get('chat') ?? '';
        if (!agentId || !chatId) {
            json(res, { error: 'agent and chat required' }, 400);
            return true;
        }
    ...
    if (path === '/api/voice/modality/set' && method === 'POST') {
        ...
        if (!agentId || !/^[a-zA-Z0-9_-]+$/.test(agentId)) {
            json(res, { error: 'Invalid agent_id' }, 400);
            return true;
        }
        if (!chatId || !/^\d+$/.test(chatId)) {
            json(res, { error: 'Invalid chat_id' }, 400);
        ...
        if (modality !== 'voice' && modality !== 'text') {
            json(res, { error: 'modality must be voice or text' }, 400);
            return true;
        }
        setLastInboundModality(agentId, chatId, modality);
        json(res, { ok: true });
        return true;
    }
    ...
    if (path === '/api/voice/stt' && method === 'POST') {
        if (!isVoiceInstalled()) {
            json(res, { error: 'Voice toolkit not installed' }, 503);
            return true;
        }
        const body = await readBody(req);
        let data;
        ...
        if (!SAFE_FILE_ID_RE.test(fileId)) {
            json(res, { error: 'Invalid file_id' }, 400);
            return true;
        }
        if (!isSafeStateDir(stateDir)) {
            json(res, { error: 'Invalid state_dir' }, 400);
            return true;
        }
    ...
    if (path === '/api/voice/tts' && method === 'POST') {
        ...
        const result = await runProc(VENV_PY, [VTOOLS_PY, 'speak', onnxPath, stateDir, chatId, text], { timeoutMs: 90_000 });
        if (result.code !== 0) {
    ...
    if (path === '/api/voice/install' && method === 'POST') {
        ...
        child.on('close', (code) => {
            ...
            if (code !== 0)
                logger.warn({ code }, '/api/voice/install: install-voice.sh exited non-zero');
            else
                logger.info('/api/voice/install: install-voice.sh completed successfully');
        });
