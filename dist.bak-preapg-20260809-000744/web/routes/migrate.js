...
export async function tryHandleMigrate(ctx) {
    const { req, res, path, method } = ctx;
    if (path === '/api/migrate/scan' && method === 'POST') {
        const body = await readBody(req);
        const { sourcePath } = JSON.parse(body.toString());
        if (!sourcePath?.trim()) {
            json(res, { error: 'Útvonal megadása kötelező' }, 400);
            return true;
        }
        if (!existsSync(sourcePath)) {
            json(res, { error: 'A megadott útvonal nem létezik' }, 404);
            return true;
        }
        const findings = [];
        const addFinding = (type, filePath) => {
        ...
        try {
            ...
            for (const dir of scanDirs) {
                ...
                if (!existsSync(scanPath))
                    continue;
                const files = readdirSync(scanPath).filter(f => (f.endsWith('.md') || f.endsWith('.txt') || f.endsWith('.json')) &&
                    !['package.json', 'tsconfig.json', 'package-lock.json', '.mcp.json'].includes(f));
                for (const f of files) {
                    const fullPath = join(scanPath, f);
                    if (findings.some(fi => fi.path === fullPath))
                        continue;
                    try {
                        ...
                        if (stat.isFile() && stat.size > 20) {
                            ...
                            let type = 'memory';
                            if (lower.includes('soul') || lower.includes('personality'))
                                type = 'personality';
    ...
    if (path === '/api/migrate/run' && method === 'POST') {
        const body = await readBody(req);
        const { findings, agentId: targetAgent } = JSON.parse(body.toString());
                ...
                if (ext === 'json') {
                        ...
                        if (Array.isArray(data)) {
                            for (const item of data) {
                                ...
                                if (String(text).trim().length > 20)
                                    chunks.push(String(text).slice(0, 2000));
                            }
                        }
                    ...
                    for (const section of sections) {
                        if (section.trim().length > 20)
                            chunks.push(section.trim().slice(0, 2000));
                    }
                }
            }
            catch { }
