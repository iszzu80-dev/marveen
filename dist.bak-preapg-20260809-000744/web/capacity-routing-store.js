...
export function readRuntimeOverlay(agent, path = OVERLAY_PATH) {
    const file = readOverlayFile(path);
    const entry = file[agent];
    return entry ?? null;
}
export function writeRuntimeOverlay(agent, entry, path = OVERLAY_PATH) {
    const file = readOverlayFile(path);
    file[agent] = entry;
...
export function clearRuntimeOverlay(agent, path = OVERLAY_PATH) {
    const file = readOverlayFile(path);
    if (!(agent in file))
        return;
    delete file[agent];
...
export function listRuntimeOverlays(path = OVERLAY_PATH) {
    return readOverlayFile(path);
}
const DEFAULT_CAPACITY_ROUTING_CONFIG = {
    enabled: false,
    candidates: [],
...
function normalizeCandidate(raw) {
    ...
    if (typeof o.provider !== 'string' || !o.provider.trim())
        return null;
    if (typeof o.authProfile !== 'string' || !o.authProfile.trim())
        return null;
...
export function normalizeCapacityRoutingConfig(raw) {
...
export function readCapacityRoutingConfig(path = CONFIG_PATH) {
    try {
...
export function isEnabledForRouting(provider, authProfile, config = readCapacityRoutingConfig()) {
 ...
 */
export function resolveRuntimeModel(agent, configuredModel, paths = {}) {
    ...
    if (!isEnabledForRouting(overlay.provider, overlay.authProfile, config))
        return configuredModel;
...
// ---------------------------------------------------------------------------
export function ensureCapacityRoutingConfigExample(exampleDir) {
