...
import { getFederationConfig, DEFAULT_ROUTING_MODE } from './config.js';
...
export function renderRoutingDirective(mode, lang) {
    if (lang === 'hu') {
        switch (mode) {
            case 'strong':
...
export function renderFederationBlock(cfg, id) {
...
export function renderSubAgentFederationBlock(id) {
    const hu = `${FEDERATION_BLOCK_BEGIN}
...
export function renderPolicySeed(lang) {
    if (lang === 'en') {
...
export function applyFederationBlock(content, block) {
...
function ensureBlockInFile(path, block, lang, seedPolicy) {
    try {
        if (!existsSync(path))
            return false; // no persona yet: nothing to onboard
...
export function ensureFederationClaudeMdSection() {
    try {
        ...
        const identity = { botName: BOT_NAME, mainAgentId: MAIN_AGENT_ID, webPort: WEB_PORT, lang };
        ...
        let changed = ensureBlockInFile(join(PROJECT_ROOT, 'CLAUDE.md'), cfg.enabled ? renderFederationBlock(cfg, identity) : null, lang, true);
        ...
        for (const name of catalogAgentNames()) {
            if (ensureBlockInFile(join(agentDir(name), 'CLAUDE.md'), subBlock, lang, false))
                changed = true;
        }
        if (changed)
            logger.info({ fed: true, enabled: cfg.enabled, lang }, 'federation: CLAUDE.md onboarding reconciled (main + sub-agents)');
        return changed;
    }
    catch (err) {
