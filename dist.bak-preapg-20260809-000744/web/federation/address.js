...
export function isQualifiedId(raw) {
...
export function parseQualifiedId(raw) {
    ...
    if (agent.includes('/'))
        return null;
    if (!SEGMENT_RE.test(system) || !SEGMENT_RE.test(agent))
        return null;
    return { system, agent };
}
/** A single id segment (system id, local agent name) valid on its own. */
export function isValidIdSegment(raw) {
    return typeof raw === 'string' && SEGMENT_RE.test(raw);
}
export function formatQualifiedId(system, agent) {
    return `${system}/${agent}`;
}
