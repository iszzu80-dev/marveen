...
let db = null;
export function initIngestDb(dbPath = join(STORE_DIR, DB_FILENAME)) {
    if (db)
        return db;
...
function requireDb() {
    if (!db)
        throw new Error('ingest db not initialized -- call initIngestDb() first');
    return db;
}
...
export function insertIncomingEvent(source, ev) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)
  `).run(source, ev.update_id, ev.chat_id, ev.user_id, ev.username, ev.message_id, ev.kind, ev.content, JSON.stringify(ev.meta), ev.tg_date, now);
...
export function createHandoffMessage(content) {
    const now = Math.floor(Date.now() / 1000);
    const info = requireDb().prepare('INSERT INTO agent_messages (from_agent, to_agent, content, status, created_at) VALUES (?, ?, ?, ?, ?)').run(COORDINATOR_AGENT_ID, MAIN_AGENT_ID, content, 'pending', now);
    return Number(info.lastInsertRowid);
}
export function markEventDelivered(eventId, agentMessageId) {
    const now = Math.floor(Date.now() / 1000);
    requireDb().prepare("UPDATE incoming_events SET status = 'delivered', agent_message_id = ?, delivered_at = ? WHERE id = ?").run(agentMessageId, now, eventId);
}
export function markEventFailed(eventId, error) {
...
export function getEventsNeedingHandoff(source, limit = 50) {
...
export function getOffset(source) {
    const row = requireDb().prepare('SELECT last_update_id FROM poll_offset WHERE source = ?').get(source);
...
export function setOffset(source, lastUpdateId) {
    const now = Math.floor(Date.now() / 1000);
    requireDb().prepare(`
