 ...
 */
export function findOwnNodeHolders(port, ctx) {
    ...
    for (const pid of pids) {
        ...
        if (opts.scopeToProjectRoot && ctx.selfProjectRoot != null) {
            ...
            if (candidateCwd != null && candidateCwd !== ctx.selfProjectRoot) {
                ctx.log.warn({ pid, candidateCwd, selfProjectRoot: ctx.selfProjectRoot }, 'Binary-pattern match runs from a different project root, leaving alone');
                continue;
            }
        }
        holders.push(pid);
 ...
 */
export async function terminateProcesses(pids, ctx, opts) {
    if (!pids.length)
        return;
    for (const pid of pids) {
        try {
            ...
            if (out === 'sent')
                ctx.log.info({ pid }, 'SIGTERM sent to previous instance');
        }
        catch (err) {
 ...
 */
export async function acquirePortLock(port, ctx, opts = {}) {
    ...
    let waited = 0;
    while (true) {
        if (!findOwnNodeHolders(port, ctx).length)
            return;
        if (waited >= drainMs)
            break;
 ...
 */
export function writeBufferFully(writer, buf) {
    let written = 0;
 ...
 * instead of spinning.
 */
export async function acquirePidfileLock(path, selfPid, ctx, opts = {}) {
