...
export function buildContextPacket(input) {
    const bullets = (xs) => (xs ?? []).map(s => String(s).trim()).filter(s => s.length > 0);
    return {
        ...input,
 ...
 */
export function artifactRefFromContent(path, ref, content, opts = {}) {
...
export function truncateExcerpt(text, max = MAX_EXCERPT_CHARS) {
    const t = String(text);
    if (t.length <= max)
        return t;
 ...
 */
export function renderContextPacket(p) {
    const out = [];
    out.push(`# Context Packet${p.cardId ? ` -- card ${p.cardId}` : ''}`);
    out.push('');
    const head = [`packetVersion: ${p.packetVersion}`];
    if (p.taskSize)
        head.push(`taskSize: ${p.taskSize}`);
    if (p.contextBudgetClass)
        head.push(`contextBudgetClass: ${p.contextBudgetClass}`);
    out.push(`> ${head.join(' | ')}`);
    out.push('');
    out.push('## Goal');
    out.push(p.goal);
    ...
    for (const n of p.dataSensitivityNotes)
        out.push(`- ${n}`);
    out.push('');
    out.push('## Done when');
    for (const d of p.doneWhen)
        out.push(`- ${d}`);
 ...
 */
export function estimateFreshTokens(text) {
...
export function estimatePacketFreshTokens(p) {
 ...
 *    budget anywhere.
 */
export function validateContextPacket(p) {
    const errors = [];
    const warnings = [];
    const err = (code, at, message) => errors.push({ code, at, message });
    if (!p.goal || p.goal.trim().length === 0)
        err('goal_missing', 'Goal', 'Goal is required');
    if (!p.doneWhen || p.doneWhen.length === 0)
        err('done_when_missing', 'Done when', 'At least one Done-when bullet is required');
    ...
    p.references.forEach((r, i) => {
        if (!r.path)
            err('reference_path_missing', `references[${i}].path`, 'A reference must carry a path');
        if (!r.ref)
            err('reference_ref_missing', `references[${i}].ref`, 'A reference must pin a commit/tag/version');
        ...
        if (ex.length > MAX_EXCERPT_CHARS) {
            err('excerpt_too_long', `references[${i}].excerpt`, `Excerpt is ${ex.length} chars (limit ${MAX_EXCERPT_CHARS}) -- reference the path, do not re-inline the document`);
        }
        ...
        if (r.bytes != null && r.bytes >= MIN_ARTIFACT_BYTES_FOR_INLINE_CHECK && ex.length >= r.bytes * MAX_EXCERPT_FRACTION_OF_ARTIFACT) {
            err('artifact_inlined', `references[${i}].excerpt`, `Excerpt is ${ex.length} of ${r.bytes} artifact bytes -- that is the document inlined, not an excerpt`);
        }
    });
    ...
    for (const [at, body] of freeTextSections(p)) {
        if (body.length > MAX_SECTION_CHARS) {
            err('section_inlined', at, `Section body is ${body.length} chars (limit ${MAX_SECTION_CHARS}) -- carry large material by path+commit+hash+excerpt instead of inlining it`);
        }
    }
 ...
 */
export function derivePacketMetadata(p) {
