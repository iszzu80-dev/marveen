...
import { serializeEvidence, deserializeEvidence, reconcileAlerts } from './alerts.js';
function rowToRecord(row) {
    return {
        dedup_key: row.dedup_key, type: row.type, severity: row.severity,
...
export function listAllAlerts(db) {
    return db.prepare(`SELECT * FROM costops_alerts`).all().map(rowToRecord);
}
...
export function listAlerts(db, opts = {}) {
    ...
    const rows = db.prepare(`SELECT * FROM costops_alerts ${where} ORDER BY last_seen DESC`).all(...params);
 ...
 */
export function applyAlertReconciliation(db, result, now) {
...
export function reconcileAndPersist(db, candidates, now, cooldownSeconds) {
...
export function acknowledgeAlertByKey(db, dedupKey, actor, now) {
    const existing = db.prepare(`SELECT dedup_key FROM costops_alerts WHERE dedup_key = ?`).get(dedupKey);
    if (!existing)
        return { ok: false, error: `no alert with dedup_key '${dedupKey}'`, status: 404 };
...
export function resolveAlertByKey(db, dedupKey, now) {
    const existing = db.prepare(`SELECT dedup_key FROM costops_alerts WHERE dedup_key = ?`).get(dedupKey);
    if (!existing)
        return { ok: false, error: `no alert with dedup_key '${dedupKey}'`, status: 404 };
