...
function prefersConsumption(sourceType) {
 ...
 */
export function resolveSourceWinners(lines, now, tierOf) {
    if (lines.length <= 1)
        return lines;
    const rep = lines.reduce((a, b) => {
        ...
        if (aIsConsumption !== bIsConsumption) {
            if (consume)
                return bIsConsumption ? b : a; // top-up: consumption wins
 ...
 */
export function resolveOperational(lines, win, now = Math.floor(Date.now() / 1000)) {
...
export function getCostSummary(db, config, now, opts = {}) {
  ...
  `).all({ start: win.start, end: win.end });
    ...
    for (const [sid, ls] of bySource) {
        ...
        const rep = winners[0];
        const resolved = winners.length === 1 ? rep : {
            ...rep,
            billed_cost: winners.reduce((s, l) => s + l.billed_cost, 0),
    ...
    const srcRows = db.prepare(`SELECT id, name, provider, source_type FROM cost_sources WHERE active = 1 OR lifecycle_state = 'decommissioned'`).all();
    ...
    const manual_render_estimate = round2(renderNonPlanSources.reduce((s, r) => s + (perSource.get(r.id) || 0), 0));
    ...
    const sourceTypeBySource = new Map(srcRows.map(r => [r.id, r.source_type]));
