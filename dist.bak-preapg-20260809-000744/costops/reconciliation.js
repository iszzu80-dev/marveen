 ...
 */
export function buildReconciliation(db, now, month) {
    const win = monthWindow(now, month);
    const sources = db.prepare(`SELECT id, name, provider FROM cost_sources WHERE active = 1 AND lifecycle_state != 'decommissioned'`).all();
  ...
  `);
    return sources.map((s) => {
        ...
        const forecastRow = latestForecast.get(s.id, win.key);
