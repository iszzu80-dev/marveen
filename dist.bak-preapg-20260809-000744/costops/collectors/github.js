 ...
 */
export function mapGitHubUsage(raw, opts) {
    const report = (raw && typeof raw === 'object') ? raw : null;
    if (!report || !Array.isArray(report.usageItems))
        return [];
    let usdTotal = 0;
...
export function makeGitHubCollector(user = GITHUB_BILLING_USER) {
    return {
        provider: 'github',
...
export const githubCollector = makeGitHubCollector();
 ...
 */
export async function syncGitHubCollector(db, now, deps = {}) {
