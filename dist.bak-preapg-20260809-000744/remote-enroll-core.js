 ...
 */
export function dashboardTokenDecision(includeToken, token) {
    if (!includeToken)
        return { include: false };
    if (token === null || token === '') {
        return {
            ok: false,
            reason: 'no dashboard access token found (store/.dashboard-token is missing, and DASHBOARD_TOKEN is unset). ' +
 ...
 */
export function checkEnrollHost(raw, isIP) {
    const host = raw.trim();
    if (!host)
        return { ok: false, reason: 'target address is empty' };
    ...
    if (host.includes('@')) {
        return {
            ok: false,
            reason: `"${seen}" looks like an email address. The target address is the machine's IP address or ` +
 ...
 * rejected.
 */
function validateEd25519Blob(base64) {
    ...
    let off = 0;
    if (buf.length < 4)
        throw new RemoteEnrollError('key blob is too short');
    ...
    if (embeddedType !== ACCEPTED_KEY_TYPE) {
        throw new RemoteEnrollError(`embedded key type must be ${ACCEPTED_KEY_TYPE}, found "${embeddedType}"`);
    }
    if (off + 4 > buf.length)
        throw new RemoteEnrollError('key blob is truncated');
 ...
 */
export function validatePublicKeyLine(rawLine) {
    ...
    if (fields.length !== 3) {
        throw new RemoteEnrollError('line must contain exactly three fields: type, key, comment (no options, no extra fields)');
    }
...
export function restrictOptions(webPort = REMOTE_PORT) {
 ...
 */
export function buildRestrictedLine(parsed, webPort = REMOTE_PORT) {
 ...
 */
export function mergeAuthorizedKeys(existing, restrictedLine, installId) {
 ...
 */
export function removeAuthorizedKey(existing, installId) {
    const target = `${COMMENT_PREFIX}${installId}`;
    const lines = existing.length ? existing.split('\n') : [];
    if (lines.length > 0 && lines[lines.length - 1] === '') {
        lines.pop();
 ...
 */
export function parseHostKeyPub(content) {
 ...
 */
export function parseKeyscanEd25519(output) {
    for (const raw of output.split('\n')) {
        ...
        if (fields.length < 3)
            continue;
        if (fields[1] !== ACCEPTED_KEY_TYPE)
            continue;
        if (isCanonicalBase64(fields[2]))
            return fields[2];
    }
    return null;
}
 ...
 */
export function resolveHostKey(sources, candidates = HOST_KEY_PUB_CANDIDATES) {
    ...
    if (output !== null) {
        ...
        if (body !== null)
            return { body, source: 'ssh-keyscan' };
    }
    return null;
}
/**
