...
function ensureWatching() {
    ...
    try {
        ...
        watcher = watch(STORE_DIR, { persistent: false }, (_event, filename) => {
            if (filename === 'config-overrides.json')
                cache = loadFromDisk();
        });
    }
    catch {
...
export function getEffectiveSettingValue(key) {
    ensureWatching();
    const def = getSettingDefinition(key);
    if (!def)
        throw new Error(`Unknown setting key: ${key}`);
    if (key in cache)
        return coerce(def, cache[key]);
    const envValue = readEnvFile([key])[key];
    if (envValue !== undefined)
        return coerce(def, envValue);
    return def.default;
}
...
export function setOverride(key, rawValue) {
    const def = getSettingDefinition(key);
    if (!def)
        return { ok: false, error: `Ismeretlen kulcs: ${key}` };
...
export function reloadOverridesForTest() {
    cache = loadFromDisk();
}
