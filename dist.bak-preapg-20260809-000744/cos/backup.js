...
export function encryptBytes(plain, passphrase) {
...
export function decryptBytes(blob, passphrase) {
    ...
    if (!blob.subarray(0, MAGIC.length).equals(MAGIC))
        throw new Error('bad backup magic (not a COS backup)');
    let o = MAGIC.length;
...
export function restoreEncryptedBackup(args) {
