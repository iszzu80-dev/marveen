...
export function dueZstItems(db, horizonDays = 45) {
    ...
    const contracts = db.prepare(`SELECT contract_id, title, status, expiry_date, termination_deadline, notice_period_days,
       CASE
     ...
     ORDER BY COALESCE(termination_deadline, expiry_date)`).all({ h: horizon });
    const licenses = db.prepare(`SELECT license_id, product_name, renewal_date, auto_renew, cancellation_candidate,
       CASE
    ...
    const obligations = db.prepare(`SELECT obligation_id, contract_id, description, due_date FROM zst_obligations
     WHERE status = 'OPEN' AND due_date IS NOT NULL AND date(due_date) <= date('now', @h)
     ORDER BY due_date`).all({ h: horizon });
    const oppPh = OPP_DECISION_STATUSES.map(() => '?').join(',');
    const opportunities = db.prepare(`SELECT opportunity_id, title, status, next_action FROM zst_opportunities
     WHERE status IN (${oppPh}) ORDER BY updated_at DESC`).all(...OPP_DECISION_STATUSES);
    return {
...
export function seedZstProducts(db, now) {
    const stmt = db.prepare(`INSERT INTO zst_products (product_id, product_name, version, updated_at)
     VALUES (@id, @name, 1, @now) ON CONFLICT(product_id) DO NOTHING`);
    let n = 0;
    for (const p of SEED_PRODUCTS) {
        if (stmt.run({ id: p.id, name: p.name, now }).changes)
            n++;
    }
    return n;
}
