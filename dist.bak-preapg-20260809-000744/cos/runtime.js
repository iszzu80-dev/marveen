...
export function registerCosConnectors(db = getDb(), now = Math.floor(Date.now() / 1000)) {
    ...
    try {
        recordSuccess(db, 'rental', now);
    }
    catch { /* row may not exist on a read-only db */ }
    try {
        recordSuccess(db, 'emag', now);
    }
    catch { /* row may not exist on a read-only db */ }
}
 ...
 *  safe defaults. */
export async function runCosTickOnce(db = getDb(), deps = safeCosDeps(), now = Math.floor(Date.now() / 1000)) {
...
export function startCosBackgroundTasks() {
    if (timer)
        return timer;
    try {
        registerCosConnectors();
    }
    catch (err) {
        logger.error({ err }, 'COS connector registration failed');
    }
    timer = setInterval(() => {
        runCosTickOnce()
            .then((res) => {
            ...
            for (const hit of res.radarHits) {
                ...
                try {
                    alertRadarHit(getDb(), hit);
                }
                catch (err) {
                    logger.error({ err, radarId: hit }, 'radar HIT alert failed');
                }
            }
            ...
            if (res.recoveryRequired.length) {
                ...
                try {
                    alertOutboundRecovery(getDb());
                }
                catch (err) {
                    logger.error({ err }, 'outbound recovery alert failed');
                }
            }
        })
...
export function stopCosBackgroundTasks() {
    if (timer) {
        clearInterval(timer);
        timer = undefined;
    }
