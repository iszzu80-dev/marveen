...
export function createAgentMessage(from, to, content, originNote, traceCtx) {
    const now = Math.floor(Date.now() / 1000);
    const info = db.prepare('INSERT INTO agent_messages (from_agent, to_agent, content, status, created_at, origin_note, trace_id, span_id, parent_span_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(from, to, content, 'pending', now, originNote ?? null, traceCtx?.trace_id ?? null, traceCtx?.span_id ?? null, traceCtx?.parent_span_id ?? null);
    return {
