...
import { getPeriodStatus, isPeriodClosed, checkPeriodWritable, checkCloseReadiness, closePeriod, reopenPeriod, getPeriodCloseHistory, getCloseSnapshot, } from '../costops/period-close.js';
...
describe('write-path guards on a closed month (CostOps Phase 2, GAP-13 integration)', () => {
    ...
    it('createCorrection is NOT blocked on a closed month -- it is the sanctioned path', () => {
        ...
        closePeriod(db, cfg, MONTH, 'istvan', 'x', NOW + 5, { force: true });
        const r = createCorrection(db, { originalLineId: info.lastInsertRowid, newAmount: 9500, reason: 'late credit applied after close' }, { now: NOW + 20 });
        expect(r.ok).toBe(true);
    });
});
