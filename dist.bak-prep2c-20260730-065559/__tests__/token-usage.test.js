...
function makeToolCallLine(toolName, ts) {
    return JSON.stringify({
        type: 'assistant',
...
describe('pruneTokenUsage', () => {
    ...
    const DAY = 86400;
    function insertRow(sessionId, ageDays) {
        getDb().prepare(`INSERT INTO token_usage (agent, session_id, timestamp, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens)
       VALUES ('test-prune', ?, ?, 1, 1, 0, 0)`).run(sessionId, NOW - ageDays * DAY);
    }
    beforeEach(() => {
    ...
    it('is a no-op when nothing is older than the window', () => {
        ...
        const cnt = getDb().prepare("SELECT COUNT(*) c FROM token_usage WHERE agent = 'test-prune'").get();
        expect(cnt.c).toBe(1);
    });
});
...
describe('deduplication', () => {
    ...
    `);
        ...
        insert.run('test-dedup', 'sess-dedup', ts, 100, 50, 0, 0, 'dedup test', null);
        insert.run('test-dedup', 'sess-dedup', ts, 100, 50, 0, 0, 'dedup test', null);
        const count = db.prepare("SELECT COUNT(*) as c FROM token_usage WHERE agent = 'test-dedup' AND session_id = 'sess-dedup' AND timestamp = ?").get(ts);
...
describe('correlateWithKanban', () => {
    ...
    `).run('test-kanban-1', 'Test Task', 'in_progress', 'normal', 'test-main', 'test-project', baseTs, baseTs);
        ...
        correlateWithKanban();
        const rows = db.prepare("SELECT task_title, project FROM token_usage WHERE agent = 'test-main' AND task_title IS NOT NULL").all();
...
describe('tryHandleTokenUsage route handler', () => {
    ...
    function makeCtx(path, method, searchParams = {}) {
        ...
        const res = {
            writeHead: (status, _headers) => { responseStatus = status; },
            end: (body) => { responseBody = body || ''; },
        };
        return {
            ctx: { req: {}, res: res, path, method, url },
