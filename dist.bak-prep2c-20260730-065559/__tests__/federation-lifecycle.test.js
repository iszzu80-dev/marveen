...
import { initDatabase, createAgentMessage, markMessageDelivered, markMessageDone, failPendingFederatedMessages, getAgentMessage, } from '../db.js';
import { tryHandleFederation, _resetInboxDedupForTest } from '../web/routes/federation.js';
import { _setFederationStoreDirForTest, reloadFederationForTest, getFederationConfig, } from '../web/federation/config.js';
...
function fakeCtx(method, path, body) {
    ...
    const res = {
        ...
        end(data) { state.body = String(data ?? ''); },
        setHeader() { },
    };
    if (body !== undefined) {
        process.nextTick(() => {
...
describe('peer lifecycle endpoints', () => {
    it('POST /peers mints an inbound token, returns it once, list stays redacted', async () => {
        ...
        const minted = created.json.inboundToken;
    ...
    it('shareCapabilitySummaries survives every mutation path (L5): add, PATCH, rotate, sibling DELETE, enabled flip', async () => {
        ...
        const list = await call('GET', '/api/federation/peers');
        const teodor = list.json.peers.find((p) => p.id === 'teodor');
        ...
        expect(after.json.peers[0].shareCapabilitySummaries).toBe(false);
    });
    ...
    it('rotate mints a fresh token and invalidates the old one in config', async () => {
        ...
        const fresh = rotated.json.inboundToken;
...
describe('master switch + full removal', () => {
    it('POST /enabled false is LOSSLESS for config but deterministic for the queue', async () => {
        ...
        expect(off.json.peers.length).toBe(1); // peers survive
        ...
        expect(on.json.peers.length).toBe(1); // one-click re-enable
    });
    ...
    it('PUT with a valid enabled:false document persists it as given', async () => {
        ...
        expect(put.json.peers.length).toBe(1);
    });
