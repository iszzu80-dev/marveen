...
import { initDatabase, createAgentMessage, markMessageDone, markMessageFailed, getAgentMessage, listAgentMessages, } from '../db.js';
...
describe('completion-notification contract', () => {
    ...
    it('notification message has correct from/to/content and is pending', () => {
        ...
        const notif = createAgentMessage(done.to_agent, done.from_agent, `[Eredmény] msg_id:${done.id} status:done\n\n${summary}`);
    ...
    it('failed message also triggers notification with status:failed', () => {
        ...
        const notif = createAgentMessage(failed.to_agent, failed.from_agent, `[Eredmény] msg_id:${failed.id} status:failed\n\n${summary}`);
