...
function fakePost(path, body) {
    ...
    return { ctx: { req, res, path: url.pathname, method: 'POST', url }, out };
}
function fakePatch(id, body) {
    const out = { status: 0, body: null };
    const res = {
        writeHead(status) { out.status = status; return res; },
        end(chunk) { if (chunk)
    ...
    return { ctx: { req, res, path, method: 'PATCH', url }, out };
}
