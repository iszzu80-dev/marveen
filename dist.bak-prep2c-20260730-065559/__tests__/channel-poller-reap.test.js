...
import { parsePollerPidsFromPs, findOrphanChannelClaudes } from '../web/channel-poller-reap.js';
...
describe('parsePollerPidsFromPs', () => {
    it('returns the bun poller pid matching the TELEGRAM_STATE_DIR for samu', () => {
        const pids = parsePollerPidsFromPs(PS_SAMPLE, 'TELEGRAM_STATE_DIR', '/Users/x/ClaudeClaw/agents/samu/.claude/channels/telegram');
        expect(pids).toEqual([90798]);
    });
    it('returns the slack poller pid for the SLACK_STATE_DIR variant', () => {
        const pids = parsePollerPidsFromPs(PS_SAMPLE, 'SLACK_STATE_DIR', '/Users/x/ClaudeClaw/agents/samu/.claude/channels/slack');
    ...
    it('returns empty array when no row matches', () => {
        const pids = parsePollerPidsFromPs(PS_SAMPLE, 'TELEGRAM_STATE_DIR', '/Users/x/ClaudeClaw/agents/nobody/.claude/channels/telegram');
        expect(pids).toEqual([]);
    });
    it('returns multiple pids when several rows match (a real orphan scenario)', () => {
        ...
        const pids = parsePollerPidsFromPs(orphans, 'TELEGRAM_STATE_DIR', '/Users/x/ClaudeClaw/.claude/channels/telegram');
    ...
    it('ignores rows where the path appears only in argv (not as an env-var value)', () => {
        ...
        const pids = parsePollerPidsFromPs(argvMention, 'TELEGRAM_STATE_DIR', '/Users/x/ClaudeClaw/.claude/channels/telegram');
...
const LIVE_PANES = new Set([76621, 77189, 44349]);
