...
import { createDeviceKey, resolveDeviceKey, listDeviceKeys, _clearDeviceKeyCacheForTest, } from '../web/auth-device-keys.js';
...
function ageDeviceKeyCacheStamp(rawKey, seconds) {
    const e = deviceKeyCache.get(sha256hexOf(rawKey));
    if (!e)
        throw new Error('device key not in cache');
    if (e.lastUsedAt !== null)
        e.lastUsedAt -= seconds;
}
function ageSessionCacheStamp(cookie, seconds) {
