...
function getIdea(id) {
    return getDb().prepare('SELECT * FROM idea_box WHERE id = ?').get(id);
}
const VALID_PRIORITIES = new Set(['low', 'normal', 'high', 'urgent']);
export async function tryHandleIdeas(ctx) {
    ...
    if (path === '/api/ideas' && method === 'POST') {
        ...
        if (data.impact !== undefined && data.impact !== null) {
            ...
            if (!Number.isFinite(v) || v < 1 || v > 5) {
                json(res, { error: 'impact must be 1-5 or null' }, 400);
                return true;
            }
            impact = v;
        }
        ...
        if (data.effort !== undefined && data.effort !== null) {
            ...
            if (!Number.isFinite(v) || v < 1 || v > 5) {
                json(res, { error: 'effort must be 1-5 or null' }, 400);
                return true;
            }
            effort = v;
        }
    ...
    if (commentsMatch && method === 'POST') {
        ...
        if (!content || typeof content !== 'string' || !content.trim()) {
            json(res, { error: 'content required' }, 400);
            return true;
        }
    ...
    if (promoteMatch && method === 'POST') {
        ...
        const idea = getDb().prepare('SELECT * FROM idea_box WHERE id = ?').get(ideaId);
        if (!idea) {
            json(res, { error: 'Ötlet nem található' }, 404);
            return true;
        }
    ...
    if (promoteBreakdownMatch && method === 'POST') {
        ...
        const body = await readBody(req);
        const { subtasks, success_criteria } = JSON.parse(body.toString());
        ...
        for (const st of subtasks) {
            ...
            createKanbanCard({
                ...
                status: 'planned',
                priority: (st.priority && VALID_PRIORITIES.has(st.priority) ? st.priority : 'normal'),
    ...
    if (revertMatch && method === 'POST') {
        ...
        if (idea.status !== 'kanban') {
            json(res, { error: 'Csak kanban státuszú ötlet vonható vissza' }, 400);
            return true;
        }
