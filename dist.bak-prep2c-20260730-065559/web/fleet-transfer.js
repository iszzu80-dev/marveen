...
export function _encryptForTest(plaintext, password) {
    return encryptWithPassword(plaintext, password);
}
export function _decryptForTest(packed, password) {
...
function denormalizePaths(text) {
    return text
...
function looksLikeHeaderSecret(key, value) {
    if (NON_SECRET_HEADER_KEY_RE.test(key))
        return false;
    if (ALWAYS_SECRET_HEADER_KEY_RE.test(key))
        return true;
...
function buildBindingLookup() {
    ...
    for (const binding of getBindings()) {
        for (const target of binding.targets) {
            if (!lookup.has(target.mcpFilePath))
                lookup.set(target.mcpFilePath, new Map());
            const byServer = lookup.get(target.mcpFilePath);
            if (!byServer.has(target.serverName))
                byServer.set(target.serverName, new Map());
...
function placeholderMcp(mcpObj, mcpFilePath, lookup) {
    const result = JSON.parse(JSON.stringify(mcpObj));
    ...
    for (const [serverName, cfg] of Object.entries(servers)) {
        ...
        if (blockingFields.length > 0) {
            throw new UserFacingError(`Titkosítatlan secret az .mcp.json-ban: szerver="${serverName}": ${blockingFields.join('; ')}. ` +
...
function deplaceholderMcp(mcpObj) {
    const result = JSON.parse(JSON.stringify(mcpObj));
    const servers = result.mcpServers;
    if (!servers)
        return result;
...
// Export
// ---------------------------------------------------------------------------
function exportChannelsAccess(channelsDir) {
...
function exportMainAgent(bindingLookup, withSecrets) {
    ...
    if (!withSecrets && typeof settings.env === 'object' && settings.env !== null) {
        for (const [key, val] of Object.entries(settings.env)) {
            if (typeof val === 'string' && looksLikeSecret(val)) {
                throw new UserFacingError(`Titkosítatlan secret a settings.json env blokkjában: kulcs="${key}". ` +
...
function exportAgent(name, bindingLookup, withSecrets) {
    ...
    if (!withSecrets && typeof settings.env === 'object' && settings.env !== null) {
        for (const [key, val] of Object.entries(settings.env)) {
            if (typeof val === 'string' && looksLikeSecret(val)) {
                throw new UserFacingError(`Titkosítatlan secret az agent "${name}" settings.json env blokkjában: kulcs="${key}". ` +
                    `Adj meg vault jelszót az exporthoz, vagy távolítsd el a titkot a settings.json-ból.`);
            }
        }
    }
...
function exportDashboardSettings() {
    const read = (name) => safeReadJson(join(STORE_DIR, name));
    return {
...
function exportVault() {
    ...
    if (!existsSync(vaultKeyPath)) {
        ...
        if (existsSync(vaultKeyMigratedPath)) {
            throw new Error('A vault kulcs macOS Keychain-be lett migrálva (.vault-key.migrated megtalálható). ' +
    ...
    const entries = vaultStore.entries ?? [];
    const bindingsStore = safeReadJson(bindingsPath);
    const bindings = bindingsStore.bindings ?? [];
...
export function exportFleet(options = {}) {
    ...
    const memories = db.prepare(`SELECT agent_id, content, sector, salience, created_at, accessed_at,
            category, auto_generated, keywords
     FROM memories ORDER BY agent_id ASC, created_at ASC`).all();
    const dailyLogs = db.prepare('SELECT agent_id, date, content, created_at FROM daily_logs ORDER BY agent_id ASC, date ASC').all();
    const kanban = {
        cards: db.prepare('SELECT * FROM kanban_cards').all(),
        comments: db.prepare('SELECT * FROM kanban_comments').all(),
        cardEvents: db.prepare('SELECT * FROM kanban_card_events').all(),
        labels: db.prepare('SELECT * FROM labels').all(),
        cardLabels: db.prepare('SELECT * FROM kanban_card_labels').all(),
    };
    const ideaBox = {
        ideas: db.prepare('SELECT * FROM idea_box').all(),
        comments: db.prepare('SELECT * FROM idea_comments').all(),
        statusLog: db.prepare('SELECT * FROM idea_status_log').all(),
    };
...
function validateSchema(fleet) {
    ...
    if (f.schemaVersion !== FLEET_SCHEMA_VERSION) {
        ...
        errors.push(`Az export schema v${v}, a telepített dashboard v${FLEET_SCHEMA_VERSION}-t támogat. ` +
            (Number(v) > FLEET_SCHEMA_VERSION
                ? 'Frissítsd a dashboardot az import előtt.'
                : 'Az export túl régi.'));
        return errors;
    }
    if (!Array.isArray(f.agents))
        errors.push('agents mező hiányzik vagy nem tömb.');
    return errors;
}
// B1: validate all untrusted names before any file operation
function validateNames(fleet) {
    const errors = [];
...
function buildDiffReport(fleet) {
    const db = getDb();
    const warnings = [];
    ...
    for (const card of fleet.kanban?.cards ?? []) {
        if (!db.prepare('SELECT 1 FROM kanban_cards WHERE id = ?').get(card.id))
            newCards++;
    }
    let newLabels = 0;
    for (const label of fleet.kanban?.labels ?? []) {
        if (!db.prepare('SELECT 1 FROM labels WHERE id = ?').get(label.id))
            newLabels++;
    }
    let newDailyLogs = 0;
    ...
    if (drySourceId && typeof drySourceId === 'string') {
        warnings.push(`Fő-agent identitás átvéve: ${drySourceId}. Apply után újraindítás szükséges hogy a dashboard ${drySourceId}-ként induljon.`);
    }
    return {
        dryRun: true,
        wouldCreate: {
...
function trackedMkdir(path, tracker) {
    if (!existsSync(path)) {
        mkdirSync(path, { recursive: true });
        tracker.dirs.push(path);
    }
}
function trackedWrite(path, content, tracker, opts) {
    const preexisted = existsSync(path);
    atomicWriteFileSync(path, content, opts);
    tracker.files.push({ path, preexisted });
}
function cleanupTracked(tracker) {
...
function writeMainAgentFiles(ma, tracker) {
    const claudeDir = join(PROJECT_ROOT, '.claude');
    trackedMkdir(claudeDir, tracker);
    if (ma.claudeMd)
        trackedWrite(join(PROJECT_ROOT, 'CLAUDE.md'), ma.claudeMd, tracker);
    if (ma.soulMd)
        trackedWrite(join(PROJECT_ROOT, 'SOUL.md'), ma.soulMd, tracker);
...
function writeAgentFiles(agent, tracker) {
    ...
    if (agent.claudeMd)
        trackedWrite(join(dir, 'CLAUDE.md'), agent.claudeMd, tracker);
    if (agent.soulMd)
        trackedWrite(join(dir, 'SOUL.md'), agent.soulMd, tracker);
...
function importVaultSection(vault, tracker) {
...
export function importFleet(rawBody, options) {
    ...
    const fleet = JSON.parse(denormalizePaths(jsonBody));
    ...
    try {
        ...
        for (const task of fleet.scheduledTasks ?? []) {
            ...
            if (task.skillMd)
                trackedWrite(join(dir, 'SKILL.md'), task.skillMd, tracker);
            trackedWrite(join(dir, 'task-config.json'), JSON.stringify({ ...task.config, enabled: false }, null, 2), tracker);
        }
        ...
        const importTx = db.transaction(() => {
            ...
            for (const label of fleet.kanban?.labels ?? []) {
                const l = label;
                if (!l.id || !l.name) {
                    logger.warn({ id: l.id }, 'Fleet import: skipping label with missing required fields');
                    continue;
                }
            ...
            for (const card of fleet.kanban?.cards ?? []) {
                ...
                if (!c.id || !c.title || !c.status || !c.priority || c.sort_order == null) {
                    logger.warn({ id: c.id }, 'Fleet import: skipping kanban card with missing required fields');
                    continue;
                }
                db.prepare(`INSERT OR IGNORE INTO kanban_cards
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(c.id, c.title, c.description ?? null, c.status, c.assignee ?? null, c.priority, c.project ?? null, c.due_date ?? null, c.sort_order, c.created_at, c.updated_at, c.archived_at ?? null, c.parent_id ?? null, c.dispatched_at ?? null);
            }
            // kanban comments (idempotent: card_id + content)
            for (const comment of fleet.kanban?.comments ?? []) {
                const c = comment;
                if (!c.card_id || !c.content)
                    continue;
            ...
            for (const mem of fleet.memories ?? []) {
             ...
             VALUES ('', NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(mem.content, mem.sector, mem.salience, mem.created_at, mem.accessed_at ?? now, mem.agent_id, mem.category, mem.auto_generated ?? 0, mem.keywords ?? null);
                }
            }
            ...
            for (const idea of fleet.ideaBox?.ideas ?? []) {
                const i = idea;
                db.prepare(`INSERT OR IGNORE INTO idea_box
           (id, title, description, category, status, source, kanban_id, impact, effort, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(i.id, i.title, i.description ?? null, i.category, i.status, i.source ?? '', i.kanban_id ?? null, i.impact ?? null, i.effort ?? null, i.created_at, i.updated_at);
            }
            ...
            for (const log of fleet.ideaBox?.statusLog ?? []) {
                ...
                if (!db.prepare('SELECT 1 FROM idea_status_log WHERE idea_id = ? AND created_at = ? AND to_status = ?')
                    ...
                    db.prepare('INSERT INTO idea_status_log (idea_id, from_status, to_status, actor, note, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(l.idea_id, l.from_status ?? null, l.to_status, l.actor, l.note ?? null, l.created_at);
                }
            }
        ...
        if (sourceAgentId && typeof sourceAgentId === 'string') {
            ...
            applyWarnings.push(`Fő-agent identitás átvéve: ${sourceAgentId}. Újraindítás kell hogy a dashboard ${sourceAgentId}-ként induljon.`);
        }
