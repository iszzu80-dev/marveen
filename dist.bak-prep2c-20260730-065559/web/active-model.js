...
export function projectsDirFor(workingDir, configDir, homeDirOverride) {
...
export function readActiveModelFromProjectDir(workingDir, sinceUnixSec, configDir) {
    const now = Date.now();
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            ...
            try {
                ...
                if (sinceUnixSec !== undefined) {
                    ...
                    if (!Number.isFinite(lineUnix) || lineUnix < sinceUnixSec)
                        continue;
                }
                value = model;
                break;
...
export function readContextTokensFromProjectDir(workingDir, configDir) {
    const now = Date.now();
