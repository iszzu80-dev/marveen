...
export function checkThrottle(usernameLc, now = Date.now()) {
    pruneGlobal(now);
...
export function recordFailure(usernameLc, now = Date.now()) {
...
export async function runDummyVerify(password) {
    try {
