...
export const INITIAL_NUDGE_STATE = Object.freeze({
    lastNudgeAt: 0,
...
export function decideNudgePreflight(input, state) {
    ...
    if (oldestAgeMs < MIN_PENDING_AGE_MS)
        return { proceed: false, state };
    if (now - state.lastNudgeAt < NUDGE_DEBOUNCE_MS)
        return { proceed: false, state };
    ...
    if (state.lastNudgeOldestId === oldestId && state.staleNudges > 0) {
        ...
        if (now - state.lastNudgeAt < STALE_NUDGE_COOLDOWN_MS)
            return { proceed: false, state };
    }
...
export function recordNudge(state, now, oldestId) {
    return {
        ...state,
        lastNudgeAt: now,
...
let state = { ...INITIAL_NUDGE_STATE };
/** Test seam. */
export function _resetNudgeStateForTest() {
...
async function tick() {
    ...
    try {
        ...
        const pre = decideNudgePreflight({ now, oldestId: oldest ? oldest.id : null, oldestAgeMs: oldest ? now - oldest.created_at * 1000 : 0 }, state);
        state = pre.state;
        if (!pre.proceed) {
            if (pre.staleAlert) {
                ...
                sendAlert(`⚠️ A fő-ügynök inbox auto-drain ${MAX_STALE_NUDGES} noszogatás után sem vette át a függő üzenetet (#${oldest?.id}). ` +
        ...
        if (state.absenceLogged)
            state = { ...state, absenceLogged: false };
        ...
        logger.info({ inboxNudge: true, pending: pending.length, oldestId: oldest.id, nudgesInLastHour: state.recentNudges.length }, 'inbox nudge: prompted the main agent to drain its inbox');
    }
    catch (err) {
        logger.warn({ err }, 'inbox nudge: tick error');
    }
}
export function startInboxNudgeWatcher() {
