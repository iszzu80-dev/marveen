...
export const FEDERATION_ROUTING_MODES = ['strong', 'catalog-first', 'advisory'];
export const DEFAULT_ROUTING_MODE = 'catalog-first';
const DISABLED = Object.freeze({ enabled: false, systemId: '', routingMode: DEFAULT_ROUTING_MODE, peers: [] });
...
function configPath() { return join(storeDir, CONFIG_FILENAME); }
function legacyTokenPath() { return join(storeDir, LEGACY_TOKEN_FILENAME); }
// https is mandatory except on loopback, which the local two-instance smoke
// test needs (http://127.0.0.1:<port> peers).
export function isAcceptablePeerBaseUrl(raw) {
...
export function generatePeerInboundToken() {
...
export function validateFederationConfig(parsed) {
    ...
    if (!isValidIdSegment(rawSystemId))
    ...
    const systemId = rawSystemId.toLowerCase();
    ...
    if (!Array.isArray(rawPeers))
        return 'peers is not an array';
    const peers = [];
    const seenIds = new Set();
    const seenInbound = new Set();
    for (const raw of rawPeers) {
        if (raw === null || typeof raw !== 'object')
            return 'peer entry is not an object';
        const p = raw;
        if (!isValidIdSegment(p.id))
            return 'invalid peer id';
        const id = p.id.toLowerCase(); // case-insensitive ids, stored lowercase
        if (id === systemId)
            return `peer id equals own systemId (${id})`;
        if (seenIds.has(id))
            return `duplicate peer id (${id})`;
        seenIds.add(id);
        if (!isAcceptablePeerBaseUrl(p.baseUrl))
            return `invalid peer baseUrl for ${id} (https required; http only on loopback)`;
        ...
        if (!isValidToken(p.inboundToken))
            return `invalid inbound token for ${id} (min ${FEDERATION_MIN_TOKEN_LENGTH} chars)`;
        const inbound = p.inboundToken.trim();
        if (seenInbound.has(inbound))
            return `duplicate inbound token (${id})`;
        ...
        if (p.outboundToken !== undefined && p.outboundToken !== null && p.outboundToken !== '') {
            if (!isValidToken(p.outboundToken))
                return `invalid outbound token for ${id} (min ${FEDERATION_MIN_TOKEN_LENGTH} chars, or empty while pairing)`;
            outbound = p.outboundToken.trim();
        }
        const trust = p.trust === undefined ? 'untrusted' : p.trust;
        if (trust !== 'untrusted')
            return `unsupported trust value for ${id}`;
        let abandonWindowMinutes;
        ...
        if (p.shareCapabilitySummaries !== undefined) {
            if (typeof p.shareCapabilitySummaries !== 'boolean')
                return `invalid shareCapabilitySummaries for ${id} (boolean)`;
    ...
    let routingMode = DEFAULT_ROUTING_MODE;
    if (obj.routingMode !== undefined) {
        if (typeof obj.routingMode !== 'string' || !FEDERATION_ROUTING_MODES.includes(obj.routingMode)) {
...
export function abandonWindowMsForPeer(cfg, system) {
...
export function federationFileHealth() {
    try {
        if (!existsSync(configPath()))
            return 'absent';
        const result = validateFederationConfig(JSON.parse(readFileSync(configPath(), 'utf-8')));
...
export function identifyFederationCaller(authHeader, check) {
    try {
        ...
        for (const peer of cfg.peers) {
            ...
            if (typeof peer.inboundToken !== 'string' || peer.inboundToken.length < FEDERATION_MIN_TOKEN_LENGTH)
                continue;
            if (check(authHeader, peer.inboundToken))
                return peer.id;
        }
        return null;
    }
...
export function writeFederationConfig(cfg) {
    mkdirSync(storeDir, { recursive: true });
...
export function setFederationEnabledPreservingFile(enabled) {
    try {
        let raw = {};
        if (existsSync(configPath())) {
 ...
 *  unreadable garbage. */
export function setFederationRoutingModePreservingFile(mode) {
    try {
        let raw = {};
        if (existsSync(configPath())) {
            const parsed = JSON.parse(readFileSync(configPath(), 'utf-8'));
...
export function removeFederationStore() {
...
// ---- test seams -------------------------------------------------------------
export function _setFederationStoreDirForTest(dir) {
    storeDir = dir;
    if (watcher) {
        try {
            watcher.close();
...
export function reloadFederationForTest() {
    cachedConfig = null;
    lastConfigWarning = '';
}
