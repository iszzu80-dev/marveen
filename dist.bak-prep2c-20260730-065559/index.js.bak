import { readFileSync, readlinkSync, realpathSync, unlinkSync, mkdirSync, openSync, closeSync, writeSync, } from 'node:fs';
...
import { acquirePortLock, acquirePidfileLock, writeBufferFully, DeferToPeerError, } from './process-lock.js';
const BANNER = `
 ██████╗██╗      █████╗ ██╗   ██╗██████╗ ███████╗
██╔════╝██║     ██╔══██╗██║   ██║██╔══██╗██╔════╝
██║     ██║     ███████║██║   ██║██║  ██║█████╗
...
function argvBelongsToThisInstall(argv, pid) {
    // Boundary-suffixed so /path/marveen never matches /path/marveen2.
    if (argv.includes(PROJECT_ROOT + '/'))
        return true;
...
// or node:fs directly.
function buildProcessLockContext() {
        ...
        listOwnProcessesMatching(pattern) {
            ...
            try {
                ...
                for (const line of raw.split('\n')) {
                    ...
                    if (pid === process.pid)
                        continue;
                    if (uid != null && rowUid !== uid)
                        continue;
                    if (!pattern.test(argv))
                        continue;
                    if (!argvBelongsToThisInstall(argv, pid))
                        continue;
                    out.push(pid);
                }
                return out;
...
function readRecordedPidFrom(path) {
    try {
...
function isLegitimateDashboardPid(pid, procCtx) {
    const cmd = procCtx.getProcessCommand(pid);
    if (cmd == null)
        return false;
    if (procCtx.uid != null) {
        const ownerUid = procCtx.getProcessUid(pid);
        if (ownerUid == null || ownerUid !== procCtx.uid)
            return false;
    }
...
function buildPidfileLockContext(procCtx) {
    return {
        tryCreateExclusive(path, pid) {
            try {
                ...
                try {
                    ...
                    writeBufferFully((b, off, len) => writeSync(fd, b, off, len), Buffer.from(String(pid)));
                }
                finally {
                    closeSync(fd);
                }
        ...
        unlinkIfMatches(path, expected) {
            ...
            try {
                const raw = readFileSync(path, 'utf-8').trim();
                const current = /^\d+$/.test(raw) ? parseInt(raw, 10) : null;
                if (current !== expected)
                    return;
                unlinkSync(path);
            }
...
function checkFreshStartupRace(procCtx) {
    const recorded = readRecordedPidFrom(PID_FILE);
    if (recorded == null || recorded === process.pid)
        return;
    let alive = false;
    try {
    ...
    if (!alive)
        return;
    if (!isLegitimateDashboardPid(recorded, procCtx))
        return;
    ...
    if (portHolders.includes(recorded))
        return;
    ...
    try {
        ...
        if (heartbeatStarted) {
            try {
                ...
                logger.warn({ err }, 'stopHeartbeat threw during shutdown');
            }
        }
        try {
            stopInviteMonitor();
            ...
            logger.warn({ err }, 'stopInviteMonitor threw during shutdown');
        }
        try {
            stopChannelRequestWatcher();
        }
        catch (err) {
            logger.warn({ err }, 'stopChannelRequestWatcher threw during shutdown');
        }
        try {
            stopStoreWatcher();
        }
        catch (err) {
            logger.warn({ err }, 'stopStoreWatcher threw during shutdown');
        }
        if (decayInterval)
...
async function main() {
    ...
    backfillEmbeddings().then(count => {
        if (count > 0)
            logger.info({ count }, 'Embedding backfill befejezve');
    ...
    function scheduleDailyDigest() {
        ...
        digestTimer = setTimeout(() => {
            runDailyDigest(ALLOWED_CHAT_ID).catch((err) => logger.error({ err }, 'Napi naplo hiba'));
