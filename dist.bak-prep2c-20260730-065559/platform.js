...
function detect() {
    ...
    if (override === 'macos' || override === 'linux-server' || override === 'linux-gui')
        return override;
...
export function tryResolveFromPath(name) {
...
export function resolveFromPath(name) {
...
// successful lookup.
export function makeLazyBinResolver(name) {
    let cached = null;
    return () => {
        if (cached === null)
