...
export function normalizeModelFallbackConfig(raw) {
    const o = (raw && typeof raw === 'object') ? raw : {};
    const enabled = o.enabled === true;
    let chain = DEFAULT_MODEL_FALLBACK.chain;
    if (Array.isArray(o.chain)) {
        const cleaned = o.chain.filter((m) => typeof m === 'string' && m.trim().length > 0);
...
// switch.
const USAGE_LIMIT_RX = /(usage limit reached|reached your usage limit|hit (?:your|the) usage limit|approaching (?:your )?usage limit|usage limit (?:will )?reset|limit will reset at|\d+-hour limit reached|upgrade to increase your usage limit)/i;
/**
 ...
 */
export function detectsUsageLimit(pane) {
    if (!pane || !pane.trim())
        return false;
 ...
 */
export function nextFallbackModel(current, chain) {
    if (chain.length < 2)
        return null;
