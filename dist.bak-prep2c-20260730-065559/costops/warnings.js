...
export function categoryForProvider(provider) {
    const p = provider.toLowerCase();
    if (p === 'render' || p === 'vercel' || p === 'cloudflare')
        return 'hosting';
    if (p === 'anthropic' || p === 'openai' || p === 'deepseek' || p === 'google-ai')
        return 'ai';
    if (p === 'google-workspace' || p === 'workspace')
        return 'productivity';
    if (p === 'domain' || p === 'ssl' || p === 'dns')
        return 'domains';
    if (p === 'all')
        return 'budget';
    return 'other';
...
export function getWarnings(db, _config, now, summary, subscriptions, 
...
limits = []) {
  ...
  `).all({ start: win.start, end: win.end });
    ...
    const dsRows = db.prepare(`SELECT balance, currency, captured_at FROM provider_balance_snapshots WHERE provider = 'deepseek' ORDER BY captured_at DESC`).all();
    if (dsRows.length === 0) {
        warnings.push({
    ...
    for (const l of limits) {
        if (l.usage_pct === null || l.limit_type === 'build_minutes')
            continue;
