...
import { resolveAuth, requiresAuth, isFederationWireEndpoint } from './web/auth-gate.js';
...
export function startWebServer(port = 3420) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (origin && (allowedOrigins.has(origin) ||
            originMatchesServedHost(origin, req.headers.host, req.headers['x-forwarded-host']))) {
        ...
        if (isBlockedCrossOriginWrite(method, origin, req.headers.host, req.headers['x-forwarded-host'], allowedOrigins)) {
        ...
        const auth = resolveAuth(req, url, path, method, DASHBOARD_TOKEN);
        ...
        const fedPeerForCtx = auth.kind === 'federation' ? auth.peer : null;
        const ctxAuth = auth.kind === 'token' ? { kind: 'token' }
            : auth.kind === 'device' ? { kind: 'device', device: auth.device }
                : auth.kind === 'session' ? { kind: 'session', user: auth.user }
                    : auth.kind === 'federation' ? { kind: 'federation', peer: auth.peer }
                        : undefined;
        ...
        try {
            const routeCtx = { req, res, path, method, url, fedPeer: fedPeerForCtx, auth: ctxAuth };
            if (await tryHandleAuth(routeCtx))
                return;
            if (await tryHandleSecurity(routeCtx))
                return;
            if (await tryHandleProfiles(routeCtx))
                return;
            if (await tryHandleMessages(routeCtx))
                return;
            if (await tryHandleFederation(routeCtx))
                return;
            if (await tryHandleDailyLog(routeCtx))
                return;
            if (await tryHandleMemories(routeCtx))
                return;
            if (await tryHandleMigrate(routeCtx))
                return;
            if (await tryHandleKanban(routeCtx))
                return;
            if (await tryHandleSchedules(routeCtx))
                return;
            if (await tryHandleConnectorsHu(routeCtx))
                return;
            if (await tryHandleConnectors(routeCtx))
                return;
            if (await tryHandleDocs(routeCtx))
                return;
            if (await tryHandleResearch(routeCtx))
                return;
            if (await tryHandleAgentsSkills(routeCtx))
                return;
            if (await tryHandleSkills(routeCtx))
                return;
            if (await tryHandleAgentTerminal(routeCtx))
                return;
            if (await tryHandleAgentConversation(routeCtx))
                return;
            if (await tryHandleAgentTaskState(routeCtx))
                return;
            if (await tryHandleAgents(routeCtx, WEB_DIR))
                return;
            if (await tryHandleMarveen(routeCtx, WEB_DIR))
                return;
            if (await tryHandleBackgroundTasks(routeCtx))
                return;
            if (await tryHandleRecall(routeCtx))
                return;
            if (await tryHandleOverview(routeCtx))
                return;
            if (await tryHandleUpdates(routeCtx))
                return;
            if (await tryHandleOnboarding(routeCtx))
                return;
            if (await tryHandleStatus(routeCtx))
                return;
            if (await tryHandleAutonomy(routeCtx))
                return;
            if (await tryHandleApprovals(routeCtx))
                return;
            if (await tryHandleTokenUsage(routeCtx))
                return;
            if (await tryHandleCostOps(routeCtx))
                return; // LOCAL-FORK: costops seam (keep on rebase)
            if (await tryHandleCos(routeCtx))
                return; // LOCAL-FORK: cos seam (keep on rebase)
            if (await tryHandleOptimization(routeCtx))
                return;
            if (await tryHandleApg(routeCtx))
                return;
            if (await tryHandleIdeas(routeCtx))
                return;
            if (await tryHandleSpans(routeCtx))
                return;
            if (await tryHandleToolLog(routeCtx))
                return;
            if (await tryHandleSkillUsage(routeCtx))
                return;
            if (await tryHandleSettings(routeCtx))
                return;
            if (await tryHandleVoice(routeCtx))
                return;
            if (await tryHandleVaultSshKeys(routeCtx))
                return;
            if (await tryHandleVaultSsh(routeCtx))
                return;
            if (await tryHandleAuditLog(routeCtx))
                return;
            if (await tryHandleFleetQ(routeCtx))
                return;
            if (await tryHandleFleet(routeCtx))
                return;
            if (await tryHandleStatic(routeCtx, WEB_DIR))
                return;
            res.writeHead(404);
            res.end('Not found');
        }
                ...
                for (const pid of pids) {
                    ...
                    if (uid !== null) {
                        try {
                            ...
                            if (Number.isFinite(ownerUid) && ownerUid !== uid)
                                continue;
                        }
                        catch {
                            continue;
    ...
    if (!webOnly)
        logger.info('Inbox nudge watcher started (20s poll, 55s offset)');
    const reauthHealerInterval = webOnly ? undefined : startReauthHealer();
    if (!webOnly && reauthHealerInterval)
        logger.info('Reauth healer started (3min poll, 90s offset)');
    ...
    if (!webOnly)
        logger.info('Capacity-routing runner started (60s poll, 55s offset)');
    const contextGuardInterval = webOnly ? undefined : startContextGuardRunner();
    if (!webOnly)
        logger.info('Context-guard runner started (5min poll, 4.5min initial delay)');
    ...
    if (!webOnly)
        logger.info('Federation manifest poller started (10min poll, 25s offset)');
    const capabilityRunnerInterval = webOnly ? undefined : startCapabilitySummaryRunner();
    if (!webOnly)
        logger.info('Capability summary runner started (5min poll, 65s offset; idle while federation is off)');
    ...
    const authSessionSweepInterval = setInterval(() => {
        try {
            ...
            if (swept > 0)
                logger.info({ swept }, 'Expired auth sessions swept');
            const sweptKeys = sweepExpiredDeviceKeys();
            if (sweptKeys > 0)
                logger.info({ swept: sweptKeys }, 'Expired device keys swept');
        }
        catch (err) {
    ...
    if (!webOnly)
        logger.info('CostOps background tasks started (reliability-snapshot: 24h poll + startup; collector sync: 15min due-check + startup)');
    ...
    if (!webOnly) {
        startCosBackgroundTasks();
        logger.info('COS autonomous loop started (radar checks every 6h; no outbound adapter → no autonomous send)');
    }
    ...
    if (!hookDecision.register) {
        ...
        try {
            ...
            for (const agentName of [MAIN_AGENT_ID, ...listAgentNames()]) {
                ...
                if (ensureAgentHooks(agentName))
                    patched.push(agentName);
                if (ensureAgentStalenessHook(agentName))
                    stalePatched.push(agentName);
                if (ensureEgressGate(agentName))
                    egressPatched.push(agentName);
                if (ensureGovernanceGateCommands(agentName))
                    govPatched.push(agentName);
                ensureQuarantineReader(agentName);
            }
            if (pruned.length)
                logger.info({ pruned }, 'Stale hook entries pruned from agent settings.json');
            if (patched.length)
                logger.info({ patched }, 'PreCompact hook backfilled into agent settings.json');
            if (stalePatched.length)
                logger.info({ patched: stalePatched }, 'staleness-guard UserPromptSubmit hook backfilled into agent settings.json');
            if (egressPatched.length)
                logger.info({ patched: egressPatched }, 'egress-gate WebFetch hook backfilled into agent settings.json');
            if (govPatched.length)
                logger.info({ patched: govPatched }, 'governance gate hook commands upgraded to absolute node path in agent settings.json');
        }
        catch (err) {
    ...
    try {
        ...
        if (swept > 0)
            logger.info({ swept }, 'Orphan agent task-state records swept');
    }
    catch (err) {
    ...
    server.close = (cb) => {
        ...
        if (pluginMonitorInterval)
            clearInterval(pluginMonitorInterval);
        workerLivenessCancelled = true;
        if (workerLivenessInterval)
            clearInterval(workerLivenessInterval);
        clearInterval(channelHealthInterval);
        clearInterval(stuckInputInterval);
        clearInterval(stuckToolCallInterval);
        if (inboxNudgeInterval)
            clearInterval(inboxNudgeInterval);
        if (reauthHealerInterval)
            clearInterval(reauthHealerInterval);
        ...
        if (federationPollerInterval)
            clearInterval(federationPollerInterval);
        if (capabilityRunnerInterval)
            clearInterval(capabilityRunnerInterval);
