...
// revoked.
export function createDeviceKey(name, opts = {}) {
...
export function resolveDeviceKey(raw) {
...
export function getDeviceKey(id) {
    const row = getDb().prepare(`SELECT ${INFO_COLUMNS} FROM device_keys WHERE id = ?`).get(id);
...
export function findDeviceKeyByInstallId(installId) {
    const row = getDb().prepare(`SELECT ${INFO_COLUMNS} FROM device_keys WHERE install_id = ?`).get(installId);
    return row ? rowToInfo(row) : null;
}
...
export function revokeDeviceKey(id) {
...
export function revokeAllDeviceKeys() {
...
export function sweepExpiredDeviceKeys() {
    const now = nowSec();
...
export function _clearDeviceKeyCacheForTest() {
    cache.clear();
}
