...
export function readAgentTelegramConfig(name) {
...
export function readAgentDiscordConfig(name) {
...
export function readMarveenTelegramConfig() {
...
export function readMarveenDiscordConfig() {
...
export function readMarveenGooglechatConfig() {
...
export function readMarveenTeamsConfig() {
...
export function readMarveenSlackConfig() {
    const envPath = join(homedir(), '.claude', 'channels', 'slack', '.env');
    if (!existsSync(envPath))
        return { hasSlack: false };
...
export const marveenBotUsernameCache = { fetchedAt: 0 };
export async function refreshMarveenBotUsername() {
    const envPath = join(homedir(), '.claude', 'channels', 'telegram', '.env');
    if (!existsSync(envPath))
        return;
...
export async function sendTelegramPhoto(token, chatId, photoPath, caption) {
...
export async function sendWelcomeMessage(agentName, token) {
    const chatId = resolveOwnerChatId();
    if (!chatId) {
        logger.warn('Telegram send skipped: no owner chat on this install');
        return;
    }
...
export async function sendMarveenAvatarChange(avatarPath) {
...
export async function sendAvatarChangeMessage(agentName, avatarPath) {
...
export function parseTelegramToken(name) {
    const envPath = join(agentDir(name), '.claude', 'channels', 'telegram', '.env');
    if (!existsSync(envPath))
        return null;
    const content = readFileOr(envPath, '');
...
export async function sendMarveenAlert(text) {
    try {
        const envPath = join(PROJECT_ROOT, '.env');
        const envContent = readFileOr(envPath, '');
        const tokenMatch = envContent.match(/TELEGRAM_BOT_TOKEN=(.+)/);
