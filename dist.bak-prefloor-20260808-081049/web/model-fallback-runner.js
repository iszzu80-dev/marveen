...
import { listAgentNames, readAgentRemoteHost, readAgentModel, writeAgentModel, resolveModelId, DEFAULT_MODEL, } from './agent-config.js';
...
function writeMainModel(model) {
    let cfg = {};
    try {
        cfg = JSON.parse(readFileSync(MAIN_SETTINGS_PATH, 'utf-8'));
    }
    catch { }
    cfg.model = model;
    atomicWriteFileSync(MAIN_SETTINGS_PATH, JSON.stringify(cfg, null, 2));
}
function readModelFor(name) {
    return name === MAIN_AGENT_ID ? readMainModel() : readAgentModel(name);
}
function writeModelFor(name, model) {
    if (name === MAIN_AGENT_ID)
        writeMainModel(model);
    else
...
async function restartFor(name) {
    if (name === MAIN_AGENT_ID) {
...
async function checkAgent(name, nowMs, revertAfterMs, chain) {
    ...
    try {
        ...
        if (action.kind === 'downgrade')
            downgradedAt.set(name, nowMs);
        else
            downgradedAt.delete(name);
        logger.info({ name, from: currentModel, to: action.model, action: action.kind }, 'model-fallback: switched model');
    }
    catch (err) {
        logger.warn({ err, name }, 'model-fallback: switch failed');
    }
}
export function startModelFallbackRunner() {
    async function sweep() {
        const cfg = readModelFallbackConfig();
        if (!cfg.enabled) {
            if (downgradedAt.size > 0)
                downgradedAt.clear(); // re-seed cleanly if re-enabled
            return;
        }
        const now = Date.now();
        const revertAfterMs = cfg.revertAfterMinutes * 60_000;
        try {
            await checkAgent(MAIN_AGENT_ID, now, revertAfterMs, cfg.chain);
        }
        catch (err) {
            logger.debug({ err }, 'model-fallback: main check error');
        }
        for (const name of listAgentNames()) {
            try {
                await checkAgent(name, now, revertAfterMs, cfg.chain);
            }
            catch (err) {
                logger.debug({ err, agent: name }, 'model-fallback: agent check error');
            }
        }
    }
