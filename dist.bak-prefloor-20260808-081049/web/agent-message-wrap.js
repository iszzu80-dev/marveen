...
import { wrapUntrusted, wrapTrustedPeer, wrapChannelInbound, UNTRUSTED_PREAMBLE, TRUSTED_PEER_PREAMBLE, CHANNEL_INBOUND_PREAMBLE, sanitizeAgentIdent, sanitizeOriginNote, } from '../prompt-safety.js';
...
const CHANNEL_COORDINATOR_AGENTS = new Set([COORDINATOR_AGENT_ID]);
...
export function classifyAgentMessage(fromAgent, toAgent) {
    ...
    if (!safeFrom)
        return null;
    if (CHANNEL_COORDINATOR_AGENTS.has(safeFrom))
        return { category: 'channel-inbound', safeFrom };
...
export function wrapAgentMessageForDelivery(category, safeFrom, fromAgent, content, msgId, originNote) {
