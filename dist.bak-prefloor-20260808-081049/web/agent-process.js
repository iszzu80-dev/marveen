...
import { paneLooksIdle, decideSubmitFollowup, shouldClearTruncatedPreamble, detectsPastePlaceholder, detectPaneState, parkedInputText, parkedInputRowCount, parkedClearSequence, stripGhostSuggestion, stripSessionTitleBanner, stripAllAnsi, paneShowsContextSaturation, idleConsideringDimGhost, detectsFirstRunGate, detectsModelConsentDialog, } from '../pane-state.js';
import { agentDir, listAgentNames, readAgentModel, readAgentClaudePlan, readAgentChannelProvider, readAgentAuthMode, readAgentDisplayName, readAgentRemoteConfig, readAgentRemoteHost, readAgentMemoryIsolation } from './agent-config.js';
...
import { withSessionSendLock, tryAcquireSessionSendLane } from './session-send-lock.js';
import { buildTmuxInvocation, buildSshExec, buildRemoteLaunchCommand, buildContinueProbeCommand, classifyRunState, classifyRunStateFromExit, sessionInList, ensureControlDir, cleanStaleSshSockets, } from './ssh-tmux.js';
import { parseTelegramToken } from './telegram.js';
import { getProvider, getProviderType, channelStateDir, readChannelToken } from '../channel-provider.js';
...
// its own plugin.
export function scopeChannelPlugins(explicitProvider, existing) {
    const out = { ...(existing ?? {}) };
...
export function ownChannelProviderForScope(hasOwnToken, resolvedProvider) {
...
export function buildTelegramMcpServerConfig(bunBin, pluginDir, stateDir) {
...
export function hasFleetOauthToken() {
    try {
...
export function shouldAlertSharedConfigCollision(hasToken, sameProviderContenderCount, platform = process.platform) {
    if (platform === 'darwin')
        return false;
...
export function maxSameProviderContenders(agents) {
    const counts = new Map();
    for (const a of agents) {
        if (!a.running || !a.hasChannel)
            continue;
...
export function countSameProviderChannelContenders(startingName) {
    return maxSameProviderContenders(listAgentNames()
...
export function resetSharedConfigCollisionAlert() {
...
function maybeAlertSharedConfigCollision(name) {
    const count = countSameProviderChannelContenders(name);
    if (!shouldAlertSharedConfigCollision(false, count) || sharedConfigCollisionAlerted)
        return;
    sharedConfigCollisionAlerted = true;
    logger.error({ name, sameProviderContenders: count }, 'isolated-config: fleet OAuth token missing with multiple RUNNING same-provider channel sub-agents -- shared ~/.claude plugin-slot collision, bots may go deaf');
    void notifyChannel(`⚠️ Flotta-figyelmeztetes: hianyzik a fleet OAuth token (store/.claude-oauth-token), es ${count} AZONOS csatorna-providerü sub-agent fut egyszerre. Izolacio nelkul mind a kozos ~/.claude-ot hasznalja, igy a plugin-slot utkozhet es bot nemulhat el. Javitas: futtasd a \`claude setup-token\`-t, mentsd a store/.claude-oauth-token fajlba, majd inditsd ujra az agenseket.`).catch(() => { });
}
...
export function ensureIsolatedChannelConfigDir(name, 
...
export function ensureMainAgentIsolatedConfigDir(provider, platform = process.platform) {
    let enabled = false;
    try {
        enabled = String(getEffectiveSettingValue('MAIN_AGENT_ISOLATED_CONFIG')) === '1';
    }
    catch {
        enabled = false;
    }
    ...
    if (!hasFleetOauthToken())
        return null;
    return provisionIsolatedConfigDir(join(PROJECT_ROOT, '.channels-config'), PROJECT_ROOT, getProviderType(provider), MAIN_AGENT_ID);
}
...
export function resolveMainAgentConfigDir() {
    let raw = '';
    try {
        raw = String(getEffectiveSettingValue('MAIN_AGENT_CONFIG_DIR') ?? '').trim();
    }
    catch {
        return null;
    }
...
function reconcileMcpServers(cur, sharedDot, name) {
    if (!existsSync(sharedDot))
        return false;
    let shared;
    try {
        shared = JSON.parse(readFileSync(sharedDot, 'utf-8'));
    }
    catch {
        return false;
    } // unparseable shared config -> leave the isolated one alone
    if (!isPlainObject(shared.mcpServers))
        return false;
...
function provisionIsolatedConfigDir(cfg, cwd, providerType, name) {
    try {
        ...
        for (const entry of readdirSync(realClaude)) {
            ...
            if (needsLink) {
                try {
                    symlinkSync(join(realClaude, entry), link);
                }
                catch (err) {
                    logger.warn({ err, entry, name }, 'isolated-config: symlink failed');
                }
            }
        }
        ...
        if (existsSync(sharedSettings)) {
            try {
                settings = JSON.parse(readFileSync(sharedSettings, 'utf-8'));
            }
            catch {
                settings = {};
            }
        }
        const scopedPlugins = scopeChannelPlugins(providerType, settings.enabledPlugins);
        ...
        for (const sub of ['cache', 'marketplaces', 'data']) {
            ...
            if (needsLink) {
                try {
                    ...
                    logger.warn({ err, sub, name }, 'isolated-config: plugin symlink failed');
                }
            }
        }
        ...
        if (existsSync(sharedInstalled)) {
            try {
                const inst = JSON.parse(readFileSync(sharedInstalled, 'utf-8'));
        ...
        try {
            ...
            if (!existsSync(dotClaude)) {
                let seed = { hasCompletedOnboarding: true };
                if (existsSync(sharedDot)) {
                    try {
                        seed = JSON.parse(readFileSync(sharedDot, 'utf-8'));
                    }
                    catch { /* keep minimal */ }
                }
                ...
                try {
                    const cur = JSON.parse(readFileSync(dotClaude, 'utf-8'));
                    let dirty = false;
                    if (cur.hasCompletedOnboarding !== true) {
                        cur.hasCompletedOnboarding = true;
                        dirty = true;
                    }
                    if (reconcileMcpServers(cur, sharedDot, name))
                        dirty = true;
                    if (dirty)
                        writeJsonAtomic(dotClaude, cur);
                }
...
export function ensureSharedClaudeOnboarded(dotClaudePath = join(homedir(), '.claude.json')) {
    try {
        ...
        const cur = JSON.parse(readFileSync(dotClaudePath, 'utf-8'));
        if (cur.hasCompletedOnboarding === true)
            return false;
        cur.hasCompletedOnboarding = true;
...
export function stampProjectTrustForDir(dotClaudePath, projectDir) {
    try {
        let data = {};
        if (existsSync(dotClaudePath)) {
            data = JSON.parse(readFileSync(dotClaudePath, 'utf-8'));
        }
        const dirs = new Set([projectDir]);
        try {
            dirs.add(realpathSync(projectDir));
        }
        catch { /* dir may not resolve yet */ }
        ...
        for (const dir of dirs) {
            const base = (projects[dir] && typeof projects[dir] === 'object')
                ? projects[dir]
                : {};
            if (base.hasTrustDialogAccepted === true && base.hasCompletedProjectOnboarding === true)
                continue;
            projects[dir] = {
                ...base,
...
export function stampFableOverageConsent(dotClaudePath) {
    try {
        if (!existsSync(dotClaudePath))
            return false;
        const data = JSON.parse(readFileSync(dotClaudePath, 'utf-8'));
...
export function stampFableOverageConsentSharedRoots() {
    ...
    for (const p of candidates) {
        if (p && existsSync(p))
            stampFableOverageConsent(p);
    }
}
function resolveAgentProvider(name) {
    const perAgent = readAgentChannelProvider(name);
    if (perAgent === 'slack' || perAgent === 'telegram' || perAgent === 'discord' || perAgent === 'googlechat' || perAgent === 'teams')
        return perAgent;
    return CHANNEL_PROVIDER;
}
export function agentSessionName(name) {
    return `agent-${name}`;
}
/**
...
function runTmux(host, tmuxArgs, opts = {}) {
...
function captureTmux(host, tmuxArgs, opts = {}) {
    if (host)
        ensureControlDir();
    const inv = buildTmuxInvocation(host, tmuxBin(), tmuxArgs);
...
export function agentRunState(name) {
...
export function isAgentRunning(name) {
...
export function sessionExistsOnHost(host, session) {
    try {
...
export function getAgentRunningSince(name) {
    try {
        const host = readAgentRemoteHost(name);
...
export function agentHasChannel(name) {
    ...
    if (token)
        return true;
    if (agentProvider === 'telegram')
        return !!parseTelegramToken(name);
    return false;
}
...
function startRemoteAgentProcess(name, host, workdir, opts) {
...
export function startAgentProcess(name, opts = {}) {
    const dir = agentDir(name);
    if (!existsSync(dir))
    ...
    if (readAgentMemoryIsolation(name))
        provisionMemoryBoundaryDir(dir);
    ...
    try {
        ...
        if (name !== MAIN_AGENT_ID) {
            ...
            try {
                ...
                s.enabledPlugins = scopeChannelPlugins(scopeProvider, s.enabledPlugins);
        ...
        if (planResolution.planUnresolved) {
            ...
            logger.warn({ name, plan: readAgentClaudePlan(name) }, 'claude-plan: configured plan id does not resolve in store/claude-plans.json; falling back to raw config-dir / default login');
        }
        ...
        stampProjectTrustForDir(claudeConfigDir ? join(claudeConfigDir, '.claude.json') : join(homedir(), '.claude.json'), dir);
        ...
        stampFableOverageConsent(claudeConfigDir ? join(claudeConfigDir, '.claude.json') : join(homedir(), '.claude.json'));
...
export function stopAgentProcess(name) {
    const session = agentSessionName(name);
    if (!isAgentRunning(name))
...
export function getAgentProcessInfo(name) {
...
export function restartAgentProcess(name, opts = {}) {
    if (isAgentRunning(name)) {
        const stopResult = stopAgentProcess(name);
        if (!stopResult.ok)
            return { ok: false, error: stopResult.error || 'Failed to stop running agent before restart' };
    }
...
async function dismissSurveyModalIfPresent(session, host = null) {
    try {
        const pane = captureTmux(host, ['capture-pane', '-t', session, '-p']);
        if (!SURVEY_MODAL_RX.test(pane))
            return;
...
export async function dismissResumeSummaryModalIfPresent(session, host = null) {
    try {
        const pane = captureTmux(host, ['capture-pane', '-t', session, '-p']);
        if (!RESUME_SUMMARY_MODAL_RX.test(pane))
            return;
...
export async function dismissModelConsentDialogIfPresent(session, host = null) {
    try {
        const pane = captureTmux(host, ['capture-pane', '-t', session, '-p']);
        if (!detectsModelConsentDialog(pane))
            return;
        runTmux(host, ['send-keys', '-t', session, '1'], { timeout: 5000 });
        await delay(150);
        runTmux(host, ['send-keys', '-t', session, 'Enter'], { timeout: 5000 });
...
export async function answerFirstRunGates(session, host = null) {
    let acted = false;
    for (let i = 0; i < FIRST_RUN_ANSWER_MAX_STEPS; i++) {
        const pane = capturePane(session, host);
        const gate = pane != null ? detectsFirstRunGate(pane) : null;
        if (gate == null)
...
export function identitySlashCommands(displayName) {
...
export async function scheduleIdentitySetup(session, displayName, host = null) {
    setTimeout(() => {
        void (async () => {
            try {
...
export async function waitForPaneIdle(session, host = null, timeoutMs = PANE_IDLE_WAIT_TIMEOUT_MS) {
...
export async function clearInputBuffer(session, host = null) {
    try {
        const pane = capturePane(session, host);
...
export async function sendPromptToSession(session, text, host = null, opts = {}) {
    ...
    const lockMode = opts.lockMode ?? 'deliver';
...
export function sendEnterToSession(session, host = null) {
    try {
        runTmux(host, ['send-keys', '-t', session, 'Enter'], { timeout: 5000 });
        return true;
    }
...
export function capturePane(session, host = null) {
    try {
...
export function captureParkedInputView(session, host = null) {
    try {
...
export async function isSessionReadyForPrompt(session, host = null) {
    ...
    const idleOrGhost = (plain) => idleConsideringDimGhost(plain, detectPaneState(plain) === 'typing' ? captureParkedInputView(session, host) : null);
...
export async function clearStaleParkedInput(session, host = null) {
    const a = capturePane(session, host);
    if (a == null || detectPaneState(a) !== 'typing')
        return false;
    ...
    if (prev && prev.sig === parked && nowMs - prev.last < UNWEDGE_COOLDOWN_MS)
        return false;
    ...
    if (b == null || detectPaneState(b) !== 'typing' || parkedInputText(captureParkedInputView(session, host) ?? b) !== parked)
        return false;
    ...
    for (let i = 0; i < sequence.length; i++) {
        ...
        if (i % PARKED_CLEAR_RECHECK_EVERY === PARKED_CLEAR_RECHECK_EVERY - 1) {
            ...
            if (after == null || detectPaneState(after) !== 'typing')
                break;
        }
    }
    await delay(PARKED_CLEAR_SETTLE_MS);
    ...
    if (stillStuck) {
        ...
        if (!escalated && fails >= SUBAGENT_PARKED_ESCALATE_AFTER) {
            ...
            notifyChannel(`⚠️ Egy sub-agent (${session}) input-mezojebe beragadt egy parkolt sor, ` +
                `az auto-tisztitas ${fails}x sikertelen -- lehet kezi beavatkozas kell. Reszlet: "${preview}"`).catch(() => { });
            escalated = true;
