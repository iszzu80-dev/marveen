 ...
 *   logs is not a skip).
 */
export async function withSessionSendLock(session, host, mode, fn, opts = {}) {
...
export function tryAcquireSessionSendLane(session, host) {
    const lane = laneFor(keyFor(session, host));
    if (lane.busy)
        return null;
    lane.busy = true;
...
export function __resetSessionSendLocks() {
    lanes.clear();
}
