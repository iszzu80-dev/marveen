...
export function isEmptyTaskState(r) {
    return (r.doneSteps.length === 0 &&
 ...
 */
export function shouldReplayTaskState(record, source, nowMs, ttlMs = TASKSTATE_TTL_MS) {
    if (!record)
        return false;
    if (record.consumed)
        return false;
    if (!REPLAY_SOURCES.has(source))
        return false;
    if (nowMs - record.ts > ttlMs)
        return false;
    if (isEmptyTaskState(record))
        return false;
    return true;
}
 ...
 */
export function buildTaskStateInjection(r) {
    const lines = [
        SENTINEL,
    ...
    if (r.summary.trim())
        lines.push(`FELADAT: ${r.summary.trim()}`);
    if (r.doneSteps.length)
        lines.push('MAR KESZ (NE ismeteld meg):\n' + r.doneSteps.map((s) => `  - ${s}`).join('\n'));
    if (r.alreadyDelegated.length)
        lines.push('MAR DELEGALVA (NE kuldd ujra):\n' + r.alreadyDelegated.map((s) => `  - ${s}`).join('\n'));
    if (r.nextAction.trim())
        lines.push(`KOVETKEZO AKCIO (innen folytasd): ${r.nextAction.trim()}`);
    if (r.pendingDecision.trim())
        lines.push(`NYITOTT DONTES / BLOKKOLO: ${r.pendingDecision.trim()}`);
    return lines.join('\n\n');
}
export function readTaskState(agent) {
...
export function writeTaskState(agent, fields, nowMs) {
...
export function markConsumed(agent) {
...
export function clearTaskState(agent) {
    const path = recordPath(agent);
    try {
        if (existsSync(path))
            unlinkSync(path);
...
export function sweepOrphanTaskStates(nowMs, ttlMs = TASKSTATE_TTL_MS) {
    if (!existsSync(STORE_DIR))
        return 0;
    let swept = 0;
    for (const f of readdirSync(STORE_DIR)) {
        if (!f.endsWith('.json'))
            continue;
        try {
            const raw = JSON.parse(readFileSync(join(STORE_DIR, f), 'utf-8'));
