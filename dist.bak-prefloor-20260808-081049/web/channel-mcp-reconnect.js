 ...
 */
async function dismissMcpMenu(session) {
    for (let i = 0; i < 4; i++) {
        try {
...
export function resolveAgentSession(agentName) {
    if (agentName === MAIN_AGENT_ID)
        return MAIN_CHANNELS_SESSION;
    return agentSessionName(agentName);
}
export function resolveAgentProviderType(agentName) {
    const perAgent = readAgentChannelProvider(agentName);
    if (perAgent === 'slack' || perAgent === 'telegram')
        return perAgent;
    return CHANNEL_PROVIDER;
}
function getPluginPattern(providerType) {
...
export function selectedSubmenuLine(pane) {
    for (const raw of pane.split('\n')) {
        if (POINTER_RX.test(raw))
            return raw;
    }
    return null;
}
 ...
 * "Disable".
 */
export function chooseSubmenuTarget(pane) {
    // Status-first: ground truth, immune to footer false-positives.
    if (DISABLED_STATUS_RX.test(pane))
        return ENABLE_RX;
    if (FAILED_STATUS_RX.test(pane))
        return RECONNECT_RX;
    ...
    if (RECONNECT_RX.test(pane))
        return RECONNECT_RX;
    if (ENABLE_RX.test(pane))
        return ENABLE_RX;
    return null;
}
/**
 ...
 * target.
 */
export async function attemptChannelMcpReconnect(agentName) {
    ...
    try {
        ...
        if (matchedAt < 0) {
            logger.warn({ agentName, session, maxUpAttempts: MAX_UP_ATTEMPTS, pluginPattern: pluginPattern.source }, 'channel-mcp-reconnect: plugin submenu not found');
            await dismissMcpMenu(session);
        ...
        if (!onTarget) {
            logger.warn({ agentName, session, target: target.source, maxSteps: SUBMENU_MAX_STEPS }, 'channel-mcp-reconnect: could not place cursor on target option');
            await dismissMcpMenu(session);
