...
export const DEFAULT_CONTEXT_GUARD = {
    enabled: false,
...
export function normalizeContextGuardConfig(raw) {
    const o = (raw && typeof raw === 'object') ? raw : {};
    const pct = (v, dflt) => (typeof v === 'number' && Number.isFinite(v) && v > 0 && v < 1) ? v : dflt;
    const mins = (v, dflt) => (typeof v === 'number' && Number.isFinite(v) && v > 0) ? v : dflt;
    let actPct = pct(o.actPct, DEFAULT_CONTEXT_GUARD.actPct);
    let hardPct = pct(o.hardPct, DEFAULT_CONTEXT_GUARD.hardPct);
    if (hardPct < actPct)
        hardPct = actPct; // hard tier can never sit below act
...
export const CONTEXT_LIMIT_TIERS = [200_000, 500_000, 1_000_000];
/**
...
export function contextLimitForModel(model) {
    ...
    if (m.includes('[1m]'))
        return 1_000_000;
    if (ONE_MILLION_FAMILIES.some(rx => rx.test(m)))
        return 1_000_000;
    if (DEEPSEEK_FAMILIES.some(rx => rx.test(m)))
        return DEEPSEEK_CONTEXT_LIMIT;
    return 200_000;
}
/**
 ...
 */
export function isRecognizedContextModel(model) {
    if (typeof model !== 'string')
        return false;
    const m = model.toLowerCase();
    if (m.includes('[1m]'))
    ...
    if (ONE_MILLION_FAMILIES.some(rx => rx.test(m)))
        return true;
    if (TWO_HUNDRED_K_FAMILIES.some(rx => rx.test(m)))
        return true;
    if (DEEPSEEK_FAMILIES.some(rx => rx.test(m)))
        return true;
    return false;
 ...
 */
export function calibrateLimit(observedTokens, baseLimit) {
    const explains = (limit) => observedTokens <= limit * CALIBRATION_OVERSHOOT_TOLERANCE;
    let limit = baseLimit;
...
export const INITIAL_GUARD_STATE = {
    phase: 'idle',
...
function cooldown(nowMs, cfg, reason) {
    return {
        action: 'none',
        reason,
        nextState: {
...
function restartDecision(nowMs, reason) {
    return {
        action: 'restart',
        reason,
        nextState: {
 ...
 */
export function decideGuard(state, inputs, cfg) {
    const { nowMs } = inputs;
    const none = (reason, next = state) => ({ action: 'none', reason, nextState: next });
    if (!cfg.enabled && !cfg.saturationRestart)
        return none('disabled', INITIAL_GUARD_STATE);
    switch (state.phase) {
        case 'cooldown': {
            if (nowMs >= state.cooldownUntilMs)
                return none('cooldown elapsed', INITIAL_GUARD_STATE);
            return none('cooling down');
        }
        case 'idle': {
            if (!inputs.running)
                return none('not running', INITIAL_GUARD_STATE);
            ...
            if (!cfg.enabled)
                return none('proactive guard disabled (saturation net armed)', cleared);
            if (inputs.pct === null)
                return none('context unmeasurable', cleared);
        ...
        case 'await-handoff': {
            ...
            const handoffWritten = inputs.handoffMtime !== null &&
            ...
            if (inputs.pct !== null && inputs.pct >= cfg.hardPct) {
                if (inputs.paneBusy)
                    return none('hard threshold during await-handoff but agent mid-turn -- deferring restart');
                return restartDecision(nowMs, 'hard threshold during await-handoff');
            }
            if (nowMs >= state.deadlineMs) {
                if (inputs.paneBusy)
                    return none('handoff timeout but agent mid-turn -- deferring restart');
 ...
 */
export function findContextWindowViolations(observations) {
    const violations = [];
    for (const obs of observations) {
        if (!isRecognizedContextModel(obs.model))
            continue;
 ...
 */
export function findUnrecognizedModelsInUse(observations) {
    const gaps = [];
    for (const obs of observations) {
        if (isRecognizedContextModel(obs.model))
            continue;
        if (obs.peak <= 0)
            continue;
        if (obs.turnCount < MIN_TURNS_FOR_REQUIRED_RECOGNITION)
            continue;
