 ...
 */
export function matchesProviderPollerCmd(cmd, provider) {
    if (!cmd)
        return false;
    if (!RUNTIME_TOKEN_RX.test(cmd))
        return false;
    if (SLUG_RX[provider].test(cmd))
        return true;
    if (provider === 'slack' && SLACK_SOCKET_MODE_RX.test(cmd))
        return true;
    return false;
}
