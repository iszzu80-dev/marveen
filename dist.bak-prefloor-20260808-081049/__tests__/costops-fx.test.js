...
import { resolveFxRate, roundHuf, resolveRateDate, convertToHufWithProvenance, canRecomputeFx, convertCorrectionToHuf, lookupHistoricalRate, fxRateDedupKey, initFxSchema, } from '../costops/fx.js';
const RATES = { USD: 350, EUR: 380 };
...
describe('initFxSchema', () => {
    ...
    `).run({ now });
        const row = db.prepare(`SELECT fx_source, conversion_method FROM cost_line_items WHERE source_id = 'anthropic-api'`).get();
    ...
    it('creates the fx_rates table and enforces one row per dedup_key', () => {
        ...
        const row = db.prepare(`SELECT rate FROM fx_rates WHERE dedup_key = ?`).get(dk);
