...
import { registerConnector, getHealth, setMode, recordSuccess, recordFailure, isUsable, DEGRADED_THRESHOLD, DOWN_THRESHOLD, } from '../cos/connector-health.js';
...
describe('COS connector health', () => {
    ...
    it('degrades OK → DEGRADED → DOWN by threshold, then a success recovers', () => {
        ...
        for (let i = 1; i < DEGRADED_THRESHOLD; i++)
            expect(recordFailure(db, 'rental', 'e', 1000 + i).status).toBe('OK');
        expect(recordFailure(db, 'rental', 'e', 2000).status).toBe('DEGRADED'); // at threshold
        for (let i = DEGRADED_THRESHOLD + 1; i < DOWN_THRESHOLD; i++)
            recordFailure(db, 'rental', 'e', 2000 + i);
    ...
    it('isUsable: fail-closed on unregistered/DOWN/DISABLED; write needs READ_WRITE', () => {
        ...
        for (let i = 0; i < DOWN_THRESHOLD; i++)
            recordFailure(db, 'gmail', 'e', 1100 + i);
