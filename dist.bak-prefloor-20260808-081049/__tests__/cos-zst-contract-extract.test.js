...
describe('ZST contract/renewal extractor (v1 heuristic)', () => {
    ...
    it('registers the contract idempotently and keeps it UNDER_REVIEW (never auto-signed)', () => {
        ...
        expect(db.prepare(`SELECT COUNT(*) n FROM zst_contracts`).get().n).toBe(1);
        const c = db.prepare(`SELECT status FROM zst_contracts WHERE contract_id=?`).get(r1.contractId);
    ...
    it('end-to-end: a ZST renewal email through intake creates a case AND a zst_contract', () => {
        ...
        const c = db.prepare(`SELECT contract_id, expiry_date, case_id FROM zst_contracts`).get();
