...
function withScript(content, ext = '.sh') {
...
describe('schedule-runner pre-check integration (source-level)', () => {
    ...
    it('skips and records the run when pre-check returns skip in cron loop', () => {
        ...
        expect(afterCronPc.slice(afterCronPc.indexOf('if (cronPc.skip)'), afterCronPc.indexOf('if (pendingKeys.has(key))'))).toMatch(/appendTaskRun/);
    });
