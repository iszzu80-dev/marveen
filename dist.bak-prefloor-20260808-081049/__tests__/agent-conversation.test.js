...
import { initDatabase, createAgentMessage, getAgentConversation, getAgentConversationThreads, } from '../db.js';
beforeAll(() => {
...
describe('getAgentConversation', () => {
    ...
    it('respects the limit', () => {
        const a = 'tconv-limit';
        for (let i = 0; i < 5; i++)
            createAgentMessage(a, 'marveen', `m${i}`);
    ...
    it('paginates older with beforeId (scroll-up)', () => {
        ...
        const ids = [];
        for (let i = 0; i < 6; i++)
            ids.push(createAgentMessage(a, 'marveen', `p${i}`).id);
