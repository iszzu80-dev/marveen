...
class MockRental {
    ...
    async search(_p) { return [offer(84000)]; }
}
const RENTAL_QUERY = {
...
describe('cosTick (one full cycle)', () => {
    ...
    it('sends+verifies a planned email, runs a due radar check that hits, and surfaces due work', async () => {
        ...
        // effects landed
        const ledger = db.prepare(`SELECT status FROM outbound_ledger WHERE ledger_id=?`).get(p.ledgerId);
    ...
    it('surfaces RECOVERY_REQUIRED outbound rows for a human (never auto-driven)', async () => {
        ...
        // and it stays put
        expect(db.prepare(`SELECT status FROM outbound_ledger WHERE ledger_id=?`).get(p.ledgerId).status).toBe('RECOVERY_REQUIRED');
    });
