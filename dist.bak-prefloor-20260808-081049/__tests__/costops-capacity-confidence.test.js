...
import { writeRateLimitSnapshot, latestRateLimitSnapshot, assertSnapshotConfidence, defaultConfidenceForSource, MEASURED_SNAPSHOT_SOURCES, } from '../costops/capacity-snapshots.js';
...
describe('P2-C: only a real provider read may claim "measured"', () => {
    ...
    it('6. re-running on the next tick does not turn one manual reading into many observations', () => {
        ...
        const n = db.prepare("SELECT COUNT(*) c FROM provider_ratelimit_snapshots WHERE provider='anthropic'").get();
        expect(n.c).toBe(1);
