...
import { MODEL_PROFILE_IDS, isModelProfileId, resolveAgentModelFromConfig, validateModelProfileMap, } from '../model-profiles.js';
const DEFAULT_MODEL = 'claude-opus-5';
const ALIASES = { sonnet: 'claude-sonnet-5', 'sonnet-5': 'claude-sonnet-5' };
const alias = (raw) => ALIASES[raw] ?? raw;
...
describe('model profile map validation', () => {
    ...
    it('rejects a PARTIAL map instead of filling the gap with a default', () => {
        ...
        if (!state.ok)
            expect(state.error).toContain('analysis_efficient');
    });
    ...
    it('rejects an unknown profile id in the map', () => {
        ...
        expect(state.ok).toBe(false);
        if (!state.ok)
            expect(state.error).toContain('cheap_and_cheerful');
    });
...
describe('resolver precedence', () => {
    ...
    it('an explicit model BEATS a modelProfile', () => {
        const r = resolveAgentModelFromConfig({ model: 'deepseek-v4-pro', modelProfile: 'premium_reasoning' }, GOOD_MAP_STATE, DEFAULT_MODEL, alias);
...
describe('failure semantics -- no silent model change', () => {
    ...
    it('a broken map with ONLY a modelProfile carries the map error forward', () => {
        const r = resolveAgentModelFromConfig({ modelProfile: 'build_strong' }, { ok: false, error: 'profile_map_unparseable' }, DEFAULT_MODEL, alias);
        expect(r.source).toBe('default');
        expect(r.error).toBe('profile_map_unparseable');
    });
    it('a broken map does NOT disturb an agent that names an explicit model', () => {
        const r = resolveAgentModelFromConfig({ model: 'deepseek-v4-pro', modelProfile: 'build_strong' }, { ok: false, error: 'profile_map_unparseable' }, DEFAULT_MODEL, alias);
        expect(r.model).toBe('deepseek-v4-pro');
        expect(r.error).toBe(undefined);
    });
});
describe('behaviour neutrality (spec 5.5 acceptance)', () => {
    ...
    it.each(LIVE_SNAPSHOT)('the canary agent $agent resolves to the SAME model through its profile as through its explicit model', ({ model, profile }) => {
