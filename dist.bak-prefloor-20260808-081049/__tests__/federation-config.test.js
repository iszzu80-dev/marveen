...
import { validateFederationConfig, getFederationConfig, writeFederationConfig, identifyFederationCaller, setFederationEnabledPreservingFile, setFederationRoutingModePreservingFile, removeFederationStore, generatePeerInboundToken, isAcceptablePeerBaseUrl, abandonWindowMsForPeer, _setFederationStoreDirForTest, reloadFederationForTest, FEDERATION_MIN_TOKEN_LENGTH, DEFAULT_ABANDON_WINDOW_MINUTES, } from '../web/federation/config.js';
...
describe('validateFederationConfig (pure)', () => {
    ...
    it('is strict about enabled === true', () => {
        for (const enabled of ['true', 1, undefined]) {
            ...
            if (typeof r !== 'string')
                ...
                expect.fail(`expected config, got: ${r}`);
        }
    });
    ...
    it('allows an EMPTY outboundToken (pairing pending) but rejects a short one', () => {
        ...
        if (typeof pending !== 'string')
            expect(pending.peers[0].outboundToken).toBe('');
    ...
    it('validates abandonWindowMinutes bounds', () => {
        ...
        if (typeof ok !== 'string')
            expect(ok.peers[0].abandonWindowMinutes).toBe(1440);
        else
            expect.fail(ok);
    ...
    it('rejects invalid ids and baseUrls; strips trailing slashes', () => {
        ...
        if (typeof r !== 'string')
            expect(r.peers[0].baseUrl).toBe('https://x.example');
    });
});
