...
describe('dashboard modal CSS contract', () => {
    ...
    it('checkbox/radio inputs in a form-group override the text-field width:100% (overflow guard)', () => {
        ...
        expect(body, 'missing `.form-group input[type="checkbox"]` width override in web/style.css').not.toBeNull();
        expect(body).toMatch(/width\s*:\s*auto/i);
    });
    it.each(['.btn-compact[hidden]', '.tg-notice[hidden]'])('%s stays out of layout when hidden', (selector) => {
