...
describe('outbound_ledger P1.1 migration', () => {
    it('rebuilds a pre-P1.1 table, mapping legacy states and back-filling the marker', () => {
        ...
        const ins = db.prepare(`INSERT INTO outbound_ledger (ledger_id, case_id, action_type, sequence_number,
       ...
       VALUES (?,?,?,?,?,?,?,0,?,?)`);
        ...
        const cols = db.prepare('PRAGMA table_info(outbound_ledger)').all().map(c => c.name);
        ...
        const rows = db.prepare('SELECT ledger_id, status, external_idempotency_marker, internal_idempotency_key, external_ref FROM outbound_ledger ORDER BY ledger_id').all();
        ...
        for (const r of rows)
            expect(r.external_idempotency_marker).toBe(r.internal_idempotency_key);
    ...
    it('is idempotent — a second init on the already-migrated table is a no-op', () => {
       ...
       VALUES ('ob-1','c1','EMAIL_SEND',1,'k1','k1','PLANNED',0,900,900)`).run();
...
describe('existing-db column migrations (P1.2/P1.5/P1.6 regression)', () => {
    ...
    it('re-running initCosSchema on the OLD tables does NOT throw and adds the columns + index', () => {
        ...
        expect(db.prepare(`SELECT COUNT(*) c FROM email_processing`).get().c).toBe(1);
        expect(db.prepare(`SELECT COUNT(*) c FROM radar_observations`).get().c).toBe(1);
    });
