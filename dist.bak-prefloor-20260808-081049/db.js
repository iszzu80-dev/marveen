...
function tightenDbPermissions(dbPath) {
...
export function initDatabase(dbPathOverride) {
  ...
  `);
    ...
    try {
        const kcSchema = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='kanban_cards'").get();
  ...
  `);
    ...
    try {
        const current = db.prepare("SELECT sql FROM sqlite_master WHERE name='memories'").get();
  ...
  `);
    ...
    try {
        db.exec('ALTER TABLE agent_messages ADD COLUMN trace_id TEXT');
    }
    catch { /* exists */ }
    try {
        db.exec('ALTER TABLE agent_messages ADD COLUMN span_id TEXT');
    }
    catch { /* exists */ }
    try {
        db.exec('ALTER TABLE agent_messages ADD COLUMN parent_span_id TEXT');
    }
    catch { /* exists */ }
    ...
    try {
        db.exec('ALTER TABLE agent_messages ADD COLUMN dispatch_id TEXT');
    }
    catch { /* exists */ }
  ...
  `);
    ...
    try {
        db.exec('ALTER TABLE pending_channel_requests ADD COLUMN resolved_at INTEGER');
    }
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE task_runs ADD COLUMN status TEXT NOT NULL DEFAULT 'fired'`);
    }
  ...
  `);
    ...
    try {
        db.exec('ALTER TABLE token_usage ADD COLUMN thinking_tokens INTEGER NOT NULL DEFAULT 0');
    }
    catch { /* already exists */ }
    try {
  ...
  `);
    ...
    try {
        db.exec('ALTER TABLE idea_box ADD COLUMN impact INTEGER');
    }
    catch { /* already exists */ }
    try {
        db.exec('ALTER TABLE idea_box ADD COLUMN effort INTEGER');
    }
    catch { /* already exists */ }
    // --- Idea Comments ---
    db.exec(`
  ...
  `);
    ...
    const toolLogCols = db.prepare('PRAGMA table_info(tool_call_log)').all().map(r => r.name);
    if (!toolLogCols.includes('agent_id'))
        db.exec('ALTER TABLE tool_call_log ADD COLUMN agent_id TEXT');
    if (!toolLogCols.includes('trace_id'))
        db.exec('ALTER TABLE tool_call_log ADD COLUMN trace_id TEXT');
    if (!toolLogCols.includes('duration_ms'))
        db.exec('ALTER TABLE tool_call_log ADD COLUMN duration_ms INTEGER');
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE store_file_audit ADD COLUMN agent TEXT`);
    }
  ...
  `);
    ...
    try {
        db.exec('ALTER TABLE vault_ssh_servers DROP COLUMN key_type');
    }
    catch { /* column absent or SQLite pre-3.35 */ }
    try {
        db.exec('ALTER TABLE vault_ssh_servers DROP COLUMN fingerprint');
    }
    catch { /* column absent or SQLite pre-3.35 */ }
    try {
        db.exec('ALTER TABLE vault_ssh_servers DROP COLUMN vault_key_id');
    }
    catch { /* column absent or SQLite pre-3.35 */ }
    try {
        db.exec('ALTER TABLE vault_ssh_servers DROP COLUMN key_expires_at');
    }
    catch { /* column absent or SQLite pre-3.35 */ }
    try {
        db.exec('ALTER TABLE vault_ssh_servers ADD COLUMN ssh_key_id TEXT REFERENCES vault_ssh_keys(id)');
    }
    catch { /* already exists */ }
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE device_keys ADD COLUMN install_id TEXT`);
    }
    catch { /* column already exists */ }
...
function migrateTaskRunsFromJson() {
    const legacyPath = join(STORE_DIR, 'task-run-history.json');
    if (!existsSync(legacyPath))
        return;
    const existingCount = db.prepare('SELECT COUNT(*) as c FROM task_runs').get().c;
    if (existingCount > 0) {
        ...
        const tx = db.transaction((rows) => {
            for (const e of rows) {
                if (!e || typeof e !== 'object')
                    continue;
                const { name, agent, ts } = e;
                if (typeof name !== 'string' || typeof agent !== 'string' || typeof ts !== 'number')
                    continue;
...
export function setSession(chatId, sessionId, messageCount = 0) {
    db.prepare('INSERT OR REPLACE INTO sessions (chat_id, session_id, updated_at, message_count) VALUES (?, ?, ?, ?)').run(chatId, sessionId, Math.floor(Date.now() / 1000), messageCount);
}
export function incrementSessionCount(chatId) {
    db.prepare('UPDATE sessions SET message_count = message_count + 1 WHERE chat_id = ?').run(chatId);
    const row = db.prepare('SELECT message_count FROM sessions WHERE chat_id = ?').get(chatId);
    return row?.message_count ?? 0;
}
export function clearSession(chatId) {
    db.prepare('DELETE FROM sessions WHERE chat_id = ?').run(chatId);
}
export function createDashboardUser(username, passwordHash) {
...
export function countDashboardUsers(includeDisabled = false) {
...
export function updateDashboardUserPassword(userId, passwordHash) {
    db.prepare('UPDATE dashboard_users SET password_hash = ?, updated_at = ? WHERE id = ?')
        .run(passwordHash, Math.floor(Date.now() / 1000), userId);
}
export function deleteDashboardUser(username) {
    const info = db.prepare('DELETE FROM dashboard_users WHERE username = ? COLLATE NOCASE').run(username);
    return info.changes > 0;
}
export function saveMemory(chatId, content, sector, topicKey) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('INSERT INTO memories (chat_id, topic_key, content, sector, salience, created_at, accessed_at) VALUES (?, ?, ?, ?, 1.0, ?, ?)').run(chatId, topicKey ?? null, content, sector, now, now);
}
...
export function buildFtsMatchExpression(query) {
    const MAX_TOKENS = 20;
...
export function recencyWeightedScore(row, nowSec, lambda = RECENCY_LAMBDA, tauSec = RECENCY_TAU_SEC) {
...
export function reRankByRecency(rows, limit, nowSec = Math.floor(Date.now() / 1000)) {
    return rows
...
export function searchMemories(query, chatId, limit = 3) {
    ...
    try {
        const candidates = db
            .prepare(`SELECT m.*, f.rank AS rank FROM memories m
         JOIN memories_fts f ON m.id = f.rowid
         WHERE f.content MATCH ? AND m.chat_id = ?
         ORDER BY rank
         LIMIT ?`)
            .all(terms, chatId, limit * RECENCY_OVERSAMPLE);
        return withoutRank(reRankByRecency(candidates, limit));
    }
    catch {
        return [];
    }
}
export function recentMemories(chatId, limit = 5) {
    return db
...
export function touchMemoriesAccessed(ids) {
    if (ids.length === 0)
        return;
    const now = Math.floor(Date.now() / 1000);
...
export function getMemoriesForChat(chatId, limit = 10) {
    return db
        .prepare('SELECT * FROM memories WHERE chat_id = ? ORDER BY accessed_at DESC LIMIT ?')
        .all(chatId, limit);
}
...
function memoryCacheInvalidate(agentId) {
...
export function saveAgentMemory(agentId, content, category, // hot, warm, cold, shared
keywords, autoGenerated = false) {
    const now = Math.floor(Date.now() / 1000);
    const info = db.prepare('INSERT INTO memories (chat_id, topic_key, content, sector, salience, created_at, accessed_at, agent_id, category, auto_generated, keywords) VALUES (?, ?, ?, ?, 1.0, ?, ?, ?, ?, ?, ?)').run(ALLOWED_CHAT_ID, null, content, 'semantic', now, now, agentId, category, autoGenerated ? 1 : 0, keywords ?? null);
...
export function getAgentMemories(agentId, limit = 20, category) {
    const key = `${agentId}:${limit}:${category ?? ''}`;
    const cached = memoryCacheGet(key);
    if (cached)
        return cached;
    const result = (category
        ? db.prepare("SELECT * FROM memories WHERE (agent_id = ? OR category = 'shared') AND category = ? ORDER BY accessed_at DESC LIMIT ?").all(agentId, category, limit)
        : db.prepare("SELECT * FROM memories WHERE (agent_id = ? OR category = 'shared') ORDER BY accessed_at DESC LIMIT ?").all(agentId, limit));
    memoryCacheSet(key, result);
    return result;
}
export function searchAgentMemories(agentId, query, limit = 10) {
    const terms = buildFtsMatchExpression(query);
    if (!terms)
        return [];
    try {
        const candidates = db.prepare(`SELECT m.*, f.rank AS rank FROM memories m
       JOIN memories_fts f ON m.id = f.rowid
       WHERE f.memories_fts MATCH ? AND (m.agent_id = ? OR m.category = 'shared')
       ORDER BY rank LIMIT ?`).all(terms, agentId, limit * RECENCY_OVERSAMPLE);
        return withoutRank(reRankByRecency(candidates, limit));
    }
    catch {
        return db.prepare("SELECT * FROM memories WHERE (agent_id = ? OR category = 'shared') AND (content LIKE ? OR keywords LIKE ?) ORDER BY accessed_at DESC LIMIT ?").all(agentId, `%${query}%`, `%${query}%`, limit);
    }
}
export function getMemoryStats() {
    const total = db.prepare('SELECT COUNT(*) as c FROM memories').get().c;
    const withEmbedding = db.prepare('SELECT COUNT(*) as c FROM memories WHERE embedding IS NOT NULL').get().c;
    const agentRows = db.prepare('SELECT agent_id, COUNT(*) as c FROM memories GROUP BY agent_id').all();
    const tierRows = db.prepare('SELECT category, COUNT(*) as c FROM memories GROUP BY category').all();
    const byAgent = {};
    const byTier = {};
    for (const r of agentRows)
        byAgent[r.agent_id] = r.c;
    for (const r of tierRows)
        byTier[r.category] = r.c;
    return { total, byAgent, byTier, withEmbedding };
}
export function updateMemory(id, content, category, agentId, keywords) {
    const now = Math.floor(Date.now() / 1000);
    ...
    const before = db.prepare('SELECT agent_id, category FROM memories WHERE id = ?').get(id);
    const sets = ['content = ?', 'accessed_at = ?'];
    ...
    if (changed) {
        if (before?.category === 'shared' || category === 'shared') {
            ...
            if (before?.agent_id)
                memoryCacheInvalidate(before.agent_id);
            if (agentId && agentId !== before?.agent_id)
                memoryCacheInvalidate(agentId);
        }
    }
    return changed;
...
// --- Daily logs ---
export function appendDailyLog(agentId, content) {
    const now = Math.floor(Date.now() / 1000);
...
export function getDailyLog(agentId, date) {
    return db.prepare('SELECT id, content, created_at FROM daily_logs WHERE agent_id = ? AND date = ? ORDER BY created_at ASC').all(agentId, date);
}
export function getDailyLogDates(agentId, limit = 14) {
    return db.prepare('SELECT DISTINCT date FROM daily_logs WHERE agent_id = ? ORDER BY date DESC LIMIT ?').all(agentId, limit).map(r => r.date);
}
function toBudapestTs(dateStr, endOfDay) {
...
export function recallByDateRange(from, to, agentId) {
    ...
    const logs = db.prepare(logSql).all(...logParams);
    ...
    const memories = db.prepare(memSql).all(...memParams);
    return { logs, memories, dateRange: { from, to } };
}
export function recallSearch(query, agentId, limit = 50) {
    const terms = buildFtsMatchExpression(query);
    let memories = [];
    ...
    if (terms) {
        try {
            ...
            const candidates = agentId
                ? db.prepare(sql).all(terms, agentId, limit * RECENCY_OVERSAMPLE)
                : db.prepare(sql).all(terms, limit * RECENCY_OVERSAMPLE);
            memories = withoutRank(reRankByRecency(candidates, limit));
        }
        catch {
            const sql = agentId
            ...
            const pat = `%${escaped}%`;
            memories = agentId
                ? db.prepare(sql).all(agentId, pat, pat, limit)
                : db.prepare(sql).all(pat, pat, limit);
        }
    }
    const logSql = agentId
    ...
    const logs = agentId
        ? db.prepare(logSql).all(logPat, agentId, limit)
        : db.prepare(logSql).all(logPat, limit);
    const dates = logs.map(l => l.date);
    const from = dates.length ? dates[dates.length - 1] : '';
    const to = dates.length ? dates[0] : '';
    return { logs, memories, dateRange: { from, to } };
}
export function createBackgroundTaskAtomic(id, agentId, prompt, tmuxSession, maxConcurrent) {
    const now = Math.floor(Date.now() / 1000);
    const result = db.transaction(() => {
        const running = db.prepare("SELECT COUNT(*) as c FROM background_tasks WHERE agent_id = ? AND status = 'running'").get(agentId).c;
...
export function getRunningBackgroundTasks() {
    return db.prepare("SELECT * FROM background_tasks WHERE status = 'running'").all();
}
export function finishBackgroundTask(id, status, output) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('UPDATE background_tasks SET status = ?, finished_at = ?, output = ? WHERE id = ?')
        .run(status, now, output, id);
}
export function getBackgroundTasks(agentId, includeFinished = false) {
    if (agentId) {
        ...
        return db.prepare(sql).all(agentId);
    }
    const sql = includeFinished
...
export function getBackgroundTask(id) {
    return db.prepare('SELECT * FROM background_tasks WHERE id = ?').get(id);
}
export function countRunningBackgroundTasks(agentId) {
    return db.prepare("SELECT COUNT(*) as c FROM background_tasks WHERE agent_id = ? AND status = 'running'").get(agentId).c;
}
export function markOrphanedTasksFailed() {
    const now = Math.floor(Date.now() / 1000);
...
export function createTask(id, chatId, prompt, schedule, nextRun) {
    db.prepare('INSERT INTO scheduled_tasks (id, chat_id, prompt, schedule, next_run, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(id, chatId, prompt, schedule, nextRun, Math.floor(Date.now() / 1000));
}
...
export function updateTaskAfterRun(id, nextRun, result) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('UPDATE scheduled_tasks SET last_run = ?, next_run = ?, last_result = ? WHERE id = ?').run(now, nextRun, result, id);
}
...
export function pauseTask(id) {
    return (db.prepare("UPDATE scheduled_tasks SET status = 'paused' WHERE id = ?").run(id).changes > 0);
}
export function resumeTask(id) {
    return (db.prepare("UPDATE scheduled_tasks SET status = 'active' WHERE id = ?").run(id).changes > 0);
}
export function getTask(id) {
    return db.prepare('SELECT * FROM scheduled_tasks WHERE id = ?').get(id);
}
export function updateTask(id, prompt, schedule, nextRun) {
...
export function listKanbanCards() {
    ...
    db.prepare("UPDATE kanban_cards SET archived_at = ? WHERE status = 'done' AND archived_at IS NULL AND updated_at < ?").run(Math.floor(Date.now() / 1000), archiveCutoff);
    return db
        .prepare('SELECT rowid AS seq, * FROM kanban_cards WHERE archived_at IS NULL ORDER BY sort_order ASC')
        .all();
}
export function listKanbanCardsSummary() {
    return db
...
export function getKanbanCard(id) {
    return db.prepare('SELECT rowid AS seq, * FROM kanban_cards WHERE id = ?').get(id);
}
export function createKanbanCard(card) {
    const now = Math.floor(Date.now() / 1000);
    const status = card.status ?? 'planned';
    const maxRow = db.prepare('SELECT MAX(sort_order) as m FROM kanban_cards WHERE status = ? AND archived_at IS NULL').get(status);
    const sortOrder = (maxRow?.m ?? -1) + 1;
    db.prepare(`INSERT INTO kanban_cards (id, title, description, status, assignee, priority, project, parent_id, due_date, sort_order, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(card.id, card.title, card.description ?? null, status, card.assignee ?? null, card.priority ?? 'normal', card.project ?? null, card.parent_id ?? null, card.due_date ?? null, sortOrder, now, now);
}
export function updateKanbanCard(id, fields) {
    const card = getKanbanCard(id);
    if (!card)
        return false;
    const now = Math.floor(Date.now() / 1000);
    const f = { ...card, ...fields, updated_at: now };
    return db.prepare(`UPDATE kanban_cards SET title=?, description=?, status=?, assignee=?, priority=?, project=?, parent_id=?, due_date=?, sort_order=?, updated_at=?, archived_at=?
     WHERE id=?`).run(f.title, f.description, f.status, f.assignee, f.priority, f.project, f.parent_id, f.due_date, f.sort_order, f.updated_at, f.archived_at, id).changes > 0;
}
export function getChildCards(parentId) {
    return db.prepare('SELECT * FROM kanban_cards WHERE parent_id = ? AND archived_at IS NULL ORDER BY sort_order ASC').all(parentId);
}
export function moveKanbanCard(id, status, sortOrder, actor) {
    const now = Math.floor(Date.now() / 1000);
    ...
    const prev = db.prepare('SELECT status FROM kanban_cards WHERE id=?').get(id)?.status;
    const changed = db.prepare('UPDATE kanban_cards SET status=?, sort_order=?, updated_at=? WHERE id=?').run(status, sortOrder, now, id).changes > 0;
    if (changed && prev !== undefined && prev !== status) {
        db.prepare('INSERT INTO kanban_card_events (card_id, from_status, to_status, actor, created_at) VALUES (?, ?, ?, ?, ?)').run(id, prev, status, actor ?? null, now);
    }
    return changed;
}
// Stamp the once-only kanban -> agent dispatch guard. Returns false if the
// card id does not exist.
export function markKanbanCardDispatched(id) {
    const now = Math.floor(Date.now() / 1000);
    return db.prepare('UPDATE kanban_cards SET dispatched_at=? WHERE id=?').run(now, id).changes > 0;
}
export function archiveKanbanCard(id) {
    const now = Math.floor(Date.now() / 1000);
    return db.prepare('UPDATE kanban_cards SET archived_at=?, updated_at=? WHERE id=?').run(now, now, id).changes > 0;
}
export function unarchiveKanbanCard(id) {
    const now = Math.floor(Date.now() / 1000);
...
export function listKanbanProjects() {
    const rows = db.prepare("SELECT DISTINCT project FROM kanban_cards WHERE project IS NOT NULL AND project != '' AND archived_at IS NULL ORDER BY project").all();
    return rows.map(r => r.project);
}
export function deleteKanbanCard(id) {
...
export function getKanbanComments(cardId) {
    return db.prepare('SELECT * FROM kanban_comments WHERE card_id = ? ORDER BY created_at ASC').all(cardId);
}
export function getKanbanCardEvents(cardId) {
    return db.prepare('SELECT * FROM kanban_card_events WHERE card_id = ? ORDER BY created_at ASC, id ASC').all(cardId);
}
...
export function getKanbanSeqByIdPrefix(prefix) {
    const rows = db.prepare('SELECT rowid AS seq FROM kanban_cards WHERE id = ? COLLATE NOCASE LIMIT 2').all(prefix);
    if (rows.length !== 1)
        return null;
    return rows[0].seq;
}
...
export function findActiveKanbanCardByTitle(title) {
    return db.prepare('SELECT rowid AS seq, * FROM kanban_cards WHERE title = ? AND archived_at IS NULL LIMIT 1').get(title);
}
...
export function markScheduledTaskKanbanWaiting(taskName) {
    const card = findActiveKanbanCardByTitle(taskName);
    if (!card)
        return null;
    const maxResult = db.prepare("SELECT MAX(sort_order) as m FROM kanban_cards WHERE status = 'waiting' AND archived_at IS NULL").get();
    const sortOrder = (maxResult.m ?? 0) + 100;
    moveKanbanCard(card.id, 'waiting', sortOrder, 'scheduler');
    return card.id;
}
export function addKanbanComment(cardId, author, content) {
    const now = Math.floor(Date.now() / 1000);
    const info = db.prepare('INSERT INTO kanban_comments (card_id, author, content, created_at) VALUES (?, ?, ?, ?)').run(cardId, author, content, now);
...
export function createLabel(label) {
    ...
    const existing = db.prepare('SELECT id, name, color, created_at FROM labels WHERE name = ?').get(label.name);
    if (existing)
        return existing;
    db.prepare('INSERT INTO labels (id, name, color, created_at) VALUES (?, ?, ?, ?)').run(label.id, label.name, label.color, now);
...
export function addLabelToCard(cardId, labelId) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('INSERT OR IGNORE INTO kanban_card_labels (card_id, label_id, created_at) VALUES (?, ?, ?)').run(cardId, labelId, now);
}
export function removeLabelFromCard(cardId, labelId) {
    return db.prepare('DELETE FROM kanban_card_labels WHERE card_id = ? AND label_id = ?').run(cardId, labelId).changes > 0;
}
export function getLabelsForCard(cardId) {
    return db.prepare(`
...
export function getLabelsForAllCards() {
...
export const HEARTBEAT_URGENT_SQL = "SELECT * FROM kanban_cards WHERE archived_at IS NULL AND priority = 'urgent' AND status != 'done'";
export const HEARTBEAT_IN_PROGRESS_SQL = "SELECT * FROM kanban_cards WHERE archived_at IS NULL AND status = 'in_progress'";
export const HEARTBEAT_WAITING_SQL = "SELECT * FROM kanban_cards WHERE archived_at IS NULL AND status = 'waiting'";
export function getHeartbeatKanbanSummary() {
    const urgent = db.prepare(HEARTBEAT_URGENT_SQL).all();
    const in_progress = db.prepare(HEARTBEAT_IN_PROGRESS_SQL).all();
    const waiting = db.prepare(HEARTBEAT_WAITING_SQL).all();
    return { urgent, in_progress, waiting };
}
export function createAgentMessage(from, to, content, originNote, traceCtx, dispatchId) {
    const now = Math.floor(Date.now() / 1000);
    const info = db.prepare('INSERT INTO agent_messages (from_agent, to_agent, content, status, created_at, origin_note, trace_id, span_id, parent_span_id, dispatch_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(from, to, content, 'pending', now, originNote ?? null, traceCtx?.trace_id ?? null, traceCtx?.span_id ?? null, traceCtx?.parent_span_id ?? null, dispatchId ?? null);
    return {
...
export function getPendingMessages(toAgent) {
    if (toAgent) {
...
// result either.
export function markMessageDelivered(id) {
    const now = Math.floor(Date.now() / 1000);
...
export function getPendingBacklogByAgent() {
    const now = Math.floor(Date.now() / 1000);
    const rows = db.prepare(`SELECT to_agent AS agent, COUNT(*) AS pending, MIN(created_at) AS oldest
       FROM agent_messages
      WHERE status = 'pending'
      GROUP BY to_agent`).all();
    return rows
...
export function closeMessagesWithoutDelivery(ids, reason) {
    if (!ids.length)
        return 0;
    const now = Math.floor(Date.now() / 1000);
    const note = `closed-without-delivery: ${reason}`;
    const stmt = db.prepare(`UPDATE agent_messages SET status = 'delivered', delivered_at = ?, result = ?
      WHERE id = ? AND status = 'pending'`);
    const run = db.transaction((rows) => {
        let n = 0;
        for (const id of rows)
            n += stmt.run(now, note, id).changes;
        return n;
    });
    return run(ids);
}
...
export function setMessageResult(id, result) {
...
export function failPendingFederatedMessages(peerId, reason) {
    const now = Math.floor(Date.now() / 1000);
    const rows = peerId === undefined
        ? db.prepare(`UPDATE agent_messages SET status = 'failed', result = ?, completed_at = ?
           WHERE status = 'pending' AND instr(to_agent, '/') > 0
         RETURNING id`).all(reason, now)
        : db.prepare(`UPDATE agent_messages SET status = 'failed', result = ?, completed_at = ?
           WHERE status = 'pending' AND instr(to_agent, '/') > 0
             AND lower(substr(to_agent, 1, instr(to_agent, '/') - 1)) = lower(?)
         RETURNING id`).all(reason, now, peerId);
...
export function claimPendingForAgent(toAgent, limit) {
    const now = Math.floor(Date.now() / 1000);
    const rows = db.prepare(`UPDATE agent_messages SET status = 'delivered', delivered_at = ?
       WHERE id IN (
     ...
     RETURNING id, from_agent, to_agent, content, status, result, created_at, delivered_at, completed_at`).all(now, toAgent, limit);
...
export function markMessageDone(id, result) {
    const now = Math.floor(Date.now() / 1000);
...
export function markMessageFailed(id, error) {
    const now = Math.floor(Date.now() / 1000);
...
export function markPendingFederatedFailed(id, error) {
    const now = Math.floor(Date.now() / 1000);
...
export function listAgentMessages(limit = 50) {
    return db.prepare('SELECT * FROM agent_messages ORDER BY created_at DESC LIMIT ?').all(limit);
}
...
export const CHAT_SYSTEM_AGENTS = ['heartbeat', 'telegram-coordinator', 'channel-coordinator', 'system'];
...
export function getAgentConversation(agent, limit = 50, beforeId) {
    const cap = Math.min(Math.max(1, Math.floor(limit) || 1), AGENT_MESSAGE_LIMIT_CAP);
    if (beforeId !== undefined && Number.isFinite(beforeId)) {
        return db.prepare('SELECT * FROM agent_messages WHERE (from_agent = ? OR to_agent = ?) AND id < ? ORDER BY created_at DESC, id DESC LIMIT ?').all(agent, agent, beforeId, cap);
    }
    return db.prepare('SELECT * FROM agent_messages WHERE (from_agent = ? OR to_agent = ?) ORDER BY created_at DESC, id DESC LIMIT ?').all(agent, agent, cap);
}
...
export function getAgentConversationThreads() {
    ...
    FROM parties p
  `).all();
    const lastStmt = db.prepare('SELECT * FROM agent_messages WHERE from_agent = ? OR to_agent = ? ORDER BY created_at DESC, id DESC LIMIT 1');
    const system = new Set(CHAT_SYSTEM_AGENTS);
    const threads = [];
    for (const p of parties) {
        if (!p.agent || system.has(p.agent))
            continue;
        const lastMessage = lastStmt.get(p.agent, p.agent) ?? null;
...
export function appendTaskRun(name, agent, status = 'fired') {
    const now = Date.now();
...
export function listTaskRunHistory(name, limit) {
    const rows = db.prepare('SELECT ts, status, agent FROM task_runs WHERE name = ? ORDER BY ts DESC LIMIT ?').all(name, limit);
    // token_usage.timestamp is in seconds; task_runs.ts is in ms -- divide by 1000
    const tokenStmt = db.prepare(`SELECT COALESCE(SUM(input_tokens + output_tokens + cache_read_tokens), 0) as total
    ...
    return rows.map((row, i) => {
        ...
        const tokenRow = tokenStmt.get(row.agent, Math.floor(row.ts / 1000), Math.floor(windowEnd / 1000));
        return { ts: row.ts, status: row.status, tokens_est: tokenRow.total > 0 ? tokenRow.total : null };
    });
}
export function countTaskRunsBetween(fromTs, toTs) {
    if (toTs === undefined) {
        const row = db.prepare('SELECT COUNT(*) as c FROM task_runs WHERE ts >= ?').get(fromTs);
        return row.c;
    }
    const row = db.prepare('SELECT COUNT(*) as c FROM task_runs WHERE ts >= ? AND ts < ?').get(fromTs, toTs);
    return row.c;
}
export function getAgentMessage(id) {
    return db.prepare('SELECT * FROM agent_messages WHERE id = ?').get(id);
}
export function getActiveScheduledTaskCount() {
    const row = db
 ...
 * from the cron loop.
 */
export function insertPendingTaskRetryIfNew(taskName, agentName, now, reason) {
    return db.prepare(`
 ...
 */
export function updatePendingTaskRetry(taskName, agentName, now, reason) {
    return db.prepare(`
...
export function upsertPendingTaskRetry(taskName, agentName, now, reason) {
...
export function clearPendingTaskRetryAlert(taskName, agentName) {
    return db
...
export function listPendingTaskRetries() {
    return db
        .prepare('SELECT * FROM pending_task_retries ORDER BY first_attempt ASC')
        .all();
}
export function getPendingTaskRetry(taskName, agentName) {
    return db
        .prepare('SELECT * FROM pending_task_retries WHERE task_name = ? AND agent_name = ?')
        .get(taskName, agentName);
}
export function deletePendingTaskRetry(taskName, agentName) {
    return db
        .prepare('DELETE FROM pending_task_retries WHERE task_name = ? AND agent_name = ?')
        .run(taskName, agentName).changes > 0;
}
export function deletePendingTaskRetryById(id) {
    return db
        .prepare('DELETE FROM pending_task_retries WHERE id = ?')
        .run(id).changes > 0;
}
export function markPendingTaskRetryAlert(taskName, agentName, ts) {
    return db
...
function vectorSearch(agentId, queryEmbedding, limit = 10) {
    const rows = db.prepare("SELECT * FROM memories WHERE embedding IS NOT NULL AND (agent_id = ? OR category = 'shared')").all(agentId);
...
export async function hybridSearch(agentId, query, limit = 10) {
...
export async function backfillEmbeddings() {
    const rows = db.prepare('SELECT id, content, keywords FROM memories WHERE embedding IS NULL').all();
    let count = 0;
    for (const row of rows) {
...
export function upsertChannelRequest(agent, channelId, userId) {
    const now = Math.floor(Date.now() / 1000);
    const sevenDaysAgo = now - 7 * 86400;
    const existing = db.prepare("SELECT id FROM pending_channel_requests WHERE agent = ? AND channel_id = ? AND (status = 'pending' OR (status = 'denied' AND COALESCE(resolved_at, requested_at) > ?))").get(agent, channelId, sevenDaysAgo);
    if (existing)
        return false;
    db.prepare('INSERT INTO pending_channel_requests (agent, channel_id, user_id, requested_at, status) VALUES (?, ?, ?, ?, ?)').run(agent, channelId, userId ?? null, now, 'pending');
    return true;
}
export function listPendingChannelRequests(agent) {
    return db.prepare("SELECT * FROM pending_channel_requests WHERE agent = ? AND status = 'pending' ORDER BY requested_at DESC").all(agent);
}
export function updateChannelRequestStatus(id, status) {
    const now = Math.floor(Date.now() / 1000);
    return db.prepare('UPDATE pending_channel_requests SET status = ?, resolved_at = ? WHERE id = ? AND status = ?').run(status, now, id, 'pending').changes > 0;
}
export function updateChannelRequestName(id, channelName) {
    db.prepare('UPDATE pending_channel_requests SET channel_name = ? WHERE id = ?').run(channelName, id);
}
// --- Telegram History ---
export function saveTelegramMessage(chatId, messageId, direction, text, userId, ts) {
    const now = ts ?? Math.floor(Date.now() / 1000);
    db.prepare(`INSERT OR IGNORE INTO telegram_history (chat_id, message_id, user_id, direction, text, ts)
     VALUES (?, ?, ?, ?, ?, ?)`).run(chatId, messageId, userId ?? null, direction, text, now);
}
export function getTelegramHistory(chatId, limit = 50) {
    return db.prepare('SELECT * FROM telegram_history WHERE chat_id = ? ORDER BY ts DESC LIMIT ?').all(chatId, limit);
}
...
export function createIdea(idea) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare(`INSERT INTO idea_box (id, title, description, category, status, source, kanban_id, impact, effort, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(idea.id, idea.title, idea.description ?? null, idea.category, idea.status, idea.source, idea.kanban_id ?? null, idea.impact ?? null, idea.effort ?? null, now, now);
}
export function updateIdea(id, patch) {
    ...
    if (patch.kanban_id !== undefined) {
        sets.push('kanban_id = ?');
        params.push(patch.kanban_id);
    }
...
export function listIdeaCategories() {
    return db.prepare('SELECT DISTINCT category FROM idea_box ORDER BY category').all().map(r => r.category);
}
export function getIdeaComments(ideaId) {
    return db.prepare('SELECT * FROM idea_comments WHERE idea_id = ? ORDER BY created_at ASC').all(ideaId);
}
export function addIdeaComment(ideaId, author, content) {
    const now = Math.floor(Date.now() / 1000);
    const info = db.prepare('INSERT INTO idea_comments (idea_id, author, content, created_at) VALUES (?, ?, ?, ?)').run(ideaId, author, content, now);
...
export function logIdeaStatusChange(ideaId, fromStatus, toStatus, actor, note) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('INSERT INTO idea_status_log (idea_id, from_status, to_status, actor, note, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(ideaId, fromStatus ?? null, toStatus, actor, note ?? null, now);
}
export function getIdeaStatusLog(ideaId) {
    return db.prepare('SELECT * FROM idea_status_log WHERE idea_id = ? ORDER BY created_at ASC').all(ideaId);
}
...
export function revertIdeaFromKanban(kanbanId) {
    const idea = db.prepare("SELECT id, status FROM idea_box WHERE kanban_id = ? AND status = 'kanban'").get(kanbanId);
    if (!idea)
        return null;
    const now = Math.floor(Date.now() / 1000);
...
// --- Tool Call Log ---
export function logToolCall(sessionId, toolName, inputSummary, success = true, agentId = null, traceId = null, durationMs = null) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('INSERT INTO tool_call_log (session_id, tool_name, input_summary, success, created_at, agent_id, trace_id, duration_ms) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(sessionId, toolName, inputSummary, success ? 1 : 0, now, agentId, traceId, durationMs);
}
export function getRecentToolCalls(sinceSecs) {
    const cutoff = Math.floor(Date.now() / 1000) - sinceSecs;
    return db.prepare('SELECT * FROM tool_call_log WHERE created_at >= ? ORDER BY created_at ASC').all(cutoff);
}
export function analyzeWorkflowCandidates(sinceSecs = 3600, minToolCalls = 5, gapSecs = 300) {
...
export function pruneToolCallLog(olderThanSecs = 86400) {
    const cutoff = Math.floor(Date.now() / 1000) - olderThanSecs;
    db.prepare('DELETE FROM tool_call_log WHERE created_at < ?').run(cutoff);
}
export function logSkillUsage(agentId, skillName, triggerType, sessionId) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('INSERT INTO skill_usage (agent_id, skill_name, trigger_type, session_id, created_at) VALUES (?, ?, ?, ?, ?)').run(agentId, skillName, triggerType, sessionId ?? null, now);
}
export function getSkillUsageRows(opts) {
    ...
    params.push(limit);
    return db.prepare(`SELECT * FROM skill_usage WHERE ${conditions.join(' AND ')} ORDER BY created_at DESC LIMIT ?`).all(...params);
}
export function getSkillUsageStats(sinceSecs) {
...
export function logConfigChange(key, oldValue, newValue, actor) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('INSERT INTO config_change_log (key, old_value, new_value, actor, created_at) VALUES (?, ?, ?, ?, ?)').run(key, oldValue === null ? null : String(oldValue), newValue === null ? null : String(newValue), actor, now);
}
export function getRecentConfigChanges(limit = 200) {
    ...
    return db.prepare('SELECT * FROM config_change_log ORDER BY created_at DESC, id DESC LIMIT ?').all(limit);
}
export function logStoreFileEvent(relPath, eventType, isSensitive, fileSize, agent = null) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare('INSERT INTO store_file_audit (rel_path, event_type, is_sensitive, file_size, agent, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(relPath, eventType, isSensitive, fileSize, agent, now);
}
export function getRecentStoreFileEvents(limit = 200) {
    return db.prepare('SELECT * FROM store_file_audit ORDER BY created_at DESC, id DESC LIMIT ?').all(limit);
}
export function queryAuditLog(opts) {
    const { sources, from, to, q, agent, limit } = opts;
    const all = ['config', 'idea', 'store', 'diary'];
    ...
    if (active.includes('config')) {
        ...
        if (q) {
            sql += ' AND (key LIKE ? OR old_value LIKE ? OR new_value LIKE ? OR actor LIKE ?)';
            const p = `%${q}%`;
            params.push(p, p, p, p);
        }
    ...
    if (active.includes('idea')) {
        ...
        if (q) {
            sql += ' AND (idea_id LIKE ? OR to_status LIKE ? OR note LIKE ? OR actor LIKE ?)';
            const p = `%${q}%`;
            params.push(p, p, p, p);
        }
        sql += ' ORDER BY created_at DESC, id DESC LIMIT ?';
    ...
    if (active.includes('store')) {
        ...
        if (q) {
            sql += ' AND (rel_path LIKE ? OR agent LIKE ?)';
            const p = `%${q}%`;
            params.push(p, p);
        }
        sql += ' ORDER BY created_at DESC, id DESC LIMIT ?';
    ...
    if (active.includes('diary')) {
        ...
        if (agent) {
            logSql += ' AND agent_id = ?';
            logParams.push(agent);
        }
        if (q) {
            logSql += ' AND content LIKE ?';
            logParams.push(`%${q}%`);
        }
        logSql += ' ORDER BY created_at DESC, id DESC LIMIT ?';
        logParams.push(limit);
        const logRows = db.prepare(logSql).all(...logParams);
        for (const r of logRows)
            parts.push({ id: r.id, source: 'diary', created_at: r.created_at, agent_id: r.agent_id, content: r.content, entry_type: 'log' });
        // memories
        ...
        if (agent) {
            memSql += ' AND agent_id = ?';
            memParams.push(agent);
        }
        if (q) {
            memSql += ' AND (content LIKE ? OR keywords LIKE ?)';
            memParams.push(`%${q}%`, `%${q}%`);
        }
        memSql += ' ORDER BY created_at DESC, id DESC LIMIT ?';
        memParams.push(limit);
        const memRows = db.prepare(memSql).all(...memParams);
        for (const r of memRows)
            parts.push({ id: r.id, source: 'diary', created_at: r.created_at, agent_id: r.agent_id, content: r.content, category: r.category, keywords: r.keywords ?? undefined, entry_type: 'memory' });
    }
...
export function listVaultSshKeys() {
    return db.prepare('SELECT * FROM vault_ssh_keys ORDER BY label ASC').all();
}
export function getVaultSshKey(id) {
    return db.prepare('SELECT * FROM vault_ssh_keys WHERE id = ?').get(id);
}
export function createVaultSshKey(key) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare(`INSERT INTO vault_ssh_keys (id, label, username, vault_key_id, public_key, fingerprint, key_type, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(key.id, key.label, key.username, key.vault_key_id, key.public_key, key.fingerprint, key.key_type, now);
...
export function deleteVaultSshKey(id) {
    return db.transaction(() => {
        const unassigned = db.prepare('UPDATE vault_ssh_servers SET ssh_key_id = NULL, updated_at = ? WHERE ssh_key_id = ?').run(Math.floor(Date.now() / 1000), id).changes;
        const deleted = db.prepare('DELETE FROM vault_ssh_keys WHERE id = ?').run(id).changes > 0;
        return { deleted, unassigned };
    })();
}
export function computeSshKeyStatus(server) {
    return server.ssh_key_id ? 'ok' : 'missing';
}
export function listVaultSshServers() {
    return db.prepare('SELECT * FROM vault_ssh_servers ORDER BY name ASC').all();
}
export function getVaultSshServer(id) {
    return db.prepare('SELECT * FROM vault_ssh_servers WHERE id = ?').get(id);
}
export function createVaultSshServer(server) {
    const now = Math.floor(Date.now() / 1000);
    db.prepare(`INSERT INTO vault_ssh_servers (id, name, host, port, username, description, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(server.id, server.name, server.host, server.port, server.username, server.description ?? null, now, now);
    return { ...server, ssh_key_id: null, created_at: now, updated_at: now };
}
export function updateVaultSshServer(id, patch) {
    const now = Math.floor(Date.now() / 1000);
    const sets = ['updated_at = ?'];
    const params = [now];
    ...
    if (patch.ssh_key_id !== undefined) {
        sets.push('ssh_key_id = ?');
        params.push(patch.ssh_key_id);
    }
    if (patch.description !== undefined) {
        sets.push('description = ?');
        params.push(patch.description);
...
export function deleteVaultSshServer(id) {
...
export function createApproval(params) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(params.id, params.agent_id, params.category, params.action_description, params.action_payload ?? null, params.timeout_at ?? null, now);
    return {
        id: params.id,
...
export function resolveApproval(id, status, resolvedBy, telegramMessageId) {
    const now = Math.floor(Date.now() / 1000);
    return db.prepare(`
    UPDATE approvals
...
export function listApprovals(opts) {
    ...
    params.push(limit);
    return db.prepare(`SELECT * FROM approvals ${where} ORDER BY requested_at DESC LIMIT ?`).all(...params);
}
...
export function stampMessageTrace(id, traceId, spanId, parentSpanId) {
    return db.prepare(`
    UPDATE agent_messages
...
export function expireTimedOutApprovals() {
    const now = Math.floor(Date.now() / 1000);
    return db.prepare(`
...
export function saveSensitivityAuditEntry(entry) {
    db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(entry.content_hash, entry.verdict, entry.category, JSON.stringify(entry.matched_patterns), entry.target_agent, entry.target_model, entry.message_id ?? null, entry.mode, entry.reason, Math.floor(Date.now() / 1000));
}
export function upsertOtelSpan(span) {
    db.prepare(`
  ...
  `).run(span.trace_id, span.span_id, span.parent_span_id ?? null, span.agent_id, span.operation, span.start_ms, span.end_ms ?? null, span.status ?? 'running', span.attributes ?? null);
}
export function closeOtelSpan(traceId, spanId, endMs, status) {
    return db.prepare(`
    UPDATE otel_spans SET end_ms = ?, status = ? WHERE trace_id = ? AND span_id = ?
  `).run(endMs, status, traceId, spanId).changes > 0;
}
export function getOtelTrace(traceId) {
    return db.prepare('SELECT * FROM otel_spans WHERE trace_id = ? ORDER BY start_ms ASC')
        .all(traceId);
}
export function listOtelTraces(limit = 50) {
    return db.prepare(`
    SELECT
      s.trace_id,
