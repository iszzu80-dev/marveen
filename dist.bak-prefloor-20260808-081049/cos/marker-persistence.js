...
export function probeAction(marker, payload) {
    return {
 ...
 * not on every boot.
 */
export async function verifyMarkerPersistence(adapter, sample) {
    let sentRef = null;
    try {
        ...
        return { passed: false, sent: false, readbackFound: false, readbackAvailable: false, refMatches: false, detail: `send threw: ${String(err?.message ?? err)}` };
    }
    let rb;
    try {
        ...
        return { passed: false, sent: !!sentRef, readbackFound: false, readbackAvailable: false, refMatches: false, detail: `readback threw: ${String(err?.message ?? err)}` };
    }
...
export function markerPersistenceGate(report) {
