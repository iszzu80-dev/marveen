...
function priceToMinor(price, currency) {
...
export function parseProductJsonLd(html) {
    ...
    for (const b of blocks) {
        ...
        for (const o of objs) {
            ...
            if (!Number.isFinite(price))
                continue;
            const currency = String(offer['priceCurrency'] ?? 'HUF').toUpperCase();
            const avRaw = String(offer['availability'] ?? '');
...
export class EmagAdapter {
    ...
    async priceFor(url, fallbackName) {
