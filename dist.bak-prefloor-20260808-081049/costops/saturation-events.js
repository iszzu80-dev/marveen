...
export function initSaturationEventsSchema(db) {
    db.exec(`
 ...
 */
export const SATURATION_EVENT_STATES = [
 ...
 */
export function recordSaturationEvent(db, input, now = Date.now()) {
    if (!input.measured)
        return false;
    if (!input.agent)
        throw new Error('recordSaturationEvent: agent is required');
    if (!input.state)
        throw new Error('recordSaturationEvent: state is required');
    db.prepare(`
 ...
 */
export function recordSaturationEventSafe(db, input, now = Date.now()) {
    try {
 ...
 */
export function countSaturationEvents(db, opts = {}) {
    const conds = [];
    const params = [];
    if (opts.agent) {
        conds.push('agent = ?');
  ...
  `).get(...SATURATION_EVENT_STATES, ...params);
    return {
