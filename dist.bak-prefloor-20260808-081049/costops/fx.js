 ...
 */
export function resolveRateDate(invoiceDate, serviceDate) {
    if (invoiceDate != null)
        return { date: invoiceDate, basis: 'invoice_date' };
    return { date: serviceDate, basis: 'service_date' };
}
function methodForBasis(basis) {
 ...
 */
export function convertToHufWithProvenance(amount, currency, rates, opts) {
    const cur = currency.toUpperCase();
 ...
 */
export function canRecomputeFx(status) {
 ...
 */
export function convertCorrectionToHuf(input) {
    return {
 ...
 */
export function lookupHistoricalRate(records, currency, atDate) {
    const cur = currency.toUpperCase();
 ...
 */
export function fxRateDedupKey(currency, source, effectiveDate) {
 ...
 */
export function initFxSchema(db) {
    try {
        db.exec(`ALTER TABLE cost_line_items ADD COLUMN fx_source TEXT`);
    }
    catch { /* already exists */ }
    try {
        db.exec(`ALTER TABLE cost_line_items ADD COLUMN conversion_method TEXT`);
    }
    catch { /* already exists */ }
    db.exec(`
