 ...
 */
export function deriveMtdSpend(snapshotsAsc) {
    let spend = 0;
...
export function forecastDeepSeekExhaustion(snapshotsAsc, now) {
 ...
 */
export async function syncDeepSeekBalance(db, now, deps = {}) {
    ...
    if (apiKey === undefined) {
        try {
            ...
            apiKey = getSecret(DEEPSEEK_VAULT_SECRET_ID);
        }
        catch {
            apiKey = null;
        }
    ...
    const httpGetJson = deps.httpGetJson || (async (url, headers) => {
        ...
        if (!r.ok)
            throw new Error(`deepseek api ${r.status}`);
        return r.json();
    });
    let balanceUsd;
    try {
    ...
    const rows = db.prepare(`SELECT balance, captured_at FROM provider_balance_snapshots WHERE provider='deepseek' AND captured_at >= ? AND captured_at < ? ORDER BY captured_at ASC`).all(w.start, w.end);
    ...
    // points of its own.
    const allRows = db.prepare(`SELECT balance, captured_at FROM provider_balance_snapshots WHERE provider='deepseek' ORDER BY captured_at ASC`).all();
...
function statusForDeepSeekBalance(balanceUsd) {
    if (balanceUsd < 1)
        return 'critical';
    if (balanceUsd < 3)
        return 'warning';
...
function upsertDeepSeekEntitlement(db, balanceUsd, forecastExhaustionAt, now) {
    db.prepare(`
...
function recordRun(db, status, count, w, now, errMsg) {
