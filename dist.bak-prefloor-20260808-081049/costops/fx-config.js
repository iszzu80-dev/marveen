 ...
 */
function migrateFromLegacyRenderPricingOnce() {
    if (existsSync(COSTOPS_FX_CONFIG_PATH))
        return;
    const legacy = readJson(LEGACY_RENDER_PRICING_PATH);
    if (!legacy)
        return;
    ...
    try {
        const file = {
            _doc: 'CostOps provider-neutral FX rates (LOCAL, gitignored). HUF per 1 unit of the key ' +
...
export function loadFxRates() {
    ...
    const raw = readJson(COSTOPS_FX_CONFIG_PATH);
