...
export function sanitizeManifest(raw, expectedSystem) {
    if (raw === null || typeof raw !== 'object' || Array.isArray(raw))
        return 'manifest is not an object';
    const m = raw;
    const system = truncate(m.system, MAX_SHORT_FIELD);
    if (!system)
        return 'manifest has no system id';
    ...
    if (system.toLowerCase() !== expectedSystem.toLowerCase())
        return `system mismatch: peer reports '${system}'`;
    ...
    const agents = agentsRaw.slice(0, MANIFEST_MAX_AGENTS).flatMap((a) => {
        ...
        if (typeof o.id !== 'string' || !isValidIdSegment(o.id))
            return [];
...
async function pollOnePeer(peer, now, fetchImpl) {
    ...
    try {
        ...
        statusCache.set(peer.id, { ...base, state: 'error', error: 'manifest is not JSON' });
        return;
    }
...
export async function pollPeerManifests(now, fetchImpl = fetch) {
...
export function refreshFederationStatus(fetchImpl = fetch) {
    if (inflightRefresh)
...
export function getFederationStatus() {
    ...
    return cfg.peers.map((peer) => statusCache.get(peer.id) ?? {
        id: peer.id,
        baseUrl: peer.baseUrl,
        state: (!peer.outboundToken ? 'unpaired' : 'unknown'),
        lastChecked: 0,
        lastOkAt: 0,
    });
}
/** Production reset (peer removal / full removal). */
export function resetFederationPollerCache(peerId) {
    if (peerId === undefined)
        statusCache.clear();
    else
        statusCache.delete(peerId);
}
export function startFederationPoller() {
