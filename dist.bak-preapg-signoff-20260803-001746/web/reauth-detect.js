 ...
 */
function liveStatusRegion(pane) {
    const lines = pane.split('\n');
    const borders = [];
    for (let i = lines.length - 1; i >= 0 && borders.length < 2; i--) {
        if (BOX_BORDER_RX.test(lines[i]))
            borders.push(i);
    }
    if (borders.length < 2)
        return null;
 ...
 */
export function detectReauthNeeded(pane) {
    if (!pane)
        return { needsReauth: false };
    const region = liveStatusRegion(pane) ?? tailOf(pane, TAIL_LINES);
    if (ESCALATION_QUOTE_MARKERS.some((rx) => rx.test(region)))
        return { needsReauth: false };
    for (const m of REAUTH_MARKERS) {
        if (m.rx.test(region))
            return { needsReauth: true, reason: m.reason };
    }
    return { needsReauth: false };
}
