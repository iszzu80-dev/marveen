...
export function reportsToCreatesCycle(agentName, proposedReportsTo, readTeam, mainAgentId) {
    if (!proposedReportsTo || proposedReportsTo === mainAgentId)
        return false;
    if (proposedReportsTo === agentName)
        return true;
    let cursor = proposedReportsTo;
    const visited = new Set();
    while (cursor && cursor !== mainAgentId) {
        if (cursor === agentName)
            return true;
        if (visited.has(cursor))
            break; // pre-existing loop elsewhere; don't spin
        visited.add(cursor);
