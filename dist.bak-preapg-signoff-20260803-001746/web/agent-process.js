...
import { paneLooksIdle, decideSubmitFollowup, shouldClearTruncatedPreamble, detectsPastePlaceholder, detectPaneState, parkedInputText, stripGhostSuggestion, paneShowsContextSaturation, idleConsideringDimGhost, detectsFirstRunGate, detectsModelConsentDialog, } from '../pane-state.js';
...
export async function clearStaleParkedInput(session, host = null) {
    ...
    if (post != null && detectPaneState(post) === 'typing' && parkedInputText(post) === parked) {
        ...
        for (let i = 0; i < PARKED_CLEAR_MAX; i++) {
            ...
            if (post == null || detectPaneState(post) !== 'typing')
                break;
        }
    }
