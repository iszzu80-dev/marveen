...
import { agentRunState, agentSessionName, restartAgentProcess, capturePane, sendPromptToSession, isSessionReadyForPrompt, } from './agent-process.js';
...
import { decideGuard, contextLimitForModel, calibrateLimit, INITIAL_GUARD_STATE, } from '../context-guard.js';
...
const remoteSkipLogged = new Set();
...
let highwater = null;
function observedHighwater(name, model, observedNow) {
    if (highwater === null)
        highwater = readHighwater();
    ...
    if (observedNow > prior) {
        ...
        try {
            writeFileSync(HIGHWATER_PATH, JSON.stringify(highwater, null, 2));
        }
        catch (err) {
            logger.warn({ err }, 'context-guard: highwater persist failed');
        }
    }
...
function handoffPathFor(name) {
    return join(workingDirFor(name), 'HANDOFF.md');
}
function handoffMtime(name) {
    try {
        return statSync(handoffPathFor(name)).mtimeMs;
    }
    catch {
        return null;
    }
}
export function handoffPrompt(pctRound, handoffPath) {
    return (`[CONTEXT-GUARD] A munkakontextusod ~${pctRound}%-on van -- kritikus. ` +
...
export function resumePrompt(name, handoffPath, hadHandoff) {
    const base = `[CONTEXT-GUARD] Friss kontextussal indultál, mert az előző session kontextusa megtelt (auto-handoff). `;
...
function measurePct(name, cfgLimit) {
 ...
 */
export function readLiveSaturationSignals(name) {
    if (name !== MAIN_AGENT_ID && readAgentRemoteHost(name))
        return { pct: null, paneSaturated: false };
    let pct = null;
    try {
        pct = measurePct(name, readContextGuardConfig(name).limitTokens);
    }
    catch {
        pct = null;
    }
...
function performRestart(name) {
    if (name === MAIN_AGENT_ID) {
        ...
        if (!res.ok)
            throw new Error(res.error ?? 'main channels hard restart failed');
    }
    else {
        restartAgentProcess(name, { fresh: true });
    }
}
async function checkAgent(name, nowMs) {
    ...
    if (decision.action === 'restart' && name === MAIN_AGENT_ID) {
        ...
        if (shouldDeferForRecentRespawn(lastRespawn, nowMs)) {
            logger.info({ name, sinceRespawnMs: lastRespawn ? nowMs - lastRespawn : null, graceMs: MARVEEN_POST_RESPAWN_GRACE_MS }, 'context-guard: recent main respawn within grace, deferring restart (avoid restart loop / boot churn)');
    ...
    try {
        switch (decision.action) {
            ...
            case 'restart': {
                ...
                performRestart(name);
                try {
                    createAgentMessage(name, MAIN_AGENT_ID, `[CONTEXT-GUARD] Ujrainditottam a(z) "${name}" agentet -- ok: ${decision.reason}` +
                        ...
                        (snapshotPath ? ` Pane-snapshot a restart elotti allapotrol: ${snapshotPath}` : ''), 'context-guard restart notice');
                }
                catch (err) {
...
export function getContextGuardStatus() {
...
export function startContextGuardRunner() {
    async function sweep() {
        const now = Date.now();
        try {
            await checkAgent(MAIN_AGENT_ID, now);
        }
        catch (err) {
            logger.debug({ err }, 'context-guard: main check error');
        }
        for (const name of listAgentNames()) {
            try {
                await checkAgent(name, now);
            }
            catch (err) {
                logger.debug({ err, agent: name }, 'context-guard: agent check error');
            }
        }
    }
