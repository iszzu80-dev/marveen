...
function isCoreAgent(name, config) {
 ...
 */
export function memoryPressureGate(agentName) {
...
function listAgentRss() {
    try {
        ...
        if (parsed.status === "error" || !parsed.agents)
            return [];
...
function isAgentIdle(name) {
    try {
        ...
        // false positives.
        const capture = execSync(`tmux capture-pane -t "${session}" -p 2>/dev/null | tail -8`, { timeout: 2000, encoding: "utf-8" });
        const lines = capture.trim().split("\n").filter(l => l.trim());
        if (lines.length === 0)
            return false; // empty pane = can't tell → assume busy
        ...
        for (const spinner of ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]) {
            if (lastLine.includes(spinner))
                return false;
        }
        // "Thinking..." or "API Error" — model is mid-turn or recovering.
        if (lastLine.includes("Thinking") || lastLine.includes("API Error"))
            return false;
 ...
 */
function verifyProcessTreeGone(name) {
    try {
        ...
        if (!agent)
            return true; // not in the tree → process tree is gone
        ...
        if (agent.rssBytes <= 10 * 1024 * 1024) { // <= 10 MiB = exiting
            ...
            if (!recheckAgent)
                return true; // gone after the extra beat
            if (recheckAgent.rssBytes === 0)
 ...
 */
function stopAgentViaApi(name) {
    const token = readDashboardToken();
    if (!token)
        return false;
    try {
        const stopUrl = `http://localhost:${DASHBOARD_PORT}/api/agents/${encodeURIComponent(name)}/stop`;
        const result = execSync(`curl -s --max-time 8 -X POST "${stopUrl}" -H "Authorization: Bearer ${token}" -H "Content-Type: application/json"`, { timeout: 10000, encoding: "utf-8" });
 ...
 */
function stopAgentFallback(name) {
    try {
 ...
 */
export function relievePressure(state) {
    const config = loadConfig();
    const agents = listAgentRss();
    // Filter: non-core only, idle only
    const candidates = agents.filter(a => {
        if (config.coreAgents.includes(a.name))
            return false;
    ...
    try {
        ...
        const existsBefore = execSync(`tmux ls 2>/dev/null | grep -c "^agent-${target.name}:" || echo 0`, { timeout: 2000, encoding: "utf-8" });
        if (Number(existsBefore.trim()) === 0)
            return null; // already gone, race with manual stop
 ...
 */
export function updateStateAction(action) {
    if (!action)
        return;
