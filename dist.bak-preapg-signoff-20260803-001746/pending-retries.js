 ...
 */
export function shouldSendAlert(now, firstAttempt, alertSentAt, thresholdMs = ALERT_THRESHOLD_MS) {
    if (alertSentAt != null)
        return false;
    if (!Number.isFinite(firstAttempt) || firstAttempt <= 0)
        return false;
    if (!Number.isFinite(now) || now < firstAttempt)
        return false;
 ...
 */
export function classifyTelegramSendError(errMessage) {
    const m = /Telegram API (\d{3})\b/.exec(errMessage);
    if (!m)
        return 'transient'; // no HTTP status -> network-level failure
 ...
 */
export function toPendingRetryView(row, now, thresholdMs = ALERT_THRESHOLD_MS) {
    return {
        id: row.id,
