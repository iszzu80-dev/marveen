...
export const SATURATION_ORDER = [
    'ok', 'warning', 'no_new_large_task', 'checkpoint_required', 'hard_stop',
];
export function saturationSeverity(s) {
    const i = SATURATION_ORDER.indexOf(s);
    return i < 0 ? 0 : i;
}
export function atOrAbove(state, floor) {
 ...
 */
export function normalizeSaturationThresholds(raw) {
    ...
    if (noNewLargeTaskPct < warningPct)
        noNewLargeTaskPct = warningPct;
    if (checkpointRequiredPct < noNewLargeTaskPct)
        checkpointRequiredPct = noNewLargeTaskPct;
    if (hardStopPct < checkpointRequiredPct)
        hardStopPct = checkpointRequiredPct;
    return {
        warningPct,
        noNewLargeTaskPct,
 ...
 */
export function classifySaturation(inputs, thresholds = DEFAULT_SATURATION_THRESHOLDS) {
    ...
    const at = (p) => `${Math.round(pct * 100)}% >= ${Math.round(p * 100)}%`;
...
export const DEFAULT_TASK_SIZE_POLICY = {
    labels: {},
    cardTypes: {},
...
function coerceSize(v) {
    return (typeof v === 'string' && VALID_SIZES.has(v)) ? v : null;
}
...
export function normalizeTaskSizePolicy(raw) {
    const o = (raw && typeof raw === 'object') ? raw : {};
    const table = (v) => {
        const out = {};
 ...
 */
export const LARGE_TASK_REFUSAL_FLOOR = 'no_new_large_task';
/**
 ...
 * classifySaturation).
 */
export function decideDispatchAdmission(inputs) {
