...
export function getClaudePidForSession(session) {
    try {
...
export function decideHasPluginAlive(ctx) {
...
export function snapshotProcsWithRetry(run, timeouts = [PS_PROBE_TIMEOUT_MS, PS_PROBE_RETRY_TIMEOUT_MS]) {
    try {
...
export function probeChannelPluginLiveness(claudePid, providerType, agentName) {
    let psOutput;
    try {
        ...
        psOutput = snapshotProcsWithRetry((timeoutMs) => execFileSync('/bin/ps', ['-axww', '-o', 'pid,ppid,command'], {
            timeout: timeoutMs,
            encoding: 'utf-8',
...
export function hasChannelPluginAlive(claudePid, providerType, agentName) {
    return probeChannelPluginLiveness(claudePid, providerType, agentName) === 'alive';
}
// --- coordinator-side decision layer ---
export function readRespawnStampMs() {
    try {
...
export function readKeepaliveAgeMs(nowMs) {
    try {
...
export function decideNativeChannelDown(f) {
    if (f.msSinceLastRespawn != null && f.msSinceLastRespawn < STARTUP_GRACE_MS)
        return false;
    if (f.claudePid == null)
        return true;
    if (!f.pluginAlive)
        return true;
    if (f.keepaliveAgeMs != null && f.keepaliveAgeMs > KEEPALIVE_STALE_MS)
        return true;
    return false;
}
// Side-effecting: gather the live facts for the main channels session and apply
// the pure decision.
export function probeNativeChannelDown(session, provider, agentName) {
    const now = Date.now();
