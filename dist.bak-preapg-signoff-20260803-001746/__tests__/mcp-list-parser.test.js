...
import { parseMcpListLine, parseMcpList, applyRefreshOutcome, scrubPaths, catalogMatchesConfigured, } from '../mcp-list-parser.js';
...
describe('parseMcpListLine -- claude.ai connectors', () => {
    ...
    it('slugifies a two-word connector name', () => {
        const result = parseMcpListLine('claude.ai Google Calendar: https://calendarmcp.googleapis.com/mcp/v1 - Connected');
        expect(result?.normalizedId).toBe('google-calendar');
    });
    it('handles multiple spaces between words in the name', () => {
        const result = parseMcpListLine('claude.ai Google   Drive: https://example.com/mcp - Connected');
        expect(result?.normalizedId).toBe('google-drive');
    });
    it('parses a needs-auth status', () => {
        const result = parseMcpListLine('claude.ai PostHog: https://mcp.posthog.com/mcp - ! Needs authentication');
        expect(result?.status).toBe('needs_auth');
    });
    it('parses a failed status', () => {
        const result = parseMcpListLine('claude.ai Broken: https://example.com - Failed to connect');
        expect(result?.status).toBe('failed');
    });
    it('preserves URLs containing colons in the endpoint', () => {
        const result = parseMcpListLine('claude.ai n8n: https://example.app.n8n.cloud/mcp-server/http - ! Needs authentication');
...
describe('parseMcpListLine -- plugin entries', () => {
    it('parses a plugin:package:name entry to the trailing slug', () => {
        const result = parseMcpListLine('plugin:telegram:telegram: bun run --cwd /path ... - Connected');
    ...
    it('handles a plugin entry with distinct package and server slugs', () => {
        const result = parseMcpListLine('plugin:acme-bundle:my-server: node /path/server.js - Connected');
        expect(result?.source).toBe('plugin');
        expect(result?.normalizedId).toBe('my-server');
    });
    it('parses a plugin:slack:slack entry (Slack channel provider)', () => {
        const result = parseMcpListLine('plugin:slack:slack: node /path/to/slack-server.js - Connected');
        expect(result).not.toBeNull();
        expect(result?.source).toBe('plugin');
    ...
    it('uses the last colon-separated segment for a two-part plugin name', () => {
        ...
        // emits.
        const result = parseMcpListLine('plugin:telegram: some-endpoint - Connected');
        expect(result?.source).toBe('plugin');
        expect(result?.normalizedId).toBe('telegram');
    });
});
...
describe('applyRefreshOutcome -- cache update rules', () => {
    ...
    const sampleOutput = 'Checking MCP server health...\n' +
        'claude.ai Gmail: https://gmailmcp.googleapis.com/mcp/v1 - Connected\n';
...
describe('scrubPaths -- sensitive-path removal', () => {
    ...
    it('scrubs quoted absolute paths (fs ENOENT shape)', () => {
        ...
        const result = scrubPaths("ENOENT: no such file or directory, open '/Users/foo/x.json'", HOME);
