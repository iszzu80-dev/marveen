...
import { stageAgentDirForExport, importAgentBundle, readBundleManifest, sanitizeImportedConfig, bundleFilename, importAllAgentsBundle, peekBundleKind, readFleetManifest, fleetBundleFilename, BUNDLE_SCHEMA_VERSION, } from '../web/agent-bundle.js';
...
function packBundle(stageRoot, agentName, includesSecrets) {
    const manifest = {
...
describe('agent bundle export/import', () => {
    ...
    it('rejects a name collision unless overwrite is set', () => {
        ...
        const destBase = join(tmp, 'agents');
        const resolveDest = (n) => join(destBase, n);
...
function packFleetBundle(stageRoot, agents, includesSecrets = false) {
