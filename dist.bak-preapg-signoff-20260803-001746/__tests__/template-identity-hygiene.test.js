...
describe('runtime-seeded placeholders are all substituted', () => {
    ...
    it('resolveTemplatePlaceholders replaces every known identity placeholder', () => {
        ...
        expect(survivors, `Known placeholders the seed failed to substitute: ${survivors.join(', ')}`).toEqual([]);
    });
    ...
    it('every placeholder used under scheduled-tasks/ is in the known set', () => {
        ...
        expect(unknown, `Placeholders used in scheduled-tasks/ that the seed does not substitute: ${unknown.join(', ')}`).toEqual([]);
    });
    ...
    it('update.sh stays host-agnostic (no hardcoded roster or operator identity)', () => {
        ...
        for (const name of ['samu', 'zara', 'boni', 'iris', 'deeper', 'slacker']) {
            expect(new RegExp(`\\b${name}\\b`, 'i').test(updateSh), `update.sh hard-codes fleet roster name "${name}" -- it must compare against agents/ at runtime instead`).toBe(false);
        }
