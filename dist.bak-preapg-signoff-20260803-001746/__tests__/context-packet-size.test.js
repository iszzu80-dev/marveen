...
import { buildContextPacket, renderContextPacket, estimateFreshTokens, artifactRefFromContent, validateContextPacket, TARGET_FRESH_TOKENS, } from '../context-packet.js';
...
const SMALL = buildContextPacket({
    cardId: 'b7f0aa12',
...
const KANBAN_DISPATCH = buildContextPacket({
    cardId: 'c31d99f4',
...
const LARGE = buildContextPacket({
    cardId: 'd5510cbe',
...
describe('P2-B measured packet sizes (estimates -- see the counting method at the top)', () => {
    it('reports the fresh-token estimate for the representative set', () => {
        ...
        for (const [name, p] of SET) {
            ...
            rows.push(`${name.padEnd(28)}  ${String(text.length).padStart(5)}   ${String(est.tokens).padStart(9)}  ` +
    ...
    it('reports the saving vs inlining the same material, which is the whole point', () => {
        ...
        // eslint-disable-next-line no-console
        console.log(`inline-vs-reference (same 4 artifacts):\n` +
