...
const isoAt = (offsetSec) => new Date((T0_SEC + offsetSec) * 1000).toISOString();
function assistantLine(offsetSec, inputTokens) {
...
function writeFixtureTranscript() {
    writeFileSync(join(FIXTURE_DIR, `${FIXTURE_SESSION}.jsonl`), [assistantLine(10, 1000), assistantLine(20, 1100)].join('\n'));
}
describe('P2-A: the correlation actually RUNS after a collection (runtime)', () => {
    ...
    it('collect -> the freshly-written token rows are attributed to the dispatch', async () => {
        const db = getDb();
        const dispatchId = createDispatch(db, { source: 'scheduler', agent: FIXTURE_AGENT, sessionId: FIXTURE_SESSION }, T0_SEC * 1000);
    ...
    it('rows outside any dispatch window stay unattributed (no invented bucket)', async () => {
        ...
        createDispatch(db, { source: 'scheduler', agent: FIXTURE_AGENT, sessionId: FIXTURE_SESSION }, (T0_SEC + 3600) * 1000);
        const result = await collectTokenUsage();
        expect(result.inserted).toBe(2);
    ...
    it('collecting twice does not double-attribute or re-link rows (idempotent)', async () => {
        ...
        const dispatches = db.prepare('SELECT COUNT(*) c FROM dispatches').get();
        expect(dispatches.c).toBe(1);
    });
    it('a REAL correlation fault does not break the collection (fault isolation)', async () => {
        const db = getDb();
        createDispatch(db, { source: 'scheduler', agent: FIXTURE_AGENT, sessionId: FIXTURE_SESSION }, T0_SEC * 1000);
        ...
        const stored = db.prepare('SELECT COUNT(*) c FROM token_usage').get();
