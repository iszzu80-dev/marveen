...
describe('kanban parent_id schema and subtask queries', () => {
    ...
    it('creates a card with parent_id', () => {
        ...
        const child = db.prepare('SELECT * FROM kanban_cards WHERE id = ?').get('CHILD001');
    ...
    it('queries children of a parent', () => {
        ...
        const children = db.prepare('SELECT * FROM kanban_cards WHERE parent_id = ? AND archived_at IS NULL ORDER BY sort_order ASC').all('PARENT01');
    ...
    it('excludes archived children', () => {
        ...
        const children = db.prepare('SELECT * FROM kanban_cards WHERE parent_id = ? AND archived_at IS NULL').all('PARENT01');
    ...
    it('card without parent_id has null', () => {
        ...
        const card = db.prepare('SELECT * FROM kanban_cards WHERE id = ?').get('SOLO0001');
    ...
    it('parent card lists no children when none exist', () => {
        ...
        const children = db.prepare('SELECT * FROM kanban_cards WHERE parent_id = ? AND archived_at IS NULL').all('NOCHILDS');
    ...
    it('transaction rolls back all inserts on failure', () => {
        ...
        const count = db.prepare('SELECT COUNT(*) as c FROM kanban_cards WHERE parent_id = ?').get('PARENT01').c;
        expect(count).toBe(0);
    });
});
describe('validateSubtasks (from llm-breakdown)', () => {
    ...
    it('nullifies unknown assignee (hallucination guard)', () => {
        const result = validateSubtasks([{ title: 'T', description: 'D', assignee: 'nonexistent-agent' }], validAssignees);
        expect(result[0].assignee).toBeNull();
    });
...
describe('generateBreakdown via worker (runAgent)', () => {
    ...
    it('passes the full system+card prompt to runAgent', async () => {
        ...
        const promptArg = mockedRunAgent.mock.calls[0][0];
