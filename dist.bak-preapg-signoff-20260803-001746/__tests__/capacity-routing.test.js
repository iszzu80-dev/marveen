...
import { deriveCapacityState, deriveCapacityStateFromBalance, isRoutable, classifyError, isFallbackEligible, isRoutingChangeAllowed, canAutoFallback, withinCandidateCeiling, shouldClimbBackToPrimary, resolveRuntimeRouting, capacityKeyId, MAX_FALLBACK_CANDIDATES, MAX_AUTO_FALLBACKS_PER_PACKAGE, } from '../capacity-routing.js';
...
describe('deriveCapacityStateFromBalance (card 6976aaa2: DeepSeek prepaid-balance capacity)', () => {
    const FLOOR = 1.0;
    function balanceInputs(overrides = {}) {
...
describe('resolveRuntimeRouting', () => {
    ...
    it('prefers a subscription-included candidate over a metered one (subscription-first)', () => {
        ...
        expect(d.action).toBe('fallback');
        if (d.action === 'fallback')
            expect(d.to).toBe(subscriptionIncluded);
    });
