...
import { classifyContent, evaluateDispatch, isProviderTrusted, parseTrustedProviders, normalizeConfig, } from '../data-sensitivity-gate.js';
// ---- test config -------------------------------------------------------------
const TEST_CONFIG = {
    mode: 'observe-only',
    enabled: true,
...
describe('classifyContent', () => {
    ...
    it('detects JWT token as restricted', () => {
        const result = classifyContent('Here is the token: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c', TEST_CONFIG);
        expect(result.category).toBe('restricted');
    ...
    it('restricted beats internal (higher severity)', () => {
        const result = classifyContent('Deploy to suite-postgres-08wb, admin@example.com is the contact', TEST_CONFIG);
        expect(result.category).toBe('restricted');
        expect(result.matchedPatterns).toContain('email');
    });
    it('multiple restricted patterns all reported', () => {
        const result = classifyContent('Email admin@example.com with token Bearer abcdefghijklmnopqrstuvwxyz123', TEST_CONFIG);
        expect(result.category).toBe('restricted');
        expect(result.matchedPatterns).toContain('email');
        expect(result.matchedPatterns).toContain('api_key');
    });
...
describe('evaluateDispatch', () => {
    it('allows public content on non-trusted provider', () => {
        const result = evaluateDispatch('Build a React component', 'deepseek-v4-pro', TEST_CONFIG, TRUSTED);
        expect(result.verdict).toBe('allow');
        expect(result.category).toBe('public');
    });
    it('allows restricted content on trusted provider (Claude)', () => {
        const result = evaluateDispatch('Email admin@example.com about the deploy', 'claude-opus-4-8[1m]', TEST_CONFIG, TRUSTED);
        expect(result.verdict).toBe('allow');
    });
    it('would_block restricted content on DeepSeek (observe-only)', () => {
        const result = evaluateDispatch('Email admin@example.com with the API key', 'deepseek-v4-pro', TEST_CONFIG, TRUSTED);
        expect(result.verdict).toBe('would_block');
        expect(result.category).toBe('restricted');
    });
    it('blocks restricted content on DeepSeek in enforce mode', () => {
        const enforceConfig = { ...TEST_CONFIG, mode: 'enforce' };
        const result = evaluateDispatch('Here is the JWT: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.abcdefghijklmnop', 'deepseek-v4-pro', enforceConfig, TRUSTED);
        expect(result.verdict).toBe('block');
    });
    it('allows everything when gate is disabled', () => {
        const offConfig = { ...TEST_CONFIG, enabled: false };
        const result = evaluateDispatch('Email admin@example.com with secret key', 'deepseek-v4-pro', offConfig, TRUSTED);
        expect(result.verdict).toBe('allow');
    });
    it('allows everything in off mode', () => {
        const offConfig = { ...TEST_CONFIG, mode: 'off' };
        const result = evaluateDispatch('Secret: postgresql://user:pass@host/db', 'deepseek-v4-pro', offConfig, TRUSTED);
        expect(result.verdict).toBe('allow');
    });
    it('allows internal content on trusted provider', () => {
        const result = evaluateDispatch('Backup suite-postgres-08wb before migration', 'claude-sonnet-4-6', TEST_CONFIG, TRUSTED);
        expect(result.verdict).toBe('allow');
    });
    it('would_block internal content on DeepSeek', () => {
        const result = evaluateDispatch('Backup suite-postgres-08wb before migration', 'deepseek-v4-flash', TEST_CONFIG, TRUSTED);
    ...
    it('unknown model on trusted list still passes', () => {
        ...
        const result = evaluateDispatch('Email admin@example.com', 'anthropic-claude-sonnet-4-6', TEST_CONFIG, multi);
        expect(result.verdict).toBe('allow');
    });
});
// ---- normalizeConfig ---------------------------------------------------------
...
describe('false-positive resistance', () => {
    it('code with hex hashes is not flagged', () => {
        const result = classifyContent('commit b3f4b3678fc62febf64a3894283cfa4620156e2c on branch main', TEST_CONFIG);
        expect(result.category).toBe('public');
    });
    it('normal task description is public', () => {
        const result = classifyContent('Build a kanban board with drag-and-drop and PostgreSQL persistence', TEST_CONFIG);
        expect(result.category).toBe('public');
    });
    it('inter-agent message without secrets is public', () => {
        const result = classifyContent('Checker exits 0 -- all tenant-scoped tables have ENABLE ROW LEVEL SECURITY.', TEST_CONFIG);
        expect(result.category).toBe('public');
    });
    it('10-digit port number without tax context is not flagged', () => {
        const result = classifyContent('The service listens on port 8080 and the PID is 1234567890', TEST_CONFIG);
        expect(result.matchedPatterns).not.toContain('tax_id_contextual');
    });
    it('ISO date ranges do NOT false-positive on credit card pattern', () => {
        ...
        const result = classifyContent('Period: 2024-01-01 to 2026-12-31', TEST_CONFIG);
        expect(result.category).toBe('public');
    });
});
