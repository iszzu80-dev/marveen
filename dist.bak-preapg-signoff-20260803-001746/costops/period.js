...
import { monthWindow, resolveOperational, PENDING_CONF } from './ledger.js';
function readMonth(db, win, providerBySource, now) {
  ...
  `).all({ start: win.start, end: win.end });
    ...
    const op = resolveOperational(rows.filter(l => !PENDING_CONF.has(l.confidence)).map(l => ({
 ...
 */
export function getPeriodTrend(db, _config, now, monthsBack = 6, monthKey) {
    const anchor = monthWindow(now, monthKey);
    const srcRows = db.prepare(`SELECT id, provider FROM cost_sources`).all();
