...
import { searchMemories, recentMemories, touchMemory, saveMemory, decayMemories as dbDecay, pruneAuditLogs, pruneTokenUsage, getMemoriesForChat, listKanbanCardsSummary, } from './db.js';
...
function ensureDigestConfigDir() {
    const candidates = [
...
const SEMANTIC_PATTERN = /\b(my|i am|i'm|i prefer|remember|always|never|az en|nekem|szeretem|nem szeretem|mindig|soha|emlekezzel|en|kedvenc|utokalok|fontos|ne felejtsd|jegyezd meg)\b/i;
...
export async function saveConversationTurn(chatId, userMsg, assistantMsg) {
    ...
    if (userMsg.length <= 20 || userMsg.startsWith('/'))
        return;
    if (SKIP_PATTERN.test(userMsg.trim()))
        return;
...
// --- Daily digest ---
export async function runDailyDigest(chatId) {
