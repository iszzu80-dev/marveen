...
vi.mock('../web/agent-config.js', async (orig) => {
    ...
    return {
        ...
        agentDir: (name) => join(AGENTS_TMP, name),
        agentConfigRoot: (name) => (name === MAIN_AGENT_ID ? tmpRoot : join(AGENTS_TMP, name)),
        listAgentNames: () => existsSync(AGENTS_TMP)
...
function fakeCtxWithBody(path, method, bodyObj) {
    const out = { status: 0, body: null };
    const res = {
        writeHead(status) { out.status = status; return res; },
        end(chunk) { if (chunk)
...
function seedSkill(dir, description) {
...
describe('GET /api/skills/local', () => {
    ...
    it('returns skills for the main agent (PROJECT_ROOT path)', async () => {
        ...
        const mainSkills = out.body.filter((s) => s.agentId === MAIN_AGENT_ID);
        expect(mainSkills.length).toBeGreaterThan(0);
        const seed = mainSkills.find((s) => s.name === 'zz-test-main-local-skill');
    ...
    it('returns skills for sub-agents', async () => {
        ...
        const subSkills = out.body.filter((s) => s.agentId === SUB_AGENT_ID);
    ...
    it('does not duplicate if MAIN_AGENT_ID somehow appears in listAgentNames', async () => {
        ...
        const { ctx, out } = fakeCtx('/api/skills/local');
        await tryHandleSkills(ctx);
        const keys = out.body.map((s) => `${s.agentId}::${s.name}`);
...
describe('GET /api/skills/:name?agent=<id> (agent-local detail)', () => {
    ...
    it('returns 200 with non-empty content for main-agent local skill', async () => {
        const { ctx, out } = fakeCtx(`/api/skills/zz-test-main-local-skill?agent=${encodeURIComponent(MAIN_AGENT_ID)}`);
        const handled = await tryHandleSkills(ctx);
        expect(handled).toBe(true);
        expect(out.status).toBe(200);
    ...
    it('returns 200 with non-empty content for sub-agent local skill', async () => {
        const { ctx, out } = fakeCtx(`/api/skills/zz-test-sub-local-skill?agent=${encodeURIComponent(SUB_AGENT_ID)}`);
        await tryHandleSkills(ctx);
        expect(out.status).toBe(200);
        expect(out.body).toHaveProperty('source', 'agent');
    ...
    it('returns 404 for non-existent agent-local skill', async () => {
        const { ctx, out } = fakeCtx(`/api/skills/does-not-exist?agent=${encodeURIComponent(MAIN_AGENT_ID)}`);
        await tryHandleSkills(ctx);
        expect(out.status).toBe(404);
    });
    it('rejects path traversal in skill name (agent param)', async () => {
        const { ctx, out } = fakeCtx(`/api/skills/${encodeURIComponent('../../../etc/passwd')}?agent=${encodeURIComponent(MAIN_AGENT_ID)}`);
        await tryHandleSkills(ctx);
        expect(out.status).toBe(404);
    });
    it('rejects traversal-looking agent param with 404 not 500', async () => {
        ...
        const { ctx, out } = fakeCtx(`/api/skills/some-skill?agent=${encodeURIComponent('../../etc')}`);
        await tryHandleSkills(ctx);
        expect(out.status).toBe(404);
    });
});
describe('PUT /api/skills/:name?agent=<id> (agent-local edit)', () => {
    ...
    it('writes updated content to main-agent local skill SKILL.md', async () => {
        const newContent = '---\nname: test\ndescription: updated\n---\n# Updated\n';
        const { ctx, out } = fakeCtxWithBody(`/api/skills/zz-test-main-local-skill?agent=${encodeURIComponent(MAIN_AGENT_ID)}`, 'PUT', { content: newContent });
        const handled = await tryHandleSkills(ctx);
        expect(handled).toBe(true);
        expect(out.status).toBe(200);
    ...
    it('writes updated content to sub-agent local skill SKILL.md', async () => {
        ...
        const { ctx, out } = fakeCtxWithBody(`/api/skills/zz-test-sub-local-skill?agent=${encodeURIComponent(SUB_AGENT_ID)}`, 'PUT', { content: newContent });
        await tryHandleSkills(ctx);
        expect(out.status).toBe(200);
    ...
    it('returns 404 for PUT on non-existent agent-local skill', async () => {
        const { ctx, out } = fakeCtxWithBody(`/api/skills/no-such-skill?agent=${encodeURIComponent(MAIN_AGENT_ID)}`, 'PUT', { content: '# x' });
        await tryHandleSkills(ctx);
        expect(out.status).toBe(404);
    });
    it('rejects path traversal in PUT skill name', async () => {
        const { ctx, out } = fakeCtxWithBody(`/api/skills/${encodeURIComponent('../../../etc/passwd')}?agent=${encodeURIComponent(MAIN_AGENT_ID)}`, 'PUT', { content: 'evil' });
        await tryHandleSkills(ctx);
        expect(out.status).toBe(400);
    });
    it('rejects traversal-looking agent param in PUT with 404 not 500', async () => {
        const { ctx, out } = fakeCtxWithBody(`/api/skills/some-skill?agent=${encodeURIComponent('../../etc')}`, 'PUT', { content: 'evil' });
        await tryHandleSkills(ctx);
        expect(out.status).toBe(404);
    });
});
