...
import { shQuote, buildTmuxInvocation, buildSshExec, sessionInList, SSH_OPTS, controlDir, } from '../web/ssh-tmux.js';
describe('shQuote', () => {
...
describe('buildTmuxInvocation', () => {
    ...
    it('host set wraps every tmux arg in one shell-quoted ssh command string', () => {
        ...
        expect(inv.args[SSH_OPTS.length + 1]).toBe("tmux 'send-keys' '-t' 'agent-x' '-l' 'a'\\''b c'");
    });
