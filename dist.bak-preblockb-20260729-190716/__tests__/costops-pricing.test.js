...
import { deriveProvider, validatePricing, getTokenCostEstimate } from '../costops/pricing.js';
...
let tsCounter = 0;
function insertUsage(model, input, output, cacheRead = 0, cacheCreate = 0, session = 's1') {
    ...
    VALUES ('marveen', ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(session, ts, input, output, cacheRead, cacheCreate, model, model ? deriveProvider(model) : null, model ? 'transcript' : null);
}
