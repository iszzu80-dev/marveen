...
import { pollPeerManifests, getFederationStatus, refreshFederationStatus, resetFederationPollerCache, sanitizeManifest, MANIFEST_MAX_BODY_BYTES, MANIFEST_MAX_AGENTS, } from '../web/federation/poller.js';
...
describe('sanitizeManifest (pure)', () => {
    it('accepts a good manifest and enforces the system-id match', () => {
        ...
        if (typeof m !== 'string')
            expect(m.agents[0].id).toBe('teodor');
...
describe('pollPeerManifests state machine', () => {
    ...
    it('unpaired peer (empty outboundToken) is reported without a network attempt', async () => {
        ...
        let calls = 0;
        const counting = (async () => { calls++; return new Response('{}', { status: 200 }); });
