...
describe('decideStuckInputAction (recovery-decision unit)', () => {
    ...
    it('multi-row sub-agent plain text -> re-inject plain, never enter', () => {
        const a = decideStuckInputAction(facts({ rowCount: 2, allowPlainReinject: true, hasPlainText: true }));
    ...
    it('sub-agent plain re-inject keeps precedence over clear-scheduled (existing path preserved)', () => {
        const a = decideStuckInputAction(facts({ rowCount: 3, scheduledTaskBlock: true, allowPlainReinject: true, hasPlainText: true }));
        expect(a).toBe('reinject-plain');
    });
});
