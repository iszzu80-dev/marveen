...
function emptyConfig() { return { version: 1, currency: 'HUF', fixed_costs: [], budgets: [{ id: 'global-monthly', amount: 200000, warning_threshold: 0.8, hard_threshold: 1.0 }] }; }
function seedSource(db, id, provider) {
...
function seedLine(db, sid, amount, conf, cat = 'subscription', win = monthWindow(NOW), dk) {
...
describe('getCostSummary v0.4 operational fields', () => {
    ...
    it('operational_spend prefers provider; current_spend legacy unaffected; render uses plan', () => {
        const db = getDb();
        seedSource(db, 'render-hosting', 'render');
        seedLine(db, 'render-hosting', 3000, 'manual');
        seedSource(db, 'render-plan', 'render');
        seedLine(db, 'render-plan', 2500, 'provider_plan_estimate', 'usage');
        seedSource(db, 'sub-a', 'acme');
        seedLine(db, 'sub-a', 5000, 'manual');
