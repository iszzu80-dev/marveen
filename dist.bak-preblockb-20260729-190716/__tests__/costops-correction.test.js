...
function insertLine(db, sourceId, amount, confidence = 'manual', actualSource = 'manual_entry') {
...
describe('createCorrection (CostOps Phase 1, GAP-05/06/14 -- correction relationship)', () => {
    ...
    it('voids the original and inserts a linked correction row with the new amount', () => {
        ...
        const original = db.prepare(`SELECT voided_at, void_reason, billed_cost, corrects_line_id FROM cost_line_items WHERE id = ?`).get(originalId);
        ...
        const corrected = db.prepare(`SELECT billed_cost, corrects_line_id, voided_at FROM cost_line_items WHERE id = ?`).get(r.newLineId);
    ...
    it('preserves source_id/period/currency/confidence/actual_source from the original', () => {
        ...
        const corrected = db.prepare(`SELECT source_id, currency, confidence, actual_source FROM cost_line_items WHERE id = ?`).get(r.newLineId);
