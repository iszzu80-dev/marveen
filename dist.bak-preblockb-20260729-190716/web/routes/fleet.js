...
import { exportFleet, importFleet, MIN_VAULT_PASSWORD_LEN, UserFacingError } from '../fleet-transfer.js';
export async function tryHandleFleet(ctx) {
    const { req, res, path, method } = ctx;
    if (path !== '/api/fleet/export' && path !== '/api/fleet/import')
        return false;
    // H1: vault password via header, not query string (avoids access-log / proxy-log / browser-history leakage)
    const vaultPassword = req.headers['x-vault-password'];
    if (path === '/api/fleet/export' && method === 'GET') {
        ...
        try {
            const exported = exportFleet({ vaultPassword: vaultPassword || undefined });
