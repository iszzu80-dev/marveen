...
export const TRANSCRIPT_DIR = join(process.env.HOME ?? homedir(), '.claude', 'projects', PROJECT_ROOT.replace(/\//g, '-'));
...
let _cachedProbeIntervalMs = null;
let _cachedAllowedChatId = undefined; // undefined = not yet read
...
export function shouldTriggerDeafnessRespawn(opts) {
    ...
    if (nowMs - markerTs < probeTimeoutMs)
        return false;
 ...
 */
export function readLastIngestionTimestamp(transcriptDir) {
    try {
        ...
        for (const line of lines) {
            if (!line.includes('<channel source='))
                continue;
            try {
...
function readProbeIntervalMs() {
    if (_cachedProbeIntervalMs !== null)
...
function readAllowedChatId() {
    if (_cachedAllowedChatId !== undefined)
...
function spawnProber() {
    ...
    try {
        ...
        proberProcess.stdout?.on('data', (data) => {
            // Do not log stdout content as it may carry debug info with timing data.
            const text = data.toString('utf-8').trim();
            if (text)
                logger.debug({ prober: 'stdout' }, text);
        });
        proberProcess.stderr?.on('data', (data) => {
            const text = data.toString('utf-8').trim();
            if (text)
                logger.warn({ prober: 'stderr' }, text);
        });
...
function checkInboundProbeDeafness(probeTimeoutMs) {
 ...
 */
export function startInboundProber() {
