...
const STATE_PATH = process.env.MARVEEN_MEM_PRESSURE_TEST_STATE;
function writeFixture(state, ageSeconds) {
    const defaultSample = {
    ...
    const fixture = {
        ...
        lastSample: state.lastSample
            ? { ...defaultSample, timestamp: sampleTs, ...state.lastSample }
...
async function run() {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    {
        ...
        ok("T1: failure mode is MONITOR_EXECUTION_FAILED", health.failureMode === "MONITOR_EXECUTION_FAILED", health.failureMode);
        ok("T1: details mention 'failed'", health.details.toLowerCase().includes("failed"), health.details);
        // Verify the state file mtime is recent (timer IS firing — just failing)
        ok("T1: state file is fresh (mtime within limit)", health.stateFileMtimeMs !== null && (NOW - health.stateFileMtimeMs) < MAX_AGE_MS, `stateFileMtimeMs=${health.stateFileMtimeMs}`);
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    {
        ...
        ok("T2: failure mode is MONITOR_STATE_STALE", health.failureMode === "MONITOR_STATE_STALE", health.failureMode);
        ok("T2: details mention age exceeds", health.details.includes("age") && health.details.includes("exceeds"), health.details);
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    {
        ...
        ok("T3: failure mode is MONITOR_EXECUTION_FAILED (not stale — timer fires, service crashes)", health.failureMode === "MONITOR_EXECUTION_FAILED", health.failureMode);
        ok("T3: lastSuccessAgeSeconds reflects 300s gap", health.lastSuccessAgeSeconds !== null && health.lastSuccessAgeSeconds >= 290, `lastSuccessAgeSeconds=${health.lastSuccessAgeSeconds}`);
            ...
            if (releaseExists) {
                ...
                if (existsSync(srcMonitor)) {
                    try {
                        ...
                        ok("T4: after source removal, release copy still intact (branch-switch-proof)", stillExists, `release=${monitorJs}`);
                        // The source is gone, proving the release is independent
                        const srcGone = !existsSync(srcMonitor);
                        ok("T4: source file successfully removed (simulated branch switch)", srcGone, srcMonitor);
                        // Restore the source
                        ...
                        // Restore on failure
                        try {
                            execSync(`mv "${srcBackup}" "${srcMonitor}" 2>/dev/null`, { timeout: 5000 });
                        }
                        catch { /* ok */ }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    {
        ...
        if (existsSync(installScript)) {
            try {
                ...
                if (existsSync(PREVIOUS_LINK)) {
                    ...
                    if (prevExists) {
                        ...
                        try {
                            ...
                            ok("T5: after rollback, current symlink points to previous release", newCurrent === prevTarget || newCurrent.includes(prevTarget), `current=${newCurrent} prevTarget=${prevTarget}`);
                ...
                ok("T5: install script is executable and parseable (mechanism exists)", existsSync(installScript), installScript);
            }
        }
        else {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    {
        ...
        if (existsSync(CURRENT_LINK)) {
            ...
            for (const [label, bundlePath] of [["monitor.js", monitorJs], ["gate.js", gateJs]]) {
                ...
                ok(`T8: ${label} does NOT reference INSTALL_DIR for scripts (release-local deps only)`, clean, scriptsRef ? `FOUND: ${scriptsRef.join("; ")}` : "clean");
            }
        }
        else {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    {
        ...
        ok("T6a: reason mentions MONITOR_EXECUTION_FAILED", rA.reason.includes("MONITOR_EXECUTION_FAILED"), rA.reason);
    ...
    }
    {
        ...
        ok("T6b: failure mode is MONITOR_STATE_STALE", healthB.failureMode === "MONITOR_STATE_STALE", healthB.failureMode);
    }
    {
        ...
        // the guard silently protects using stale logic.
        {
            ...
            ok("T7: failure mode is MONITOR_RELEASE_MISMATCH", hM.failureMode === "MONITOR_RELEASE_MISMATCH", hM.failureMode);
            ok("T7: details name BOTH releases (auditable)", !!hM.details && hM.details.includes("monitor-98222ef28") && hM.details.includes("monitor-99999999"), hM.details);
