...
// content itself.
const SUB_TELEGRAM_WAKE_NUDGE = '[telegram-wake] Bejövő Telegram üzenet(ek) várnak; a drain hook behúzta őket a kontextusba fentebb. Dolgozd fel és válaszolj.';
const _subWakeState = new Map();
/**
 ...
 */
export function wakeBackoffMs(attempts, baseMs, maxMs) {
...
export function shouldWakeForTelegramInbox(params) {
    ...
    if (!hasPending)
        return false;
    if (inboxAgeMs <= minAgeMs)
        return false;
    if (attempts >= maxAttempts)
        return false; // budget exhausted for this backlog
    if (now - lastWakeAt < wakeBackoffMs(attempts, debounceMs, maxDebounceMs))
        return false;
    if (!sessionExists)
        return false;
    if (!sessionIdle)
...
export async function maybeWakeSubAgentsForTelegram(now) {
    ...
    if (!SUBAGENT_TELEGRAM_WAKE_ENABLED)
        return;
    let names;
    try {
    ...
    for (const name of names) {
        ...
        try {
            ...
            if (inboxAgeMs <= SUB_TELEGRAM_WAKE_MIN_AGE_MS)
                continue;
...
export function _resetSubWakeStateForTest() {
    _subWakeState.clear();
}
