...
export function credentialsGuardEnabled() {
...
export function looksLikeSetupToken(token) {
    return OAUTH_TOKEN_RX.test(token);
}
function tokenHash(token) {
    return createHash('sha256').update(token).digest('hex');
}
function readVerifiedHash() {
    try {
        return readFileSync(VERIFIED_STAMP, 'utf-8').trim() || null;
    }
    catch {
        return null;
    }
 ...
 */
export function liveTestToken(token, claudeBin) {
    let dir = null;
    try {
        dir = mkdtempSync(join(tmpdir(), 'cred-guard-'));
        const out = execFileSync(claudeBin, ['-p', 'Reply with exactly: OK', '--model', LIVE_TEST_MODEL], {
            encoding: 'utf-8',
            timeout: 60_000,
 ...
 */
function tokenIsValid(claudeBin) {
    const token = readFleetToken();
    if (!token) {
        logger.warn('credentials-guard: no fleet setup-token (store/.claude-oauth-token); leaving credentials.json untouched');
        return false;
    }
    if (!looksLikeSetupToken(token)) {
        logger.warn('credentials-guard: fleet token is not a well-formed setup-token; leaving credentials.json untouched');
        return false;
    }
    const h = tokenHash(token);
    if (readVerifiedHash() === h)
        return true;
    logger.info('credentials-guard: live-testing the fleet setup-token (once per token value)');
    if (!liveTestToken(token, claudeBin)) {
        logger.warn('credentials-guard: fleet token failed the live test; leaving credentials.json untouched');
        return false;
    }
    try {
        writeFileSync(VERIFIED_STAMP, h + '\n', { mode: 0o600 });
    }
    catch { /* stamp is an optimisation; a write failure only costs a re-test */ }
    return true;
}
const AUTH_FAILURE_RX = /\b401\b|Invalid bearer token|Invalid API key|authentication_error|Invalid authentication|Failed to authenticate|OAuth (?:token|authentication) (?:has )?expired/i;
/** Pure classifier for a probe run. Exported for tests. */
export function classifyAuthProbe(input) {
    if (!input.ran)
        return 'inconclusive';
    if (!input.exitedNonZero && /\bOK\b/.test(input.output))
        return 'ok';
    if (AUTH_FAILURE_RX.test(input.output))
 ...
 */
export async function liveProbeAuth(envOverride, claudeBin) {
    let dir = null;
    try {
        ...
        const run = await new Promise((resolve) => {
            execFile(bin, ['-p', 'Reply with exactly: OK', '--model', LIVE_TEST_MODEL], { timeout: 60_000, encoding: 'utf-8', env: env }, (err, stdout, stderr) => {
                if (err && err.code === 'ENOENT') {
                    resolve({ ran: false, exitedNonZero: true, output: '' });
                    return;
                }
...
export function stampTokenVerified(token) {
    try {
        writeFileSync(VERIFIED_STAMP, tokenHash(token) + '\n', { mode: 0o600 });
    }
...
export function isPromotableSetupCredential(cred, nowMs) {
    if (!looksLikeSetupToken((cred.accessToken ?? '').trim()))
        return false;
    if (typeof cred.expiresAt !== 'number')
        return false;
    return cred.expiresAt - nowMs >= MIN_PROMOTABLE_LIFETIME_MS;
}
export async function syncFleetTokenFromSharedCredentials(claudeBin) {
    try {
        if (readFleetToken())
            return 'fleet-token-present';
        let cred = {};
        try {
            const parsed = JSON.parse(readFileSync(HOME_CREDENTIALS, 'utf-8'));
            cred = parsed?.claudeAiOauth ?? {};
        }
        catch {
            return 'no-credentials';
        }
        if (!isPromotableSetupCredential(cred, Date.now()))
            return 'not-setup-token';
...
export function quarantineFleetToken(reasonLabel) {
    try {
        if (!existsSync(FLEET_OAUTH_TOKEN_PATH))
            return false;
        renameSync(FLEET_OAUTH_TOKEN_PATH, badFleetTokenPath());
        try {
            rmSync(VERIFIED_STAMP, { force: true });
        }
        catch { /* best effort */ }
        logger.error({ to: badFleetTokenPath(), reason: reasonLabel }, 'credentials-guard: QUARANTINED the fleet token (dead on live probe); newly launched agents fall back to shared-file auth. Restore with mv if this was a false positive.');
        return true;
    }
    catch (err) {
 ...
 */
export async function quarantineFleetTokenIfDead(claudeBin) {
    const token = readFleetToken();
    if (!token)
        return 'no-token';
    ...
    if (probe === 'auth-rejected') {
        quarantineFleetToken('reauth-healer: agent 401 + fleet token failed live probe');
        return 'quarantined';
    }
    return 'inconclusive';
}
export async function fleetTokenBootPass(claudeBin) {
    const token = readFleetToken();
    if (!token)
        return syncFleetTokenFromSharedCredentials(claudeBin);
    if (!looksLikeSetupToken(token)) {
    ...
    if (readVerifiedHash() === tokenHash(token))
    ...
    if (probe === 'auth-rejected') {
        quarantineFleetToken('boot validation: live probe auth-rejected');
        return 'quarantined';
    }
    return 'validate-inconclusive';
}
export function renameSharedCredentialsIfSafe(claudeBin) {
    try {
        if (!credentialsGuardEnabled())
            return 'disabled';
        if (PLATFORM === 'macos')
            return 'not-linux';
