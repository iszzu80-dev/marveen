...
function computeNextState(current, sample, since, thresholds) {
    const av = sample.memAvailableGiB;
    if (av < thresholds.emergencyMemAvailableGiB)
        return "emergency";
    if (av < thresholds.criticalMemAvailableGiB)
        return "critical";
    if (av < thresholds.warningMemAvailableGiB)
        return "warning";
    // Recovery → normal after sustained stability
    if (current === "recovery") {
        const sinceMs = new Date(since).getTime();
        const stableMs = thresholds.recoveryMinutes * 60 * 1000;
        if (Date.now() - sinceMs >= stableMs)
            return "normal";
        return "recovery";
    }
...
async function run() {
    ...
    ok("warning → recovery after 5+ min stable normal", computeNextState("warning", sample(3.0), new Date(Date.now() - 6 * 60 * 1000).toISOString(), T) === "recovery");
    ok("critical → recovery after 5+ min stable normal", computeNextState("critical", sample(4.0), new Date(Date.now() - 10 * 60 * 1000).toISOString(), T) === "recovery");
    ok("emergency → recovery after 5+ min stable normal", computeNextState("emergency", sample(5.0), new Date(Date.now() - 7 * 60 * 1000).toISOString(), T) === "recovery");
    // ── recovery → normal after staying normal ──────────────────────────────
    ok("recovery stays recovery at 3GiB, only 1min stable (not yet)", computeNextState("recovery", sample(3.0), new Date(Date.now() - 1 * 60 * 1000).toISOString(), T) === "recovery");
    ok("recovery → normal after 5+ min stable normal (incident fully resolved)", computeNextState("recovery", sample(4.0), new Date(Date.now() - 6 * 60 * 1000).toISOString(), T) === "normal");
    ok("recovery → normal after 10min stable normal", computeNextState("recovery", sample(5.0), new Date(Date.now() - 10 * 60 * 1000).toISOString(), T) === "normal");
