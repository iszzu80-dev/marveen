...
function upsertProviderLines(db, lines, now) {
    ...
    if (t === 'object') {
        ...
        if (depth < 6)
            for (const k of Object.keys(value)) {
                keys[k] = describeShape(value[k], depth + 1);
            }
 ...
 */
export async function dryRunCollector(args) {
...
export async function runCollector(args) {
  ...
  `);
    ...
    if (!lockResult.ok) {
        insertRun.run({
            provider: collector.provider, collector: collector.collectorName,
            started: now, finished: now, status: 'locked',
