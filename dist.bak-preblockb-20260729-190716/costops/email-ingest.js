 ...
 */
export function toHuf(amount, currency, fxUsdHuf, fxEurHuf = 0) {
    const cur = (currency || 'HUF').toUpperCase();
    if (cur === 'HUF')
        return Math.round(amount * 100) / 100;
    if (cur === 'USD')
        return Math.round(amount * fxUsdHuf * 100) / 100;
    ...
    if (cur === 'EUR' && fxEurHuf > 0)
        return Math.round(amount * fxEurHuf * 100) / 100;
...
export function fxRateFor(currency, fxUsdHuf, fxEurHuf) {
    const cur = (currency || 'HUF').toUpperCase();
    if (cur === 'USD')
        return fxUsdHuf;
    if (cur === 'EUR' && fxEurHuf > 0)
 ...
 */
export function ingestEmailCosts(db, entries, opts) {
  ...
  `);
    ...
    const tx = db.transaction((list) => {
        for (const e of list) {
            if (!e || typeof e.source_id !== 'string' || !e.source_id) {
                out.errors.push({ source_id: String(e?.source_id), reason: 'missing source_id' });
                continue;
            }
            const win = monthWindow(e.month);
            if (!win) {
                out.errors.push({ source_id: e.source_id, reason: `bad month '${e.month}'` });
                continue;
            }
            ...
            if (!writable.writable) {
                out.errors.push({ source_id: e.source_id, reason: writable.reason });
                continue;
            }
            const amountHuf = toHuf(Number(e.amount), e.currency, opts.fxUsdHuf, fxEurHuf);
            if (amountHuf === null || !isFinite(amountHuf)) {
                out.errors.push({ source_id: e.source_id, reason: `uncconvertible currency '${e.currency}'` });
                continue;
            }
