...
import { deriveMtdSpend } from './collectors/deepseek.js';
 ...
 */
export function forecastFixedSubscription(periodAmount, dataConfidence = 'manual') {
    const confidence = dataConfidence === 'actual_invoice' || dataConfidence === 'provider_api' || dataConfidence === 'billing_export'
        ? 'high' : 'medium';
    return {
 ...
 */
export function forecastTimeProportionalUsage(mtdAmount, win) {
    const projected = mtdAmount / win.fractionElapsed;
    const confidence = win.fractionElapsed < EARLY_MONTH_FRACTION ? 'low'
 ...
 */
export function forecastBalanceDelta(snapshotsAsc, win, now) {
    ...
    const confidence = spanSeconds < 3 * 86400 ? 'low'
 ...
 */
export function forecastInvoiceCadence(lastInvoiceAmount, cadence, opts = {}) {
 ...
 */
export function forecastOneTime(occurredAmount, plannedAmount = null) {
...
export function forecastManual(manualEstimate) {
 ...
 */
export function resolveSourceForecast(ctx) {
    if (ctx.manual_override != null)
        return forecastManual(ctx.manual_override);
    if (ctx.balance_snapshots && ctx.balance_snapshots.length > 0)
        return forecastBalanceDelta(ctx.balance_snapshots, ctx.win, ctx.now);
    ...
    if (ctx.charge_category === 'purchase')
        return forecastOneTime(ctx.occurred_amount ?? null, ctx.planned_amount ?? null);
    if (ctx.charge_category === 'usage')
        return forecastTimeProportionalUsage(ctx.mtd_amount, ctx.win);
...
export function computeForecastError(forecastAmount, actualAmount) {
    return {
 ...
 */
export function summarizeForecastAccuracy(records, keyOf) {
    ...
    for (const [key, rs] of groups) {
        ...
        const pcts = errors.map(e => e.percent_error).filter((p) => p !== null);
 ...
 */
export function forecastSnapshotDedupKey(sourceId, month, snapshotAt) {
 ...
 * the actual.
 */
export function initForecastSchema(db) {
    db.exec(`
