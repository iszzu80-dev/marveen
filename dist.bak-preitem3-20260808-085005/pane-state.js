...
function pastePlaceholderRegion(pane) {
...
export function detectsPastePlaceholder(pane) {
    if (!pane)
        return false;
...
export function stripGhostSuggestion(coloredPane) {
    let out = '';
    let dim = false;
    let i = 0;
    ...
    while (i < n) {
        ...
        if (ch === '\x1b') {
            ...
            let j = i + 2;
            while (j < n && (coloredPane[j] < '@' || coloredPane[j] > '~'))
                j++;
...
export function stripAllAnsi(s) {
    let out = '';
    let i = 0;
    const n = s.length;
    while (i < n) {
...
function lineHasBackgroundFill(line) {
    ...
    for (const seq of seqs) {
        ...
        while (k < params.length) {
            const c = params[k];
            if (c === 48)
                return true; // any background colour set (256/truecolor)
            if (c === 38) {
            ...
            if (c === 7)
                return true; // reverse video
            if ((c >= 40 && c <= 47) || (c >= 100 && c <= 107))
                return true; // basic / bright bg
            k++;
        }
    }
    return false;
...
export function stripSessionTitleBanner(coloredPane) {
    const lines = coloredPane.split('\n');
    let end = lines.length;
    while (end > 0 && lineHasBackgroundFill(lines[end - 1]))
        end--;
    if (end === lines.length)
        return coloredPane;
 ...
 */
export function detectsThinkingBlockError(pane) {
    if (!pane)
        return false;
    const lines = pane.split('\n');
    ...
    for (let i = 0; i < tail.length; i++) {
        if (!ERROR_CHROME_RX.test(tail[i]))
            continue;
 ...
 */
export function detectsBlockingMenu(pane) {
    if (!pane || !pane.trim())
        return false;
    ...
    if (BUSY_ESC_TO_INTERRUPT_RX.test(footerRegion))
        return false;
 ...
 */
export function detectsFirstRunGate(pane) {
    if (!pane || !pane.trim())
        return null;
    const lines = pane.split('\n');
    const busyRegion = lines.slice(-BUSY_LIVE_REGION_LINES).join('\n');
    for (const rx of BUSY_INDICATORS) {
        if (rx.test(busyRegion))
            return null;
    }
    ...
    for (const g of FIRST_RUN_GATES) {
        ...
        if (g.kind === 'welcome' && pane.includes('❯'))
            continue;
        return g.kind;
    }
    return null;
...
export function detectsModelConsentDialog(pane) {
    if (!pane || !pane.trim())
        return false;
    const lines = pane.split('\n');
    const busyRegion = lines.slice(-BUSY_LIVE_REGION_LINES).join('\n');
...
export function detectPermissionMode(pane) {
    if (!pane || !pane.trim())
        return null;
    const m = pane.match(/((?:[A-Za-z][\w-]* ){1,3})on(?: \(shift\+tab to cycle\)| · [^\n]*?(?:ctrl\+t|↓ to manage|← for agents))/);
    if (m)
        return m[1].trim();
    return /\? for shortcuts/.test(pane) ? 'default' : null;
}
export function detectPaneState(pane, opts = {}) {
    if (!pane || !pane.trim())
        return 'unknown';
    ...
    if (detectsThinkingBlockError(pane))
        return 'error';
 ...
 */
export function paneLooksIdle(capture) {
 ...
 */
export function isReadyForPrompt(pane) {
 ...
 */
export function idleConsideringDimGhost(plain, dimStripped) {
    if (paneLooksIdle(plain))
        return true;
    if (detectPaneState(plain) !== 'typing')
        return false;
...
function liveInputBoxFooterless(lines) {
    const seps = [];
    ...
    if (firstNonEmpty == null || !/^\s*\u276F/.test(firstNonEmpty))
        return null;
    return inner.join('\n');
}
function liveInputBox(pane) {
    const lines = pane.split('\n');
    const footerIdx = lines.findIndex(l => IDLE_FOOTER_RX.test(l));
    if (footerIdx < 0)
        return liveInputBoxFooterless(lines);
    let bottomSep = -1;
    for (let i = footerIdx - 1; i >= 0; i--) {
        if (BOX_SEP_RX.test(lines[i])) {
            bottomSep = i;
 ...
 */
export function shouldRetrySubmit(pane, payloadHint, opts = {}) {
    if (!pane || !pane.trim())
        return false;
    ...
    if (BUSY_ESC_TO_INTERRUPT_RX.test(retryFooterRegion))
        return false;
 ...
 * opening tag.
 */
export function shouldClearTruncatedPreamble(pane) {
    if (!pane)
        return false;
    const inputBox = liveInputBox(pane);
    if (inputBox == null)
        return false;
    const hasPreamble = TRUSTED_PREAMBLE_MARKER.test(inputBox) ||
        UNTRUSTED_PREAMBLE_MARKER.test(inputBox);
    if (!hasPreamble)
        return false;
    // A real opening tag means the wrapped content landed -- not stuck.
    if (REAL_OPENING_TAG_RX.test(inputBox))
        return false;
    return true;
}
/**
 ...
 *                     stuck.
 */
export function decideSubmitFollowup(pane, payloadHint, attempt, maxAttempts) {
    if (pane == null)
        return 'give-up';
    if (!shouldRetrySubmit(pane, payloadHint))
        return 'done';
 ...
 */
export function decidePaneErrorAlert(isError, prev, now, thresholds) {
    if (!isError) {
...
export function stuckInputSignature(pane) {
    if (detectPaneState(pane) !== 'typing')
        return null;
    const box = liveInputBox(pane);
    if (box == null)
        return null;
...
export function parkedPasteSignature(pane) {
    if (!pane || !pane.trim())
        return null;
    const lines = pane.split('\n');
    const busyRegion = lines.slice(-BUSY_LIVE_REGION_LINES).join('\n');
...
export function parkedChannelInput(pane) {
    if (detectPaneState(pane) !== 'typing')
        return null;
    const box = liveInputBox(pane);
    if (box == null)
    ...
    if (!/<channel\s+source="plugin:/.test(flat))
        return null; // human draft -> not ours
    const m = flat.match(/<channel\s+source="plugin:[^"]*"[^>]*>.*?<\/channel>/);
    if (!m)
        return { complete: false, block: null, chatId: null }; // opening tag only -> truncated
    const block = m[0];
    ...
    if (!cm || /\s/.test(cm[1]))
        return { complete: false, block, chatId: null };
...
export function parkedInputText(pane) {
    if (detectPaneState(pane) !== 'typing')
        return null;
    const box = liveInputBox(pane);
    if (box == null)
...
export function parkedMachineOriginInput(pane) {
...
export function parkedScheduledTaskInput(pane) {
    const flat = parkedInputText(pane);
    if (flat == null)
        return false;
    if (/^SCHEDULED TASK NOTICE/.test(flat) || /^<scheduled-task[\s>]/.test(flat))
        return true;
...
export function parkedClearSequence(rowCount) {
    const rounds = Math.min(PARKED_CLEAR_ROUNDS_MAX, Math.max(PARKED_CLEAR_ROUNDS_MIN, Math.max(0, rowCount) * PARKED_CLEAR_ROUNDS_PER_ROW));
    // Home first: the first kill must start at the beginning even when the cursor
    // was left mid-buffer by an earlier attempt.
    const keys = ['C-a'];
    for (let i = 0; i < rounds; i++)
        keys.push('C-k', 'Delete');
    return keys;
}
...
export function parkedInputRowCount(pane) {
    const box = liveInputBox(pane);
    if (box == null)
        return 0;
    return box
...
export function submitLanded(prevSig, paneAfter) {
    if (paneAfter == null)
        return false;
    return stuckInputSignature(paneAfter) !== prevSig;
}
const NO_STUCK_INPUT = {
    parkedSig: null,
    firstSeenAt: null,
    lastRecoverAt: null,
    attempts: 0,
 ...
 */
export function decideStuckInputRecovery(parkedSig, prev, now, thresholds) {
 ...
 */
export function decideStuckInputAction(f) {
    ...
    if (f.truncatedPreamble && f.escalate)
        return 'clear-preamble';
...
export function parkedMainInputHasRemedy(pane) {
...
export function stuckToolCallSignature(pane) {
 ...
 */
export function decideStuckToolCallRecovery(sig, prev, now, thresholds) {
...
export function paneShowsContextSaturation(capture) {
