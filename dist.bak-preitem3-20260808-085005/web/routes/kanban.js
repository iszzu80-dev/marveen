...
import { listKanbanCards, createKanbanCard, updateKanbanCard, deleteKanbanCard, moveKanbanCard, archiveKanbanCard, unarchiveKanbanCard, getKanbanComments, addKanbanComment, getKanbanCardEvents, listKanbanProjects, getKanbanCard, getChildCards, getDb, createAgentMessage, markKanbanCardDispatched, getKanbanSeqByIdPrefix, listLabels, getLabel, createLabel, updateLabel, deleteLabel, addLabelToCard, removeLabelFromCard, getLabelsForAllCards, getLabelsForCard, listArchivedKanbanCards, revertIdeaFromKanban, getHeartbeatKanbanSummary, } from '../../db.js';
...
export function kanbanMoveInstructions(id, target) {
...
function fireKanbanDispatch(id) {
    try {
        const card = getKanbanCard(id);
        if (!card || card.dispatched_at)
            return;
        ...
        if (!admission.admit) {
            ...
            logger.warn({ id, target, state: admission.state, pct: admission.pct, taskSize: admission.taskSize }, 'Kanban dispatch refused by P2-B admission gate (large task into saturated session)');
            try {
                addKanbanComment(id, MAIN_AGENT_ID, `[CONTEXT-GUARD/P2-B] Dispatch to ${target} deferred: ${admission.reason}. ` +
...
export async function tryHandleKanban(ctx) {
    ...
    if (path === '/api/kanban/labels' && method === 'POST') {
        ...
        const { name, color } = JSON.parse(body.toString());
        if (!name || !name.trim()) {
            json(res, { error: 'Címke neve kötelező' }, 400);
            return true;
        }
    ...
    if (cardLabelsMatch && method === 'POST') {
        ...
        if (!labelId) {
            json(res, { error: 'labelId mező kötelező' }, 400);
            return true;
        }
        if (!getLabel(labelId)) {
    ...
    if (cardLabelDeleteMatch && method === 'DELETE') {
        ...
        if (removeLabelFromCard(cardId, labelId)) {
            json(res, { ok: true });
            return true;
        }
    ...
    if (kanbanMoveMatch && method === 'POST') {
        ...
        if (moveKanbanCard(id, status, sort_order ?? 0, actor)) {
            ...
            if (status === 'done') {
                try {
                    recordAcceptedOutcomeForCard(getDb(), id);
                }
                catch (err) {
    ...
    if (kanbanCommentsMatch && method === 'POST') {
        ...
        if (!author || !content) {
            json(res, { error: 'Szerző és tartalom kötelező' }, 400);
            return true;
        }
    ...
    if (breakdownMatch && method === 'POST') {
        ...
        if (existing.length > 0) {
            json(res, { error: 'A kártya már rendelkezik subtask-okkal' }, 409);
            return true;
        }
        try {
    ...
    if (acceptMatch && method === 'POST') {
        ...
        if (!parent) {
            json(res, { error: 'Szülő kártya nem található' }, 404);
            return true;
        }
        const body = await readBody(req);
        const { subtasks } = JSON.parse(body.toString());
