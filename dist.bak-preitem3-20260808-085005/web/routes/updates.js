import { readFileSync, writeFileSync, mkdirSync, openSync, closeSync, statSync, unlinkSync, } from 'node:fs';
import { join } from 'node:path';
import { spawn, execFileSync } from 'node:child_process';
import { PROJECT_ROOT, STORE_DIR } from '../../config.js';
import { logger } from '../../logger.js';
import { getUpdateStatus, refreshUpdateStatus, } from '../update-checker.js';
import { checkUpdatePreflight, checkNoConcurrentUpdate, classifyLockWriteError, } from '../../update-preflight.js';
...
function readLastResult() {
    try {
        return JSON.parse(readFileSync(join(STORE_DIR, 'update.last-result'), 'utf-8'));
    }
    catch {
        return null;
    }
...
function isDiagnosable(r) {
    return r?.status === 'rolled-back' || r?.status === 'failed';
}
export async function tryHandleUpdates(ctx) {
    ...
    if (path === '/api/updates/status' && method === 'GET') {
        ...
        let running = false;
        try {
            running = statSync(UPDATE_PIDFILE).isFile();
        }
        catch { /* not running */ }
    ...
    if (path === '/api/updates/diagnose' && method === 'POST') {
        ...
        try {
            writeFileSync(DIAGNOSE_MARKER, key, { mode: 0o600 });
        }
        catch { /* best-effort */ }
    ...
    if (path === '/api/updates/apply' && method === 'POST') {
        ...
        const git = {
            currentBranch: () => execFileSync('/usr/bin/git', ['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: PROJECT_ROOT, timeout: 3000, encoding: 'utf-8' }),
            porcelainStatus: () => execFileSync('/usr/bin/git', ['status', '--porcelain', '--untracked-files=no'], { cwd: PROJECT_ROOT, timeout: 3000, encoding: 'utf-8' }),
            aheadCount: () => {
                try {
                    const out = execFileSync('/usr/bin/git', ['rev-list', '--count', '@{u}..HEAD'], { cwd: PROJECT_ROOT, timeout: 3000, encoding: 'utf-8' }).trim();
