...
import { createApproval, getApproval, resolveApproval, listApprovals, expireTimedOutApprovals, createAgentMessage, } from '../../db.js';
import { logger } from '../../logger.js';
import { readBody, json } from '../http-helpers.js';
const AUTONOMY_CONFIG_PATH = join(PROJECT_ROOT, 'store', 'autonomy-config.json');
function getTimeoutAt(category) {
    try {
        if (!existsSync(AUTONOMY_CONFIG_PATH))
            return null;
        const config = JSON.parse(readFileSync(AUTONOMY_CONFIG_PATH, 'utf-8'));
        const cat = config.categories.find(c => c.key === category);
        if (!cat || cat.timeout_minutes == null)
            return null;
...
function notifyMainAgent(approval) {
    try {
        const content = [
            `[APPROVAL_REQUEST]`,
            `id=${approval.id}`,
...
export async function tryHandleApprovals(ctx) {
