...
import { resolveProfilePlaceholders } from './profiles.js';
...
export function resolveDashboardOrigin(publicUrl, port) {
...
export function hookCommandWired(ptuJson, command) {
...
// install location.
export function substituteTemplatePlaceholders(content, id) {
    return content
...
export function resolveTemplatePlaceholders(content) {
...
export function agentSettingsPath(name) {
 ...
 */
export function isUnsafeHookCommand(command) {
    if (_TMP_PREFIXES.some((p) => command.includes(p)))
        return true;
...
function _hookScriptBasename(command) {
    const m = command.match(/\/([^/\s'"]+\.(?:py|mjs|js|sh))\b/);
    return m ? m[1] : null;
}
/**
...
export function ensureAgentHooks(name) {
    ...
    if (existing.hooks) {
        ...
        for (const [event, handlers] of Object.entries(tplHooks)) {
            if (!existingHooks[event]) {
                ...
                const tplEntries = handlers;
                const existEntries = existingHooks[event];
                // Collect all command strings already present in this event's hook groups.
                const existingCommands = new Set(existEntries.flatMap((e) => (e.hooks ?? []).map((h) => h.command).filter(Boolean)));
                for (const tplEntry of tplEntries) {
                    // Add hooks that are missing AND safe to register (registration guard).
                    const newHooks = (tplEntry.hooks ?? []).filter((h) => h.command && !existingCommands.has(h.command) && !isUnsafeHookCommand(h.command));
                    ...
                    for (const tplHook of tplEntry.hooks ?? []) {
                        if (!tplHook.command || tplHook.timeout == null)
                            continue;
                        for (const existEntry of existEntries) {
                            for (const existHook of existEntry.hooks ?? []) {
...
export function ensureAgentStalenessHook(name) {
    ...
    if (isUnsafeHookCommand(STALENESS_HOOK_CMD))
        return false;
...
export function writeAgentSettingsFromProfile(name, profile) {
    ...
    if (agentGetsGovernanceGates(name))
        denyList.push(...SELF_PACE_TOOL_DENY);
...
export function agentGetsEmailGate(name) {
...
export function injectEmailSendGate(existing) {
    ...
    if (isUnsafeHookCommand(command))
        return;
    const entry = {
        matcher: 'Bash|send_email',
        hooks: [{ type: 'command', command, timeout: 10 }],
    };
    const prev = Array.isArray(hooks.PreToolUse) ? hooks.PreToolUse : [];
...
export function agentGetsGovernanceGates(name) {
    return name !== MAIN_AGENT_ID;
}
...
export function injectSelfPaceGate(existing) {
    const hooks = (existing.hooks && typeof existing.hooks === 'object'
        ? existing.hooks
        : (existing.hooks = {}));
...
export function injectEgressGate(existing) {
    const hooks = (existing.hooks && typeof existing.hooks === 'object'
        ? existing.hooks
        : (existing.hooks = {}));
...
export function ensureEgressGate(name) {
    const settingsPath = agentSettingsPath(name);
    let settings = {};
    if (existsSync(settingsPath)) {
        try {
    ...
    if (ptuJson.includes('egress-gate.mjs') && hookCommandWired(ptuJson, command))
        return false;
    if (isUnsafeHookCommand(command))
        return false;
...
// path or space in it.
export function isPublicFetchHost(value) {
    const host = value.trim().toLowerCase();
    if (!host || host.length > 253)
        return false;
    if (/[^a-z0-9.-]/.test(host))
        return false; // scheme, port, path, wildcard, space
    if (host.startsWith('.') || host.endsWith('.'))
        return false;
    if (host.startsWith('-') || host.endsWith('-'))
        return false;
    if (/^\d+(\.\d+)*$/.test(host))
        return false; // IPv4 literal or a bare number
    const labels = host.split('.');
    if (labels.length < 2)
        return false; // single label: localhost and friends
    ...
    if (INTERNAL_SUFFIX.includes(labels[labels.length - 1]))
        return false;
    return true;
}
export function ownerAllowedDomains(storeDir = STORE_DIR) {
    try {
...
export function renderQuarantineReader(template, domains) {
    ...
    const already = new Set([...stripped.matchAll(/^- `([^`]+)`/gm)].map((m) => m[1].toLowerCase()));
...
export function ensureGovernanceGateCommands(name) {
    if (name === MAIN_AGENT_ID)
        return false;
    const settingsPath = agentSettingsPath(name);
    if (!existsSync(settingsPath))
...
export function ensureQuarantineReader(name) {
    ...
    if (existsSync(destPath)) {
        try {
            if (readFileSync(destPath, 'utf-8') === rendered)
                return false;
        }
...
function copyTaskConfigWithAgentRewrite(srcPath, destPath) {
    try {
...
export function ensureDefaultScheduledTasks() {
    const repoTasks = join(PROJECT_ROOT, 'scheduled-tasks');
    if (!existsSync(repoTasks))
        return;
...
export function scaffoldAgentDir(name) {
...
const FLEET_ROSTER_BLOCK_RE = new RegExp(`${FLEET_ROSTER_BEGIN.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\S]*?${FLEET_ROSTER_END.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`);
...
const AUTONOMY_BLOCK_RE = new RegExp(`${AUTONOMY_BEGIN.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\S]*?${AUTONOMY_END.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`);
...
function buildFleetRosterBody(selfName) {
    let agentNames;
    try {
...
// guess.
function buildAutonomyBody(name) {
    return [
...
export function ensureAutonomySection(name) {
...
export function ensureFleetRosterSection(name) {
...
export async function generateClaudeMd(name, description, model) {
...
Output ONLY the markdown content, no code fences.`;
    ...
    if (!text)
        throw new Error(error ? blockedHint('CLAUDE.md', error) : noOutputHint('CLAUDE.md'));
...
function noOutputHint(target) {
    return (`Failed to generate ${target}: the Claude Code CLI returned no output. ` +
...
function blockedHint(target, reason) {
    return (`Failed to generate ${target}: the model returned a blocked/errored result ` +
...
export async function generateSoulMd(name, description) {
...
Output ONLY the markdown content, no code fences.`;
    const { text, error } = await runAgent(prompt);
    if (!text)
        throw new Error(error ? blockedHint('SOUL.md', error) : noOutputHint('SOUL.md'));
    let cleaned = text.trim();
    if (cleaned.startsWith('```')) {
        cleaned = cleaned.replace(/^```\w*\n?/, '').replace(/\n?```$/, '');
    }
...
export async function generateSkillMd(skillName, description) {
...
Output ONLY the markdown content, no code fences.`;
    const { text, error } = await runAgent(prompt);
    if (!text)
        throw new Error(error ? blockedHint('SKILL.md', error) : noOutputHint('SKILL.md'));
    let cleaned = text.trim();
    if (cleaned.startsWith('```')) {
        cleaned = cleaned.replace(/^```\w*\n?/, '').replace(/\n?```$/, '');
    }
