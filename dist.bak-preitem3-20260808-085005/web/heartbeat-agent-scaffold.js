...
import { PROJECT_ROOT, STORE_DIR, OWNER_NAME, BOT_NAME, MAIN_AGENT_ID, WEB_PORT, HEARTBEAT_CALENDAR_ACCOUNT, APP_TZ, DASHBOARD_PUBLIC_URL, } from '../config.js';
...
const CHANNEL_PLUGIN_DISABLES = Object.fromEntries(Object.values(CHANNEL_PLUGIN_IDS).map(id => [id, false]));
...
export function currentHeartbeatIdentity() {
    return {
...
// spawns it at boot.
export function shouldBootHeartbeatAgent(opts) {
...
export function renderHeartbeatClaudeMd(id) {
...
function renderAgentConfigJson() {
    return JSON.stringify(HEARTBEAT_AGENT_CONFIG, null, 2) + '\n';
}
function renderClaudeSettingsJson() {
 ...
 */
export function ensureHeartbeatAgent() {
    try {
