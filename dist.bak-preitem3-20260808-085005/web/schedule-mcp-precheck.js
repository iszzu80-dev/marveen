...
export function deriveProcessPattern(def) {
...
export function resolveMcpProcessPatterns(agentName) {
    const out = {};
...
export function collectSubtreeCmdlines(psOutput, rootPid) {
...
export function decideMcpPrecheck(required, patterns, cmdlines) {
    const missing = [];
    const unknown = [];
    for (const name of required) {
        ...
        if (!cmdlines.some((cmd) => cmd.includes(pattern)))
            missing.push(name);
    }
 ...
 */
export function checkTaskMcpRequirements(required, agentName, session, host) {
    ...
    const result = decideMcpPrecheck(required, resolveMcpProcessPatterns(agentName), collectSubtreeCmdlines(psOutput, claudePid));
    if (result.unknown.length) {
        logger.debug({ agent: agentName, session, unknown: result.unknown }, 'MCP pre-check: no process pattern derivable for some required servers (fail-open)');
    }
    return result;
}
