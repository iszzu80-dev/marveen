...
function writeMockPs(processes) {
    let output = "";
...
async function run() {
    ...
    try {
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test1: claude counted (direct child of pane PID, despite no cmdline match)", marveen && marveen.rssBytes === expectedKiB * 1024, marveen ? `${marveen.rssBytes} vs ${expectedKiB * 1024}` : "agent not found");
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test2: all direct children counted (grandchildren are known awk one-level limit)", cw && cw.rssBytes === expectedKiB * 1024, cw ? `${cw.rssBytes} vs ${expectedKiB * 1024}` : "agent not found");
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test3: MCP children counted via parent ancestry (not cmdline match)", qa && qa.rssBytes === expectedKiB * 1024, qa ? `${qa.rssBytes} vs ${expectedKiB * 1024}` : "agent not found");
        }
        // ═══════════════════════════════════════════════════════════════════════════
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test4: browser child counted (direct child of pane, not cmdline match)", uat && uat.rssBytes === expectedKiB * 1024, uat ? `${uat.rssBytes} vs ${expectedKiB * 1024}` : "agent not found");
        }
        // ═══════════════════════════════════════════════════════════════════════════
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test5: marveen RSS = own tree only (not codeworker)", mv && mv.rssBytes === mvKiB * 1024, mv ? `${mv.rssBytes} vs ${mvKiB * 1024}` : "marveen not found");
            ok("test5: codeworker RSS = own tree only (not marveen)", cw && cw.rssBytes === cwKiB * 1024, cw ? `${cw.rssBytes} vs ${cwKiB * 1024}` : "codeworker not found");
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test6: non-reparented direct children counted", st && st.rssBytes === countedKiB * 1024, st ? `${st.rssBytes} vs ${countedKiB * 1024}` : "sentinel not found");
            ok("test6: reparented grandchild NOT in tree RSS (known awk limit; ps scan in verifyProcessTreeGone covers this)", st && st.rssBytes !== withReparentKiB * 1024);
        }
        // ═══════════════════════════════════════════════════════════════════════════
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test7a: totalRssBytes is null on error/partial with no data", r.status === "error" ? r.totalRssBytes === null : true, `status=${r.status} totalRssBytes=${r.totalRssBytes}`);
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test9: gate per-agent RSS = script per-agent RSS", gateAgents.every(ga => {
                const sa = scriptResult.agents.find(a => a.name === ga.name);
                return sa && sa.rssBytes === ga.rssBytes;
            }));
            ok("test9: gate total RSS = monitor total RSS", gateTotal === monitorTotal, `gate=${gateTotal} monitor=${monitorTotal}`);
            ok("test9: monitor measuredAgentCount = script count", monitorCount === scriptResult.agents.length, `monitor=${monitorCount} script=${scriptResult.agents.length}`);
        ...
        // ═══════════════════════════════════════════════════════════════════════════
        {
            ...
            ok("test10: full tree measured", agent && agent.rssBytes === fullKiB * 1024, agent ? `${agent.rssBytes} vs ${fullKiB * 1024}` : "agent not found");
            ...
            ok("test10: GOES RED — incomplete count does NOT match (proves suite can fail)", !redCheck, `measured=${agent?.rssBytes} vs incomplete=${incompleteKiB * 1024} — should NOT match`);
        }
    }
    finally {
