...
import { expandAndValidateConfigDir, readAgentClaudeConfigDir, readAgentClaudePlan, } from './agent-config.js';
...
const VALID_PLAN_TYPES = new Set(['personal', 'team']);
...
function validatePlan(raw, homeDir) {
    if (!raw || typeof raw !== 'object')
        return null;
    const o = raw;
    const id = isNonEmptyString(o.id) ? o.id.trim() : null;
    if (!id || !PLAN_ID_ALLOWED.test(id))
        return null;
    if (!isNonEmptyString(o.label))
        return null;
    if (!isNonEmptyString(o.configDir))
        return null;
    ...
    if (typeof planType !== 'string' || !VALID_PLAN_TYPES.has(planType)) {
        return null;
    }
    if (typeof o.channelsAllowed !== 'boolean')
        return null;
    const plan = {
        id,
        label: o.label.trim(),
    ...
    if (isNonEmptyString(o.expectedOrgType))
        plan.expectedOrgType = o.expectedOrgType.trim();
    if (isNonEmptyString(o.expectedEmail))
        plan.expectedEmail = o.expectedEmail.trim();
    return plan;
}
...
export function resolveClaudePlans(rawJson, homeDir) {
    let parsed;
    try {
...
let plansCache = null;
export function readClaudePlans() {
    let mtimeMs;
    try {
        mtimeMs = statSync(CLAUDE_PLANS_PATH).mtimeMs;
    }
    catch {
        mtimeMs = -1;
    }
    if (plansCache && plansCache.mtimeMs === mtimeMs)
    ...
    if (mtimeMs === -1) {
        ...
        let rawJson;
        try {
            rawJson = readFileSync(CLAUDE_PLANS_PATH, 'utf8');
        }
        catch {
            rawJson = '';
        }
...
export function getClaudePlan(id) {
    if (!id)
        return null;
...
export function resolveAgentConfigDir(name) {
    const planId = readAgentClaudePlan(name);
    if (planId) {
        const plan = getClaudePlan(planId);
        if (plan)
            return { configDir: plan.configDir, planId, planUnresolved: false };
