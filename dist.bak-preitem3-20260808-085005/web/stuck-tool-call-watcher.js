...
import { stuckToolCallSignature, decideStuckToolCallRecovery, detectPaneState, } from '../pane-state.js';
...
export function confirmsWedgeProfile(cpuPercent, maxCpuPercent) {
    if (cpuPercent === null)
        return true;
...
function sampleMainClaudeCpuPercent(session) {
    try {
...
export function shouldDeferForRecentRespawn(lastRespawnMs, nowMs, graceMs = MARVEEN_POST_RESPAWN_GRACE_MS) {
    return lastRespawnMs > 0 && nowMs - lastRespawnMs < graceMs;
}
async function checkSession(label, session) {
    ...
    if (recover) {
        ...
        if (pane != null && detectPaneState(pane) === 'idle') {
            logger.info({ label, session, tag: next.tag, seconds: next.lastSeconds, spellPeakSeconds: next.spellPeakSeconds }, 'stuck-tool-call-watcher: counter stagnant but pane is at the idle prompt (residual footer of a completed turn, not a wedge) -- skipping recovery');
            watchState.delete(session);
            return;
        }
        ...
        if (shouldDeferForRecentRespawn(lastRespawn, Date.now())) {
            logger.info({ label, session, sinceRespawnMs: lastRespawn ? Date.now() - lastRespawn : null, graceMs: MARVEEN_POST_RESPAWN_GRACE_MS }, 'stuck-tool-call-watcher: recent respawn within grace, deferring recovery (avoid double-respawn / boot churn)');
            return;
        }
        ...
        if (!confirmsWedgeProfile(cpuPercent, WEDGE_MAX_CPU_PERCENT)) {
            logger.info({ label, session, cpuPercent, maxCpuPercent: WEDGE_MAX_CPU_PERCENT, seconds: next.lastSeconds }, 'stuck-tool-call-watcher: counter stagnant but claude is CPU-active (not the idle wedge profile) -- deferring recovery');
            return;
        }
...
export function startStuckToolCallWatcher() {
