...
const pluginAbsentAt = new Map();
function markPluginAbsent(session) {
    pluginAbsentAt.set(session, Date.now());
}
/** Clear the absent mark once the plugin is observed healthy (recovery). */
export function clearPluginAbsent(session) {
 ...
 */
export function wasPluginConfirmedAbsent(session, withinMs) {
    const at = pluginAbsentAt.get(session);
    return at != null && Date.now() - at <= withinMs;
}
function getSessionClaudePid(session) {
    try {
...
function hasBunChild(claudePid) {
    try {
...
function isSessionReadyForUnlock(session) {
    try {
        ...
        // for the 'idle' state.
        if (!/bypass permissions on/.test(pane))
            return false;
        // Refuse if any modal is visible.
        if (/Resume from summary/.test(pane))
            return false;
        if (/Open System Settings/.test(pane))
            return false;
        return true;
    }
    catch (err) {
...
async function sendUnlockKeystrokes(session, provider) {
    try {
...
async function runUnlockProbe(state) {
    ...
    if (hasBunChild(claudePid)) {
        ...
        logger.info({ session: state.session, claudePid, provider: state.provider }, 'channel-plugin-unlock: bun child present, plugin healthy - no unlock needed');
        return;
    }
    if (!isSessionReadyForUnlock(state.session)) {
        if (state.retriesLeft > 0) {
            logger.info({ session: state.session, retriesLeft: state.retriesLeft }, 'channel-plugin-unlock: pane not idle yet, retrying');
    ...
    if (!releaseLane) {
        if (state.retriesLeft > 0) {
            logger.info({ session: state.session, retriesLeft: state.retriesLeft }, 'channel-plugin-unlock: pane send lane busy (delivery in flight), retrying');
    ...
    logger.warn({ session: state.session, claudePid, provider: state.provider }, 'channel-plugin-unlock: bun child absent after cold-start window, firing /mcp unlock sequence');
    try {
 ...
 */
export function schedulePluginUnlockAfterRespawn(session, provider) {
    setTimeout(() => { void runUnlockProbe({ session, provider, retriesLeft: UNLOCK_PROBE_MAX_RETRIES }); }, UNLOCK_PROBE_DELAY_MS);
    logger.info({ session, provider, delayMs: UNLOCK_PROBE_DELAY_MS }, 'channel-plugin-unlock: probe scheduled after respawn');
}
