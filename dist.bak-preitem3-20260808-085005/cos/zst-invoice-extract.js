...
import { upsertZstInvoice } from './zst-finance.js';
...
function supplierFromSender(from) {
...
export function extractInvoice(src) {
    ...
    if (gross != null)
        extracted.push('gross_amount');
    const dates = parseDates(text);
    const issueDate = dates[0];
    if (issueDate)
        extracted.push('issue_date');
    const dueDate = dates.length > 1 ? dates[dates.length - 1] : undefined;
    if (dueDate && dueDate !== issueDate)
        extracted.push('due_date');
    const invoiceNumber = invNoMatch?.[1];
    if (invoiceNumber)
        extracted.push('invoice_number');
    const supplierId = supplierFromSender(src.from) ?? undefined;
    if (supplierId)
        extracted.push('supplier_id');
    const confidence = gross != null && invoiceNumber ? 'HIGH' : gross != null || invoiceNumber ? 'PARTIAL' : 'LOW';
    return {
...
export function ingestZstInvoiceEmail(db, src, now) {
