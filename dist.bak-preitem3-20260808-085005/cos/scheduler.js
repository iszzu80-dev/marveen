...
const TERMINAL_CASE_STATUSES = ['COMPLETED', 'CANCELLED', 'ARCHIVED'];
...
export function dueCases(db, now, limit = 100) {
    return db.prepare(`SELECT ${DUE_COLUMNS} FROM personal_cases
     WHERE ${NOT_TERMINAL} AND next_wake_at IS NOT NULL AND next_wake_at <= ?
     ORDER BY next_wake_at ASC LIMIT ?`).all(...TERMINAL_CASE_STATUSES, now, limit);
}
/** Active cases whose follow_up_at has arrived (a nudge is due). */
export function dueFollowUps(db, now, limit = 100) {
    return db.prepare(`SELECT ${DUE_COLUMNS} FROM personal_cases
     WHERE ${NOT_TERMINAL} AND follow_up_at IS NOT NULL AND follow_up_at <= ?
     ORDER BY follow_up_at ASC LIMIT ?`).all(...TERMINAL_CASE_STATUSES, now, limit);
}
/** Set (or clear, with null) a case's next wake time. */
export function setNextWake(db, caseId, wakeAt, now) {
...
const OUTBOUND_TERMINAL = ['VERIFIED', 'FAILED_TERMINAL', 'CANCELLED'];
...
const OUTBOUND_NO_AUTO_WORK = [...OUTBOUND_TERMINAL, 'RECOVERY_REQUIRED'];
/**
 ...
 * drains in order.
 */
export function reconcileOutbound(db, limit = 100) {
    const ph = OUTBOUND_NO_AUTO_WORK.map(() => '?').join(',');
    return db.prepare(`SELECT ledger_id, case_id, action_type, status, attempt FROM outbound_ledger
     WHERE status NOT IN (${ph}) ORDER BY created_at ASC LIMIT ?`).all(...OUTBOUND_NO_AUTO_WORK, limit);
}
/**
 ...
 */
export function outboundNeedingHuman(db, limit = 100) {
    return db.prepare(`SELECT ledger_id, case_id, action_type, status, attempt FROM outbound_ledger
...
export function openEmailBatches(db, limit = 100) {
    return db.prepare(`SELECT batch_id, gmail_account_id, status FROM email_processing_batches
     WHERE status IN ('OPEN','PROCESSING') ORDER BY created_at ASC LIMIT ?`).all(limit);
}
