...
import { syncRenderCollector } from '../costops/collectors/render.js';
...
const stub = async (url) => url.includes('postgres') ? POSTGRES : SERVICES;
...
describe('syncRenderCollector (offline, v0.5 sync spine)', () => {
    ...
    it('imports idempotently, records an ok run with detail_json (no raw IDs)', async () => {
        ...
        const lines = db.prepare("SELECT COUNT(*) n FROM cost_line_items WHERE source_id='render-plan'").get();
        expect(lines.n).toBeLessThanOrEqual(1); // 0 if fx=0 in store pricing, else 1 -- never duplicated
        const runs = db.prepare("SELECT detail_json FROM import_runs WHERE provider='render' AND status='ok' ORDER BY started_at DESC LIMIT 1").get();
