...
import { upsertZstInvoice, markInvoicePaid, suggestBankMatches, ensureAccountingPackage, invoiceDuplicateHash, } from '../cos/zst-finance.js';
const T0 = 1_700_000_000;
describe('ZST Slice 2 finance', () => {
    beforeEach(() => { initDatabase(':memory:'); });
    it('all ZST business tables exist after initCosSchema (schema smoke)', () => {
        const db = getDb();
        const tables = new Set(db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'zst_%'`).all().map(r => r.name));
    ...
    it('AT-ZF02: an invoice cannot be marked paid without evidence', () => {
        ...
        expect(db.prepare(`SELECT payment_status, bank_transaction_id FROM zst_invoices WHERE invoice_id=?`).get(invoiceId))
    ...
    it('AT-ZF03/ZF04: bank match is read-only SUGGESTED, one txn not two invoices', () => {
      ...
      VALUES ('t1', 5000, 'ACME Kft', 'UNMATCHED', 'HUF', ?, ?)`).run(T0, T0);
        ...
        const t = db.prepare(`SELECT reconciliation_status, matched_invoice_id, match_confidence FROM zst_bank_transactions WHERE bank_transaction_id='t1'`).get();
        ...
        const recon = db.prepare(`SELECT COUNT(*) n FROM zst_reconciliation_items`).get();
    ...
    it('accounting package rolls up counts for a period (read-only assembly)', () => {
        ...
        const p = db.prepare(`SELECT * FROM zst_accounting_packages WHERE package_id=?`).get(packageId);
        ...
        expect(db.prepare(`SELECT COUNT(*) n FROM zst_accounting_packages`).get().n).toBe(1);
    });
