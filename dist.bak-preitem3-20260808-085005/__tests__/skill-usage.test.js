...
function insert(db, row) {
    ...
    db.prepare('INSERT INTO skill_usage (agent_id, skill_name, trigger_type, session_id, created_at) VALUES (?, ?, ?, ?, ?)').run(row.agent_id, row.skill_name, row.trigger_type, row.session_id ?? null, row.created_at ?? now);
}
// ---- Schema tests ----
describe('skill_usage schema', () => {
    ...
    it('accepts tool_call trigger_type', () => {
        ...
        const row = db.prepare('SELECT * FROM skill_usage LIMIT 1').get();
    ...
    it('allows null session_id', () => {
        ...
        const row = db.prepare('SELECT session_id FROM skill_usage LIMIT 1').get();
    ...
    it('autoincrement id is unique per row', () => {
        insert(db, { agent_id: 'agent-a', skill_name: 'fleet-helper', trigger_type: 'tool_call' });
        insert(db, { agent_id: 'agent-a', skill_name: 'fleet-helper', trigger_type: 'tool_call' });
        const rows = db.prepare('SELECT id FROM skill_usage').all();
    ...
    it('allows same skill to appear multiple times (no unique constraint)', () => {
        ...
        const count = db.prepare('SELECT COUNT(*) AS c FROM skill_usage').get().c;
        expect(count).toBe(5);
    });
});
// ---- Query tests ----
...
describe('skill_usage stats aggregation', () => {
    ...
    it('aggregates fleet-helper correctly', () => {
        const rows = stats();
        const fh = rows.find((r) => r.skill_name === 'fleet-helper');
...
describe('skill_usage has no prune mechanism', () => {
    it('table definition has no expiry column (row stays forever)', () => {
        ...
        const cols = db.prepare("PRAGMA table_info(skill_usage)").all().map((c) => c.name);
