...
import { detectUnderusedSubscriptionRecommendation, detectDuplicateSubscriptionRecommendations, detectDuplicateHostingSaasRecommendations, detectAnnualBillingRecommendation, detectUnusedDomainOrStorageRecommendation, detectForgottenServiceRecommendation, detectOversizedFixedPackageRecommendation, detectAutomateLongManualSourceRecommendation, detectProviderCreditOrDiscountRecommendation, reconcileRecommendations, acceptRecommendation, dismissRecommendation, serializeEvidence, deserializeEvidence, initOptimizationSchema, } from '../costops/optimization.js';
// ---- detectors ---------------------------------------------------------------------
...
// ---- lifecycle ------------------------------------------------------------------------
function candidate(dedup_key, over = {}) {
    return {
...
function record(dedup_key, status, over = {}) {
    return {
        ...candidate(dedup_key),
...
describe('initOptimizationSchema', () => {
    ...
    `).run({ ev: serializeEvidence({ x: 1 }), exp: now + 1000, now });
        const row = db.prepare(`SELECT * FROM costops_recommendations WHERE dedup_key = 'k1'`).get();
