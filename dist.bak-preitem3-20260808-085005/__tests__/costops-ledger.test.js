...
import { monthWindow, hashRef, confidenceBucket, syncFixedCostsToLedger, getCostSummary, } from '../costops/ledger.js';
...
describe('costops ledger + summary', () => {
    ...
    it('syncs fixed costs idempotently (no duplicates on re-run)', () => {
        ...
        syncFixedCostsToLedger(db, c, NOW);
        const rows = db.prepare('SELECT COUNT(*) as n FROM cost_line_items').get();
        expect(rows.n).toBe(2); // still 2, not 6
        const sources = db.prepare('SELECT COUNT(*) as n FROM cost_sources').get();
    ...
    it('reflects updated config amounts on re-sync (upsert, not insert)', () => {
        ...
        const row = db.prepare("SELECT billed_cost FROM cost_line_items WHERE source_id='anthropic-max'").get();
