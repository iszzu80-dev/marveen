...
import { initDispatchSchema, createDispatch, createDispatchSafe, recordOutcome, recordAcceptedOutcomeForCard, resolveOutcome, correlateTokenUsageToDispatches, loadDispatchAttributionConfig, DEFAULT_MAX_ATTRIBUTION_WINDOW_SECONDS, TERMINAL_OUTCOMES, resolveBillingMode, loadBillingMap, costPerAcceptedTask, } from '../costops/dispatch.js';
...
import { estimateModelCost } from '../costops/pricing.js';
...
describe('P2-A schema (installed via the CostOps seam)', () => {
    ...
    it('initDispatchSchema is idempotent and creates the three tables + link column', () => {
        ...
        const cols = db.prepare("SELECT name FROM pragma_table_info('token_usage')").all().map((r) => r.name);
        expect(cols).toContain('dispatch_id');
    });
    it('the dispatches table carries NO prompt/PII/secret column', () => {
        const cols = getDb().prepare("SELECT name FROM pragma_table_info('dispatches')").all().map((r) => r.name);
...
describe('P2-A createDispatch + routing_event', () => {
    ...
    it('creates a dispatch (opaque uuid) plus a default_route routing_event', () => {
        ...
        const d = db.prepare('SELECT * FROM dispatches WHERE dispatch_id = ?').get(id);
        ...
        const ev = db.prepare('SELECT * FROM routing_events WHERE dispatch_id = ?').get(id);
...
describe('P2-A window correlation (agent, session_id)', () => {
    ...
    it('attributes token_usage to the dispatch whose [created_at, next) window contains its timestamp', () => {
        ...
        const at = (ts) => db.prepare('SELECT dispatch_id FROM token_usage WHERE timestamp = ?').get(T0_SEC + ts).dispatch_id;
    ...
    it('does not cross session boundaries and never overwrites an existing link', () => {
        ...
        const s1 = db.prepare("SELECT dispatch_id FROM token_usage WHERE session_id='s1'").get();
        expect(s1.dispatch_id).toBe('MANUAL'); // not overwritten
        const s2 = db.prepare("SELECT dispatch_id FROM token_usage WHERE session_id='s2'").get();
...
describe('P2-A window bound 1: a TERMINAL outcome closes the window', () => {
    ...
    for (const outcome of ['accepted', 'failed', 'cancelled']) {
        it(`a row AFTER the '${outcome}' outcome is not attributed; a row before it still is`, () => {
            ...
            const at = (off) => db.prepare('SELECT dispatch_id FROM token_usage WHERE timestamp = ?').get(T0_SEC + off).dispatch_id;
            expect(at(50)).toBe(d);
    ...
    for (const outcome of ['retry', 'unknown']) {
        it(`a '${outcome}' outcome does NOT close the window (a row after it is still attributed)`, () => {
            ...
            const row = db.prepare('SELECT dispatch_id FROM token_usage WHERE timestamp = ?').get(T0_SEC + 150);
    ...
    it('another dispatch outcome does not close THIS dispatch (bound is per-dispatch)', () => {
        ...
        expect(correlateTokenUsageToDispatches(db, { configPath: NO_CONFIG })).toBe(1);
        const row = db.prepare("SELECT dispatch_id FROM token_usage WHERE session_id = 's1'").get();
...
describe('P2-A window bound 2: the max-window cap (the "absorbs a conversation hours later" case)', () => {
    ...
    it('a row beyond the DEFAULT cap is not attributed even as the session\'s last dispatch with no outcome', () => {
        ...
        const orphan = db.prepare('SELECT COUNT(*) AS n FROM token_usage WHERE dispatch_id IS NULL').get();
    ...
    it('the cap is configurable: a LARGER cap moves the boundary out', () => {
        ...
        const row = db.prepare('SELECT dispatch_id FROM token_usage WHERE timestamp = ?').get(beyondDefault);
        expect(row.dispatch_id).toBe(d);
    });
...
describe('P2-A measurement faults never block a dispatch', () => {
    it('createDispatchSafe returns null instead of throwing when the write fails', () => {
        const broken = { prepare() { throw new Error('simulated measurement failure (disk/schema fault)'); } };
        let result;
        expect(() => {
    ...
    it('createDispatch (unsafe) DOES throw -- proving the safe wrapper is what isolates the fault', () => {
        const broken = { prepare() { throw new Error('simulated measurement failure'); } };
