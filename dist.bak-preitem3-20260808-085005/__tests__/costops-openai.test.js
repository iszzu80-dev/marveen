...
describe('openaiCollector + syncOpenAiCollector (offline stub, no live call)', () => {
    ...
    it('sync imports a provider_api line + import_run using a stubbed key and fetcher (idempotent)', async () => {
        ...
        const lineCount = () => db.prepare("SELECT COUNT(*) c FROM cost_line_items WHERE source_id='openai-api'").get().c;
    ...
    it('reports error (no import) when the vault key is missing', async () => {
        ...
        expect(db.prepare("SELECT COUNT(*) c FROM cost_line_items WHERE source_id='openai-api'").get().c).toBe(0);
    });
    ...
    it('once a real rate is configured, the same run imports normally', async () => {
        ...
        expect(db.prepare("SELECT billed_cost FROM cost_line_items WHERE source_id='openai-api'").get().billed_cost).toBe(4.0 * 360);
    });
});
