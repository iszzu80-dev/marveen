...
const ACCT = 'private';
const NOW = 1_000_000;
function seed(messageId, status, contentHash) {
    const db = getDb();
    openBatch(db, { batchId: `b-${messageId}`, accountId: ACCT, cursorBefore: null, cursorAfter: 'c1', messages: [{ messageId }] }, NOW);
    if (status === 'processed')
        sourceCommit(db, ACCT, messageId, NOW);
    else
        claimMessage(db, ACCT, messageId, NOW);
    if (contentHash)
        db.prepare(`UPDATE email_processing SET content_hash=? WHERE gmail_account_id=? AND message_id=?`).run(contentHash, ACCT, messageId);
}
describe('gmail history self-event guard (AC-28)', () => {
    ...
    it('the COS/Processed label WE added on a known message → SELF_LABEL_NOOP (no business)', () => {
        ...
        const row = getDb().prepare(`SELECT status, self_event_count, last_self_event_at FROM email_processing WHERE message_id='m1'`).get();
