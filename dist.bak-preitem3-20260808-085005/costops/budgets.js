...
import { getCostSummary } from './ledger.js';
import { saveCostopsConfig } from './config.js';
export function initBudgetAuditSchema(db) {
    db.exec(`
...
function spendForScope(budget, summary) {
...
export function resolveBudgetStatus(budget, summary) {
    ...
    const status = used_pct >= hard_threshold ? 'hard' : used_pct >= warning_threshold ? 'warning' : 'ok';
    return {
...
export function getAllBudgetStatuses(db, config, now, monthKey) {
 ...
 */
export function upsertBudget(db, config, input, actor, now) {
    if (!actor || !actor.trim())
        return { ok: false, error: 'actor is required for a budget change (audit trail)', status: 400 };
...
export function deleteBudget(db, config, id, actor, now) {
    if (!actor || !actor.trim())
        return { ok: false, error: 'actor is required for a budget change (audit trail)', status: 400 };
    const existingIdx = config.budgets.findIndex(b => b.id === id);
    if (existingIdx < 0)
        return { ok: false, error: `no budget '${id}'`, status: 404 };
...
export function getBudgetAuditHistory(db, budgetId) {
    const rows = (budgetId
    ...
    return rows.map(r => ({
        budget_id: r.budget_id, action: r.action, actor: r.actor,
