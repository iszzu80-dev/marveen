...
//
//   npx tsx scripts/zst-import-bank-statement.ts <statement.csv> [--account OTP-HUF] [--apply]
//
...
import { parseBankStatementCsv, importBankStatement } from '../src/cos/zst-bank-import.js'
...
function main(): void {
  ...
  if (!csvPath || csvPath.startsWith('--')) {
    console.error('usage: zst-import-bank-statement.ts <statement.csv> [--account ID] [--db PATH] [--apply]')
    process.exit(2)
  }
