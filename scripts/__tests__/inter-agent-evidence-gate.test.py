...
class TestCommaConjunctionClauseBoundary(unittest.TestCase):
    ...
    # -- colon was tried twice (bare colon-boundary, then found unsafe for
    # label-style phrasing) and REVERTED both times. deliverylead's 3
    # label-form cases are the reason: in "Missing: <path>." / "Absent:
    # <path>." / "The deliverable does not exist: <path>.", the absence word
    # sits BEFORE the colon and the path AFTER it, so a colon boundary
    # strips the path's own clause of all absence vocabulary -- and this is
    # not a rare shape, it is the exact format this gate's OWN output uses
    # ("evidence-check: MISSING -- ..."). Pinned as named guards so colon
    # cannot quietly return a third time.

    def test_3_deliverylead_label_missing_colon_path(self):
        content = f"DONE: Missing: {FAKE_ABSENT_TRACKED}."
        row = (26, "marveen", "buildfejleszto", content, "pending", 0)
        result = eg.check_message(None, row, token=None, dry_run=True)
        self.assertEqual(result["verdict"], "PASS")

    def test_3b_deliverylead_label_absent_colon_path(self):
        content = f"DONE: Absent: {FAKE_ABSENT_TRACKED}."
        row = (27, "marveen", "buildfejleszto", content, "pending", 0)
        result = eg.check_message(None, row, token=None, dry_run=True)
        self.assertEqual(result["verdict"], "PASS")

    def test_3c_deliverylead_label_does_not_exist_colon_path(self):
        content = f"DONE: The deliverable does not exist: {FAKE_ABSENT_TRACKED}."
        row = (28, "marveen", "buildfejleszto", content, "pending", 0)
        result = eg.check_message(None, row, token=None, dry_run=True)
        self.assertEqual(result["verdict"], "PASS")
