...
function transportFor(agentConfigJson, session) {
...
describe('delivery transport selection', () => {
    it('a remote-configured agent routes its tmux send over ssh', () => {
        const { host, inv } = transportFor('{"remoteHost":"devbox","remoteWorkdir":"/home/user/proj"}', 'agent-dev');
    ...
    it('a half-configured (invalid) remote agent falls back to local transport', () => {
        const { host, inv } = transportFor('{"remoteHost":"devbox","remoteWorkdir":"~/bad"}', 'agent-dev');
        expect(host).toBeNull();
        expect(inv.file).toBe(TMUX);
    });
});
