...
// real git invocation.
function makeGit(branch, porcelain = '', ahead = 0) {
    return {
...
describe('checkUpdatePreflight --local commits (diverged history)', () => {
    it('rejects when the local checkout has commits ahead of upstream', () => {
        ...
        if (result.reason !== 'local-commits')
            return;
...
function makePidfile(raw, alivePids = [], nowMs = 1_000_000_000_000) {
    return {
        readPidfile: () => raw,
...
describe('checkNoConcurrentUpdate -- age-based staleness', () => {
    ...
    it('falls back to alive-only check when epoch line is missing (legacy format)', () => {
        const result = checkNoConcurrentUpdate(makePidfile('7777', [7777], 2_000_000_000_000));
        expect(result.ok).toBe(false);
    });
