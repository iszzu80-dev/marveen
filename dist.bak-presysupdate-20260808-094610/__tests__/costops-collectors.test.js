...
const FX = 308.91;
function emptyConfig() { return { version: 1, currency: 'HUF', fixed_costs: [], budgets: [] }; }
...
describe('runCollector (offline stub, no live call)', () => {
    ...
    it('imports a provider_api line and records an ok import_run; idempotent', async () => {
        ...
        const n = getDb().prepare("SELECT COUNT(*) n FROM cost_line_items WHERE confidence='provider_api'").get();
        expect(n.n).toBe(1);
        const runs = getDb().prepare("SELECT COUNT(*) n FROM import_runs").get();
    ...
    it('does NOT overwrite the manual estimate; both coexist; headline resolves to actual', async () => {
      ...
      VALUES ('anthropic-api',?,?,'usage','Anthropic API',17655,'HUF','estimate',?,'fixed|anthropic-api|2026-07',?)`).run(w.start, w.end, NOW, NOW);
        ...
        // both lines exist
        const both = db.prepare("SELECT confidence, billed_cost FROM cost_line_items WHERE source_id='anthropic-api' ORDER BY confidence").all();
    ...
    it('on collector error: records status=error, sanitized, deletes NOTHING', async () => {
        ...
        const throwing = async () => { throw new Error('rate limited, key sk-abcdef1234567890abcdef'); };
        ...
        // nothing deleted
        const n = db.prepare("SELECT COUNT(*) n FROM cost_line_items").get();
        expect(n.n).toBe(1);
