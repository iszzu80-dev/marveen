...
import { createRadarItem, getRadarItem, dueRadarChecks, recordObservation, setRadarStatus, markNotified, decideNotify, } from '../cos/radar.js';
...
describe('COS price radar', () => {
    ...
    it('records observations, tracks the new low, and does NOT hit above target', () => {
        ...
        expect(db.prepare(`SELECT COUNT(*) n FROM radar_observations WHERE radar_id='r1'`).get().n).toBe(3);
    });
    ...
    it('decideNotify: a non-hit never notifies', () => {
        const fake = { last_notified_at: null, last_notified_offer_id: null, last_notified_price: null };
    ...
    it('P1.5: a single-currency observation records identity FX (converted == best)', () => {
        ...
        const row = db.prepare(`SELECT * FROM radar_observations WHERE radar_id='r1' ORDER BY observed_at DESC LIMIT 1`).get();
