...
import { snapshotProcsWithRetry, PS_PROBE_TIMEOUT_MS, PS_PROBE_RETRY_TIMEOUT_MS, } from '../channel-coordinator/liveness.js';
describe('snapshotProcsWithRetry', () => {
    it('returns the first attempt without retrying on the fast path', () => {
        const run = vi.fn((_t) => 'PS-OUTPUT');
    ...
    it('retries once with the longer deadline when the fast path throws', () => {
        const run = vi.fn((timeoutMs) => {
            if (timeoutMs === PS_PROBE_TIMEOUT_MS)
    ...
    it('propagates the error when BOTH attempts throw (verdict stays unknown upstream)', () => {
        const run = vi.fn((_t) => { throw new Error('ETIMEDOUT'); });
