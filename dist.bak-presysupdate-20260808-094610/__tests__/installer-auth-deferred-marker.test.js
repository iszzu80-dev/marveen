...
const sh = (code) => execFileSync('/bin/bash', ['-c', code], { encoding: 'utf8' }).trim();
describe('deliberate auth skip is recorded, not inferred', () => {
    it('sets the flag in the SKIP branch, not anywhere else', () => {
        ...
        const skipBranch = script.slice(script.indexOf('Kihagyva. Kesobb allitsd be'), script.indexOf('Pre-flight headless probe'));
    ...
    it('is never derived from missing credentials', () => {
        ...
        const persistBlock = script.slice(script.indexOf('if [ "${CLAUDE_AUTH_DEFERRED:-}" = "1" ]'), script.indexOf('env_merge_key CLAUDE_AUTH_DEFERRED 1') + 60);
