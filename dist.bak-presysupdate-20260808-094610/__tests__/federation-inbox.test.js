...
import { validateInboxPayload, tryHandleFederation, _resetInboxDedupForTest, purgeInboxDedup, INBOX_MAX_BODY_BYTES, } from '../web/routes/federation.js';
...
const DEPS = { isKnownAgent: (n) => n === 'marketing', mainAgentId: 'arthur' };
function writeEnabledConfigFile() {
...
function fakeCtx(method, path, body, fedPeer = null) {
...
describe('tryHandleFederation -- inbox endpoint (HTTP + DB)', () => {
    ...
    it('413s an oversized body (Content-Length precheck) instead of a retryable-looking failure', async () => {
        ...
        ctx.req.headers['content-length'] = String(INBOX_MAX_BODY_BYTES + 1);
        expect(await tryHandleFederation(ctx)).toBe(true);
...
describe('tryHandleFederation -- manifest + peers view', () => {
    it('manifest 403s when disabled; reports system/version/agents when enabled; no operator-private data', async () => {
        ...
        expect(manifest.agents.some((a) => a.id === MAIN_AGENT_ID)).toBe(true);
    ...
    it('capabilitySummary in the manifest is per-peer opt-in AND dashboard-token visible (L5)', async () => {
        ...
        async function manifestAs(fedPeer) {
        ...
        const mainOf = (m) => m.agents.find((a) => a.id === MAIN_AGENT_ID);
