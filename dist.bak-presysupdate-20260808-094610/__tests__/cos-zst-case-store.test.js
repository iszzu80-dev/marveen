...
import { createZstCase, getZstCase, transitionZstCase, appendZstCaseEvent, acquireZstClaim, releaseZstClaim, listActiveZstCases, listTodayZstCases, } from '../cos/zst-case-store.js';
...
describe('ZST corporate case engine (Slice 0)', () => {
    ...
    function events(caseId) {
        return getDb().prepare(`SELECT event_type, previous_status, new_status, case_version FROM zst_case_events
    ...
    describe('transitionZstCase (optimistic concurrency)', () => {
        ...
        it('throws CaseConcurrencyError on a stale version (never clobbers)', () => {
            ...
            expect(getZstCase(db, 'z1').status).toBe('READY'); // unchanged
        });
    });
