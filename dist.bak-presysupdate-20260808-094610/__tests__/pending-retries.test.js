...
import { shouldSendAlert, toPendingRetryView, classifyTelegramSendError, ALERT_THRESHOLD_MS, } from '../pending-retries.js';
describe('shouldSendAlert', () => {
    ...
    it('returns false if an alert was already sent', () => {
        expect(shouldSendAlert(firstAttempt + 10 * threshold, firstAttempt, firstAttempt + threshold + 1, threshold)).toBe(false);
    });
...
describe('toPendingRetryView', () => {
    ...
    it('sets alertDue=false if an alert was already sent', () => {
        const view = toPendingRetryView({ ...baseRow, alert_sent_at: 1_000_000 + ALERT_THRESHOLD_MS + 100 }, 1_000_000 + 2 * ALERT_THRESHOLD_MS);
