 ...
 */
export function mainRestartMechanism(launchctlPresent) {
    return launchctlPresent ? 'launchd' : 'tmux-respawn';
}
export const DEFAULT_AUTO_RESTART = {
    enabled: false,
    mode: 'continue',
    dailyTime: null,
    intervalHours: null,
 ...
 */
export function normalizeAutoRestartConfig(raw) {
    const o = (raw && typeof raw === 'object') ? raw : {};
    const mode = o.mode === 'fresh' ? 'fresh' : 'continue';
    const dailyTime = parseHHMM(o.dailyTime) !== null ? o.dailyTime.trim() : null;
 ...
 */
export function restartDue(lastRestartAtMs, nowMs, dueAtMs) {
    if (!Number.isFinite(dueAtMs))
        return false;
    if (nowMs < dueAtMs)
        return false;
    if (lastRestartAtMs !== null && lastRestartAtMs >= dueAtMs)
        return false;
    return true;
}
/**
 ...
 */
export function dailyDueAtMs(localMidnightMs, minutesSinceMidnight) {
