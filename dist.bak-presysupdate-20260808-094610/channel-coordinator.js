...
import { initIngestDb, insertIncomingEvent, createHandoffMessage, markEventDelivered, getEventsNeedingHandoff, getOffset, setOffset, closeIngestDb, } from './channel-coordinator/ingest.js';
...
export function inNative409Cooldown(confirmedUpUntilMs, nowMs) {
...
function sendAlert(message) {
    ...
    execFile('/bin/bash', [script, message], { timeout: 10_000 }, (err) => {
        if (err)
            logger.warn({ err }, 'channel-coordinator: notify.sh alert failed');
    });
}
// ---- handoff content -----------------------------------------------------
...
export function neutralizeChannelTags(text) {
...
export function transientBackoffMs(attempt) {
...
function processBatch(updates) {
    ...
    for (const raw of updates) {
        ...
        const ev = mapUpdate(raw);
        if (!ev)
            continue; // unhandled update kind: offset still advances past it
        let ins;
        try {
        ...
        if (!ins.inserted || ins.eventId == null)
            continue; // dedup: already handed off
        try {
...
async function runLoop(token) {
    ...
    while (!stopping) {
        ...
        if (updates.length === 0)
            continue; // long-poll timed out; re-loop re-checks liveness
