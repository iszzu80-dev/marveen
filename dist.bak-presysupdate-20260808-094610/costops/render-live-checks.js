 ...
 */
export async function checkRenderBuildMinutes(now, deps = {}) {
    const apiKey = deps.apiKey !== undefined ? deps.apiKey : getRenderApiKey();
    if (!apiKey)
        return []; // no key configured -- render.ts's own sync already surfaces this gap
    ...
    if (ageSecs > LOOKBACK_SECS)
        return []; // stale event, pool has very likely reset since
    return [{
