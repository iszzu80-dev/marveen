...
export function fromSubscriptions(subs) {
    const out = [];
    for (const s of subs) {
...
export function fromDeepSeekBalance(db) {
    const rows = db.prepare(`SELECT balance, currency, captured_at FROM provider_balance_snapshots WHERE provider = 'deepseek' ORDER BY captured_at DESC`).all();
    if (rows.length === 0) {
        return [{
...
export function fromCodexRateLimit(db) {
    const rows = db.prepare(`SELECT used_percent, resets_at, captured_at FROM provider_ratelimit_snapshots WHERE provider = 'codex' ORDER BY captured_at DESC LIMIT 1`).all();
    if (rows.length === 0) {
        return [{
...
function fromWorkspaceAlerts(db, now) {
...
let liveChecksCache = null;
export async function getLiveCheckWarnings(now, sslHosts, domains) {
    ...
    promise.catch(() => { if (liveChecksCache?.promise === promise)
        liveChecksCache = null; });
    return promise;
}
async function fromLiveChecks(now, sslHosts, domains) {
    const out = [];
    ...
    for (const w of renderWarnings) {
        if (w.code !== 'render_build_minutes_exhausted')
            continue; // access-failure warnings aren't a limit signal
        out.push({
 ...
 */
export async function getLimitStatus(db, now, subscriptions) {
