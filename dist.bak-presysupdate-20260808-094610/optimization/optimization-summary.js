...
import { buildMonthlyPortfolioReview, } from '../costops/monthly-portfolio-review.js';
import { buildMarveenBenchmarkPack, } from '../costops/marveen-benchmark-pack.js';
import { loadCostopsConfig } from '../costops/config.js';
import { listRuntimeOverlays, readCapacityRoutingConfig, } from '../web/capacity-routing-store.js';
import { readOptimizationConfig, } from './optimization-config.js';
const MODULE_KEYS = [
    'measurement',
    'contextEfficiency',
    'capacityMonitoring',
...
const MARKET_WATCH_NOTE = 'market watch scheduler not yet wired to any cron/runner -- runMarketWatchCycle exists but has no caller in production';
function errorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
function topActionableRecommendation(recommendations) {
    ...
    return recommendations
        .filter(({ verdict }) => verdict !== 'NO_DECISION'
        && verdict !== 'INSUFFICIENT_EVIDENCE'
        && verdict !== 'KEEP')
        .sort((a, b) => confidenceRank[b.confidence] - confidenceRank[a.confidence])[0] ?? null;
}
function recommendationBlocker(recommendations) {
    if (recommendations.every(({ verdict }) => verdict === 'NO_DECISION' || verdict === 'INSUFFICIENT_EVIDENCE')) {
...
function buildAttentionQueue(input) {
    ...
    for (const subscription of input.capacityReport?.subscriptions ?? []) {
        if (subscription.overflow.confidence !== 'unknown'
            ...
            items.push({
                ...
                explanation: `${subscription.name} has observed overflow of ${subscription.overflow.value} `
    ...
    if (input.topRecommendation) {
        ...
        items.push({
            ...
            explanation: `The portfolio report returned ${recommendation.verdict} with ${recommendation.confidence} confidence.`,
            action: 'Review the recommendation evidence before deciding',
        });
    }
    const measuredFallback = input.kpiReport?.rows.find(({ fallback_rate: fallbackRate }) => fallbackRate.state === 'measured'
    ...
    if (measuredFallback) {
        items.push({
            ...
            explanation: `Measured fallback activity is present for ${measuredFallback.agent ?? 'an unassigned agent'} `
...
export function buildOptimizationSummary(db, now, opts = {}) {
    ...
    const routingErrors = [routingConfigError, overlaysError].filter((value) => value !== null);
            ...
            if (inventory.config.packages.length === 0) {
                ...
                try {
                    ...
                    for (const recommendation of recommendations)
                        freshness.push(recommendation.generated_at);
                ...
                try {
                    const review = buildMonthlyPortfolioReview(db, loadCostopsConfig().config, now, inventory.config.packages, fxContext);
    ...
    const runtimeRoutingConfig = routingConfig
        ? {
    ...
    else if (config.modules.runtimeRouting
