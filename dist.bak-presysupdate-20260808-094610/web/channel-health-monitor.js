...
import { resolveAgentSession, resolveAgentProviderType, } from './channel-mcp-reconnect.js';
...
function spawnDetachedReconnect(agentName) {
...
function isPluginFailedInPane(pane, pluginPaneId) {
    if (!pane.includes(pluginPaneId))
        return false;
    return PLUGIN_FAILED_RX.test(pane);
}
export function getChannelHealth(agentName) {
    const state = reconnectState.get(agentName);
    if (!state)
        return { healthy: true, reconnectAttempts: 0, lastAttemptAt: null };
    return {
        healthy: false,
...
function checkAgent(agentName, session) {
    const now = Date.now();
    const state = reconnectState.get(agentName);
    ...
    if (state && now < state.nextRetryAt)
        return;
    ...
    logger.warn({ agentName, attempt, provider: providerType }, 'channel-health-monitor: plugin failure detected, spawning detached reconnect');
