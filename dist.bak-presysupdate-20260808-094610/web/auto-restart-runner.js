...
import { restartDue, dailyDueAtMs, parseHHMM, mainRestartMechanism } from '../auto-restart.js';
...
function computeDueAt(cfg, name, nowMs) {
    if (cfg.dailyTime) {
...
function paneIsIdle(session, host) {
...
function restartMainChannelsSession() {
...
function performRestart(name, cfg) {
...
function checkAgent(name, nowMs) {
    ...
    if (dueAt === null)
        return;
    if (!restartDue(lastRestart.get(name) ?? null, nowMs, dueAt))
        return;
...
export function startAutoRestartRunner() {
    function sweep() {
        const now = Date.now();
        try {
            checkAgent(MAIN_AGENT_ID, now);
        }
        catch (err) {
            logger.debug({ err }, 'auto-restart: main check error');
        }
        for (const name of listAgentNames()) {
            try {
                ...
                logger.debug({ err, agent: name }, 'auto-restart: agent check error');
            }
        }
    }
