...
import { listVaultSshKeys, getVaultSshKey, createVaultSshKey, deleteVaultSshKey, } from '../../db.js';
import { setSecret, getSecret, deleteSecret } from '../vault.js';
function fingerprintFromPubKey(authorizedKeyLine) {
...
export function extractPublicKeyFromVault(vaultKeyId) {
...
export async function tryHandleVaultSshKeys(ctx) {
    const { req, res, path, method } = ctx;
    if (!path.startsWith('/api/vault/ssh-keys'))
        return false;
    ...
    if (pubKeyMatch && method === 'GET') {
        ...
        if (!key) {
            json(res, { error: `Key "${id}" not found` }, 404);
            return true;
        }
