...
import { hardRestartMarveenChannels, mainChannelsSessionExists, createMainChannelsSession, } from '../channel-monitor.js';
...
function keychainHasClaudeCredentials() {
    if (process.platform !== 'darwin')
        return false;
    try {
        execFileSync('/usr/bin/security', ['find-generic-password', '-s', 'Claude Code-credentials', '-a', userInfo().username], { timeout: 3000, stdio: 'ignore' });
        return true;
    }
    catch {
        return false;
...
function claudeAuthPresent() {
    if (readEnvValue('CLAUDE_CODE_OAUTH_TOKEN'))
        return true;
    if (readEnvValue('ANTHROPIC_API_KEY'))
        return true;
    try {
        const d = JSON.parse(readFileSync(HOME_CREDENTIALS, 'utf-8'));
        if (d?.claudeAiOauth?.accessToken)
            return true;
        if (d?.apiKey)
            return true;
    }
    catch { /* no / unreadable credentials.json */ }
    if (fleetTokenPresent())
        return true;
...
function existingChannelTokens() {
...
function paired() {
    try {
        const a = JSON.parse(readFileSync(join(channelStateDir(CHANNEL_PROVIDER), 'access.json'), 'utf-8'));
...
function agentsRunning() {
    try {
        return sessionExistsOnHost(null, MAIN_CHANNELS_SESSION);
    }
    catch {
        return false;
    }
...
function renameInPersonaFile(file, from, to) {
    if (!from || !to || from === to)
        return;
    let content;
    try {
...
export function identitySavePlan(servicesUp, freshSetup, nameChanged) {
    return {
        restart: servicesUp && freshSetup && nameChanged,
        restartNeeded: servicesUp && !freshSetup && nameChanged,
    };
}
export async function tryHandleOnboarding(ctx) {
    ...
    if (path === '/api/onboarding/identity' && method === 'POST') {
        ...
        if (!agentName || !ownerName) {
            json(res, { error: 'agentName es ownerName szukseges.', reason: 'missing' }, 400);
            return true;
        }
        ...
        try {
            for (const f of [join(PROJECT_ROOT, 'CLAUDE.md'), join(PROJECT_ROOT, 'SOUL.md')]) {
                ...
                if (prevOwnerName)
                    renameInPersonaFile(f, prevOwnerName, ownerName);
            }
        }
        catch (err) {
        ...
        if (plan.restart) {
            ...
            restarted = r.ok;
            if (!r.ok)
                restartError = r.error || 'restart failed';
            if (r.ok)
                logger.info('onboarding: channels restarted so the new identity is picked up');
            else
                logger.error({ error: restartError }, 'onboarding: channels restart after identity save FAILED');
        }
    ...
    if (path === '/api/onboarding/claude-auth' && method === 'POST') {
        ...
        if (!token && !apiKey) {
            json(res, { error: 'token vagy apiKey szukseges.', reason: 'missing' }, 400);
            return true;
        }
        if (token && !/^sk-ant-oat/.test(token)) {
            json(res, { error: 'A setup-token formatuma nem stimmel (sk-ant-oat...).', reason: 'bad-token' }, 400);
            return true;
        }
        if (apiKey && !/^sk-ant-/.test(apiKey)) {
            json(res, { error: 'Az API-kulcs formatuma nem stimmel (sk-ant-...).', reason: 'bad-key' }, 400);
            return true;
        }
        ...
        if (!hadAuthBefore && agentsRunning()) {
            ...
            if (!r.ok)
                restartError = r.error || 'restart failed';
            if (r.ok)
                logger.info('onboarding: channels restarted so the fresh auth is picked up');
            else
                logger.error({ error: restartError }, 'onboarding: channels restart after first auth FAILED');
        }
    ...
    if (path === '/api/onboarding/launch' && method === 'POST') {
        if (agentsRunning()) {
            json(res, { ok: true, alreadyRunning: true });
            return true;
        }
        if (!claudeAuthPresent()) {
            json(res, { error: 'Eloszor allitsd be a Claude-autentikaciot.', reason: 'no-auth' }, 409);
            return true;
        }
        ...
        const r = hardRestartMarveenChannels();
        if (!r.ok) {
            json(res, { error: r.error || 'Nem sikerult eletre kelteni az agenteket.', reason: 'launch-failed' }, 500);
            return true;
        }
