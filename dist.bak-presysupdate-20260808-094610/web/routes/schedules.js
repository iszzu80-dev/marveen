...
import { listPendingTaskRetries, deletePendingTaskRetryById, listTaskRunHistory, } from '../../db.js';
...
import { SCHEDULED_TASKS_DIR, MAX_SCHEDULED_TASK_PROMPT_LEN, listScheduledTasks, writeScheduledTask, } from '../scheduled-tasks-io.js';
...
function resolveScheduleDir(rawName) {
    let decoded;
    try {
...
export async function tryHandleSchedules(ctx) {
    ...
    if (path === '/api/schedules/expand-questions' && method === 'POST') {
        ...
        const { prompt, agent } = JSON.parse(body.toString());
        if (!prompt?.trim()) {
...
[
  ...
  {"question": "Masik kerdes?", "options": ["A", "B"]}
    ...
    if (path === '/api/schedules/expand-prompt' && method === 'POST') {
        const body = await readBody(req);
        const { prompt, answers } = JSON.parse(body.toString());
        if (!prompt?.trim()) {
            json(res, { error: 'Prompt is required' }, 400);
            return true;
        }
...
Az eredmeny CSAK a kibovitett prompt szovege legyen, semmi mas. Ne hasznalj code fence-t.`;
    ...
    if (path === '/api/schedules' && method === 'POST') {
        ...
        if (!data.schedule?.trim()) {
            json(res, { error: 'Schedule is required' }, 400);
            return true;
        }
        if (!isValidCronShape(data.schedule)) {
            json(res, { error: 'Invalid cron expression' }, 400);
            return true;
        }
        const dir = join(SCHEDULED_TASKS_DIR, name);
        if (existsSync(dir)) {
            json(res, { error: 'Schedule already exists' }, 409);
            return true;
        }
    ...
    if (pendingCancelMatch && method === 'DELETE') {
        ...
        if (!removed) {
            json(res, { error: 'Pending retry not found' }, 404);
            return true;
        }
