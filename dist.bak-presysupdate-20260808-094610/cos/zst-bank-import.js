...
function mapColumns(headerCells) {
    const idx = {};
    for (const role of Object.keys(COLUMN_CUES)) {
        idx[role] = -1;
        for (let c = 0; c < headerCells.length; c++) {
            const h = deaccent(headerCells[c]);
            if (COLUMN_CUES[role].some(cue => h.includes(cue))) {
                idx[role] = c;
                break;
            }
        }
...
export function parseBankStatementCsv(csv) {
    ...
    if (col.bookingDate < 0 && col.amount < 0)
        return []; // not a recognizable statement
    const out = [];
    for (let i = 1; i < lines.length; i++) {
        const cells = splitCsvLine(lines[i], delim);
        const get = (r) => (col[r] >= 0 ? cells[col[r]] ?? '' : '');
        const amount = parseAmount(get('amount'));
        const bookingDate = normalizeDate(get('bookingDate'));
        if (amount == null && !bookingDate)
            continue; // empty/garbage row
        const row = {
            bookingDate,
        ...
        if (amount != null)
            row.direction = amount < 0 ? 'DEBIT' : 'CREDIT';
        out.push(row);
    }
    return out;
}
...
export function importBankStatement(db, args, now) {
    const rows = parseBankStatementCsv(args.csv);
    const result = { imported: 0, duplicates: 0, skipped: 0, rows: [] };
    const insert = db.prepare(`INSERT INTO zst_bank_transactions
    ...
    const tx = db.transaction(() => {
        for (const r of rows) {
            if (r.amount == null && !r.bookingDate) {
                result.skipped++;
                continue;
            }
            ...
            if (exists) {
                result.duplicates++;
                result.rows.push({ bankTransactionId: id, duplicate: true });
                continue;
            }
            insert.run({
