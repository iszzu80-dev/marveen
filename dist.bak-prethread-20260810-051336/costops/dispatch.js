...
import { loadPricingConfig, estimateModelCost } from './pricing.js';
 ...
 */
export function initDispatchSchema(db) {
  ...
  `);
    ...
    try {
        db.exec('ALTER TABLE token_usage ADD COLUMN dispatch_id TEXT');
    }
 ...
 */
export function createDispatch(db, input, now = Date.now()) {
    const id = randomUUID();
 ...
 */
export function createDispatchSafe(db, input, now = Date.now()) {
    try {
...
export function insertRoutingEvent(db, ev, atSec = Math.floor(Date.now() / 1000)) {
    const id = randomUUID();
    db.prepare(`
...
export function recordOutcome(db, input, now = Date.now()) {
    const id = randomUUID();
    db.prepare(`
 ...
 */
export function recordOutcomeSafe(db, input, now = Date.now()) {
    try {
        return recordOutcome(db, input, now);
    }
    catch (err) {
        logger.warn({ err, outcome: input.outcome, dispatchId: input.dispatchId }, 'recordOutcome failed; dispatch outcome un-recorded (delivery handling unaffected)');
        return null;
    }
}
/**
 ...
 */
export function recordAcceptedOutcomeForCard(db, cardId, now = Date.now()) {
    const rows = db.prepare('SELECT dispatch_id FROM dispatches WHERE card_id = ?').all(cardId);
    let written = 0;
    for (const r of rows) {
...
export function resolveOutcome(db, dispatchId) {
    const r = db.prepare('SELECT outcome FROM dispatch_outcomes WHERE dispatch_id = ? ORDER BY created_at DESC, rowid DESC LIMIT 1').get(dispatchId);
 ...
 */
export const TERMINAL_OUTCOMES = ['accepted', 'failed', 'cancelled'];
/**
 ...
 */
export function loadDispatchAttributionConfig(path = DISPATCH_ATTRIBUTION_CONFIG_PATH) {
    if (!existsSync(path))
...
function sanitizeMaxWindowSeconds(v) {
 ...
 */
export function correlateTokenUsageToDispatches(db, opts = {}) {
    ...
    const dispatches = db.prepare(`SELECT dispatch_id, agent, session_id, created_at FROM dispatches
     WHERE ${where.join(' AND ')} ORDER BY agent, session_id, created_at ASC`).all(...params);
    ...
    const terminalRows = db.prepare(`SELECT dispatch_id, MIN(created_at) AS closed_at FROM dispatch_outcomes
     WHERE dispatch_id IS NOT NULL AND outcome IN (${TERMINAL_OUTCOMES.map(() => '?').join(', ')})
     GROUP BY dispatch_id`).all(...TERMINAL_OUTCOMES);
    for (const r of terminalRows)
        closedAt.set(r.dispatch_id, r.closed_at);
    // Bind order: dispatch_id, agent, session_id, from, nextStart, capEnd, outcomeEnd.
    const link = db.prepare(`UPDATE token_usage SET dispatch_id = ?
 ...
 */
export function correlateTokenUsageToDispatchesSafe(db, opts = {}) {
    try {
 ...
 */
export function resolveBillingMode(map, provider, authProfile) {
    if (!map || !provider || !authProfile)
        return 'unknown';
...
export function loadBillingMap(path = BILLING_MAP_PATH) {
    if (!existsSync(path))
        return null;
    try {
        const raw = JSON.parse(readFileSync(path, 'utf-8'));
        ...
        for (const e of raw.entries) {
            if (e && typeof e.provider === 'string' && typeof e.auth_profile === 'string' && typeof e.billing_mode === 'string') {
                entries.push({ provider: e.provider, auth_profile: e.auth_profile, billing_mode: e.billing_mode });
            }
        }
 ...
 */
export function costPerAcceptedTask(db, opts = {}) {
    const pricing = opts.pricing ?? loadPricingConfig().pricing;
    const conds = ["o.outcome = 'accepted'"];
    const params = [];
    if (opts.from) {
  ...
  `).all(...params);
    ...
    const providerPeriodTasks = new Map();
    for (const r of rows) {
        ...
        if (!providerPeriodTasks.has(ppKey))
            providerPeriodTasks.set(ppKey, new Set());
        ...
        if (r.input_tokens != null || r.output_tokens != null) {
            ...
            if (marginal != null) {
                acc.marginal += marginal;
                acc.marginalSeen = true;
            }
        }
    }
 ...
 */
function allocatedPerTask(db, provider, period, providerPeriodAcceptedTasks) {
    if (!provider || providerPeriodAcceptedTasks <= 0)
        return null;
