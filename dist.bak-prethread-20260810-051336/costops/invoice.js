 ...
 */
export function computeNetAmount(a) {
...
export function invoiceDedupKey(provider, invoiceRefHashOrFallback, periodKey) {
 ...
 */
export function inferExpectedInvoiceBy(periodEndsAsc, graceDays = 15) {
    if (periodEndsAsc.length < 2)
        return null;
    const gaps = [];
    for (let i = 1; i < periodEndsAsc.length; i++)
        gaps.push(periodEndsAsc[i] - periodEndsAsc[i - 1]);
 ...
 */
export function recordInvoice(db, input, opts) {
    if (!input.source_id)
        return { ok: false, error: 'source_id is required', status: 400 };
    if (!input.provider)
        return { ok: false, error: 'provider is required', status: 400 };
    if (!input.currency)
        return { ok: false, error: 'currency is required', status: 400 };
    if (!Number.isFinite(input.gross_amount) || input.gross_amount < 0)
        return { ok: false, error: 'gross_amount must be a non-negative number', status: 400 };
    ...
    const dup = db.prepare(`SELECT id FROM costops_invoices WHERE dedup_key = ? AND status != 'voided'`).get(dedupKey);
    if (dup)
        return { ok: false, error: `duplicate invoice: an active invoice already exists for ${dedupKey}`, status: 409, duplicate: true };
  ...
  `).get(input.source_id, input.billing_period_end, input.billing_period_start);
    ...
    `).run({
        ...
        if (activeLine) {
            ...
            if (!corr.ok) {
                caughtError = corr.error ?? 'correction failed';
                return null;
            }
      ...
      `).run({ source_id: input.source_id, start: input.billing_period_start, end: input.billing_period_end, net, currency: input.currency, dedup_key: lineDedup, now: opts.now });
            ledgerLineId = lineInfo.lastInsertRowid;
        }
    ...
    if (caughtError || !result)
        return { ok: false, error: caughtError ?? 'record invoice failed', status: 500 };
 ...
 */
export function applyInvoiceAdjustment(db, input, opts) {
    if (!input.reason || !input.reason.trim())
        return { ok: false, error: 'reason is required for an invoice adjustment (auditability)', status: 400 };
    const row = db.prepare(`SELECT id, gross_amount, tax_amount, discount_amount, credit_amount, refund_amount, late_charge_amount, status, ledger_line_id FROM costops_invoices WHERE id = ?`).get(input.invoiceId);
    if (!row)
        return { ok: false, error: `no invoice with id ${input.invoiceId}`, status: 404 };
    if (row.status === 'voided')
        return { ok: false, error: `invoice ${input.invoiceId} is voided -- cannot adjust it`, status: 409 };
    const amounts = {
    ...
    tx();
    if (caughtError)
        return { ok: false, error: caughtError, status: 500 };
 ...
 */
export function voidInvoice(db, invoiceId, reason, now) {
    if (!reason || !reason.trim())
        return { ok: false, error: 'reason is required to void an invoice (auditability)', status: 400 };
    const row = db.prepare(`SELECT status FROM costops_invoices WHERE id = ?`).get(invoiceId);
    if (!row)
        return { ok: false, error: `no invoice with id ${invoiceId}`, status: 404 };
    if (row.status === 'voided')
        return { ok: false, error: `invoice ${invoiceId} is already voided`, status: 409 };
    db.prepare(`UPDATE costops_invoices SET status='voided', void_reason=?, voided_at=? WHERE id=?`).run(reason, now, invoiceId);
    return { ok: true, invoiceId };
}
export function listInvoices(db, opts = {}) {
 ...
 * fabricated).
 */
export async function buildInvoiceReconciliation(db, now, month) {
 ...
 */
export function initInvoiceSchema(db) {
    db.exec(`
