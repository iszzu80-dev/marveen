...
import { detectBudgetThresholdAlert, detectForecastBudgetBreachAlert, detectBalanceExhaustionAlert, detectStaleCollectorAlert, detectFailedSyncAlert, detectCredentialPermissionAlert, detectReconciliationMismatchAlert, detectManualProviderVarianceAlert, detectMissingInvoiceAlert, detectUnusualSpendGrowthAlert, detectNewUnknownSourceAlert, detectLongEstimateOnlySourceAlert, detectSubscriptionUtilizationAlert, reconcileAlerts, serializeEvidence, deserializeEvidence, } from './alerts.js';
 ...
 */
function gatherBudgetCandidates(db, config, now) {
    const out = [];
...
function gatherBalanceExhaustionCandidates(db, now) {
  ...
  `).all();
    ...
    for (const r of rows) {
        if (r.remaining == null)
            continue;
        const c = detectBalanceExhaustionAlert({ provider: r.provider, remaining: r.remaining, currency: r.included_unit ?? 'USD', forecast_exhaustion_at: r.forecast_exhaustion_at }, now);
        if (c)
            out.push(c);
    }
    return out;
 ...
 */
export function classifyErrorCode(raw) {
    if (!raw)
        return null;
    if (CREDENTIAL_ERROR_MARKER.test(raw))
        return 'credential_error';
    if (PERMISSION_ERROR_MARKER.test(raw))
        return 'permission_error';
    return null;
}
function gatherSyncAndCredentialCandidates(db, now) {
  ...
  `).all();
    ...
    for (const r of latestRows) {
        ...
        const signal = { provider: r.provider, status, last_success: lastOk, data_age_secs: age, error_code: classified ?? r.error_code };
        const staleAlert = detectStaleCollectorAlert(signal);
        if (staleAlert)
            out.push(staleAlert);
        const failedAlert = detectFailedSyncAlert(signal);
        if (failedAlert)
            out.push(failedAlert);
...
function gatherReconciliationCandidates(db, now) {
    ...
    for (const row of rows) {
        ...
        if (actual == null || estimate == null || row.variance == null)
            continue;
...
function gatherManualProviderVarianceCandidates(summary) {
    const providerDerived = summary.operational.provider_derived_spend;
    if (providerDerived === 0)
        return [];
...
function gatherUnusualSpendGrowthCandidates(db, config, now) {
    const trend = getPeriodTrend(db, config, now, 2);
    if (trend.current.no_data || trend.previous.no_data)
        return [];
    const out = [];
...
function gatherNewSourceAndEstimateOnlyCandidates(db, now) {
    const out = [];
    const win = monthWindow(now);
    const sources = db.prepare(`SELECT id FROM cost_sources WHERE active = 1 AND lifecycle_state != 'decommissioned'`).all();
    for (const s of sources) {
        const earliest = db.prepare(`SELECT MIN(charge_period_start) as t FROM cost_line_items WHERE source_id = ? AND voided_at IS NULL`).get(s.id);
        if (earliest.t != null) {
    ...
    `).all(s.id, ESTIMATE_ONLY_LOOKBACK_PERIODS);
        if (periodRows.length < ESTIMATE_ONLY_LOOKBACK_PERIODS)
            continue; // not enough history to claim "long" -- never fabricated
        let allEstimateOnly = true;
        for (const p of periodRows) {
            const lines = db.prepare(`SELECT confidence FROM cost_line_items WHERE source_id = ? AND charge_period_start = ? AND voided_at IS NULL`).all(s.id, p.charge_period_start);
            ...
            if (!ESTIMATE_ONLY_CONF.has(resolved)) {
                allEstimateOnly = false;
                break;
            }
        }
        if (allEstimateOnly) {
...
function gatherUtilizationCandidates(now) {
    ...
    for (const ls of limitStatuses) {
        if (ls.usage_pct == null || ls.sub_id == null)
            continue;
 ...
 */
function gatherMissingInvoiceCandidates(db, now) {
    const out = [];
    const sources = db.prepare(`SELECT DISTINCT source_id FROM costops_invoices`).all();
    for (const s of sources) {
        const rows = db.prepare(`
 ...
 */
export function gatherAlertCandidates(db, config, now) {
...
function rowToRecord(r) {
    return {
        dedup_key: r.dedup_key, type: r.type, severity: r.severity,
...
function loadExistingAlerts(db) {
    const rows = db.prepare(`SELECT * FROM costops_alerts`).all();
 ...
 */
export function captureAlerts(db, config, now, opts = {}) {
...
export function listAlerts(db, opts = {}) {
