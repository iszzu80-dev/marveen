...
export const ALLOWED_UPDATES = ['message', 'edited_message', 'channel_post', 'callback_query'];
export class TelegramApiError extends Error {
    kind;
    retryAfterSec;
    constructor(kind, message, retryAfterSec) {
        super(message);
        this.kind = kind;
...
export function mapUpdate(u) {
    const msg = u.message ?? u.edited_message ?? u.channel_post;
    if (msg) {
        const kind = u.edited_message
            ? 'edited_message'
            : u.channel_post
                ? 'channel_post'
                : 'message';
        ...
        if (Array.isArray(msg.photo) && msg.photo.length > 0)
            meta['has_photo'] = true;
        if (msg.document)
            meta['document'] = { file_id: msg.document.file_id, file_name: msg.document.file_name };
        if (msg.voice)
            meta['voice'] = { file_id: msg.voice.file_id };
        return {
...
// gets torn down.
export async function getUpdates(token, offset, timeout, limit) {
    ...
    if (!res.ok) {
        ...
        if (errorCode === 401)
            throw new TelegramApiError('fatal', `401 unauthorized: ${description}`);
        if (errorCode === 409)
            throw new TelegramApiError('conflict', `409 conflict: ${description}`);
        if (errorCode === 429)
            throw new TelegramApiError('rate_limit', `429 too many requests: ${description}`, retryAfter);
        if (errorCode >= 500)
            throw new TelegramApiError('transient', `5xx: ${description}`);
    ...
    if (!json.ok)
        throw new TelegramApiError('transient', `getUpdates ok=false: ${json.description ?? 'unknown'}`);
    return json.result ?? [];
}
...
export async function probeHighWater(token) {
    const controller = new AbortController();
    ...
    if (!res.ok) {
        if (res.status === 401)
            throw new TelegramApiError('fatal', '401 unauthorized (high-water probe)');
        if (res.status === 409)
            throw new TelegramApiError('conflict', '409 conflict (high-water probe)');
