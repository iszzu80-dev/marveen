...
async function fetchCompare(remote, base, head) {
...
function releaseBodySummary(fullMessage) {
    const lines = fullMessage.split('\n').slice(1); // drop the subject line
    const kept = [];
    for (const line of lines) {
        if (/^(Co-Authored-By|Signed-off-by|Co-authored-by):/i.test(line.trim()))
            continue;
        kept.push(line);
    }
    return kept.join('\n').trim();
...
function applyCompare(status, cmp) {
...
export function groupByRelease(commits, fullMessages) {
    const groups = [];
    let cur = null;
    const upcoming = { version: '', summary: '', commits: [] };
...
export async function refreshUpdateStatus() {
    ...
    try {
        ...
        if (!latestRes.ok)
            throw new Error(`GitHub /commits/${branch} -> ${latestRes.status}`);
        const latestJson = await latestRes.json();
        if (!latestJson.sha)
            throw new Error(`No sha on commits/${branch} response`);
