...
import { PROJECT_ROOT, MAIN_AGENT_ID, BOT_NAME, APP_TZ_INVALID, } from '../config.js';
import { resolveOwnerChatId } from '../owner-chat.js';
import { appendTaskRun, listPendingTaskRetries, deletePendingTaskRetry, updatePendingTaskRetry, insertPendingTaskRetryIfNew, markPendingTaskRetryAlert, clearPendingTaskRetryAlert, markScheduledTaskKanbanWaiting, getDb, } from '../db.js';
import { createDispatchSafe } from '../costops/dispatch.js';
import { resolveDispatchIdentitySafe } from '../costops/dispatch-identity.js';
import { toPendingRetryView, classifyTelegramSendError } from '../pending-retries.js';
import { SCHEDULED_TASK_PREAMBLE, wrapScheduledTask, } from '../prompt-safety.js';
...
import { agentSessionName, isAgentRunning, isSessionReadyForPrompt, sendPromptToSession, startAgentProcess, sessionExistsOnHost, capturePane, sendEnterToSession, clearStaleParkedInput, } from './agent-process.js';
import { MAIN_CHANNELS_SESSION } from './main-agent.js';
import { sendTelegramMessage } from './telegram.js';
import { runCommandTask } from './command-task.js';
import { paneShowsContextSaturation, detectsFirstRunGate, detectPaneState } from '../pane-state.js';
...
export function resolveStuckTimeoutMs(task, defaultMs = TASK_FIRE_TIMEOUT_MS, maxMs = TASK_FIRE_MAX_TRACK_MS) {
...
export function decideTaskTimeout(entry, paneState, now, opts) {
    const elapsed = now - entry.injectedAt;
    if (elapsed >= opts.maxTrackMs)
        return 'clear';
    if (paneState === 'idle')
        return 'clear';
    if (entry.alerted)
...
export function decideScheduledResubmitAction(attempt, stuck) {
    if (!stuck)
        return 'none';
...
export function isScheduledPromptStuck(pane, marker) {
    if (!pane || !pane.trim())
        return false;
...
export function decideCatchUp(task, ageMs, lateThresholdMs = LATE_CATCHUP_THRESHOLD_MS) {
    if (ageMs <= lateThresholdMs)
        return 'on-time';
    return ageMs <= catchUpMaxAgeMs(task) ? 'catch-up' : 'stale';
}
/** Pure: where the first post-start scan window begins. */
export function computeCatchUpStart(persistedTickMs, now, maxCatchUpMs = SCHEDULE_MAX_CATCHUP_MS, coldStartMs = SCHEDULE_COLD_START_CATCHUP_MS) {
...
function loadLastTickMs() {
    try {
        const raw = JSON.parse(readFileSync(SCHEDULE_TICK_STATE_PATH, 'utf-8'));
...
function persistLastTickMs(nowMs) {
    try {
...
export function chatIdFromAccessConfig(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const o = raw;
    ...
    if (o.groups && typeof o.groups === 'object') {
        const keys = Object.keys(o.groups);
        if (keys.length > 0)
            return keys[0];
    }
    return null;
...
export function resolveBoundChatId(agentName) {
    const dir = agentName === MAIN_AGENT_ID
        ? channelStateDir('telegram')
        : channelStateDir('telegram', agentDir(agentName));
    try {
        const raw = JSON.parse(readFileSync(join(dir, 'access.json'), 'utf-8'));
...
export function runPreCheck(task) {
    if (!task.preCheck)
        return { skip: false };
...
const lastMcpMissing = new Map();
function mcpMissingReason(taskName, agentName) {
...
async function attemptFireTask(task, agentName, now, preCheckPrefix, lateCatchUpMs) {
    ...
    if (!sessionExistsOnHost(host, session)) {
        ...
        if (!start.ok) {
            ...
            if (/already running/i.test(start.error ?? ''))
                return 'busy';
    ...
    try {
        ...
        const taskBody = preCheckPrefix
            ? `[Pre-check eredmeny]\n${preCheckPrefix}\n\n[Feladat]\n${task.prompt}`
            : task.prompt;
        const fullPrompt = SCHEDULED_TASK_PREAMBLE + '\n' +
            prefix.trimEnd() + '\n\n' +
        ...
        if (lateCatchUpMs != null) {
            ...
            logger.warn({ task: task.name, agent: agentName, session, lateCatchUpMinutes: Math.round(lateCatchUpMs / 60000) }, 'Scheduled task fired via restart catch-up window -- missed its normal tick');
        }
        else {
        ...
        const resubmit = async (attempt, laneBusySkips = 0) => {
            try {
                ...
                const res = await withSessionSendLock(session, host, 'recover', async () => {
...
export function taskInjectionRank(t) {
    if (t.forceSend)
        return 0;
...
export async function runScheduledTaskNow(taskName, opts = {}) {
    ...
    if (!task.enabled && !opts.allowDisabled)
...
function resolveSchedulerAlertToken() {
...
function sendCatchUpSummary(caughtUp, stale, gapMs) {
    ...
    const mins = (ms) => `${Math.round(ms / 60000)} perc`;
...
function sendPendingRetryAlert(view, nowMs) {
...
function sendTaskTimeoutAlert(entry, elapsedMs) {
...
export function startScheduleRunner() {
    ...
    if (cronTzSource === 'system-default' && cronTz === 'UTC') {
        logger.warn({ cronTz }, 'schedule-runner: cron timezone fell back to UTC (no SCHEDULER_TZ/TZ set) -- ' +
    ...
    if (APP_TZ_INVALID) {
        logger.warn({ rejectedTz: APP_TZ_INVALID, cronTz }, `schedule-runner: SCHEDULER_TZ="${APP_TZ_INVALID}" is not a usable timezone -- ` +
    ...
    if (persistedTickMs != null && startupGapMs > SCHEDULE_COLD_START_CATCHUP_MS) {
        logger.warn({ downtimeMinutes: Math.round(startupGapMs / 60000), cappedAtHours: SCHEDULE_MAX_CATCHUP_MS / 3_600_000 }, 'schedule-runner: scheduler was down longer than a tick -- scanning the downtime window for missed occurrences');
    }
    ...
    async function runCheck() {
        ...
        try {
            ...
            for (const row of pendingRows) {
                ...
                if (stillPresent && view.alertDue)
                    sendPendingRetryAlert(view, now);
            }
            ...
            for (const task of tasks) {
                ...
                if (decision === 'stale') {
                    logger.warn({ task: task.name, ageMinutes: Math.round(ageMs / 60000), maxAgeMinutes: catchUpMaxAgeMs(task) / 60000 }, 'Scheduled occurrence missed while the scheduler was down and is too stale to catch up -- recording as missed');
                    staleThisTick.push({ task: task.name, ageMs });
                    const missedTargets = task.agent === 'all'
                        ? [MAIN_AGENT_ID, ...listAgentNames().filter(a => isAgentRunning(a))]
                        : [task.agent || MAIN_AGENT_ID];
                    for (const agentName of missedTargets)
                        appendTaskRun(task.name, agentName, 'missed');
                    continue;
                }
                const lateCatchUpMs = decision === 'catch-up' ? ageMs : undefined;
                if (lateCatchUpMs != null)
                    caughtUpThisTick.push({ task: task.name, ageMs });
