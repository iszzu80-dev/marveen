...
import { getPendingMessages, markMessageDelivered, markMessageDone, markMessageFailed, markPendingFederatedFailed, setMessageResult, createAgentMessage, stampMessageTrace, upsertOtelSpan, getDb, } from '../db.js';
...
import { agentSessionName, isSessionReadyForPrompt, clearStaleParkedInput, sendPromptToSession, sessionExistsOnHost, } from './agent-process.js';
...
const routerLoggedMisses = new Set();
...
const routerInjectFailures = new Map();
...
const routerRetryOfDispatch = new Map();
/**
 ...
 */
export function shouldGiveUpOnInject(failCount, maxFailures) {
 ...
 */
function notifyOrchestratorOfFailedHandoff(msg, reason) {
    try {
        ...
        createAgentMessage('system', MAIN_AGENT_ID, `[handoff-failure] Inter-agent message (id ${msg.id}) ${msg.from_agent} -> ${msg.to_agent} could NOT be delivered: ${reason}. Consider re-sending or checking the target agent. Content preview: ${preview}`);
...
function notifyDelegationFailed(msg, error) {
    try {
        createAgentMessage('system', msg.from_agent, `A(z) ${msg.to_agent} címre küldött föderált üzeneted (#${msg.id}) véglegesen meghiúsult: ${error.slice(0, 200)}. ` +
...
const agentStuckSince = new Map(); // agent -> first tick stuck (Date.now)
 ...
 */
export function shouldAbandon(sessionExists, ageMs, windowMs) {
...
const deliveredTraceCtx = new Map();
function generateTraceId() { return randomUUID(); }
function generateSpanId() { return randomUUID().replace(/-/g, '').slice(0, 16); }
...
function stampTraceOnMessage(msg, nowMs) {
...
export async function deliverFederatedBatch(federated, now) {
    let attempts = 0;
    let abandons = 0;
    ...
    for (const msg of federated) {
        ...
        if (ageMs > abandonMs) {
            ...
            // the next tick.
            if (abandons >= MAX_FEDERATED_PER_TICK)
                continue;
            abandons++;
        ...
        if (attempts >= MAX_FEDERATED_PER_TICK)
            continue;
        let result;
        try {
        ...
        if (result.kind === 'skipped')
            continue; // peer in backoff: no network attempt made
        attempts++;
...
let batchedMsgIdsThisTick = new Set();
/**
 ...
 */
function batchDeliverBacklog(agent, agentPending, now) {
...
export async function runMessageRouterTick() {
    ...
    const localPending = [];
    const federatedPending = [];
    for (const m of allPending)
        (isQualifiedId(m.to_agent) ? federatedPending : localPending).push(m);
    ...
    const receiversInTick = new Set();
    for (const m of pending) {
        if (m.to_agent !== MAIN_AGENT_ID)
            receiversInTick.add(m.to_agent);
    }
    const absentNow = new Set();
        ...
        if (agentWasAbsent.has(agent) && !agentBatchedThisReconnect.has(agent)) {
            ...
            if (agentPending.length > RECONNECT_BATCH_THRESHOLD) {
                ...
                if (oldestAge > RECONNECT_BATCH_AGE_MS) {
                    logger.warn({ agent, pendingCount: agentPending.length, oldestAgeMs: oldestAge }, 'message-router: reconnect-backlog batch — summarizing old messages');
    ...
    for (const msg of pending) {
        ...
        if (batchedMsgIdsThisTick.has(msg.id))
            continue;
        ...
        const recordDispatchOutcome = (outcome, evidence, retryOf = null) => {
            if (!dispatchId || dispatchOutcomeWrittenThisTick)
                return;
        ...
        try {
            ...
            try {
                ...
                if (gateCheck.shouldBlock) {
                    ...
                    logger.warn({ id: msg.id, to: msg.to_agent, audit: gateCheck.auditEntry }, 'data-sensitivity-gate: BLOCKED restricted content to non-trusted provider');
                ...
                if (!shouldGiveUpOnInject(failCount, MAX_INJECT_FAILURES)) {
                    ...
                    recordDispatchOutcome('retry', `message-router:inject-threw-attempt-${failCount}-of-${MAX_INJECT_FAILURES}-will-retry`, priorAttempt && priorAttempt !== dispatchId ? priorAttempt : null);
                    if (dispatchId)
                        routerRetryOfDispatch.set(msg.id, dispatchId);
                    continue;
                }
...
function extractVoiceFileId(content) {
    if (!content.includes('attachment_kind="voice"'))
        return null;
...
function injectTranscript(content, transcript) {
    ...
    result = result.replace(/(<channel[^>]*>)[\s\S]*?(<\/channel>)/, (_m, open, close) => `${open}\n[Hang átirat]: ${transcript}\n${close}`);
    return result;
}
...
async function callVoiceSTT(fileId, agentId) {
    try {
        ...
        if (!existsSync(join(resolvedDir, '.env')))
            return null;
