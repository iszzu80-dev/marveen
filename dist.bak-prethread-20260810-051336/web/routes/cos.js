...
import { draftZstSend, approveZstSend, rejectZstSend, dispatchZstSend, renderedPayloadHash as zstPayloadHash, } from '../../cos/zst-send.js';
import { createEscalation, getEscalation, listOpenEscalations, transitionEscalation, isHardGated, } from '../../cos/zst-productlab.js';
...
export async function tryHandleCos(ctx) {
    ...
    if (path === '/api/cos/zst-outbound/draft' && method === 'POST') {
        ...
        if (!b.caseId || !b.to || !b.subject || !b.body) {
            json(res, { error: 'caseId, to, subject, body required' }, 400);
            return true;
        }
        try {
    ...
    if (path === '/api/cos/zst-outbound/approve' && method === 'POST') {
        ...
        try {
            json(res, await approveAndDispatchZst(getDb(), b.ledgerId, b.renderedPayloadHash, b.approvedBy ?? 'istvan', b.allowedRecipients, Math.floor(Date.now() / 1000)));
        }
        catch (e) {
            json(res, { error: String(e.message) }, 400);
        }
    ...
    if (path === '/api/cos/zst-escalations/transition' && method === 'POST') {
        ...
        if (!b.escalationId || !b.status) {
            json(res, { error: 'escalationId, status required' }, 400);
            return true;
        }
        ...
        try {
            json(res, transitionEscalation(getDb(), b.escalationId, b.status, b.actor ?? 'marveen', Math.floor(Date.now() / 1000)));
        }
        catch (e) {
            json(res, { error: String(e.message) }, 400);
        }
...
export function escalationNeedsIstvanInPerson(esc, targetStatus) {
    if (!esc || targetStatus !== 'ACCEPTED')
        return false;
    return isHardGated(esc);
}
...
export async function approveAndDispatchZst(db, ledgerId, seenPayloadHash, approvedBy, allowedRecipients, now) {
    const row = db.prepare(`SELECT l.payload AS payload, l.campaign_id AS campaign_id, l.case_id AS case_id,
