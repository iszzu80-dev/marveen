...
function scanSkillDir(dir, source, deletable) {
    if (!existsSync(dir))
        return [];
...
export async function tryHandleAgentsSkills(ctx) {
    ...
    if (skillImportMatch && method === 'POST') {
        ...
        if (!name) {
            json(res, { error: 'Invalid agent name' }, 400);
            return true;
        }
        if (!agentExistsFor(name)) {
    ...
    if (skillActionMatch && method === 'DELETE') {
        ...
        if (!name || !skillName) {
            json(res, { error: 'Invalid agent or skill name' }, 400);
            return true;
        }
        if (!agentExistsFor(name)) {
            json(res, { error: 'Agent not found' }, 404);
