...
export const KNOWN_HOOK_SCRIPTS = [
    'taskstate-replay.py',
...
export function isWorktreeRoot(projectRoot, deps = {}) {
    const normalized = normalizeSeparators(projectRoot);
    if (normalized.includes(WORKTREES_FRAGMENT))
        return true;
...
export function isTemporaryRoot(projectRoot, deps = {}) {
    const normalized = normalizeSeparators(projectRoot);
...
function ourScriptPaths(command, knownScripts) {
    const paths = [];
...
export function pruneStaleHookEntries(settings, opts) {
        ...
        for (const group of groups) {
            ...
            const keptHooks = group.hooks.filter((h) => {
                ...
                if (scriptPaths.length === 0)
                    return true; // foreign entry: never touch
...
export function pruneStaleHooksFromSettingsFile(settingsPath) {
