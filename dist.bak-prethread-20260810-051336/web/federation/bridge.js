...
import { getFederationConfig, FEDERATION_MIN_TOKEN_LENGTH } from './config.js';
...
export function isPeerInBackoff(peerId, now) {
    const st = peerBackoff.get(peerId);
    return !!st && now < st.nextAttemptAt;
}
function noteFailure(peerId, now) {
 ...
 *  clear everything. */
export function resetPeerBackoff(peerId) {
    if (peerId === undefined)
        peerBackoff.clear();
    else
        peerBackoff.delete(peerId);
...
/** Test seam. */
export function _resetBackoffForTest() {
    peerBackoff.clear();
}
 ...
 */
export async function sendFederatedMessage(msg, now, fetchImpl = fetch) {
    const target = parseQualifiedId(msg.to_agent);
    if (!target)
        return { kind: 'failed', error: `Invalid federated address: ${truncate(msg.to_agent, 80)}` };
    const cfg = getFederationConfig();
    if (!cfg.enabled)
        return { kind: 'failed', error: 'Federation is disabled on this system' };
    ...
    const peer = cfg.peers.find((p) => p.id === targetSystem);
    if (!peer)
        return { kind: 'failed', error: `Unknown federation peer '${target.system}'` };
    ...
    if (isPeerInBackoff(peer.id, now))
        return { kind: 'skipped' };
    let res;
    try {
    ...
    let bodyText = '';
    try {
        bodyText = await readBoundedBody(res, INBOX_REPLY_MAX_BYTES);
    }
    catch { /* body is best-effort for diagnostics */ }
    if (res.ok) {
        noteSuccess(peer.id);
        let remoteId = '';
        try {
            remoteId = String(JSON.parse(bodyText).id ?? '');
        }
        catch { /* non-JSON 2xx still counts */ }
...
export function logFedOut(fields, msg) {
