...
import { capturePane, isSessionReadyForPrompt, sendPromptToSession, sessionExistsOnHost, hasFleetOauthToken, FLEET_OAUTH_TOKEN_PATH, } from './agent-process.js';
...
export function makeWorkerCtx(session, homeDir) {
    return {
        session,
        home: homeDir,
 ...
 */
export function workerHomeFor(agentId, variant) {
 ...
 */
export function workerStartAllowed(env = process.env) {
...
const ctxSlow = makeWorkerCtx(process.env.MARVEEN_WORKER_SESSION || `${MAIN_AGENT_ID}-worker`, process.env.MARVEEN_WORKER_DIR || workerHomeFor(MAIN_AGENT_ID, 'slow'));
...
const ctxFast = makeWorkerCtx(process.env.MARVEEN_WORKER_SESSION_FAST || `${MAIN_AGENT_ID}-worker-fast`, process.env.MARVEEN_WORKER_DIR_FAST || workerHomeFor(MAIN_AGENT_ID, 'fast'));
...
export function classifyPriority(message) {
    if (message.length >= FAST_MESSAGE_MAX_LEN)
        return 'slow';
    if (SLOW_MESSAGE_KEYWORDS.test(message))
        return 'slow';
    return 'fast';
}
// =============================================================================
...
const WORKER_AUTH_FAILURE_RX = /Please run \/login|Not logged in|Invalid bearer token|Invalid authentication credentials|API Error:\s*401|OAuth (?:token|authentication) (?:has )?expired|Failed to authenticate/i;
/**
 ...
 */
export function configDirKeychainService(configDir) {
    const suffix = createHash('sha256').update(configDir).digest('hex').slice(0, 8);
    return `Claude Code-credentials-${suffix}`;
}
function workerKeychainService(ctx) {
...
function clearWorkerKeychainEntry(ctx) {
    try {
        execFileSync('/usr/bin/security', ['delete-generic-password', '-s', workerKeychainService(ctx), '-a', userInfo().username], { timeout: 3000, stdio: ['ignore', 'ignore', 'ignore'] });
    }
 ...
 */
function seedWorkerCredentials(ctx) {
    if (process.platform === 'darwin')
        clearWorkerKeychainEntry(ctx);
...
function workerPaneHasAuthFailure(ctx) {
 ...
 */
export function buildWorkerPrompt(callerPrompt, outPath, donePath) {
    return [
        callerPrompt,
        '',
        '---',
...
export function classifyWorkerPane(pane) {
    if (pane == null || pane.trim() === '')
        return 'empty';
    const tail = pane.split('\n').slice(-30).join('\n');
    if (WORKER_AUTH_FAILURE_RX.test(tail))
        return 'auth';
    const st = detectPaneState(pane);
    if (st === 'busy')
        return 'busy';
    if (st === 'idle' || st === 'typing')
        return 'idle';
    ...
    if (/Enter to confirm|Esc to cancel|❯\s*1\./.test(pane))
        return 'modal';
    return 'unknown';
}
/** Pure decision: should the boot poll attempt a self-heal for this pane class? */
export function shouldSelfHeal(cls) {
...
// --- per-session mutex ---------------------------------------------------------
function withWorkerLockFor(ctx, fn) {
 ...
 */
export function ensureWorkerCwd(ctx = ctxSlow) {
    if (!existsSync(ctx.home))
        mkdirSync(ctx.home, { recursive: true });
    if (!existsSync(ctx.scratchDir))
    ...
    if (existsSync(realClaude)) {
        ...
        if (skipSharedCreds) {
            ...
            if (lstatSyncSafe(credsLink)?.isSymbolicLink())
                rmSync(credsLink, { force: true });
        }
        for (const entry of readdirSync(realClaude)) {
            if (WORKER_CONFIG_SKIP.has(entry))
                continue;
            if (skipSharedCreds && entry === '.credentials.json')
                continue;
    ...
    for (const p of WORKER_DISABLED_PLUGINS)
    ...
    try {
        ...
        const parsed = existsSync(homeClaudeJson) ? JSON.parse(readFileSync(homeClaudeJson, 'utf-8')) : {};
        ...
        const keys = new Set([ctx.home]);
        try {
            keys.add(realpathSync(ctx.home));
        }
 ...
 */
export function stampWorkerFirstRun(parsed) {
...
// --- session lifecycle ---------------------------------------------------------
function workerSessionExists(ctx) {
 ...
 */
function startWorkerSessionFor(ctx) {
    ...
    const launch = (hasFleetOauthToken() ? `export CLAUDE_CODE_OAUTH_TOKEN="$(cat ${shArg(FLEET_OAUTH_TOKEN_PATH)})"; ` : '') +
 ...
 */
export function workerContexts() {
    return [ctxSlow, ctxFast];
}
export function isWorkerSessionAlive(session) {
 ...
 */
function logWorkerClaudeVersion(ctx) {
    try {
 ...
 */
function selfHealWorkerOnce(ctx) {
    const cls = classifyWorkerPane(capturePane(ctx.session));
    if (!shouldSelfHeal(cls))
        return false;
    ...
    for (let i = 0; i < WORKER_SELF_HEAL_MAX_ESCAPES; i++) {
        try {
            execFileSync(TMUX, ['send-keys', '-t', ctx.session, 'Escape'], { timeout: 5000 });
        }
        catch {
            break;
        }
...
function alertWorkerStuck(ctx, paneTail) {
    ...
    if (Date.now() - ctx.lastStuckAlert < WORKER_STUCK_ALERT_COOLDOWN_MS)
        return;
    ctx.lastStuckAlert = Date.now();
    void notifyChannel(`⚠️ Marveen worker [${ctx.session}]: a hatter-worker session nem all keszen (beragadt dialogus vagy ismeretlen kepernyo). Onjavitas lefutott (Escape + restart), de a keszenlet nem allt helyre. Erintett: agens-generalas, capability-osszefoglalo, heartbeat, digest. Nezz ra: tmux attach -t ${ctx.session}`).catch(() => { });
}
async function ensureWorkerReady(ctx) {
    ...
    while (Date.now() < deadline) {
        if (await isSessionReadyForPrompt(ctx.session))
            return true;
        if (!healed && Date.now() - start > WORKER_SELF_HEAL_GRACE_MS) {
            ...
            try {
                ...
                logger.warn({ err }, 'agent-worker: self-heal pass failed');
            }
        }
        await sleepMs(2000);
    }
...
function restartWorkerSession(ctx) {
    ...
    try {
        execFileSync(TMUX, ['kill-session', '-t', ctx.session], { timeout: 5000 });
    }
    catch { /* not running */ }
    try {
        startWorkerSessionFor(ctx);
        ...
        logger.warn({ err, session: ctx.session }, 'agent-worker: restart failed');
    }
}
// Reset context between requests so unrelated one-shots never share/grow context.
function clearWorkerContext(ctx) {
    try {
...
async function runWorkerAttempt(ctx, message, timeoutMs) {
    ...
    for (const p of [outPath, donePath]) {
        try {
            rmSync(p, { force: true });
        }
        catch { /* none */ }
    ...
    try {
        while (true) {
            ...
            if (decision === 'timeout') {
                ...
                if (dispatchId)
                    recordOutcomeSafe(getDb(), { dispatchId, outcome: 'failed', evidence: 'agent-worker:request-timeout' });
            ...
            if (decision === 'dead') {
                ...
                if (dispatchId)
                    recordOutcomeSafe(getDb(), { dispatchId, outcome: 'failed', evidence: 'agent-worker:session-died-mid-request' });
                restartWorkerSession(ctx);
 ...
 */
export async function runViaWorker(message, timeoutMs, priority) {
