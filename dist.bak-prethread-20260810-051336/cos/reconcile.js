...
const zstFrozenCases = (db) => {
    let rows = [];
    try {
        rows = db.prepare(`SELECT z.status AS status, COUNT(*) AS n, MAX(s.updated_at) AS last_change
       FROM zst_cases z
       ...
       GROUP BY z.status ORDER BY COUNT(*) DESC`).all();
    }
    catch {
        return null;
    }
...
const zstStuckSending = (db, now) => {
    const n = count(db, `SELECT COUNT(*) AS n FROM zst_outbound_ledger WHERE status='SENDING' AND sending_at < ?`, now - 3600);
    if (n === null)
        return null;
    if (n === 0)
        return null;
...
const zstMessagesWithoutThread = (db) => {
    const n = count(db, `SELECT COUNT(*) AS n FROM zst_email_processing WHERE thread_id IS NULL OR thread_id = ''`);
    if (n === null || n === 0)
        return null;
    return {
...
const stagnantCases = (db) => {
    ...
    try {
        ...
        rows = db.prepare(`SELECT s.domain AS domain, s.case_id AS case_id, s.no_progress_run_count AS n,
       ...
       ORDER BY s.no_progress_run_count DESC`).all(STAGNATION_RUNS, ...ENGINE_ACTIONABLE);
    }
    catch {
        return null;
    }
...
const awaitingOwnerTooLong = (db) => {
    let rows = [];
    try {
        rows = db.prepare(`SELECT s.case_id AS case_id, s.no_progress_run_count AS n, p.title AS title
       FROM case_progression_state s
       ...
       ORDER BY s.no_progress_run_count DESC`).all(OWNER_DECISION_RUNS);
    }
    catch {
        return null;
    }
