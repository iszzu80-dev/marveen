 ...
 */
export function followUpEligibility(db, caseId, now) {
    const c = db.prepare(`SELECT case_id, title, status, waiting_on, next_action_owner, follow_up_at, gmail_thread_ids
     FROM personal_cases WHERE case_id = ?`).get(caseId);
    if (!c)
        return { eligible: false, code: 'case_not_waiting', reason: `nincs ilyen ügy: ${caseId}` };
    ...
    const pending = db.prepare(`SELECT COUNT(*) AS n FROM outbound_ledger WHERE case_id = ? AND status = 'PLANNED'`).get(caseId);
    if (pending.n > 0) {
    ...
    const sent = db.prepare(`SELECT COUNT(*) AS n FROM outbound_ledger
     WHERE case_id = ? AND outbound_kind = 'FOLLOW_UP' AND status NOT IN ('CANCELLED','FAILED_TERMINAL')`).get(caseId);
 ...
 */
export function draftFollowUp(c) {
...
export function sweepFollowUpCandidates(db, now, limit = 10) {
    const rows = db.prepare(`SELECT case_id, title, status, waiting_on, next_action_owner, follow_up_at, gmail_thread_ids
     FROM personal_cases
     WHERE archived_at IS NULL AND status IN ('WAITING_EXTERNAL','FOLLOW_UP_DUE')
     ORDER BY follow_up_at LIMIT ?`).all(limit * 3);
    const eligible = [];
    const skipped = [];
    for (const r of rows) {
        const e = followUpEligibility(db, r.case_id, now);
        if (!e.eligible) {
            skipped.push({ caseId: r.case_id, code: e.code, reason: e.reason });
            continue;
        }
        const thread = (() => { try {
            return JSON.parse(r.gmail_thread_ids ?? '[]')[0];
        }
        catch {
            return undefined;
        } })();
