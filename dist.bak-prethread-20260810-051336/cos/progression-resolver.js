...
export class CrossDomainReadError extends Error {
    ...
    constructor(claimedDomain, caseId, source) {
...
export function domainGuard(db, claimedDomain, caseId, source) {
    ...
    if (exists)
        return; // OK — case is in the claimed domain
...
export function resolveEmailThread(db, domain, caseId) {
    ...
    const caseRow = db.prepare(`SELECT gmail_thread_ids, source_references FROM ${tableName} WHERE case_id = ?`).get(caseId);
    if (!caseRow)
        return { thread_ids: [], messages: [], has_history: false };
    ...
    try {
        ...
        if (caseRow.gmail_thread_ids)
            threadIds = [caseRow.gmail_thread_ids];
    }
    ...
    if (threadIds.length > 0) {
        ...
        const rows = db.prepare(`SELECT message_id, thread_id, gmail_account_id, status, case_id, created_at
       FROM ${epTable} WHERE message_id IN (${placeholders})
       ORDER BY created_at ASC`).all(...sourceRefs);
        for (const row of rows) {
            messages.push({
                message_id: row.message_id,
                thread_id: row.thread_id,
...
export function resolveMemory(db, domain, caseTitle, caseType, description) {
...
export function resolveContextDeep(db, domain, caseId, now) {
    ...
    const caseRow = db.prepare(`SELECT title, case_type, description, status, sensitivity, parent_case_id, created_at
     FROM ${tableName} WHERE case_id = ?`).get(caseId);
    if (!caseRow) {
