...
export function completionActor(actor) {
    return actor === PROGRESSION_ENGINE_ACTOR ? 'ENGINE' : 'OWNER';
}
// ── Internal helpers ──────────────────────────────────────────────────────
const DEFAULT_DOD_VERIFICATION = {
...
function parseVerification(raw) {
    ...
    try {
        ...
        if (!parsed || !Array.isArray(parsed.criteria))
            return null;
        const v = parsed;
        if (v.provenance !== 'CASE_SPECIFIC')
            v.provenance = 'GENERIC_STATUS_TEMPLATE';
...
export function initializeDoDVerification(db, domain, caseId, criteria, provenance, now) {
    domainGuard(db, domain, caseId, 'initializeDoDVerification');
    // Idempotent: if already initialised, return existing
    const existing = db.prepare('SELECT dod_verification_json FROM case_progression_state WHERE domain = ? AND case_id = ?').get(domain, caseId);
    ...
    const criteriaStates = criteria.map(c => ({
        label: c,
        met: false,
        met_at: null,
        met_by_run: null,
    ...
    db.prepare(`UPDATE case_progression_state
     ...
     WHERE domain = ? AND case_id = ?`).run(JSON.stringify(verification), now, domain, caseId);
    return verification;
}
 ...
 *
 *  Domain-scoped. */
export function satisfyDoDCriterion(db, domain, caseId, criterionIndex, runId, evidence, now) {
    ...
    if (typeof evidence !== 'string' || evidence.trim() === '')
        return false;
    const row = db.prepare('SELECT dod_verification_json FROM case_progression_state WHERE domain = ? AND case_id = ?').get(domain, caseId);
    if (!row)
        return false;
    const verification = parseVerification(row.dod_verification_json);
    if (!verification)
        return false;
    if (criterionIndex < 0 || criterionIndex >= verification.criteria.length)
        return false;
    const c = verification.criteria[criterionIndex];
    if (c.met)
        return false; // already met — idempotent no-op
    c.met = true;
    c.met_at = now;
    c.met_by_run = runId;
 ...
 *
 *  Domain-scoped. */
export function satisfyNextDoDCriterionWithEvidence(db, domain, caseId, runId, evidence, now) {
 ...
 *  Domain-scoped. */
export function evaluateDoDCompleteness(db, domain, caseId) {
    ...
    // satisfied now.
    const proven = (c) => c.met && typeof c.met_by_evidence === 'string' && c.met_by_evidence !== '';
 ...
 *  Read-only — no mutation.
 *  Domain-scoped. */
export function canCompleteCase(db, domain, caseId, by = 'ENGINE') {
    ...
    const state = db.prepare(`SELECT progression_enabled, dod_verification_json
    ...
    const provenance = verification?.provenance ?? 'GENERIC_STATUS_TEMPLATE';
 ...
 *  Domain-scoped. */
export function guardCaseCompletion(db, domain, caseId, by = 'ENGINE') {
