...
import { findDueCases, tryClaimProgression, releaseProgressionClaim, } from './progression-scheduler.js';
...
export function runProgressionHeartbeat(db, now = Math.floor(Date.now() / 1000), maxPerDomain = 50) {
    const result = {
        personal: 0,
        zst: 0,
        errors: [],
    ...
    for (const domain of ['personal', 'zst']) {
        ...
        for (const dc of due) {
            ...
            try {
                ...
                result.cycleErrors++;
                result.errors.push(`${domain}/${dc.case_id}: ${err.message}`);
            }
            finally {
                ...
                try {
                    ...
                    const held = db.prepare(`SELECT claim_fence FROM case_claims WHERE claim_key=? AND owner_run_id=?`)
                        .get(key, runId);
                    if (held)
                        releaseClaim(db, { claimKey: key, ownerRunId: runId, fence: held.claim_fence });
                }
