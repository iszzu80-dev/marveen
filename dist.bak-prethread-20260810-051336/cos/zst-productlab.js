...
export function createEscalation(db, input, now) {
    db.prepare(`INSERT INTO zst_product_escalations
...
export function getEscalation(db, escalationId) {
    const r = db.prepare(`SELECT * FROM zst_product_escalations WHERE escalation_id=?`).get(escalationId);
    if (!r)
        return undefined;
    return { ...r, hard_gate: isHardGated(r) };
}
const ALLOWED = {
...
export function transitionEscalation(db, escalationId, to, actor, now) {
...
export function listOpenEscalations(db) {
    const rows = db.prepare(`SELECT * FROM zst_product_escalations WHERE status NOT IN ('ACCEPTED','REJECTED','CANCELLED') ORDER BY created_at`).all();
    return rows.map(r => ({ ...r, hard_gate: isHardGated(r) }));
}
