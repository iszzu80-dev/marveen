...
export function initCosSchema(db) {
  ...
  `);
    ...
    if (_obExists) {
        const _cols = db.prepare(`PRAGMA table_info(outbound_ledger)`).all();
...
export function initCosDocumentsSchema(db) {
    db.exec(`
...
export function initZstSchema(db) {
  ...
  `);
    ...
    for (const [name, ev] of [['zevents_no_update', 'UPDATE'], ['zevents_no_delete', 'DELETE']]) {
    ...
    for (const [name, ev] of [['pcevents_no_update', 'UPDATE'], ['pcevents_no_delete', 'DELETE']]) {
        db.exec(`DROP TRIGGER IF EXISTS ${name}`);
...
export function initZstSendSchema(db) {
    db.exec(`
...
export function initZstFinanceSchema(db) {
    db.exec(`
...
export function initZstContractsSchema(db) {
    db.exec(`
...
export function initZstCommercialSchema(db) {
    db.exec(`
...
export function initZstProductLabSchema(db) {
...
export function initProgressionSchema(db) {
