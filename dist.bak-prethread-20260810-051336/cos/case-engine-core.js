...
export class CaseConcurrencyError extends Error {
    caseId;
    seenVersion;
    constructor(caseId, seenVersion) {
...
export function makeCaseEngine(tables, defaults, statusSets = PERSONAL_STATUS_SETS) {
    const T = tables;
    const TERMINAL_STATUSES = statusSets.terminal;
    const ATTENTION_STATUSES = statusSets.attention;
    function appendCaseEvent(db, ev, now) {
        const info = db.prepare(`INSERT INTO ${T.events}
       ...
       VALUES (@caseId, @caseVersion, @actor, @sourceSystem, @sourceReference, @eventType,
          @previousStatus, @newStatus, @reason, @payload, @correlationId, @now)`).run({
            caseId: ev.caseId,
    ...
    function getCase(db, caseId) {
        return db.prepare(`SELECT * FROM ${T.cases} WHERE case_id = ?`).get(caseId);
    }
    function createCase(db, input, now) {
        const status = input.status ?? defaults.status;
        const actor = input.actor ?? input.owner ?? defaults.actor;
        const tx = db.transaction(() => {
            db.prepare(`INSERT INTO ${T.cases}
         ...
         VALUES (@caseId, 1, @title, @description, @caseType, @category, @status, @priority,
            @owner, @sensitivity, @sourceSystem, @sourceReference, @now, @now)`).run({
                caseId: input.caseId,
                title: input.title,
    ...
    function listActiveCases(db) {
        const placeholders = TERMINAL_STATUSES.map(() => '?').join(',');
        return db.prepare(`SELECT ${LIST_COLUMNS} FROM ${T.cases}
       WHERE archived_at IS NULL AND status NOT IN (${placeholders})
       ORDER BY ${PRIORITY_ORDER}, updated_at DESC`).all(...TERMINAL_STATUSES);
    }
    function listTodayCases(db, horizonSec) {
       ...
       ORDER BY ${PRIORITY_ORDER}, COALESCE(due_at, follow_up_at, next_wake_at, updated_at) ASC`).all(...TERMINAL_STATUSES, horizonSec, horizonSec, horizonSec, ...ATTENTION_STATUSES);
    }
    function transitionCase(db, input, now) {
        const tx = db.transaction(() => {
            ...
            const setParts = ['status = @newStatus', 'version = version + 1', 'updated_at = @now'];
            const params = {
            ...
            for (const key of PATCHABLE_COLUMNS) {
                if (key in patch) {
                    setParts.push(`${key} = @p_${key}`);
                    params[`p_${key}`] = patch[key] ?? null;
                }
            }
            const info = db.prepare(`UPDATE ${T.cases} SET ${setParts.join(', ')}
         WHERE case_id = @caseId AND version = @seenVersion`).run(params);
            if (info.changes === 0)
                throw new CaseConcurrencyError(input.caseId, input.seenVersion);
    ...
    function acquireClaim(db, args, now) {
        db.prepare(`INSERT INTO ${T.claims} (claim_key, owner_run_id, claim_fence, claimed_at, claim_expires_at)
       ...
       WHERE ${T.claims}.claim_expires_at < @now`).run({ key: args.claimKey, owner: args.ownerRunId, now, expires: now + args.ttlSeconds });
    ...
    function releaseClaim(db, args) {
        const info = db.prepare(`DELETE FROM ${T.claims} WHERE claim_key = @key AND owner_run_id = @owner AND claim_fence = @fence`).run({ key: args.claimKey, owner: args.ownerRunId, fence: args.fence });
