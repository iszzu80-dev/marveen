...
export function draftZstSend(db, input, now) {
    ...
    const version = db.prepare(`SELECT version FROM zst_campaigns WHERE campaign_id = ?`).get(campaignId)?.version ?? 1;
    db.prepare(`UPDATE zst_outbound_ledger SET campaign_id = @c, campaign_version = @v, updated_at = @now
     WHERE ledger_id = @id`).run({ c: campaignId, v: version, now, id: planned.ledgerId });
