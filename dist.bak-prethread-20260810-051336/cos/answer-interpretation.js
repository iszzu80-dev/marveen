...
export function buildAnswerPrompt(ctx, answer) {
    return [
...
function coerce(raw) {
    ...
    if (!ballHolder || !nextAction || !basis)
        return null;
 ...
 */
export async function interpretOwnerAnswer(client, ctx, answer) {
    const text = (answer ?? '').trim();
    if (!text)
        return { ok: false, reason: 'üres válasz' };
    if (text.length > 4000)
        return { ok: false, reason: 'túl hosszú válasz az értelmezéshez' };
    let raw;
    try {
        raw = await client.complete(SYSTEM_PROMPT, buildAnswerPrompt(ctx, text));
    }
    catch (e) {
        return { ok: false, reason: `a modell nem válaszolt: ${e.message}` };
    }
    let parsed;
    try {
    ...
    if (!proposal)
        return { ok: false, reason: 'a modell válaszából hiányoznak a kötelező mezők' };
...
export function ballMoved(ctx, p) {
