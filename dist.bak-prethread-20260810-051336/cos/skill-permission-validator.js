...
export const BENIGN_CAPABILITIES = new Set([
...
export function validateSkillPermissions(meta) {
    const violations = [];
...
export function parseSkillFrontmatter(md) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (key === 'name')
            meta.name = val;
        else if (key === 'sensitive_approved')
            meta.sensitiveApproved = /^(true|yes)$/i.test(val);
...
export function validateSkillMd(md) {
