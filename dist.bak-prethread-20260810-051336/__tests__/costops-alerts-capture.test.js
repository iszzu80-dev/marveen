...
import { classifyErrorCode, gatherAlertCandidates, captureAlerts, listAlerts, } from '../costops/alerts-capture.js';
...
function insertSource(db, id, provider, source_type = 'usage') {
...
describe('gatherAlertCandidates -- missing invoice (GAP-14 wiring)', () => {
    beforeEach(() => { initDb(); });
    function insertInvoice(db, sourceId, periodEnd) {
        db.prepare(`
...
describe('captureAlerts persistence round-trip', () => {
    ...
    it('dedups on a second capture with the same condition -- no duplicate row, only a touch', () => {
        ...
        const rows = db.prepare(`SELECT COUNT(*) as n FROM costops_alerts WHERE type = 'budget_threshold'`).get();
