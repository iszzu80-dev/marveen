...
import { initializeDoDVerification, satisfyDoDCriterion, satisfyNextDoDCriterionWithEvidence, evaluateDoDCompleteness, canCompleteCase, guardCaseCompletion, completionActor, PrematureCompletionError, } from '../cos/progression-completion.js';
import { runProgressionCycle } from '../cos/progression-pipeline.js';
import { CrossDomainReadError } from '../cos/progression-resolver.js';
import { transitionCase } from '../cos/case-store.js';
...
function seedProgressionState(db, domain, caseId, overrides = {}) {
    ...
    db.prepare(`INSERT INTO case_progression_state
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(domain, caseId, overrides.progression_enabled ?? 1, overrides.progression_mode ?? 'shadow', overrides.definition_of_done_json ?? null, overrides.dod_verification_json ?? null, overrides.goal ?? null, t, t);
}
function seedCase(db, domain, caseId, status = 'NEW') {
    const t = now();
    if (domain === 'personal') {
        db.prepare(`INSERT INTO personal_cases (case_id, title, case_type, status, sensitivity, version, created_at, updated_at)
       VALUES (?, 'Test case', 'ADMIN', ?, 'PERSONAL', 1, ?, ?)`).run(caseId, status, t, t);
    }
    else {
        db.prepare(`INSERT INTO zst_cases (case_id, title, case_type, status, sensitivity, version, created_at, updated_at)
       VALUES (?, 'Test case', 'ADMIN', ?, 'ZST_INTERNAL', 1, ?, ?)`).run(caseId, status, t, t);
    }
}
...
function initDoD(db, domain, caseId, criteria, t, provenance = 'CASE_SPECIFIC') {
    return initializeDoDVerification(db, domain, caseId, criteria, provenance, t);
}
function satisfy(db, domain, caseId, idx, runId, t, evidence = `case_event:${runId}-${idx}`) {
    return satisfyDoDCriterion(db, domain, caseId, idx, runId, evidence, t);
}
function satisfyNext(db, domain, caseId, runId, t, evidence = `case_event:${runId}`) {
...
describe('Checkpoint E.4 — Semantic Completion', () => {
    describe('initializeDoDVerification()', () => {
        ...
        it('creates dod_verification_json with all criteria as unmet', () => {
            ...
            const row = db.prepare('SELECT dod_verification_json FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-001');
            const parsed = JSON.parse(row.dod_verification_json);
        ...
        it('works for ZST domain', () => {
            ...
            const row = db.prepare('SELECT dod_verification_json FROM case_progression_state WHERE domain = ? AND case_id = ?').get('zst', 'zst-001');
        ...
        it('throws CrossDomainReadError for cross-domain case', () => {
            ...
            expect(() => initDoD(db, 'personal', 'zst-cross', ['Test'], now())).toThrow(CrossDomainReadError);
        });
    });
    ...
    describe('satisfyDoDCriterion()', () => {
        ...
        it('throws CrossDomainReadError for cross-domain case', () => {
            ...
            expect(() => satisfy(db, 'personal', 'zst-cross', 0, 'run-1', t)).toThrow(CrossDomainReadError);
        });
    });
    ...
    describe('RED-PROOF: DoD not met → cannot complete', () => {
        ...
        it('3 criteria, 2 met — canCompleteCase returns false', () => {
            ...
            initDoD(db, 'personal', 'pri-red-1', ['Evidence A produced', 'Evidence B verified', 'Stakeholder C notified'], t);
        ...
        it('3 criteria, 3 met — canCompleteCase returns true', () => {
            ...
            initDoD(db, 'personal', 'pri-red-2', ['Evidence A produced', 'Evidence B verified', 'Stakeholder C notified'], t);
        ...
        it('guardCaseCompletion throws when DoD not met', () => {
            ...
            // B still unmet
            expect(() => guardCaseCompletion(db, 'personal', 'pri-red-3')).toThrow(PrematureCompletionError);
        });
        it('guardCaseCompletion does NOT throw when DoD is met', () => {
            ...
            expect(() => guardCaseCompletion(db, 'personal', 'pri-red-4')).not.toThrow();
        });
        it('RED-PROOF: single unmet criterion among 5 blocks completion', () => {
            const t = now();
            seedCaseWithProgression(db, 'personal', 'pri-red-5');
            initDoD(db, 'personal', 'pri-red-5', ['C1', 'C2', 'C3', 'C4', 'C5'], t);
            // Satisfy 4 of 5
    ...
    describe('RED-PROOF: Case close path guarded (transitionCase/transitionZstCase)', () => {
        ...
        it('transitionCase blocks the ENGINE closing on an unmet DoD', () => {
            ...
            // 'B' is still unmet
            expect(() => transitionCase(db, {
                caseId: 'pri-close-1',
                newStatus: 'COMPLETED',
                actor: 'progression-engine',
                seenVersion: 1,
            }, t + 10)).toThrow(PrematureCompletionError);
        });
        ...
        it('transitionCase allows COMPLETED when DoD is met (progression-enabled case)', () => {
            ...
            const row = db.prepare('SELECT status, completed_at FROM personal_cases WHERE case_id = ?').get('pri-close-2');
    ...
    describe('Domain-scoped — CrossDomainReadError', () => {
        ...
        it('initializeDoDVerification throws CrossDomainReadError for cross-domain', () => {
            ...
            expect(() => initDoD(db, 'personal', 'zst-dod-err', ['Test'], t)).toThrow(CrossDomainReadError);
        });
        it('satisfyDoDCriterion throws CrossDomainReadError for cross-domain', () => {
            ...
            expect(() => satisfy(db, 'personal', 'zst-dod-err2', 0, 'r1', t)).toThrow(CrossDomainReadError);
        });
        it('evaluateDoDCompleteness throws CrossDomainReadError for cross-domain', () => {
            seedCaseWithProgression(db, 'zst', 'zst-eval-err');
            expect(() => evaluateDoDCompleteness(db, 'personal', 'zst-eval-err')).toThrow(CrossDomainReadError);
        });
        it('canCompleteCase throws CrossDomainReadError for cross-domain', () => {
            seedCaseWithProgression(db, 'zst', 'zst-gate-err');
            expect(() => canCompleteCase(db, 'personal', 'zst-gate-err')).toThrow(CrossDomainReadError);
        });
        it('satisfyNextDoDCriterionWithEvidence throws CrossDomainReadError for cross-domain', () => {
            ...
            expect(() => satisfyNext(db, 'personal', 'zst-auto-err', 'r1', t)).toThrow(CrossDomainReadError);
        });
    });
