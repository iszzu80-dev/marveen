...
import { quarantinePoison, quarantineBatchPoison, poisonCandidates, POISON_ATTEMPT_THRESHOLD, } from '../cos/poison-quarantine.js';
...
function seedPoison(attempts = POISON_ATTEMPT_THRESHOLD) {
    ...
    for (let i = 0; i < attempts; i++)
        markRecoveryRequired(db, ACC, 'poison', 'sérült melléklet', NOW - 800 + i);
    return db;
}
const statusOf = (m) => (getDb().prepare(`SELECT status, quarantine_reason FROM email_processing WHERE message_id=?`).get(m));
