...
import { initPacketMetadataSchema, recordPacketMetadata, recordPacketMetadataSafe, readPacketMetadata, } from '../costops/packet-metadata.js';
import { buildContextPacket, derivePacketMetadata, artifactRefFromContent } from '../context-packet.js';
...
describe('P2-B packet metadata: schema rides the CostOps seam, idempotently', () => {
    ...
    it('carries NO column that could hold prompt text or a packet body', () => {
        const cols = getDb().prepare("SELECT name FROM pragma_table_info('dispatch_packets')").all();
        ...
        const tokenCols = getDb().prepare("SELECT name, type FROM pragma_table_info('dispatch_packets')").all();
    ...
    it('estimate_confidence is NOT NULL, so a stored estimate can never lose its marker', () => {
        const cols = getDb().prepare("SELECT name, [notnull] FROM pragma_table_info('dispatch_packets')").all();
...
describe('P2-B packet metadata: persistence', () => {
    ...
    it('is idempotent per dispatch: re-recording replaces, never duplicates', () => {
        ...
        const n = getDb().prepare('SELECT COUNT(*) AS n FROM dispatch_packets WHERE dispatch_id = ?').get(id);
        const a = getDb().prepare('SELECT COUNT(*) AS n FROM dispatch_packet_artifacts WHERE dispatch_id = ?').get(id);
        expect(n.n).toBe(1);
        expect(a.n).toBe(1);
    ...
    it('REFUSES to store a token count with no confidence marker', () => {
        const id = newDispatch();
        const bad = { ...derivePacketMetadata(EXAMPLE_PACKET), estimateConfidence: '' };
...
describe('P2-B packet metadata: fault isolation -- the dispatch still goes out', () => {
    ...
    it('a rejected estimate (no confidence marker) does not propagate through the safe wrapper', () => {
        const id = newDispatch();
        const bad = { ...derivePacketMetadata(EXAMPLE_PACKET), estimateConfidence: undefined };
...
describe('P2-B packet metadata: rollback is inert', () => {
    ...
    it('the P2-A dispatches table gained no columns (linked-table choice, not nullable columns)', () => {
        const cols = getDb().prepare("SELECT name FROM pragma_table_info('dispatches')").all().map(c => c.name);
