...
import { detectPaneState, detectPermissionMode, detectsThinkingBlockError, detectsBlockingMenu, detectsPastePlaceholder, isReadyForPrompt, shouldRetrySubmit, shouldClearTruncatedPreamble, decideSubmitFollowup, decidePaneErrorAlert, stuckInputSignature, parkedPasteSignature, decideStuckInputRecovery, parkedChannelInput, parkedInputText, parkedInputRowCount, submitLanded, paneShowsContextSaturation, } from '../pane-state.js';
...
const modeFooter = (tail) => ['', SEP, '❯ ', SEP, tail].join('\n');
...
// implausible.
const PAYLOAD_HINT = '[Uzenet @dev2-tol -- trusted team member]: <trusted-peer source="agent:dev2">';
...
describe('parkedChannelInput (stuck channel-block gate + truncation guard)', () => {
    const SEP = '─'.repeat(80);
    const wrap = (boxLines) => ['', SEP, ...boxLines, SEP, '  ⏵⏵ bypass permissions on (shift+tab to cycle)'].join('\n');
...
describe('parkedInputText', () => {
    ...
    it('returns the parked input text when typing', () => {
        expect(parkedInputText(TYPING_PARKED2)).toBe('Valami amit a felhasznalo elkezdett geppelni, meg nem kuldte el');
    });
    ...
    it('collapses terminal-wrapped lines into a single submittable line', () => {
        expect(parkedInputText(WRAPPED_PARKED)).toBe('[Uzenet @system-tol]: Uj csapattag erkezett: balazsmarveenja. Udv neki ha legkozelebb beszeltek!');
    });
});
describe('parked input rendered with a non-breaking space (U+00A0) after ❯', () => {
    ...
    it('recovers the parked text with the ❯ prompt and NBSP stripped', () => {
        expect(parkedInputText(NBSP_PARKED)).toBe('[Uzenet @dev2-tol]: please re-run the merge once CI is green');
    });
    ...
    it('collapses a terminal-wrapped NBSP-parked message into one submittable line', () => {
        ...
        expect(parkedInputText(pane)).toBe('[Uzenet @dev3-tol]: please review the latest changes when you have a moment and re-run the merge once CI is green');
    });
...
describe('submitLanded', () => {
    ...
    const parkedSig = stuckInputSignature(TYPING_PARKED);
...
describe('footer-less welcome-screen parked input', () => {
    ...
    it('submitLanded fires once the welcome wedge clears to an idle pane', () => {
        ...
        const sig = stuckInputSignature(WELCOME_STUCK);
        expect(sig).not.toBeNull();
        expect(submitLanded(sig, IDLE_BYPASS)).toBe(true);
    });
