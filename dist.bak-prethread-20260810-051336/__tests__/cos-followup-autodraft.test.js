...
import { followUpEligibility, draftFollowUp, sweepFollowUpCandidates, OVERDUE_GRACE_DAYS, MAX_AUTO_FOLLOWUPS, } from '../cos/followup-autodraft.js';
...
function mk(over = {}) {
    ...
    db.prepare(`UPDATE personal_cases SET status=@status, waiting_on=@waiting_on, next_action_owner=@owner,
       follow_up_at=@fu, gmail_thread_ids=@thread WHERE case_id='c1'`).run({
...
describe('follow-up auto-draft (option C)', () => {
    ...
    describe('what C refuses', () => {
        ...
        it('a draft is already waiting for approval — never queue a second', () => {
         ...
         VALUES ('l1','c1','EMAIL_SEND',1,'k1','PLANNED',?,?)`).run(NOW, NOW);
        ...
        it('stops after two automatic nudges — a third letter is talking to itself', () => {
           ...
           VALUES (?,'c1','EMAIL_SEND',?,?,'VERIFIED','FOLLOW_UP',?,?)`).run(`l${i}`, i + 10, `k${i}`, NOW, NOW);
            }
            const e = followUpEligibility(db, 'c1', NOW);
