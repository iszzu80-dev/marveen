...
function approvals() {
    return getDb().prepare('SELECT COUNT(*) AS n FROM zst_campaign_approvals').get().n;
}
describe('the corporate outbound door', () => {
    ...
    it('drafting writes a ledger row and sends nothing', () => {
        const d = draft();
        const row = getDb().prepare('SELECT status, case_id FROM zst_outbound_ledger WHERE ledger_id = ?').get(d.ledgerId);
    ...
    it('refuses a recipient the owner did not authorise', async () => {
        const d = draft();
        const res = await approveAndDispatchZst(getDb(), d.ledgerId, d.renderedPayloadHash, 'istvan', ['valaki.mas@example.com'], T0 + 5);
        expect(res.sent).toBe(false);
    ...
    it('defaults the authorised list to exactly the drafted addressee', async () => {
        ...
        const a = getDb().prepare('SELECT allowed_recipients FROM zst_campaign_approvals LIMIT 1').get();
...
describe('the escalation door does not let HTTP spell its way past the hard gate', () => {
    ...
    it('every commitment-bearing request type is covered, not just the one I tested', () => {
        for (const t of ['PAID_SERVICE', 'LICENSE', 'SUBCONTRACTOR', 'CONTRACT', 'SIGNIFICANT_COST']) {
            expect(escalationNeedsIstvanInPerson({ target_workspace: 'ZST', request_type: t }, 'ACCEPTED'), `${t} must need Istvan`).toBe(true);
        }
    });
});
