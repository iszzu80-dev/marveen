...
import { draftZstSend, approveZstSend, rejectZstSend, dispatchZstSend, evaluateZstSendGate, renderedPayloadHash, } from '../cos/zst-send.js';
...
function mockAdapter(over = {}) {
    ...
    return {
        ...
        adapter: {
            ...
            async send(a) { if (over.sendThrows)
                throw over.sendThrows; sent.push(a); return { externalRef: 'zmsg-1' }; },
            async readback() { return over.readback ?? { found: true, externalRef: 'zmsg-1' }; },
        },
    };
}
describe('ZST approval-gated send (Slice 1 write-half, AT-ZA)', () => {
    ...
    const dispatchInput = (d, email = EMAIL) => ({
