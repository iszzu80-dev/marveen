...
import { scheduleNextProgression, findDueCases, tryClaimProgression, releaseProgressionClaim, extendProgressionLease, setProgressionEnabled, } from '../cos/progression-scheduler.js';
...
function seedProgressionState(db, domain, caseId, overrides = {}) {
    const t = now();
    db.prepare(`INSERT INTO case_progression_state
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(domain, caseId, overrides.progression_enabled ?? 1, overrides.progression_mode ?? 'shadow', overrides.next_progression_at ?? null, overrides.progression_claimed_by ?? null, overrides.progression_claim_expires_at ?? null, overrides.goal ?? null, overrides.last_progressed_at ?? null, t, t);
}
/** Seed the parent case row so domainGuard passes. */
function seedCase(db, domain, caseId) {
    const t = now();
    if (domain === 'personal') {
        db.prepare(`INSERT INTO personal_cases (case_id, title, case_type, status, sensitivity, created_at, updated_at)
       VALUES (?, 'Test case', 'ADMIN', 'NEW', 'PERSONAL', ?, ?)`).run(caseId, t, t);
    }
    else {
        db.prepare(`INSERT INTO zst_cases (case_id, title, case_type, status, sensitivity, created_at, updated_at)
       VALUES (?, 'Test case', 'ADMIN', 'NEW', 'ZST_INTERNAL', ?, ?)`).run(caseId, t, t);
    }
}
/** Seed both the parent case and a progression state row. */
function seedCaseWithProgression(db, domain, caseId, overrides = {}) {
...
describe('Checkpoint E.3 — Wait/Wake scheduling + claim/lease', () => {
    describe('scheduleNextProgression()', () => {
        ...
        it('sets next_progression_at on an existing progression state row', () => {
            ...
            const row = db.prepare('SELECT next_progression_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-001');
        ...
        it('clears next_progression_at when null is passed', () => {
            ...
            const row = db.prepare('SELECT next_progression_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-002');
        ...
        it('works for ZST domain', () => {
            ...
            const row = db.prepare('SELECT next_progression_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('zst', 'zst-001');
            expect(row.next_progression_at).toBe(wakeAt);
        });
        it('throws CrossDomainReadError for cross-domain case', () => {
            seedCaseWithProgression(db, 'zst', 'zst-cross-001');
            expect(() => scheduleNextProgression(db, 'personal', 'zst-cross-001', now() + 3600, now())).toThrow(CrossDomainReadError);
        });
    });
    ...
    describe('tryClaimProgression()', () => {
        ...
        it('claims a due case (next_progression_at in the past, not claimed)', () => {
            ...
            // Verify DB state
            const row = db.prepare('SELECT progression_claimed_by, progression_claim_expires_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-001');
        ...
        it('returns null when case is not due (next_progression_at in the future)', () => {
            ...
            const row = db.prepare('SELECT progression_claimed_by FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-002');
        ...
        it('returns null when already claimed by another runner (non-expired lease)', () => {
            ...
            const row = db.prepare('SELECT progression_claimed_by FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-005');
        ...
        it('succeeds when claim is expired (reclaim)', () => {
            ...
            const row = db.prepare('SELECT progression_claimed_by, progression_claim_expires_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-006');
        ...
        it('throws CrossDomainReadError for cross-domain case', () => {
            ...
            expect(() => tryClaimProgression(db, 'personal', 'zst-cross-002', randomUUID(), 300, t)).toThrow(CrossDomainReadError);
        });
    });
    ...
    describe('RED-PROOF: no double-claim (concurrent runners)', () => {
        ...
        it('two claims within the same transaction fail — only one succeeds', () => {
            ...
            const row = db.prepare('SELECT progression_claimed_by FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-concurrent');
        ...
        it('second claim on same case fails even when case is still due', () => {
            ...
            const row = db.prepare('SELECT progression_claimed_by, progression_claim_expires_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-double');
        ...
        it('RED-PROOF: 10 concurrent claims on the same case — exactly ONE winner', () => {
            ...
            const results = runIds.map(runId => tryClaimProgression(db, 'personal', 'pri-ten', runId, 300, t));
            ...
            const row = db.prepare('SELECT progression_claimed_by FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-ten');
    ...
    describe('RED-PROOF: expired lease is reclaimable', () => {
        ...
        it('another runner can claim a case whose lease has expired', () => {
            ...
            const row = db.prepare('SELECT progression_claimed_by, progression_claim_expires_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-expired');
            expect(row.progression_claimed_by).toBe(newRunId);
            expect(row.progression_claim_expires_at).toBe(t + 300);
        });
        ...
        it('RED-PROOF chain: claim → expire → reclaim → verify old owner cannot release', () => {
            ...
            const row = db.prepare('SELECT progression_claimed_by FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-chain');
    ...
    describe('releaseProgressionClaim()', () => {
        ...
        it('no-op when no claim exists (progression_claimed_by IS NULL)', () => {
            ...
            // Should not throw
            expect(() => releaseProgressionClaim(db, 'personal', 'pri-003', 'any-runner', t)).not.toThrow();
        });
        it('throws CrossDomainReadError for cross-domain case', () => {
            const t = now();
            ...
            expect(() => releaseProgressionClaim(db, 'personal', 'zst-release-cross', 'runner-1', t)).toThrow(CrossDomainReadError);
        });
    });
    ...
    describe('extendProgressionLease()', () => {
        ...
        it('extends the lease for a claim held by the given runId', () => {
            ...
            const row = db.prepare('SELECT progression_claim_expires_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-001');
        ...
        it('returns false when trying to extend a claim held by a DIFFERENT runId', () => {
            ...
            const row = db.prepare('SELECT progression_claim_expires_at FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-002');
            expect(row.progression_claim_expires_at).toBe(t + 300);
        });
        ...
        it('throws CrossDomainReadError for cross-domain case', () => {
            ...
            expect(() => extendProgressionLease(db, 'personal', 'zst-extend-cross', 'runner-1', t + 900, t)).toThrow(CrossDomainReadError);
        });
    });
    ...
    describe('Domain-scoped — CrossDomainReadError on all operations', () => {
        ...
        it('scheduleNextProgression throws CrossDomainReadError for cross-domain', () => {
            ...
            expect(() => scheduleNextProgression(db, 'personal', 'zst-sched', now() + 3600, now())).toThrow(CrossDomainReadError);
        });
        it('tryClaimProgression throws CrossDomainReadError for cross-domain', () => {
            setupZstOnly('zst-claim');
            expect(() => tryClaimProgression(db, 'personal', 'zst-claim', randomUUID(), 300, now())).toThrow(CrossDomainReadError);
        });
        it('releaseProgressionClaim throws CrossDomainReadError for cross-domain', () => {
            ...
            expect(() => releaseProgressionClaim(db, 'personal', 'zst-rel', 'runner-1', t)).toThrow(CrossDomainReadError);
        });
        it('extendProgressionLease throws CrossDomainReadError for cross-domain', () => {
            ...
            expect(() => extendProgressionLease(db, 'personal', 'zst-ext', 'runner-1', t + 900, t)).toThrow(CrossDomainReadError);
        });
        it('setProgressionEnabled throws CrossDomainReadError for cross-domain', () => {
            setupZstOnly('zst-enable');
            expect(() => setProgressionEnabled(db, 'personal', 'zst-enable', true)).toThrow(CrossDomainReadError);
        });
    ...
    describe('setProgressionEnabled()', () => {
        ...
        it('enables progression on a case', () => {
            ...
            const row = db.prepare('SELECT progression_enabled, progression_mode FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-001');
        ...
        it('disables progression on a case', () => {
            ...
            const row = db.prepare('SELECT progression_enabled FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-002');
        ...
        it('sets progression mode', () => {
            ...
            const row = db.prepare('SELECT progression_mode FROM case_progression_state WHERE domain = ? AND case_id = ?').get('personal', 'pri-003');
        ...
        it('works for ZST domain', () => {
            ...
            const row = db.prepare('SELECT progression_enabled, progression_mode FROM case_progression_state WHERE domain = ? AND case_id = ?').get('zst', 'zst-010');
            expect(row.progression_enabled).toBe(1);
            expect(row.progression_mode).toBe('internal');
        });
    });
    ...
    describe('Full wake cycle (integration)', () => {
        ...
        it('schedule → wait → due → claim → release → schedule again', () => {
            ...
            // 7. Verify state
            const row = db.prepare(`SELECT next_progression_at, progression_claimed_by, progression_claim_expires_at
         FROM case_progression_state WHERE domain = ? AND case_id = ?`).get('personal', 'pri-cycle');
