...
// ---------------------------------------------------------------------------
const mockSendPrompt = vi.fn(async (..._a) => undefined);
vi.mock('../logger.js', () => ({
...
vi.mock('../web/agent-process.js', () => ({
    ...
    sendPromptToSession: (...a) => mockSendPrompt(...a),
...
const outcomesFor = (dispatchId) => getDb()
...
function enqueueWithDispatch(dispatchId) {
    const id = nextMsgId++;
    getDb()
        .prepare(`INSERT INTO agent_messages (id, from_agent, to_agent, content, status, created_at, dispatch_id)
...
describe('P2-A live writer: `failed` on the message-router give-up path', () => {
    ...
    it('writes `failed` with mechanism evidence once the inject retries are exhausted', async () => {
        ...
        const msg = db.prepare('SELECT status FROM agent_messages WHERE id = ?').get(msgId);
    ...
    it('an un-instrumented message (no dispatch anywhere) produces no outcome row on give-up', async () => {
        ...
        for (let i = 0; i < MAX_ATTEMPTS; i++)
            await runMessageRouterTick();
        const minted = db.prepare("SELECT dispatch_id FROM dispatches WHERE source = 'message'").all();
