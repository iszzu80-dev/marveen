...
function agent(name, hidden = false) {
    mkdirSync(join(AGENTS, name), { recursive: true });
    if (hidden)
        writeFileSync(join(AGENTS, name, HIDDEN_AGENT_SENTINEL), '');
}
beforeEach(() => {
...
describe('hidden technical workers and the saturation net', () => {
    ...
    it('the rescue restart is FRESH, never --continue', () => {
        ...
        const src = readFileSync(join(dirname(fileURLToPath(import.meta.url)), '..', 'web', 'context-guard-runner.ts'), 'utf8');
