...
import { normalizeContextGuardConfig, contextLimitForModel, isRecognizedContextModel, calibrateLimit, CALIBRATION_OVERSHOOT_TOLERANCE, findContextWindowViolations, findUnrecognizedModelsInUse, MIN_TURNS_FOR_REQUIRED_RECOGNITION, decideGuard, DEFAULT_CONTEXT_GUARD, INITIAL_GUARD_STATE, READY_TIMEOUT_MS, SATURATION_CONFIRM_SWEEPS, } from '../context-guard.js';
// The guard is default-off (opt-in); these behavioural cases exercise an
// explicitly-enabled guard.
const CFG = { ...DEFAULT_CONTEXT_GUARD, enabled: true };
...
describe('decideGuard: idle', () => {
    ...
    it('resets to initial state when fully disarmed (guard + net off)', () => {
        ...
        const stale = { phase: 'await-handoff', handoffMtimeAtRequest: 1, deadlineMs: 2, cooldownUntilMs: 0, saturatedStreak: 0 };
...
describe('saturation net (samu 2026-07-18 stall)', () => {
    ...
    const netOnly = { ...DEFAULT_CONTEXT_GUARD }; // enabled:false, saturationRestart:true
    ...
    it('outranks the proactive tiers when both would fire (no handoff request into a dead pane)', () => {
        const state = { ...INITIAL_GUARD_STATE, saturatedStreak: SATURATION_CONFIRM_SWEEPS - 1 };
