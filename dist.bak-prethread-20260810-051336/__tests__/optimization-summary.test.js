...
const mockedState = vi.hoisted(() => ({
    optimization: null,
    subscriptions: { version: 1, subscriptions: [] },
    inventory: { version: 1, packages: [] },
}));
vi.mock('../optimization/optimization-config.js', async () => {
    const actual = await vi.importActual('../optimization/optimization-config.js');
    return {
        ...actual,
...
vi.mock('../costops/subscriptions.js', async () => {
    const actual = await vi.importActual('../costops/subscriptions.js');
    return {
        ...actual,
...
vi.mock('../costops/package-inventory.js', async () => {
    const actual = await vi.importActual('../costops/package-inventory.js');
    return {
        ...actual,
...
vi.mock('../costops/config.js', async () => {
    const actual = await vi.importActual('../costops/config.js');
    return {
        ...actual,
        loadCostopsConfig: () => ({
...
vi.mock('../web/capacity-routing-store.js', async () => {
    const actual = await vi.importActual('../web/capacity-routing-store.js');
    return {
        ...actual,
...
function config(moduleValues, masterEnabled = true) {
    return {
        version: 1,
        masterEnabled,
        preset: 'custom',
...
function setOptimizationConfig(moduleValues, opts = {}) {
...
describe('buildOptimizationSummary', () => {
    beforeEach(() => {
        initDatabase(':memory:');
        mockedState.subscriptions = { version: 1, subscriptions: [] };
        mockedState.inventory = { version: 1, packages: [] };
