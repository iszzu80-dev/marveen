...
describe('COS radar HIT alert', () => {
    ...
    it('alertRadarHit posts a bus message to marveen and a daily-log entry', () => {
        const db = seedHit();
        alertRadarHit(db, 'r1');
        const msg = db.prepare(`SELECT from_agent, to_agent, content FROM agent_messages ORDER BY id DESC LIMIT 1`).get();
        expect(msg).toMatchObject({ from_agent: 'cos-radar', to_agent: 'marveen' });
        expect(msg.content).toContain('COS radar HIT');
        const log = db.prepare(`SELECT content FROM daily_logs ORDER BY id DESC LIMIT 1`).get();
