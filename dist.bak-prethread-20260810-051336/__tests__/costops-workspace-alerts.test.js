...
describe('workspace alerts ingest', () => {
    ...
    it('ingests a valid entry and is idempotent per message_ref', () => {
        const db = getDb();
        const entry = { account: 'google-private', issue_type: 'payment_failure', detected_at: NOW, message_ref: 'msg-1' };
        expect(ingestWorkspaceAlerts(db, [entry], NOW).ingested).toBe(1);
        ingestWorkspaceAlerts(db, [entry], NOW);
        const rows = db.prepare('SELECT COUNT(*) as n FROM workspace_alerts').get();
        expect(rows.n).toBe(1);
    });
    it('rejects an invalid issue_type without throwing', () => {
        const db = getDb();
        const result = ingestWorkspaceAlerts(db, [{ account: 'google-zst', issue_type: 'bogus', detected_at: NOW, message_ref: 'msg-2' }], NOW);
    ...
    it('never stores the raw message_ref -- only a hash', () => {
        ...
        const row = db.prepare('SELECT message_ref FROM workspace_alerts').get();
    ...
    it('buildWorkspaceAlertWarnings emits one warning per (account, issue_type), most recent only, never fabricated', () => {
        ...
        expect(warnings[0].detail.detected_at).toBe(NOW - 1 * 86400);
    });
    ...
    it('severity rises as the suspension date approaches (30/14/7-day ladder)', () => {
        ...
        const warnings = buildWorkspaceAlertWarnings(db, NOW);
        expect(warnings.find(w => w.detail.account === 'a1').severity).toBe('low');
        expect(warnings.find(w => w.detail.account === 'a2').severity).toBe('medium');
        expect(warnings.find(w => w.detail.account === 'a3').severity).toBe('high');
    });
    ...
    it('rejects an invalid suspension_date type without throwing', () => {
        const db = getDb();
        const result = ingestWorkspaceAlerts(db, [{ account: 'google-zst', issue_type: 'payment_failure', detected_at: NOW, suspension_date: 'not-a-number', message_ref: 'bad' }], NOW);
        expect(result.ingested).toBe(0);
        expect(result.errors).toHaveLength(1);
    });
