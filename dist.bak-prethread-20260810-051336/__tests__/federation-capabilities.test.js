...
import { roleSpecificHead, readSummarySource, summarySourceHash, containsPrivateData, ownerScrubNeedle, pickStaleAgents, failureBackoffMs, generateOneSummary, pruneCapabilityCache, getCapabilitySummary, mainAgentCapabilitySummary, readCapabilityCache, writeCapabilityCache, purgeCapabilityCache, buildSummaryPrompt, CAPABILITY_CACHE_FILENAME, CAPABILITY_SUMMARY_MAX_CHARS, CAPABILITY_FAILURE_BACKOFF_BASE_MS, CAPABILITY_FAILURE_BACKOFF_MAX_MS, _setCapabilityStoreDirForTest, } from '../web/federation/capabilities.js';
...
describe('pickStaleAgents (pure)', () => {
    ...
    it('picks missing entries; skips fresh ones; honours the limit', () => {
        const cache = { a: { summary: 's', sourceHash: hash, generatedAt: NOW } };
    ...
    it('skips privacy-rejected entries until the sources change', () => {
        const cache = { a: { sourceHash: hash, rejected: true, lastAttemptAt: NOW - 10 } };
    ...
    it('respects the exponential failure backoff and resets it on a hash change', () => {
        const twoFails = { a: { sourceHash: hash, consecutiveFailures: 2, lastAttemptAt: NOW } };
