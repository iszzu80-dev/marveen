...
import { resolveEmailThread, resolveMemory, resolveContextDeep, CrossDomainReadError, domainGuard, } from '../cos/progression-resolver.js';
...
// ── Helpers ─────────────────────────────────────────────────────────────
function seedMemory(db, content, sector = 'semantic', topicKey = 'test') {
    db.prepare(`INSERT INTO memories (chat_id, topic_key, content, sector, salience, created_at, accessed_at)
     VALUES ('test-chat', ?, ?, ?, 1.0, unixepoch(), unixepoch())`).run(topicKey, content, sector);
}
function snapshotProgressionState(domain, caseId, db) {
    return db.prepare('SELECT * FROM case_progression_state WHERE domain = ? AND case_id = ?').get(domain, caseId);
}
// ── Tests ───────────────────────────────────────────────────────────────
describe('Checkpoint C — Resolver depth (card 53f1fd06)', () => {
    ...
    describe('Stage 1: Email thread resolution', () => {
        beforeAll(() => {
            ...
            db.prepare(`UPDATE personal_cases SET gmail_thread_ids = ?, source_references = ?
         WHERE case_id = ?`).run(JSON.stringify(['thread-abc']), JSON.stringify(['msg-001', 'msg-002', 'msg-003']), 'case-thread-test-001');
            ...
            db.prepare(`UPDATE email_processing SET status = 'LOCAL_APPLIED', updated_at = ?
         WHERE batch_id = ?`).run(now, batchId1);
            ...
            db.prepare(`UPDATE zst_cases SET gmail_thread_ids = ?, source_references = ?
         WHERE case_id = ?`).run(JSON.stringify(['zst-thread-xyz']), JSON.stringify(['zst-msg-001', 'zst-msg-002']), 'zst-thread-test-001');
            // Seed zst_email_processing
            db.prepare(`INSERT INTO zst_email_processing (gmail_account_id, message_id, thread_id, case_id, status, created_at)
         VALUES ('zst-account', 'zst-msg-001', 'zst-thread-xyz', 'zst-thread-test-001', 'LOCAL_APPLIED', ?)`).run(now - 86400 * 2);
            db.prepare(`INSERT INTO zst_email_processing (gmail_account_id, message_id, thread_id, case_id, status, created_at)
         VALUES ('zst-account', 'zst-msg-002', 'zst-thread-xyz', 'zst-thread-test-001', 'LOCAL_APPLIED', ?)`).run(now - 86400 * 1);
        });
    ...
    describe('Stage 4: Pipeline integration — audit in progression state', () => {
        it('progression state includes resolution_audit_json after a cycle', () => {
            ...
            const audit = JSON.parse(state.resolution_audit_json);
        ...
        it('progression run progress_delta_json includes audit summary', () => {
            const runs = db.prepare("SELECT progress_delta_json FROM case_progression_runs WHERE case_id = 'case-thread-test-001' AND domain = 'personal' ORDER BY started_at DESC LIMIT 1").get();
    ...
    describe('Stage 5: GATE 0 eval corpus — missing-info-resolved-from-email scenario', () => {
        it('new corpus scenario exercises email-thread resolution in eval harness', () => {
            ...
            db.prepare(`UPDATE personal_cases SET source_references = ?, gmail_thread_ids = ?
         WHERE case_id = ?`).run(JSON.stringify(['eval-msg-1', 'eval-msg-2']), JSON.stringify(['eval-thread-1']), 'eval-email-resolve-001');
            ...
            db.prepare(`UPDATE email_processing SET status = 'LOCAL_APPLIED', case_id = 'eval-email-resolve-001', updated_at = ?
         WHERE batch_id = ?`).run(now, batchId2);
    ...
    describe('Stage 6: Read-only invariant — deep resolver writes nothing', () => {
        it('resolveEmailThread is read-only — no INSERT/UPDATE/DELETE', () => {
            const beforeCount = db.prepare('SELECT count(*) as c FROM email_processing').get().c;
            resolveEmailThread(db, 'personal', 'case-thread-test-001');
            const afterCount = db.prepare('SELECT count(*) as c FROM email_processing').get().c;
            expect(afterCount).toBe(beforeCount);
        });
        it('resolveMemory is read-only — no INSERT/UPDATE/DELETE on memories', () => {
            const beforeCount = db.prepare('SELECT count(*) as c FROM memories').get().c;
            resolveMemory(db, 'personal', 'test', 'PERSONAL', null);
            const afterCount = db.prepare('SELECT count(*) as c FROM memories').get().c;
            expect(afterCount).toBe(beforeCount);
        });
        it('resolveContextDeep does not mutate case tables', () => {
            const before = db.prepare("SELECT status, version FROM personal_cases WHERE case_id = 'case-thread-test-001'").get();
            resolveContextDeep(db, 'personal', 'case-thread-test-001', now);
            const after = db.prepare("SELECT status, version FROM personal_cases WHERE case_id = 'case-thread-test-001'").get();
    ...
    describe('Stage 7: Safety assertions with deep resolver', () => {
        it('all 7 safety assertions pass on cases progressed with deep resolver', () => {
            ...
            // Get the latest run
            const run = db.prepare("SELECT safety_assertions_json FROM case_progression_runs WHERE case_id = 'case-thread-test-001' AND domain = 'personal' ORDER BY started_at DESC LIMIT 1").get();
    ...
    describe('Stage 8: Domain-scoped reads RED-proof', () => {
        ...
        it('RED-PROOF: cross_domain_leakage safety assertion fires on the failed run', () => {
            ...
            const run = db.prepare("SELECT safety_assertions_json, error_code FROM case_progression_runs WHERE case_id = 'zst-cross-domain-test' AND domain = 'personal' AND error_code = 'CROSS_DOMAIN_LEAKAGE' ORDER BY started_at DESC LIMIT 1").get();
            expect(run).toBeTruthy();
            const assertions = JSON.parse(run.safety_assertions_json);
            const cdLeak = assertions.find((a) => a.assertion === 'cross_domain_leakage');
        ...
        it('RED-PROOF: seeded memory EXISTS in DB but is never returned — guard blocks, not missing data', () => {
            ...
            const memCount = db.prepare("SELECT count(*) as c FROM memories WHERE content LIKE '%MVM%' OR content LIKE '%Generalitol%'").get().c;
