...
function extractReapAwkProgram() {
    const line = channelsSh.split('\n').find(l => l.includes('ORPHAN_PIDS2=') && l.includes('awk'));
    if (!line)
        throw new Error('ORPHAN_PIDS2 awk line not found in channels.sh');
    const m = line.match(/awk .*?'([^']*)'/);
    if (!m)
        throw new Error('could not extract awk program from the ORPHAN_PIDS2 line');
    return m[1];
}
function runReapMatcher(psLines) {
    const program = extractReapAwkProgram();
    const out = execSync(`awk -v needle='CLAUDE_PLUGIN_ROOT=' -v prov='/${PROVIDER}' -v subdir='${INSTALL_DIR}/agents/' '${program}'`, { input: psLines.join('\n') + '\n', encoding: 'utf-8' });
