...
const NOW = 1_000_000;
function seedRecovery(seq, lastError = 'marker absent') {
    const db = getDb();
...
describe('outbound RECOVERY_REQUIRED alert', () => {
    ...
    it('alertOutboundRecovery posts to the bus + daily log and returns true', () => {
        ...
        const msg = getDb().prepare(`SELECT content FROM agent_messages WHERE from_agent='cos-outbound' ORDER BY id DESC LIMIT 1`).get();
