export function initOptimizationDecisionsSchema(db) {
    db.prepare(`
...
export function upsertDecisionsFromRecommendations(db, recommendations, now) {
    const run = db.transaction(() => {
        ...
        WHERE package_id = ?
      `).get(recommendation.package_id);
            if (!existing) {
                const initialStatus = recommendation.verdict === 'INSUFFICIENT_EVIDENCE' ? 'insufficient_evidence' : 'new';
                db.prepare(`
        ...
        `).run(recommendation.package_id, recommendation.verdict, initialStatus, evidenceJson, recommendation.confidence, now, now);
                db.prepare(`
        ...
        `).run(recommendation.package_id, initialStatus, now);
            ...
            const shouldReopen = REOPENABLE_STATUSES.has(existing.status)
          ...
          WHERE package_id = ?
        `).run(recommendation.verdict, evidenceJson, recommendation.confidence, now, now, recommendation.package_id);
                db.prepare(`
          INSERT INTO optimization_decision_events (
            package_id, from_status, to_status, actor, at, note
          ) VALUES (?, ?, 'new', 'system', ?, ?)
        `).run(recommendation.package_id, existing.status, now, `verdict changed from ${existing.verdict} to ${recommendation.verdict}, reopened for review`);
            }
            else {
                db.prepare(`
          UPDATE optimization_decisions
          ...
          WHERE package_id = ?
        `).run(recommendation.verdict, evidenceJson, recommendation.confidence, now, recommendation.package_id);
            }
            touched += 1;
        }
...
export function listOptimizationDecisions(db, opts = {}) {
    if (opts.status) {
        return db.prepare(`
      SELECT *
      FROM optimization_decisions
...
export function getOptimizationDecisionEvents(db, packageId) {
    return db.prepare(`
    SELECT *
...
export function setDecisionStatus(db, packageId, newStatus, actor, now, opts = {}) {
    const existing = db.prepare(`
    SELECT *
    FROM optimization_decisions
    WHERE package_id = ?
  `).get(packageId);
    if (!existing)
        return { ok: false, error: 'package_not_found', record: null };
    const update = db.transaction(() => {
      ...
      WHERE package_id = ?
    `).run(newStatus, now, actor, newStatus === 'deferred' ? (opts.deferredUntil ?? null) : null, now, packageId);
        db.prepare(`
      INSERT INTO optimization_decision_events (
        package_id, from_status, to_status, actor, at, note
      ) VALUES (?, ?, ?, ?, ?, ?)
