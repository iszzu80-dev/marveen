 ...
 */
export function pickLsofPath(env, isExecutable) {
    const candidates = [...ABSOLUTE_CANDIDATES];
    for (const dir of (env.PATH ?? '').split(':').filter(Boolean))
        candidates.push(join(dir, 'lsof'));
...
export function resolveLsofPath(env = process.env) {
    if (cachedLsofPath !== undefined)
 ...
 */
export function runLsof(args, timeoutMs) {
    const bin = resolveLsofPath();
    if (bin == null) {
        if (!missingWarned) {
            missingWarned = true;
            logger.warn({ path: process.env.PATH, tried: ABSOLUTE_CANDIDATES }, 'lsof not found on PATH or usual locations -- port-holder / cwd resolution DISABLED; single-instance reclaim degrades to argv-attribution. Install lsof or add /usr/sbin to PATH.');
        }
        return null;
    }
    try {
...
export function __resetLsofCache() {
