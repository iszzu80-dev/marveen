...
- a progressionig nem történt manual restart, code/config/policy change, owner intervention vagy más olyan külső beavatkozás, amely maga okozhatta a haladást;
...
### 9.2.1.a.3. Calibration emberi költség-budget
...
baseline_manual_monitoring_minutes
...
> **Nincs kötelező emberi döntés epizódonként.**
...
Nem hard-safety gate enforced promotionjához az evaluation windowban bizonyítani kell, hogy:

```text
net_human_minutes_saved > 0
```

ahol a nettó megtakarítás legalább:

```text
baseline manual monitoring/adjudication
-
calibration human labeling
-
false-block adjudication
-
manual override/recovery burden
```

Hard safety invariant kivétel lehet, de ilyenkor a magasabb human costot explicit safety justification és owner acceptance dokumentálja.

Ha nincs elég automatikusan/verifierrel címkézett data, a helyes állapot:

```text
UNCALIBRATED / OBSERVE_CALIBRATION
```

nem pedig tömeges manuális címkézési kényszer.
...
- Periodic Chain Charter, canonical phase model és producer-független calibration policy;
- Periodic Grace Calibrator + immutable calibration receipt;
- Episode Label Resolver + label receipt + intervention-aware ground-truth policy;
- calibration labeling economics telemetry;
- chain/gate scoped control maturity resolver (`OBSERVE_CALIBRATION` without blanket product downgrade);
- empirically calibrated, phase-aware periodic progress differential executor/contract;
- calibration lifecycle: validation, supersede, drift/review trigger;
...
- calibration labor-economics telemetry;
