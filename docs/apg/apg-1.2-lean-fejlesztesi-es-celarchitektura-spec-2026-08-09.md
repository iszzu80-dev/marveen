...
13. **slow-but-progressing reverse control:** periodikus lánc a deklarált grace window-n belül több cikluson át nem mutat stage/cursor változást, majd a grace lejárta előtt vagy annak határán legitim progressiont mutat; ezt a detector NEM jelölheti `STALLED`-nek.
...
periodic_chain_charter:
  ...
  grace_policy:
    ...
    max_no_progress_duration: optional
    phase_overrides: optional
...
- `max_no_progress_cycles` mondja meg, hány egymást követő completed cycle lehet legitim módon delta nélkül;
- `max_no_progress_duration` opcionálisan wall-clock felső korlátot is adhat;
- `phase_overrides` csak canonical, verziózott policy részeként használható, például tervezési fázisnál más grace lehet;
- a producer nem módosíthatja saját futása közben a chartert vagy a grace window-t;
- a charter módosítása új policy verzió és új audit receipt;
- nincs rendszer-szintű kényelmi default grace.

Ha a required progress checkhez nincs érvényes grace policy:
...
Enforced módban ez a spec általános szabálya szerint `INCOMPLETE/BLOCK`, amíg a chain contract nincs rendesen definiálva.
...
ÉS
a canonical grace window lejárt
ÉS
...
### 9.2.5. Kötelező kétirányú kontroll

A stagnálás-detektor elfogadásához három esetet kell bizonyítani:

```text
TRUE_STALL_BEYOND_GRACE
→ STALLED / FAIL
...
MISSING_GRACE_POLICY
→ UNKNOWN
```

Ezzel a detector nemcsak azt bizonyítja, hogy tud megállítani, hanem azt is, hogy **nem gyárt hibát legitim lassú haladásból**.
...
20. **Periodikus lánc grace-aware módon különbözteti meg a valódi stagnálást a legitim lassú haladástól.**  
    **RED:** bármelyik teljesül: (a) a detector a canonical grace window lejárta előtt `STALLED`-et ad; (b) a grace lejárta után runnable work + nulla business-state delta mellett healthy marad authoritative wait evidence nélkül; (c) hiányzó grace policy mellett `UNKNOWN` helyett definitív PASS/FAIL keletkezik.
...
- Periodic Chain Charter + producer-független grace policy;
- grace-aware periodic progress differential executor/contract;
- three-way progress control: true-stall / slow-progress / missing-policy;
...
- HF-03 legitimate-slow-progress / false-stall historical fixture;
- SF-03 true-stall-beyond-grace fixture;
...
9. grace-aware progress differential check, ha recurring/periodic lánc van, beleértve a slow-progress false-stall kontrollt;
...
- periodic chain charter + producer-független grace policy;
- grace-aware periodic progress differential contract + executor;
- true-stall / slow-progress / missing-policy three-way control;
- dual-sided acceptance-control (`positive_condition` + `red_condition`);
- HF-01 documented-but-not-executed historical fixture;
- HF-02 green-but-does-not-start historical fixture;
- HF-03 legitimate-slow-progress / false-stall historical fixture;
- SF-03 true-stall-beyond-grace fixture;
- missing-grace-policy → UNKNOWN fixture;
