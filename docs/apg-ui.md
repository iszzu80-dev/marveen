...
> Card `0f75d35d`. Owner spec (verbatim): `docs/apg/apg-0.4-lean-ui-integration-spec.md` (sections 1–28).
> Task 2 of 2, sequenced after the optimization dashboard (task 1, merged `1d10ba1`, sibling card `12d5c98d`).
...
> "Codex-run evidence and bugs found" below. **Owner acceptance against section 24 + stop-conditions
> section 27 belongs to marveen; this document is the producer's self-report, not a self-declared verdict.**
...
src/web/
  apg-scope-overrides.ts -- Marveen's OWN UI-side config: which mode a project/kanban_card scope
                            ...
                            off rule (global off cannot be raised by any scope override).
...
1 guarded `Apg.mount()` call + 1 `known` map entry), `web/index.html` +6 lines (2 tags + 5 hidden
container divs — wait, 4 container divs + 1 script + 1 link, 6 total). `config-registry.ts`'s +101
lines are a pure append (11 new objects), no existing entry touched.
...
2. **API routes** (`apg-scope-overrides.ts`/`routes/apg.ts` + `web.ts` wiring). Completed, self-
   ...
   spec 7.4 requires "same key → same result", and the code as generated would instead hit the
   ...
   (`store/apg-decision-idempotency.json`) and wired it in; a route-smoke test (`apg-ui-routes.test.ts`)
   now asserts the replay-vs-conflict distinction directly.
...
check at all (would show an empty-but-visible "APG döntések" section even fully off, the moment any
approval carried `category='apg_decision'`); `onActivityRendered` checked mode but rendered a visible
"APG ki van kapcsolva" block instead of hiding, plus flashed a loading placeholder before knowing the
...
- **Enforcement toggles** (`APG_REQUIRE_CLAIM_RECEIPT`/`APG_REQUIRE_INDEPENDENT_ACCEPTANCE`/
  ...
  means "enforcement toggle-ok működnek" is not literally true yet — there is nothing to toggle.
- **"Effective mode mindenhol látszik"**: shown on the Overview mode chip, the work-item detail
  overlay, and the card-detail panel; NOT shown on the Approvals decision cards.
- **Scope-override Settings UI** (spec 14.5's table + "Felülírás hozzáadása" form): API-only
