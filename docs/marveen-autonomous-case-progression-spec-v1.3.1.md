# Marveen Autonomous Case Progression Layer v1.3.1
...
## 4.4 Terminális Case-ek progression-state invariánsa

A live mérés feloldotta a `104 total Case` vs `101 progression-state row` eltérést:

```text
Personal Cases: 64
ZST Cases: 40
Total Cases: 104
Progression-state rows: 101
```

A három progression-state nélküli Personal Case mind már terminális volt: egy `CANCELLED` Revolut-ügy, egy `COMPLETED` teszt-ügy és egy `COMPLETED` Airbnb-ügy. A ZST oldalon nincs eltérés: 40 Case / 40 progression-state row.

Ez **helyes viselkedés**, és mostantól explicit invariáns:

> Olyan Case-hez, amely már a progression eligibility első értékelésekor terminális (`COMPLETED`, `CANCELLED`, vagy más domain-policy szerint terminális státusz), nem kötelező progression-state sort létrehozni.

Következmény:

```text
terminal-at-entry Case
→ no progression-state creation required
→ no progression scheduling
→ no Reader/Planner invocation
→ no missing-state alert
```

Ha egy már progression-state-tel rendelkező Case később terminálissá válik, az állapot auditcélból megmarad, de inaktívvá válik és további autonomous progression nem indul. Re-open esetén újra kell értékelni az eligibility-t, és a korábbi progression state újrahasználható, illetve ha korábban nem volt, akkor inicializálható.

A reconciliation szabály ezért nem lehet `case_count == progression_state_count`, hanem az eligibility-t kell figyelembe vennie. A terminális-at-entry Case progression-state hiánya **nem data-integrity hiba**.
...
## 22.2 Nem hamisítható Action Authorization Ticket

A 22.1 hard gate csak akkor valódi security boundary, ha a végrehajtó **nem fogadhat el a hívó által önbevallott** `gateEvaluated=true`, `authorized=true`, `policyPassed=true` vagy ezekkel ekvivalens állapotot.

Tiltott modell:

```text
caller
→ "I evaluated the gate and it passed"
→ executor trusts caller
```

Ez nem enforcement, csak convention.

A v1.3.1 kötelező modellje:
...
Deterministic Action Gate
      ↓
validate: domain, case/version, goal version, action type/intent,
target/recipient, payload hash, risk, policy, delegation/approval, kill switch
      ↓
NON-FORGEABLE AUTHORIZATION TICKET / CAPABILITY
      ↓
Controlled Action Executor
      ↓
independent ticket validation
      ↓
external adapter
```

A ticket minimum kötése:
...
delegation_envelope_id OR approval_id
issued_at
expires_at
single_use
nonce
...
- csak a trusted deterministic gate bocsáthatja ki;
- a caller nem állíthatja elő saját maga;
- más actionre nem használható;
- payload/recipient/target módosítása érvényteleníti;
- rövid élettartamú;
- external write esetén single-use;
- consumption auditált;
- authority visszavonása vagy érvénytelenedése execution előtt blokkolja a végrehajtást.

Elfogadható implementációk:

1. **Signed/HMAC capability ticket** — a gate canonical action authorizationt ír alá, az executor ellenőrzi.
2. **Opaque server-side capability** — a gate egy véletlen, kitalálhatatlan authorization ID-t ad, az executor atomikusan fogyasztja a szerveroldali authority rekordot.
3. **In-process unforgeable capability object** — csak akkor, ha a runtime/module boundary ténylegesen megakadályozza a caller általi konstrukciót és nincs serialization/bypass út.

**Ajánlott első implementáció:** opaque server-side, single-use authorization record vagy rövid élettartamú signed capability.

### Executor invariáns

External write csak akkor indulhat, ha:

```text
valid gate-issued authorization ticket
AND exact action context matches
AND ticket not expired
AND ticket not consumed
AND approval/envelope still valid
AND kill switch allows
```

Bármelyik hiányzik: `BLOCK`.

A caller által küldött boolean authorization flag **nem security proof**.

### TOCTOU-védelem

Execution előtt újraellenőrizendő legalább:

```text
case_version / goal_version
ticket expiry
approval/envelope validity
kill switch
payload_hash
target / recipient
```

Eltérés esetén az authorization érvénytelen, és új gate evaluation szükséges.

### Kötelező adversarial tesztek

```text
authorized=true without ticket → BLOCK
forged authorization_id → BLOCK
consumed ticket reuse → BLOCK
recipient changed after authorization → BLOCK
payload changed after authorization → BLOCK
action type changed → BLOCK
expired ticket → BLOCK
revoked envelope after issuance → BLOCK
invalid/withdrawn approval → BLOCK
case version changed → BLOCK or re-evaluate
kill switch disabled after issuance → BLOCK
direct adapter bypass → impossible / BLOCK
```

A security Definition of Done:

> A caller nem tud authority-t állítani vagy kitalálni; külső write-ra csak a deterministic gate által kibocsátott, az exact actionhöz kötött és az executor által ellenőrzött capability jogosíthat.
