...
export function createRadarItem(db, item, now) {
    const interval = item.checkIntervalSec ?? 86400;
    db.prepare(`INSERT INTO radar_items (radar_id, case_id, kind, label, query, target_price, max_price, currency,
     ...
     VALUES (@radarId, @caseId, @kind, @label, @query, @targetPrice, @maxPrice, @currency,
        'ACTIVE', @interval, @nextCheck, @now, @now)`).run({
...
export function getRadarItem(db, radarId) {
    return db.prepare(`SELECT * FROM radar_items WHERE radar_id = ?`).get(radarId);
}
...
export function dueRadarChecks(db, now, limit = 100) {
    return db.prepare(`SELECT * FROM radar_items WHERE status IN ('ACTIVE','HIT') AND next_check_at IS NOT NULL AND next_check_at <= ?
     ORDER BY next_check_at ASC LIMIT ?`).all(now, limit);
}
...
export function decideNotify(item, best, offerId, hit) {
    if (!hit)
        return { should: false, reason: null };
    if (item.last_notified_at == null)
        return { should: true, reason: 'NEW_HIT' };
    // A different offer now meets the target (includes the prior offer expiring and
    // being replaced) → worth surfacing.
    if (offerId != null && offerId !== item.last_notified_offer_id)
        return { should: true, reason: 'NEW_OFFER' };
    ...
    if (item.last_notified_price != null && best != null && best < item.last_notified_price) {
        ...
        if (rel >= SIGNIFICANT_DROP_FRACTION)
            return { should: true, reason: 'PRICE_DROP' };
    }
 ...
 */
export function recordObservation(db, radarId, obs, now) {
    const item = getRadarItem(db, radarId);
    if (!item)
        throw new Error(`radar item not found: ${radarId}`);
    ...
    const tx = db.transaction(() => {
        db.prepare(`INSERT INTO radar_observations
       ...
       VALUES (@radarId, @now, @best, @currency, @count, @ref, @offerId,
         @origCur, @origPrice, @cmpCur, @rate, @rateSrc, @rateTs, @best)`).run({
        ...
        const status = hit ? 'HIT' : item.status;
        const notify = decideNotify(item, obs.bestPrice, obs.offerId ?? null, hit);
        db.prepare(`UPDATE radar_items SET best_seen_price=@newLow, status=@status, next_check_at=@next, updated_at=@now WHERE radar_id=@radarId`).run({ newLow: newLow ?? null, status, next: now + item.check_interval_sec, radarId, now });
...
export function markNotified(db, radarId, n, now) {
    db.prepare(`UPDATE radar_items SET last_notified_offer_id=@offerId, last_notified_price=@price,
        last_notified_at=@now, notification_reason=@reason, updated_at=@now WHERE radar_id=@id`).run({ offerId: n.offerId, price: n.price, reason: n.reason, now, id: radarId });
}
export function setRadarStatus(db, radarId, status, now) {
