...
export function claudeAgentRunnable(plat = platform(), readCpuinfo = () => {
    try {
        return readFileSync('/proc/cpuinfo', 'utf-8');
    }
    catch {
        return '';
    }
    ...
    if (!info)
        return true;
    if (cpuinfoIsX86(info) && !cpuinfoHasAvx(info))
        return false;
    return true;
}
