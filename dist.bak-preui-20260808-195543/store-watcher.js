...
let currentWriteActor = null;
export function setStoreWriteActor(actor) {
    currentWriteActor = actor;
}
export function clearStoreWriteActor() {
...
function scanStore(dir, relBase = '') {
    try {
...
let watcher = null;
function isSystemFile(rel) {
    const name = basename(rel);
    return SYSTEM_FILES.has(name) || SYSTEM_RE.test(name);
}
export function startStoreWatcher() {
    if (watcher)
        return;
    knownFiles = new Set();
    scanStore(STORE_DIR);
...
export function stopStoreWatcher() {
    if (!watcher)
        return;
    try {
        watcher.close();
