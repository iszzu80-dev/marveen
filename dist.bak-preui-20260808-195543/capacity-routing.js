...
// ---------------------------------------------------------------------------
export const CAPACITY_STATES = ['available', 'degraded', 'limited', 'blocked', 'unknown'];
export function capacityKeyId(k) {
 ...
 */
export function deriveCapacityState(inputs, limitedThreshold = DEFAULT_LIMITED_THRESHOLD) {
    if (inputs.activeBlockingSignal)
        return 'blocked';
    if (inputs.usageConfidence === 'unknown' || inputs.usageFraction === null)
        return 'unknown';
    const stale = inputs.ageSeconds === null || inputs.ageSeconds > inputs.staleAfterSeconds;
    if (inputs.usageFraction >= 1)
        return 'blocked';
    if (inputs.usageFraction >= limitedThreshold)
        return 'limited';
    return stale ? 'degraded' : 'available';
}
/** States a resolver may route work TO (a place work can actually happen). */
export function isRoutable(state) {
 ...
 */
export function deriveCapacityStateFromBalance(inputs, floorUsd) {
    if (inputs.balanceUsd === null)
        return 'unknown';
    const stale = inputs.ageSeconds === null || inputs.ageSeconds > inputs.staleAfterSeconds;
    if (stale)
        return 'unknown';
...
const FALLBACK_ELIGIBLE_CLASSES = new Set(['capacity', 'rate_limit', 'outage']);
/**
 ...
 */
export function isFallbackEligible(cls) {
 ...
 */
export function classifyError(signal) {
    switch (signal.kind) {
        ...
        case 'http_status':
            if (signal.status === 429)
                return 'rate_limit';
            if (signal.status >= 500 && signal.status < 600)
                return 'outage';
            if (signal.status === 400 || signal.status === 422)
                return 'validation';
            return 'unclassified';
        default:
            return 'unclassified';
...
export function isRoutingChangeAllowed(input) {
 ...
 */
export function shouldClimbBackToPrimary(input) {
    if (!isRoutable(input.primaryCapacityState))
        return false;
    if (input.providerStatedResetAtMs !== null)
        return input.nowMs >= input.providerStatedResetAtMs;
 ...
 */
export function resolveRuntimeRouting(input) {
    ...
    if (input.packageOpen)
        return { action: 'hold_current_overlay', reasonCode: 'sticky_package_open' };
    ...
    const usable = input.candidates
        ...
        .map(({ c }) => c);
    if (usable.length === 0)
        return { action: 'no_eligible_fallback', reasonCode: 'no_candidate_available' };
