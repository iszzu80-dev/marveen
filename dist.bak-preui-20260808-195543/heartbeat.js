...
import { HEARTBEAT_CALENDAR_ID, STORE_DIR, DB_FILENAME, PROJECT_ROOT, OWNER_NAME, APP_TZ, } from './config.js';
...
function ensureHeartbeatWorkerCwd() {
    try {
        ...
        if (existsSync(realClaude)) {
            for (const entry of readdirSync(realClaude)) {
                if (HEARTBEAT_CONFIG_SKIP.has(entry))
                    continue;
        ...
        if (dirty || current.enabledPlugins == null || !existsSync(settingsPath)) {
            const next = { ...current, enabledPlugins };
...
function readClaudeCodeOauthJson() {
    if (process.platform !== 'darwin')
        return null;
    try {
        const out = execFileSync('/usr/bin/security', ['find-generic-password', '-s', 'Claude Code-credentials', '-a', userInfo().username, '-w'], { timeout: 3000, encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
        if (!out)
            return null;
        return out;
    }
...
export function formatHeartbeatCardLabel(card) {
    const max = 80;
...
function shouldNotify(data) {
    ...
    if (data.calendar.length > 0)
        return true;
    if (data.kanban.urgent > 0)
        return true;
    if (data.kanban.waiting > 2)
        return true;
    return false;
}
// --- Agent prompt ---
...
function scheduleNext(delayMs) {
    heartbeatTimeout = setTimeout(async () => {
        ...
        logger.info({ nextRun: nextRun.toLocaleString('hu-HU', { timeZone: APP_TZ }) }, `Heartbeat kovetkezo: ${nextRun.toLocaleTimeString('hu-HU', { timeZone: APP_TZ })}`);
...
export function initHeartbeat() {
    ...
    logger.info({ nextRun: nextRun.toLocaleString('hu-HU', { timeZone: APP_TZ }) }, `Heartbeat utemezve (kovetkezo: ${nextRun.toLocaleTimeString('hu-HU', { timeZone: APP_TZ })})`);
