...
import { buildCapacityReport, assertNoRecommendationLanguage, weakestConfidence, freshnessOf, unknownFigure, CAPACITY_STALE_AFTER_SECONDS, } from '../costops/capacity.js';
...
describe('P2-C capacity view', () => {
    ...
    it('1. every subscription carries a plan, a billing cycle and all five capacity figures', () => {
        ...
        for (const s of r.subscriptions) {
            ...
            for (const key of ['usage', 'unused_capacity', 'overflow', 'blocked_work', 'work_pushed_to_api']) {
                const f = s[key];
