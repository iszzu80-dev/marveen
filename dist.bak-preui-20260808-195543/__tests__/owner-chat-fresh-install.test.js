...
beforeEach(() => {
    ...
    writeFileSync(join(home, '.claude', 'channels', 'telegram', 'access.json'), JSON.stringify({ dmPolicy: 'allowlist', allowFrom: [REAL], groups: {}, pending: {} }));
...
afterEach(() => {
    ...
    if (originalChat === undefined)
        ...
        process.env['ALLOWED_CHAT_ID'] = originalChat;
...
describe('fresh wizard install (no usable ALLOWED_CHAT_ID, channel paired)', () => {
    ...
    it('with no pairing at all, it sends NOTHING rather than a doomed request', async () => {
        ...
        writeFileSync(join(home, '.claude', 'channels', 'telegram', 'access.json'), JSON.stringify({ dmPolicy: 'allowlist', allowFrom: [], groups: {}, pending: {} }));
        vi.resetModules();
        const { sendWelcomeMessage } = await import('../web/telegram.js');
        await sendWelcomeMessage('probe-agent', 'fake-token');
