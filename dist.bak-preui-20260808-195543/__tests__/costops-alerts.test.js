...
import { detectBudgetThresholdAlert, detectForecastBudgetBreachAlert, detectBalanceExhaustionAlert, detectStaleCollectorAlert, detectFailedSyncAlert, detectCredentialPermissionAlert, detectMissingInvoiceAlert, detectReconciliationMismatchAlert, detectManualProviderVarianceAlert, detectUnusualSpendGrowthAlert, detectNewUnknownSourceAlert, detectLongEstimateOnlySourceAlert, detectSubscriptionUtilizationAlert, reconcileAlerts, acknowledgeAlert, resolveAlert, serializeEvidence, deserializeEvidence, initAlertsSchema, } from '../costops/alerts.js';
// ---- detectors ---------------------------------------------------------------------
...
describe('initAlertsSchema', () => {
    ...
    `).run({ now, ev: serializeEvidence({ used_pct: 0.9 }) });
        const row = db.prepare(`SELECT * FROM costops_alerts WHERE dedup_key = 'budget_threshold|global'`).get();
