...
import { mkdtempSync, rmSync, existsSync, writeFileSync, mkdirSync, readFileSync, statSync, readdirSync, chmodSync, } from 'node:fs';
...
function ed25519Base64(keyByte = 0x42) {
...
const RESTRICTED = buildRestrictedLine(validatePublicKeyLine(`ssh-ed25519 ${B64} marveen-remote:${UUID}`));
describe('enrollAuthorizedKey (filesystem)', () => {
    ...
    it('fails fast when a fresh (non-stale) lockfile is held', async () => {
        ...
        await expect(enrollAuthorizedKey({
            sshDir,
