...
import { decideStuckInputAction, submitLanded, parkedInputRowCount, stuckInputSignature, } from '../pane-state.js';
...
describe('decideStuckInputAction (recovery-decision unit)', () => {
    ...
    it('multi-row sub-agent MACHINE-marked plain text -> re-inject plain, never enter', () => {
        const a = decideStuckInputAction(facts({ rowCount: 2, allowPlainReinject: true, hasPlainText: true, machineOrigin: true }));
    ...
    it('STUCKINPUT805 precedence FLIP: clear-scheduled beats plain re-inject on sub-agents too', () => {
        ...
        const a = decideStuckInputAction(facts({ rowCount: 3, scheduledTaskBlock: true, allowPlainReinject: true, hasPlainText: true, machineOrigin: true }));
        expect(a).toBe('clear-scheduled');
    });
});
