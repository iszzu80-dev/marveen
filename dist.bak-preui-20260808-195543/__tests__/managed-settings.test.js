...
import { getManagedSettingsSudoCommand, } from '../web/routes/agents.js';
...
describe('isManagedSettingsReady (algorithm)', () => {
    function check(path) {
        ...
        try {
            ...
            return plugins.some(p => p.plugin === SLACK_ENTRY.plugin && p.marketplace === SLACK_ENTRY.marketplace);
        }
        catch {
            return false;
        }
...
describe('getManagedSettingsSudoCommand merge logic', () => {
    function runMerge(existingContent, targetPath) {
        ...
        if (!innerMatch)
            throw new Error('Could not parse python script from command');
        ...
        if (!payloadMatch)
            throw new Error('Could not extract payload');
    ...
    it('does not duplicate existing entries', () => {
        ...
        const result = runMerge(existing, p);
        const slackEntries = result.allowedChannelPlugins.filter(e => e.plugin === 'slack-channel');
        expect(slackEntries).toHaveLength(1);
        expect(result.allowedChannelPlugins).toHaveLength(2);
    });
});
describe('setAgentEnabledPlugins (algorithm)', () => {
    function set(settingsPath, provider) {
