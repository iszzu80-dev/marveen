...
function sliceShellFn(src, name) {
...
function budgets(streaks) {
    const consts = [
        CHANNELS.match(/^NEVER_STARTED_BASE=\d+$/m)?.[0],
        CHANNELS.match(/^NEVER_STARTED_CAP=\d+$/m)?.[0],
    ];
    if (consts.some((c) => !c))
        throw new Error('budget constants not found in channels.sh');
...
function decisionChain() {
    ...
    if (end < 0)
        throw new Error('unterminated decision chain');
    return CHANNELS.slice(start, end);
}
function runChain(dir, opts) {
    const body = [
        'set -u',
