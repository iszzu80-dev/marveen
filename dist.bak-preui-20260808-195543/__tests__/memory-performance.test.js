...
import { initDatabase, getDb, saveAgentMemory, getAgentMemories, updateMemory, clearMemoryCache, getMemoryCacheSize, backfillEmbeddings, } from '../db.js';
...
describe('backfillEmbeddings', () => {
    ...
    it('processes rows without embeddings and updates them when Ollama responds', async () => {
        ...
        const result = db.prepare(`INSERT INTO memories (chat_id, topic_key, content, sector, salience,
       created_at, accessed_at, agent_id, category, auto_generated, keywords)
       VALUES (?, NULL, ?, 'semantic', 1.0, ?, ?, ?, 'cold', 0, NULL)`).run('test-chat', 'Backfill target content', now, now, BACKFILL_AGENT);
        ...
        const rowBefore = db.prepare('SELECT embedding FROM memories WHERE id = ?').get(id);
        ...
        const rowAfter = db.prepare('SELECT embedding FROM memories WHERE id = ?').get(id);
