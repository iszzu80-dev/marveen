...
function fixtureDb() {
    ...
    const ins = db.prepare("INSERT INTO kanban_cards (id,title,status,priority,assignee,archived_at,updated_at,created_at,sort_order) VALUES (?,?,?,?,?,?,?,?,0)");
    return { dir, db, ins };
}
describe('the heartbeat urgent list contains urgent, unfinished cards', () => {
    it('excludes done and archived, keeps waiting, in_progress AND planned', () => {
        ...
        try {
            ...
            const rows = db.prepare(HEARTBEAT_URGENT_SQL).all();
    ...
    it('the waiting list is waiting-only and drops archived rows', () => {
        ...
        try {
            ...
            const ids = db.prepare(HEARTBEAT_WAITING_SQL).all().map((r) => r.id);
...
describe('the heartbeat AGENT is handed the list, not a filter to re-apply', () => {
    const SCAFFOLD_SRC = readFileSync(join(ROOT, 'src', 'web', 'heartbeat-agent-scaffold.ts'), 'utf-8');
    const kanbanBlock = SCAFFOLD_SRC.slice(SCAFFOLD_SRC.indexOf('- **Kanban**'), SCAFFOLD_SRC.indexOf('- **Scheduled tasks**'));
