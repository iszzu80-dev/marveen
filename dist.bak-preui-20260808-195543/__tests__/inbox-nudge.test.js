...
import { decideNudgePreflight, recordNudge, nudgeText, INITIAL_NUDGE_STATE, NUDGE_MAX_CHARS, MIN_PENDING_AGE_MS, NUDGE_DEBOUNCE_MS, STALE_NUDGE_COOLDOWN_MS, MAX_STALE_NUDGES, MAX_NUDGES_PER_HOUR, } from '../web/inbox-nudge-watcher.js';
...
describe('decideNudgePreflight', () => {
    ...
    it('enforces the wall-clock debounce across spells', () => {
        const r = decideNudgePreflight({ now: NOW, oldestId: 7, oldestAgeMs: 60_000 }, st({ lastNudgeAt: NOW - NUDGE_DEBOUNCE_MS + 1 }));
        expect(r.proceed).toBe(false);
        const r2 = decideNudgePreflight({ now: NOW, oldestId: 7, oldestAgeMs: 60_000 }, st({ lastNudgeAt: NOW - NUDGE_DEBOUNCE_MS - 1 }));
    ...
    it('enforces the rolling hourly budget and logs the exhaustion once per spell', () => {
        ...
        const r = decideNudgePreflight({ now: NOW, oldestId: 9, oldestAgeMs: 60_000 }, st({ recentNudges: recent, lastNudgeAt: NOW - NUDGE_DEBOUNCE_MS - 1 }));
        expect(r.proceed).toBe(false);
