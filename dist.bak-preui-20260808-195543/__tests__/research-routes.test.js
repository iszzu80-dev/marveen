...
describe('research routes', () => {
    ...
    it('lists seeded docs for sub-agent and main agent', async () => {
        ...
        const sub = out.body.find((a) => a.agent === SUB_AGENT_ID);
        expect(sub?.docs.map((d) => d.name)).toContain('alpha.md');
        expect(sub?.docs.find((d) => d.name === 'alpha.md')?.title).toBe('Alpha Report');
        const main = out.body.find((a) => a.agent === MAIN_AGENT_ID);
        expect(main?.docs.map((d) => d.name)).toContain('zz-test-main-research.md');
    });
