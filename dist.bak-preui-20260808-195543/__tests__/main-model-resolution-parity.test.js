...
function fixture(envBody, settingsBody) {
    ...
    writeFileSync(join(root, 'dist', 'config-registry.js'), `exports.DISTRIBUTION_DEFAULT_AGENT_MODEL = ${JSON.stringify(DISTRIBUTION_DEFAULT_AGENT_MODEL)};\n`);
    if (envBody !== null)
        writeFileSync(join(root, '.env'), envBody + '\n');
    if (settingsBody !== null)
        writeFileSync(join(root, '.claude', 'settings.json'), settingsBody + '\n');
    return root;
}
/** The shell answer, via the script's own side-effect-free test seam. */
function shellResolves(root) {
