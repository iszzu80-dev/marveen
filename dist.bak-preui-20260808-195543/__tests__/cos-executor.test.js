...
import { planAction, executeAction, recoverAction, verifyAction, cancelAction, idempotencyKey, SendError, } from '../cos/executor.js';
...
class MockAdapter {
    ...
    async send(a) {
        ...
        if (this.mode === 'fail-retryable')
            throw new SendError('connection refused before request', { reachedProvider: false, terminal: false });
        if (this.mode === 'fail-terminal')
            throw new SendError('recipient rejected', { reachedProvider: false, terminal: true });
        if (this.mode === 'reach-then-throw') {
            this.provider.add(marker);
    ...
    async readback(marker) {
        if (this.readbackMode === 'throw')
            throw new Error('sent search unreachable');
        if (this.readbackMode === 'unavailable')
