...
import { withSessionSendLock, __resetSessionSendLocks, DEFAULT_DELIVER_WAIT_BUDGET_MS, } from '../web/session-send-lock.js';
...
describe('withSessionSendLock -- per-session delivery serialization', () => {
    ...
    it('RECOVER mode skips (ran:false) when a delivery holds the lane, runs when free', async () => {
        let releaseHolder;
        const held = new Promise(res => { releaseHolder = res; });
    ...
    it('DELIVER fails OPEN when the wait budget elapses against a wedged holder', async () => {
        ...
        let unblock;
        const wedged = new Promise(res => { unblock = res; });
        ...
        let secondRan = false;
        const second = await withSessionSendLock('sess', null, 'deliver', async () => { secondRan = true; }, { waitBudgetMs: 1, sleep: () => Promise.resolve() });
