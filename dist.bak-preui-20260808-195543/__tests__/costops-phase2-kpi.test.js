...
function makeDispatch(over = {}, at = NOW) {
...
function addTokenRow(dispatchId, input, output, at = NOW) {
    getDb().prepare(`
...
describe('P2-C KPI surface: empty states are UNKNOWN, never 0', () => {
    ...
    it('2. dispatches with NO outcome: acceptance/retry/failure are unknown, not 0', () => {
        ...
        for (const k of ['first_pass_acceptance', 'retry_rate', 'failure_rate']) {
    ...
    it('8. context_saturation_events comes from MEASURED gate observations, not from capacity_state', () => {
        ...
        const cs = getDb().prepare('SELECT capacity_state FROM routing_events WHERE dispatch_id = ?').get(d);
