...
import { mapGitHubUsage, syncGitHubCollector } from '../costops/collectors/github.js';
...
describe('githubCollector + syncGitHubCollector (offline stub)', () => {
    ...
    it('sync imports a provider_api line (even at 0) with a stubbed key + fetcher; idempotent', async () => {
        ...
        const row = db.prepare("SELECT billed_cost, confidence, data_freshness FROM cost_line_items WHERE source_id='github'").get();
        ...
        expect(db.prepare("SELECT COUNT(*) c FROM cost_line_items WHERE source_id='github'").get().c).toBe(1);
    ...
    it('errors (no import) when the vault token is missing', async () => {
        ...
        expect(db.prepare("SELECT COUNT(*) c FROM cost_line_items WHERE source_id='github'").get().c).toBe(0);
    });
});
