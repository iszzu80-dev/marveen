...
import { agentHasChannel, agentSessionName, capturePane, captureParkedInputView, clearInputBuffer, dismissResumeSummaryModalIfPresent, dismissModelConsentDialogIfPresent, stampFableOverageConsentSharedRoots, isAgentRunning, sendPromptToSession, startAgentProcess, stopAgentProcess, scheduleIdentitySetup, ensureMainAgentIsolatedConfigDir, ensureSharedClaudeOnboarded, hasFleetOauthToken, FLEET_OAUTH_TOKEN_PATH, answerFirstRunGates, shSingleQuote, } from './agent-process.js';
...
import { detectPaneState, decidePaneErrorAlert, detectsBlockingMenu, detectsFirstRunGate, detectsModelConsentDialog, stuckInputSignature, decideStuckInputRecovery, parkedChannelInput, parkedInputText, shouldClearTruncatedPreamble, parkedInputRowCount, submitLanded, decideStuckInputAction, parkedScheduledTaskInput, parkedMachineOriginInput, parkedMainInputHasRemedy, } from '../pane-state.js';
import { MAIN_CHANNELS_SESSION, MAIN_CHANNELS_PLIST } from './main-agent.js';
import { notifyChannel } from '../notify.js';
import { getProvider, channelStateDir, readChannelToken } from '../channel-provider.js';
...
function resolveAgentProvider(name) {
    ...
    if (perAgent === 'slack' || perAgent === 'telegram' || perAgent === 'discord')
        return perAgent;
...
const agentDownSince = new Map();
const agentLastRestart = new Map();
...
const agentNoTokenWarned = new Set();
...
const agentBusyDeferAlerted = new Set();
...
const agentRestartFailures = new Map();
let agentRestartFailuresInitialized = false;
function agentFailuresPath(agentName) {
    return join(PROJECT_ROOT, 'store', `.agent-failures-${agentName}`);
}
function loadPersistedAgentFailures(agentName) {
    try {
...
function savePersistedAgentFailures(agentName, count) {
    try {
...
function clearPersistedAgentFailures(agentName) {
    try {
        writeFileSync(agentFailuresPath(agentName), '0');
    }
    catch { /* best effort */ }
}
function ensureAgentRestartFailuresInitialized() {
    if (agentRestartFailuresInitialized)
        return;
...
let mainStuckInput = { parkedSig: null, firstSeenAt: null, lastRecoverAt: null, attempts: 0 };
...
export function decideStuckInputRestart(parked, attempts, maxAttempts, now, lastRestartAt, restartCount, minIntervalMs, maxConsecutive) {
    if (!parked || attempts < maxAttempts)
        return 'skip';
    if (now - lastRestartAt < minIntervalMs)
        return 'skip';
    if (restartCount >= maxConsecutive)
        return restartCount === maxConsecutive ? 'alert' : 'skip';
    return 'restart';
}
...
export function applyStuckRestartBusyGuard(paneState, decision, opts) {
    if (paneState === 'busy')
        return 'skip';
    if (paneState === 'typing') {
...
export async function recoverStuckInputForSession(session, prev, thresholds, allowPlainReinject) {
...
async function performStuckInputAction(session, action, paneBefore, block, prevSig, attempt) {
    let submitted = false;
    try {
        switch (action) {
            case 'reinject-block': {
    ...
    if (submitted) {
        ...
        logger.warn({ session, action, attempt, landed }, landed
...
export function shouldRunPeriodicReap(lastAt, now, intervalMs) {
...
let marveenSuspectFirstSeen = null;
let marveenDownState = null;
function getMainAgentProvider() {
    return CHANNEL_PROVIDER;
}
async function softReconnectMarveen() {
    return (await attemptChannelMcpReconnect(MAIN_AGENT_ID)).ok;
}
async function triggerMarveenMemorySave() {
    const prompt = [
...
export function readConfiguredMainModel(projectRoot = PROJECT_ROOT) {
...
export function readExtraChannelPluginIds(projectRoot = PROJECT_ROOT) {
...
export function respawnMainSessionFresh() {
...
export async function resumeMarveenSession() {
    const provider = getProvider(getMainAgentProvider());
    try {
 ...
 */
export function lastMainRespawnAt() {
...
export function mainChannelsSessionExists() {
    try {
...
export function createMainChannelsSession() {
    const now = Date.now();
...
function respawnMarveenSessionFresh() {
    const provider = getProvider(getMainAgentProvider());
    try {
...
export function shouldEscalateAfterResume(f) {
    if (f.claudePid == null)
        return true;
...
function schedulePostResumePluginGuard(provider) {
    setTimeout(() => {
        try {
...
export function hardRestartMarveenChannels() {
    ...
    catch { /* backstop handlers remain */ }
...
function maybeRestartWedgedMainChannel(state) {
    ...
    const action = applyStuckRestartBusyGuard(paneState, decideStuckInputRestart(parked, state.attempts, MAIN_STUCK_THRESHOLDS.maxAttempts, Date.now(), lastStuckRestartAt, stuckRestartCount, STUCK_RESTART_MIN_INTERVAL_MS, STUCK_RESTART_MAX_CONSECUTIVE), { machineOrigin, softRemedy });
    if (action === 'skip' && shouldDeferKeepaliveRespawn(paneState)) {
        logger.info({ paneState, attempts: state.attempts, machineOrigin, softRemedy }, paneState === 'busy'
 ...
 */
export function shouldDeferKeepaliveRespawn(paneState) {
...
export function shouldRespawnForStaleKeepalive(opts) {
    if (opts.keepaliveAgeMs == null)
        return false;
    if (opts.msSinceLastRespawn != null && opts.msSinceLastRespawn < opts.respawnGraceMs)
        return false;
...
export function shouldRefreshKeepaliveFromInbound(lastInboundTs, keepaliveMtimeMs) {
...
function refreshKeepaliveFromInbound() {
    try {
        const lastInboundTs = readLastIngestionTimestamp(TRANSCRIPT_DIR);
        let mtimeMs = 0;
        try {
            mtimeMs = statSync(KEEPALIVE_FILE).mtimeMs;
        }
        catch { /* missing -> 0 */ }
        if (!shouldRefreshKeepaliveFromInbound(lastInboundTs, mtimeMs))
            return;
        if (!existsSync(KEEPALIVE_FILE)) {
            writeFileSync(KEEPALIVE_FILE, String(Math.floor(lastInboundTs / 1000)));
        }
        const when = new Date(lastInboundTs);
...
function checkMainKeepaliveStaleness() {
...
async function handleMarveenDown() {
    const now = Date.now();
        ...
        if (providerLabel === 'telegram' && !marveenDownState.conflictProbed) {
            ...
            if (tok) {
                probeTelegramConflict(tok)
                    .then(r => {
                    if (r.conflicted) {
                        logger.warn({ status: r.status, description: r.description }, 'Telegram getUpdates 409 Conflict confirmed -- orphan poller is contending for the bot token. Recovery will reap and respawn.');
                    }
                    else if (r.status > 0) {
                        logger.info({ status: r.status, description: r.description }, 'Telegram getUpdates returned non-409 status on diagnostic probe -- the down state has a different cause than orphan poller contention');
                    }
                })
                    .catch(err => {
    ...
    if (marveenDownState.stage === 'save') {
        ...
        if (now - saveStartedAt < SAVE_WINDOW_MS)
            return;
    ...
    if (marveenDownState.stage === 'resume') {
        ...
        if (now - resumeStartedAt < RESUME_GRACE_MS)
            return;
...
function shouldEscalateMarveenDown() {
    const now = Date.now();
...
export function startChannelPluginMonitor() {
    ...
    async function check() {
        ...
        try {
            ...
            const targets = [{ session: MAIN_CHANNELS_SESSION, isMarveen: true, provider: mainProvider }];
            for (const a of listAgentNames()) {
            ...
            for (const t of targets) {
                ...
                if (!claudePid) {
                    if (!t.isMarveen && t.agentName) {
                        ...
                        if (lastRestart && Date.now() - lastRestart < AGENT_RESTART_GRACE_MS)
                            continue;
                    }
                    if (t.isMarveen) {
                ...
                if (t.isMarveen) {
                    ...
                    if (!agentDownSince.has(t.session)) {
                        ...
                        try {
                            ...
                            logger.warn({ agent: t.agentName, provider: t.provider, claudePid, ...evidence }, evidence.interpretation === 'in-tree'
                    ...
                    if (action === 'skip') {
                        logger.debug({ agent: t.agentName, provider: t.provider, failures, msDown, agentBusy, paneState: agentPaneState }, agentBusy
...
function memGateAllowsStart(agentName) {
    try {
...
async function reconcileDesiredAgents() {
    if (reconcileBurstInProgress)
        return;
    ...
    try {
        for (const name of down) {
            ...
            if (last != null && Date.now() - last < AGENT_RESTART_GRACE_MS)
                continue;
            if (!memGateAllowsStart(name))
                continue; // Commit 3 v1: safe-mode / memory gate
