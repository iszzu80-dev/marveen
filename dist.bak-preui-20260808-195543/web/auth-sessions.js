...
export function createSession(user, opts = {}) {
...
export function revokeSession(cookieValue) {
    if (!cookieValue)
        return;
...
export function sweepExpiredSessions() {
    ...
    for (const [hash, entry] of cache) {
        if (entry.lastSeenAt < idleCutoff || entry.createdAt < absoluteCutoff)
            cache.delete(hash);
    }
    return res.changes;
}
