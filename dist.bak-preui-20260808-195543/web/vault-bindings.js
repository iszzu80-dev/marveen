...
function readBindings() {
    try {
        return JSON.parse(readFileSync(BINDINGS_PATH, 'utf-8'));
    }
    catch {
        return { bindings: [] };
    }
}
function writeBindings(store) {
...
export function addBinding(binding) {
    ...
    const idx = store.bindings.findIndex(b => b.vaultSecretId === binding.vaultSecretId && b.envVar === binding.envVar);
    if (idx >= 0) {
...
export function removeBinding(vaultSecretId, envVar) {
    const store = readBindings();
    const before = store.bindings.length;
    store.bindings = store.bindings.filter(b => !(b.vaultSecretId === vaultSecretId && b.envVar === envVar));
    if (store.bindings.length === before)
        return false;
    writeBindings(store);
    return true;
}
export function removeBindingsForSecret(vaultSecretId) {
    const store = readBindings();
        ...
        for (const target of binding.targets) {
            try {
                ...
                if (binding.headerName) {
                    ...
                    applyHeadersHelper(serverCfg, headerBindingsForServer(target.mcpFilePath, target.serverName, remaining));
                }
                else {
                    if (!serverCfg.env)
                        continue;
                    delete serverCfg.env[binding.envVar];
                    if (!serverHasVaultRefs(serverCfg.env))
                        unwrapCommand(serverCfg);
                }
...
export function collectAllMcpFilePaths() {
        ...
        if (existsSync(projectsDir)) {
            try {
                for (const proj of readdirSync(projectsDir)) {
                    ...
                    if (existsSync(projMcp))
                        paths.push({ path: projMcp, label: `project:${agentName}/${proj}` });
                }
            }
            catch { /* ignore */ }
        }
    ...
    for (const extPath of getExternalProjectPaths()) {
        ...
        if (existsSync(extMcp))
            paths.push({ path: extMcp, label: `external:${basename(extPath)}` });
    }
    return paths;
}
...
function looksLikeSensitiveValue(val) {
    if (!val || val.length < 8)
        return false;
        ...
        try {
            ...
            for (const [serverName, cfg] of Object.entries(servers)) {
                ...
                for (const [envVar, envVal] of Object.entries(env)) {
                    if (!looksLikeSensitiveKey(envVar))
                        continue;
                    if (!looksLikeSensitiveValue(String(envVal)))
                        continue;
...
function wrapCommand(serverCfg) {
    if (serverCfg.command === VAULT_WRAPPER_PATH)
        return;
    serverCfg._vaultOriginalCommand = serverCfg.command;
    if (serverCfg.args?.length)
        serverCfg._vaultOriginalArgs = serverCfg.args;
    serverCfg.args = [serverCfg.command, ...(serverCfg.args || [])];
    serverCfg.command = VAULT_WRAPPER_PATH;
}
function unwrapCommand(serverCfg) {
    if (serverCfg.command !== VAULT_WRAPPER_PATH)
        return;
    if (!serverCfg._vaultOriginalCommand)
        return;
...
function serverHasVaultRefs(env) {
    if (!env)
        return false;
...
function headerBindingsForServer(mcpFilePath, serverName, all = getBindings()) {
    return all.filter(b => b.headerName && b.targets.some(t => t.mcpFilePath === mcpFilePath && t.serverName === serverName));
}
...
function applyHeadersHelper(serverCfg, headerBindings) {
    for (const b of headerBindings) {
        if (serverCfg.headers && b.headerName)
            delete serverCfg.headers[b.headerName];
    }
    if (headerBindings.length === 0) {
        delete serverCfg.headersHelper;
    }
    else {
        const args = headerBindings.map(b => `${b.headerName}=${b.headerScheme ?? 'Bearer'}:::${b.vaultSecretId}`);
        serverCfg.headersHelper = [VAULT_HEADERS_HELPER_PATH, ...args].map(shellEscape).join(' ');
    }
    if (serverCfg.headers && Object.keys(serverCfg.headers).length === 0)
        delete serverCfg.headers;
}
export function syncSecret(vaultSecretId) {
    const bindings = getBindings().filter(b => b.vaultSecretId === vaultSecretId);
    if (bindings.length === 0)
        return { updated: 0, errors: [] };
    const secret = getSecret(vaultSecretId);
    if (secret === null)
        return { updated: 0, errors: [`Vault secret "${vaultSecretId}" not found`] };
    let updated = 0;
    const errors = [];
        ...
        for (const target of binding.targets) {
            try {
                ...
                if (binding.headerName) {
                    ...
                    applyHeadersHelper(serverCfg, headerBindingsForServer(target.mcpFilePath, target.serverName));
                }
                else {
                    if (!serverCfg.env)
                        serverCfg.env = {};
    ...
    if (updated > 0)
        logger.info({ vaultSecretId, updated }, 'Vault secret synced to .mcp.json files');
    return { updated, errors };
}
export function unsyncBinding(vaultSecretId, envVar) {
    const all = getBindings();
    const bindings = all.filter(b => b.vaultSecretId === vaultSecretId && b.envVar === envVar);
    ...
    const remaining = all.filter(b => !(b.vaultSecretId === vaultSecretId && b.envVar === envVar));
    for (const binding of bindings) {
        for (const target of binding.targets) {
            try {
                const content = JSON.parse(readFileOr(target.mcpFilePath, '{}'));
...
export function syncAllBindings() {
