...
export function agentChannelDir(name, mainAgentId, provider) {
...
function invitesPathFor(accessPath) {
    return join(accessPath, '..', 'invites.json');
}
function readAccess(path) {
    try {
...
function writeInvites(path, data) {
    mkdirSync(join(path, '..'), { recursive: true });
    atomicWriteFileSync(path, JSON.stringify(data, null, 2));
}
function pruneInvites(store, now) {
    if (!store.invites)
        return false;
    let mutated = false;
...
function activeInviteCount(store, now) {
    if (!store.invites)
        return 0;
    let n = 0;
    for (const inv of Object.values(store.invites)) {
        if (!inv.used && inv.expiresAt >= now)
            n++;
    }
    return n;
}
export function createInvite(accessPath, botUsername, provider = 'telegram', ttlMs = INVITE_DEFAULT_TTL_MS) {
...
export function listInvites(accessPath) {
    const invitesPath = invitesPathFor(accessPath);
    const store = readInvites(invitesPath);
    const now = Date.now();
    if (pruneInvites(store, now))
        writeInvites(invitesPath, store);
    if (!store.invites)
        return [];
...
export function revokeInvite(accessPath, token) {
    const invitesPath = invitesPathFor(accessPath);
    const store = readInvites(invitesPath);
    if (!store.invites?.[token])
        return false;
    delete store.invites[token];
    writeInvites(invitesPath, store);
...
export function runInviteMonitorTick(mainAgentId, agentsRoot) {
    const providerTypes = ['telegram', 'slack'];
    for (const provider of providerTypes) {
        const targets = [];
        const mainAccess = join(channelStateDir(provider), 'access.json');
        if (existsSync(mainAccess))
            targets.push({ name: mainAgentId, accessPath: mainAccess });
        if (existsSync(agentsRoot)) {
            ...
            for (const e of entries) {
                ...
                if (existsSync(p))
                    targets.push({ name: e, accessPath: p });
            }
        }
        for (const { name, accessPath } of targets) {
            ...
            if (!access.allowFrom)
                access.allowFrom = [];
            if (!access.allowFrom.includes(pEntry.senderId))
                access.allowFrom.push(pEntry.senderId);
            if (access.pending)
                delete access.pending[pCode];
            tEntry.used = true;
            tEntry.usedBy = pEntry.senderId;
            tEntry.usedAt = now;
            if (activeInviteCount(store, now) === 0)
                access.dmPolicy = 'allowlist';
            try {
...
let inviteMonitorInterval = null;
export function startInviteMonitor(mainAgentId, agentsRoot, intervalMs = 3000) {
...
export function stopInviteMonitor() {
    if (inviteMonitorInterval) {
