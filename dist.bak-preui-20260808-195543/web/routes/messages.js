import { createAgentMessage, getPendingMessages, listAgentMessages, getAgentConversation, getAgentConversationThreads, getKanbanSeqByIdPrefix, markMessageDone, markMessageFailed, getAgentMessage, closeOtelSpan, getPendingBacklogByAgent, } from '../../db.js';
...
export async function tryHandleMessages(ctx) {
    const { req, res, path, method, url } = ctx;
    if (path === '/api/messages' && method === 'POST') {
        const body = await readBody(req);
        const { from, to, content, origin_note } = JSON.parse(body.toString());
    ...
    if (path === '/api/messages' && method === 'GET') {
        ...
        if (status === 'pending' && agent) {
            ...
            messages = getAgentConversation(agent, limit, Number.isFinite(before) ? before : undefined);
        }
        else {
    ...
    if (msgUpdateMatch && method === 'PUT') {
        ...
        const body = await readBody(req);
        const { status: newStatus, result } = JSON.parse(body.toString());
        let ok = false;
        if (newStatus === 'done')
            ok = markMessageDone(id, result);
        else if (newStatus === 'failed')
            ok = markMessageFailed(id, result);
        if (ok) {
            ...
            if (done && done.from_agent !== done.to_agent && !done.content.startsWith('[Eredmény]')) {
                ...
                createAgentMessage(done.to_agent, done.from_agent, `[Eredmény] msg_id:${id} status:${newStatus}\n\n${summary}`);
            }
            json(res, { ok: true });
            return true;
        }
