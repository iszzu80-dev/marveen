...
const ENROLL_KINDS = ['token', 'session'];
function str(v) {
    return typeof v === 'string' ? v : '';
}
export async function tryHandleSecurity(ctx) {
    const { req, res, path, method, auth } = ctx;
    if (path !== '/api/security/bridge-enroll' || method !== 'POST')
        return false;
    if (auth === undefined || !ENROLL_KINDS.includes(auth.kind)) {
    ...
    try {
        ...
        void notifySecurityEvent(`🔗 Bridge-párosítás a dashboardról: "${name}" (${outcome.action === 'replaced' ? 'újrapárosítás' : 'új eszköz'}). ` +
