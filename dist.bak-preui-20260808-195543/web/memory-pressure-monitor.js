...
import { relievePressure } from "./memory-pressure-gate.js";
    ...
    try {
        ...
        for (const line of raw.split("\n")) {
            if (line.startsWith("MemAvailable:"))
                memAvailableKiB = Number(line.match(/\d+/)?.[0] ?? 0);
            if (line.startsWith("MemTotal:"))
                memTotalKiB = Number(line.match(/\d+/)?.[0] ?? 0);
            if (line.startsWith("SwapFree:"))
                swapFreeKiB = Number(line.match(/\d+/)?.[0] ?? 0);
            if (line.startsWith("SwapTotal:"))
                swapTotalKiB = Number(line.match(/\d+/)?.[0] ?? 0);
        }
    }
...
function readExpectedAgentCount() {
    try {
...
function main() {
    ...
    if ((nextState === "critical" || nextState === "emergency") &&
        nextState !== prevState) {
