...
const ELSEWHERE = {
    securityProfile: name => `use PUT /api/agents/${encodeURIComponent(name)}/security with {"profile": "..."} instead`,
};
...
export function checkAgentPutFields(name, body) {
    if (body === null || typeof body !== 'object') {
        return { ok: false, rejected: [], message: 'Request body must be a JSON object.' };
    }
    const writable = new Set(AGENT_PUT_WRITABLE_FIELDS);
    const rejected = Object.keys(body).filter(k => !writable.has(k));
    if (!rejected.length)
        return { ok: true };
    const hints = rejected.filter(k => k in ELSEWHERE).map(k => `${k}: ${ELSEWHERE[k](name)}`);
    const message = `Unsupported field(s) for this endpoint: ${rejected.join(', ')}.` +
