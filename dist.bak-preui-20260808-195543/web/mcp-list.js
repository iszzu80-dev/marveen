...
import { applyRefreshOutcome, scrubPaths as scrubPathsBase, } from '../mcp-list-parser.js';
...
export function getMcpListCache() {
    return mcpListCache;
}
export function purgeFromMcpListCache(name) {
...
let mcpListWorkingDir = null;
function getMcpListWorkingDir() {
    if (mcpListWorkingDir && existsSync(mcpListWorkingDir))
...
const cleanupMcpListWorkingDir = () => {
    if (!mcpListWorkingDir)
        return;
    try {
        rmSync(mcpListWorkingDir, { recursive: true, force: true });
    }
    catch { /* best effort */ }
    mcpListWorkingDir = null;
};
process.once('exit', cleanupMcpListWorkingDir);
export function refreshMcpListCache() {
    ...
    inflightRefresh = (async () => {
        ...
        try {
            ...
            const { stdout, stderr, execError } = await new Promise((resolve, reject) => {
...
export function startMcpListChecker() {
