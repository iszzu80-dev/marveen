...
import { validateContextPacket, estimateFreshTokens, findSecretShapes, MAX_SECTION_CHARS, } from './context-packet.js';
function bullets(xs) {
...
export function renderCheckpoint(cp) {
    ...
    const ids = [];
    if (cp.cardId)
        ids.push(`card ${cp.cardId}`);
    if (cp.dispatchId)
        ids.push(`dispatch ${cp.dispatchId}`);
    ...
    if (done.length === 0)
        out.push('- (nothing completed yet)');
    else
        for (const w of done)
            out.push(`- ${w}`);
...
export function validateCheckpoint(cp) {
    const errors = [];
    if (!cp.goal || !cp.goal.trim())
        errors.push({ code: 'goal_missing', at: 'Goal', message: 'A checkpoint must state its goal' });
    if (!cp.nextStep || !cp.nextStep.trim())
        errors.push({ code: 'next_step_missing', at: 'Next Steps', message: 'A checkpoint must carry one executable next step' });
    if (!cp.agent || !cp.agent.trim())
        errors.push({ code: 'agent_missing', at: 'From', message: 'A checkpoint must name the agent it came from' });
