 ...
 */
export function resolveOwnerChatId(readAccessFile = (p) => readFileSync(p, 'utf-8'), configured = ALLOWED_CHAT_ID) {
    const fromEnv = normalizeChatId(configured);
    if (fromEnv)
        return fromEnv;
    try {
        const raw = JSON.parse(readAccessFile(join(channelStateDir('telegram'), 'access.json')));
        ...
        if (Array.isArray(raw.allowFrom)) {
            for (const entry of raw.allowFrom) {
                const id = normalizeChatId(typeof entry === 'number' ? String(entry) : entry);
                if (id)
                    return id;
            }
        }
        if (raw.groups && typeof raw.groups === 'object') {
            for (const key of Object.keys(raw.groups)) {
